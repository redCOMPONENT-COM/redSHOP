    ...
    */
		      ...
		      foreach ($orders as $order_detail)
		      {
		        ...
		        if( empty($request['errorcode']) && ($accept == "1" || $accept == "2") ) {
		        	...
		        	if ($this->orderPaymentNotYetUpdated($db, $order_id, $tid)) {
			            ...
			            if ($accept == "2") {
			            	$msg =  JText::_('EPAY_PAYMENT_CALLBACK');
			            }

			            // payment fee
			            if ($request["transfee"]) {
			            	$msg = JText::_('EPAY_PAYMENT_FEE');
			            }

			            // payment date
			            if ($request["date"]) {
			            	$msg = JText::_('EPAY_PAYMENT_DATE');
			            }

			            // payment fraud control
			            if (@$request["fraud"]) {
			            	$msg = JText::_('EPAY_FRAUD');
			            }
			            ...
			            if ($request["cardid"]) {
			             ...
			             $msg =  JText::_('EPAY_PAYMENT_CARDTYPE');

			            }

			            // creation information
			            $msg = JText::_('EPAY_PAYMENT_LOG_TID');
			           	$msg =  JText::_('EPAY_PAYMENT_TRANSACTION_SUCCESS');
			        }
			      }elseif( $accept == "0" ) {
						$values->order_status_code=$invalid_status;
					 	...
					 	$values->log=JTEXT::_('ORDER_NOT_PLACED');
					  	...
					  	$msg =  JText::_('EPAY_PAYMENT_ERROR');
			      }else{
						$values->order_status_code=$invalid_status;
					  	$values->order_payment_status_code='Unpaid';
					 	$values->log=JTEXT::_('ORDER_NOT_PLACED');
	...
	function orderPaymentNotYetUpdated($dbConn, $order_id, $tid){
 	...
 	function onCapture_Paymentrs_payment_epay($element, $data){
		...
		// get the class
		$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$element.DS.'epaysoap.php';
		include($paymentpath);
