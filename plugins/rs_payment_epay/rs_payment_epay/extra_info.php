...
{?><input type="hidden" name="callbackurl" value="<?php echo JURI::base(); ?>index2.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_epay&accept=1&sessionid=<?php echo $sessionid?>&Itemid=<?php echo $_REQUEST['Itemid']?>&orderid=<?php echo $data['order_id']?>">
...
<? } else {
?>
<input type="hidden" name="accepturl" value="<?php echo JURI::base(); ?>index.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_epay&accept=1&sessionid=<?php echo $sessionid ?>&Itemid=<?php echo $_REQUEST['Itemid'] ?>&orderid=<?php echo $data['order_id'] ?>">
<?php } ?>
<input type="hidden" name="declineurl" value="<?php echo JURI::base(); ?>index.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_epay&accept=0&sessionid=<?php echo $sessionid ?>&Itemid=<?php echo $_REQUEST['Itemid'] ?>&orderid=<?php echo $data['order_id'] ?>">
<input type="hidden" name="group" value="<?php echo $this->_params->get("payment_group"); ?>">
<input type="hidden" name="instant" value="<?php echo $this->_params->get("auth_type"); ?>">
...
<br>
<table border="0" width="100%"><tr class="epay_button_row"><td class="epay_button_data"><input type="button" onClick="open_ePay_window();" value="<?php echo JTEXT::_('OPEN EPAY PAYMENT WINDOW') ?>"></td><td width="100%" id="flashLoader"></td></tr></table><br><br><br>
