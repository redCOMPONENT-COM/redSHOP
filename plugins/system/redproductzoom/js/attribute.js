...
//zoom
function display_image_add(img,product_id,img_link)
{
	
	document.getElementById('main_image'+product_id).src=img;
	document.getElementById('zoom_img').href=img_link;
}
	...
	request.onreadystatechange=function() {
		...
		if(request.readyState == 4 )
		{
			...
			{*/
				...
				if(arrResponse[4]!="")
				{
					var gmyhtml = '<a href="'+arrResponse[2]+'" class="jqzoom" rel=\'gal1\'  title="triumph" ><img src="'+arrResponse[4]+'"  title="triumph"  style="border: 4px solid #666;"></a>'; 
					$(".redzoom").html(gmyhtml);
					//zoom
					/*if(document.getElementById('zoom_img').href)
					{
						//window.setTimeout('window.alert(\'Hello!\')', 4000);
						document.getElementById('zoom_img').href="";
						document.getElementById('zoom_img').href=arrResponse[2];
			...
			$('#additional_images'+product_id).html(arrResponse[1]);
	...
	else 
	{
		...
		request.onreadystatechange=function() 
		{
			if(request.readyState == 4 && request.status == 200) 
			{
				...
				var options = {url:detailurl,handler:'html',size: {x: 500, y: 600},htmldata:responce};
				redBOX.initialize({});
				document.attbox = redBOX.open(null,options);
		...
		if(request.readyState == 4 ) 
		{
			...
			request_inner.onreadystatechange = function()
			{
				if(request_inner.readyState == 4 && request_inner.status==200 && aj_flag ) 
				{
					...
					var options = {url:newurl,handler:'html',size: {x: 500, y: 150},htmldata:responcebox,onOpen: function(){
