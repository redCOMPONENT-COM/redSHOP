...
class zoomproducthelper extends producthelper
{
	function replaceProductImage($product, $imagename = "", $linkimagename = "", $link = "", $width, $height, $Product_detail_is_light = 2, $enableHover = 0, $preselectedResult = array(), $suffixid = 0)
	{
		...
		if ($imagename != "")
		{
			$product_img = $redhelper->watermark('product', $imagename, $width, $height, WATERMARK_PRODUCT_THUMB_IMAGE, $product_id);

			if ($cat_product_hover)
				$product_hover_img = $redhelper->watermark('product', $imagename, PRODUCT_HOVER_IMAGE_WIDTH, PRODUCT_HOVER_IMAGE_HEIGHT, WATERMARK_PRODUCT_THUMB_IMAGE, $product_id);

			if ($linkimagename != "")
			{
				$linkimage = $redhelper->watermark('product', $linkimagename, '', '', WATERMARK_PRODUCT_IMAGE, $product_id);
			}
			else
			{
				$linkimage = $redhelper->watermark('product', $imagename, '', '', WATERMARK_PRODUCT_IMAGE, $product_id);
				//$linkimage = $redhelper->watermark('product',$imagename,$width,$height,WATERMARK_PRODUCT_IMAGE, $product_id);
			}
		}
	...
	function getAdditionalImageforZoom($pid)
	{
		...
		for ($m = 0; $m < count($media_image); $m++)
		{
			...
			if ($media_image[$m]->media_name != $media_image[$m]->product_full_image && is_file($filename1))
			{
				...
				if ($media_image [$m]->media_name)
				{
					...
					$pimg = $redhelper->watermark('product', $thumb, $mpw_thumb, $mph_thumb, WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $pid);
					$linkimage = $redhelper->watermark('product', $thumb, '', '', WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $pid);
					$hoverimg_path = $redhelper->watermark('product', $thumb, ADDITIONAL_HOVER_IMAGE_WIDTH, ADDITIONAL_HOVER_IMAGE_HEIGHT, WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $pid);

					$img_path = $redhelper->watermark('product', $thumb, $pw_thumb, $ph_thumb, WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $pid);

					$hovermore_images = $redhelper->watermark('product', $thumb, '', '', WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $pid);
					...
					$img_path_org = $redhelper->watermark('product', $thumb_original, $pw_thumb, $ph_thumb, WATERMARK_PRODUCT_THUMB_IMAGE, $pid);
					$hovermore_org = $redhelper->watermark('product', $thumb_original, $pw_thumb, $ph_thumb, 0, $pid);
		...
		if (count($media_image) > 0)
		{
			...
			$img_path_org = $redhelper->watermark('product', $thumb_original, $mpw_thumb, $mph_thumb, WATERMARK_PRODUCT_THUMB_IMAGE, $pid);

			$prodmainimg = "";
			$prod_img_path = $redhelper->watermark('product', $thumb_original, $pw_thumb, $ph_thumb, 0, $pid);
	...
	function displayAdditionalImage($product_id = 0, $accessory_id = 0, $relatedprd_id = 0, $property_id = 0, $subproperty_id = 0, $main_imgwidth = 0, $main_imgheight = 0, $redview = "", $redlayout = "")
	{
		...
		for ($m = 0; $m < count($media_image); $m++)
		{
			...
			if ($media_image [$m]->media_name != $media_image [$m]->product_full_image)
			{
				...
				{
					$pimg = $redhelper->watermark('product', $thumb, $mpw_thumb, $mph_thumb, WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $product_id);
					$linkimage = $redhelper->watermark('product', $thumb, '', '', WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $product_id);

					$img_path = $redhelper->watermark('product', $thumb, $pw_thumb, $ph_thumb, WATERMARK_PRODUCT_ADDITIONAL_IMAGE, $product_id);
					...
					$img_path_org = $redhelper->watermark('product', $thumb_original, $mpw_thumb, $mph_thumb, WATERMARK_PRODUCT_THUMB_IMAGE, $product_id);
		...
		if ($val_prodadd > 0)
		{
			...
			$prodmainimg = "";
			$prod_img_path = $redhelper->watermark('product', $thumb_original, $pw_thumb, $ph_thumb, 0, $product_id);
			$prod_img_path_link = $url . "components/com_redshop/assets/images/product/" . $thumb_original;
		...
		if ($property_id > 0)
		{
			...
			if (is_file($property_filename_org))
			{
				...
				$property_img_path_org = $redhelper->watermark('property', $property_thumb_original, $pw_thumb, $ph_thumb, 0, $property_id);
				$prop_mainimg_path = $redhelper->watermark('property', $property_thumb_original, $mpw_thumb, $mph_thumb, 0, $property_id);
			...
			else
			{
				...
				$property_img_path_org = $redhelper->watermark('product', $property_thumb_original, $pw_thumb, $ph_thumb, 0, ($thumb_original == PRODUCT_DEFAULT_IMAGE) ? 1 : $product->product_id);
				$prop_mainimg_path = $redhelper->watermark('product', $property_thumb_original, $mpw_thumb, $mph_thumb, 0, ($thumb_original == PRODUCT_DEFAULT_IMAGE) ? 1 : $product->product_id);
			...
			else
			{
				for ($m = 0; $m < count($media_image); $m++)
				{
					...
					if ($thumb != $media_image [$m]->property_image)
					{
						...
						{
							$imgs_path = $redhelper->watermark('property', $thumb, $pw_thumb, $ph_thumb, 0, $media_image [$m]->section_id);
							...
							$propadditionImg .= "<img src='" . $redhelper->watermark('property', $thumb, $mpw_thumb, $mph_thumb, 0, $media_image [$m]->section_id) . "' alt='" . $alttext . "' title='" . $alttext . "' style='cursor: pointer;'></a>";
		...
		if ($subproperty_id > 0)
		{
			...
			if (is_file($subproperty_filename_org))
			{
				...
				$subproperty_img_path_org = $redhelper->watermark('subcolor', $imagename, $pw_thumb, $ph_thumb, 0, $subproperty_id);
				$subprop_mainimg_path = $redhelper->watermark('subcolor', $imagename, $mpw_thumb, $mph_thumb, 0, $subproperty_id);
			...
			else
			{
				for ($m = 0; $m < count($media_image); $m++)
				{
					...
					{
						if ($thumb && is_file(JPATH_COMPONENT_SITE . "/assets/images/subproperty/" . $thumb))
						{
							$imgs_path = $redhelper->watermark('subproperty', $thumb, $pw_thumb, $ph_thumb, 0, $media_image[$m]->section_id);
							$imgs_thumb_path = $redhelper->watermark('subproperty', $thumb, $mpw_thumb, $mph_thumb0, $media_image[$m]->section_id);
	...
	function getdisplaymainImage($product_id = 0, $property_id = 0, $subproperty_id = 0, $pw_thumb = 0, $ph_thumb = 0, $redview = "")
	{
		...
		{
			$productmainimg = $redhelper->watermark($type, $imagename, $pw_thumb, $ph_thumb, WATERMARK_PRODUCT_THUMB_IMAGE, $image_id);

			$aHrefImageResponse = $redhelper->watermark($type, $imagename, '', '', WATERMARK_PRODUCT_IMAGE, $image_id);
			//zoom
