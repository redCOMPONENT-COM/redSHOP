...
//			exit;
	...
	function createInvoice($d)
	{
		...
		try
		{
			...
			{
				$reference .= JText::_('ORDER_NUMBER').': '.$d['order_number'];
			}
			if(isset($d['requisition_number']) && $d['requisition_number']!="")
			{
				$reference .= chr(13).''.JText::_('REQUISITION_NUMBER').': '.$d['requisition_number'];
			}
			if(isset($d['customer_note']) && $d['customer_note']!="")
			{
				$reference .= chr(13).''.JText::_('CUSTOMER_NOTE_LBL').': '.$d['customer_note'];
			}
