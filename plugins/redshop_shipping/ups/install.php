...
class PlgRedshop_ShippingUpsInstallerScript
{
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('ups'))
			)
				->loadResult();
	...
	{
		$configFile = JPATH_ROOT . '/plugins/redshop_shipping/ups/config/ups.cfg.php';
		...
		$oldConfigFile = JPATH_ROOT . '/plugins/redshop_shipping/ups/ups.cfg.php';
		...
		$defaultFile = JPATH_ROOT . '/plugins/redshop_shipping/ups/config/default.cfg.php';
