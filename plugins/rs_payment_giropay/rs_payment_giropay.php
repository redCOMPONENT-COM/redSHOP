    ...
    */
 	function onPrePayment($element, $data)
   	{
	    	...
	    	if (empty($plugin))
		{
 		...
 		$mainframe =& JFactory::getApplication();
		$class_path=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$element.DS.'gsGiropay.php';
		include($class_path);
	 ...
	 */
       function onNotifyPaymentrs_payment_giropay($element, $request){
		...
		$giro_parameters=$this->getparameters('rs_payment_giropay');
		$paymentinfo = $giro_parameters[0];
		$paymentparams = new JParameter( $paymentinfo->params );
		
		$merchantId = $paymentparams->get('merchant_id','');
		$projectId = $paymentparams->get('project_id','');
