...
class PlgRedshop_AlertAlert_Min_Stock extends JPlugin
{
	/**
	...
	public function storeAlertMinStock($cart)
	{
		...
		for ($i = 0; $i <= $cart['idx']; $i++) {
			foreach ($min_value_product_in_stock as $k => $v) {
				if ($cart[$i]['product_id'] == $k)
				{
					if ($value_product_in_stock[$k]->quantity <= $v->data_txt)
					{
						...
						if($db->setQuery($query)->execute())
						{
							...
							else
							{
								foreach ($mail as $k=>$v)
								{
									$this->sendMail($template_mail['0']->template_desc,$v);
								}
							}
						}
					};
