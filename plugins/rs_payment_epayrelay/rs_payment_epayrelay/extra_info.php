...
<form action="https://ssl.ditonlinebetalingssystem.dk/auth/default.aspx" method="post" name="ePay" id="ePay" onsubmit="return validate();">
<table cellspacing="0" cellpadding="3" style="width: 400px;">
<input type="hidden" name="merchantnumber" value="<?php echo $this->_params->get("merchant_id");?>" style="width: 200px;" />
<input type="hidden" name="amount" value="<?php echo round($order_details[0]->order_total*100, 2 ) ?>" style="width: 200px;" />
<input type="hidden" name="currency" style="width: 200px;"  value="<?php echo calculateePayCurrency($data['order_id'])?>"/>
<input type="hidden" name="orderid" style="width: 200px;"  value="<?php echo $data['order_id']?>"/>
<input type="hidden" name="MD5Key" value="<?php if ($this->_params->get("epay_md5") == 2) echo md5( calculateePayCurrency($data['order_id']) . round(($order_details[0]->order_total)*100, 2 ) . $data['order_id']  . $this->_params->get("epay_paymentkey"))?>">
<input type="hidden" name="use3D" value="<?php echo $this->_params->get("epay_3dsecure"); ?>">
<input type="hidden" name="language" style="width: 200px;" value="<?php echo $this->_params->get("language"); ?>" />
<input type="hidden" name="instantcapture" style="width: 200px;" value="<?php echo $this->_params->get("auth_type"); ?>" />
...
<input type="hidden" name="transfee" value="0"/>
		<tr>
			<td><?php echo JText::_("CARD_NO"); ?></td>
			<td>
				<input type="text" name="cardno"   value="" <?php if($trans_fee==1) {?>   onchange="getFee(this.value, 1)" <?php }?> autocomplete="off"/>
			</td>

		</tr>
		<tr>
			<td><?php echo JText::_("EXP_MONTH"); ?></td>
			<td>
		...
		<tr>
			<td><?php echo JText::_("EXP_YEAR"); ?></td>
			<td>
				<select name="expyear" >
					<option value="09">09</option>
					<option value="10">10</option>
		...
		<tr>
			...
			<td>
				...
				<input type="hidden" name="declineurl" value="<?php echo $url?>index2.php?option=com_redshop&view=order_detail&task=notify_payment&payment_plugin=rs_payment_epayrelay&Itemid=<?php echo $_REQUEST['Itemid'] ?>&orderid=<?php echo $data['order_id'] ?>" />

				<?php

				if ($this->_params->get("activate_callback") == "1")
				{?>
				<input type="hidden" name="callbackurl" value="<?php echo $url?>index2.php?option=com_redshop&view=order_detail&task=notify_payment&payment_plugin=rs_payment_epayrelay&orderid=<?php echo $data['order_id'] ?>">
				<input type="hidden" name="accepturl" value="<?php echo $url?>index.php?option=com_redshop&view=order_detail&Itemid=<?php echo $_REQUEST['Itemid']?>&oid=<?php echo $data['order_id'] ?>">
				<? } else {
				?>
				<input type="hidden" name="accepturl" value="<?php echo $url?>index2.php?option=com_redshop&view=order_detail&task=notify_payment&payment_plugin=rs_payment_epayrelay&Itemid=<?php echo $_REQUEST['Itemid'] ?>&orderid=<?php echo $data['order_id'] ?>" />
				<?php } ?>
		<tr>
			<td>&nbsp;</td>
			<td><input type="submit" value="<?php echo JText::_('PAY'); ?>" class="button" style="width: 200px;"></td>
