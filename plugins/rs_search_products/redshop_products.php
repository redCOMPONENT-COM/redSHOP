...
$mainframe->registerEvent( 'onSearch', 'plgSearchredshop_products' );
...
function plgSearchredshop_products( $text, $phrase='', $ordering='', $areas=null )
{
	$db		=& JFactory::getDBO();
	...
	switch ( $ordering )
	{
	  ...
	  $query = 'SELECT a.product_id,a.product_name AS title,a.product_number as number, a.product_s_desc AS text,'
	...
	foreach($rows AS $key => $weblink) {
		if(searchHelper::checkNoHTML($weblink, $searchText, array('url', 'text', 'title','number'))) {
			$return[] = $weblink;
		}
	}
