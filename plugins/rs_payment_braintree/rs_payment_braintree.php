...
class plgRedshop_paymentrs_payment_braintree extends JPlugin
{
    ...
    */
    ...
    function onPrePayment($element, $data)
    {
    	...
    	if (empty($plugin))
        {
 		...
 		$mainframe =& JFactory::getApplication();
 		$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$plugin.DS.'creditcardform.php';
		include($paymentpath);
    }
	    ...
	    function getCredicardForm($element, $data)
	    {
			...
			if($new_user)
			{
					$cart_data = '<form action="'.JRoute::_('index.php?option=com_redshop&view=checkout&format=final&stap=2&oid='.(int) $data['order_id'].'&Itemid='.$Itemid ).'" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data" onsubmit="return CheckCardNumber(this);">';
	...
	//				$cardinfo .='<td>'.$this->_producthelper->getProductFormattedPrice($order->order_total).'</td></tr>';
				...
				$cart_data = '<form action="'.JRoute::_('index.php?option=com_redshop&view=checkout&format=final&stap=2&oid='.(int) $data['order_id'].'&Itemid='.$Itemid ).'" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data" >';
				$cart_data .='<table height="100">
								<tr><td>'.JText::_('USER_IS_ALREADY_REGISTERED_IN_BRAINTREE_VAULT').'</td></tr>
							 </table>';
			}
			
				$cart_data .= '<input type="hidden" name="option" value="com_redshop" />';
				$cart_data .= '<input type="hidden" name="Itemid" value="'.$Itemid.'" />';
				$cart_data .= '<input type="hidden" name="task" value="barintree_checkout_next" />';
				$cart_data .= '<input type="hidden" name="view" value="checkout" />';
				$cart_data .= '<input type="submit" name="submit" class="greenbutton" value="'.JText::_('BTN_CHECKOUTNEXT').'" />';
    ...
    */
 	    function getOrderAndCcdata($element,$data)
	    {
			...
			$values['payment_plugin']	= "rs_payment_braintree";//$payment_method_id;
	//		$values['task']				= $task;
	...
	function onAfterCreditcardInfo($element, $data)
    {
		...
		$apipath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$plugin.DS.'_environment.php';
		include($apipath);
		if($this->_params->get("store_in_vault"))
        {
        	...
        	if($new_user)
        	{
				$braintree_data = Braintree_TransparentRedirect::transactionData(
                    array('redirectUrl' => JURI::base()."index.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_braintree&orderid=".$data['order_id'],
                    'transaction' => array( 'options' => array('storeInVault' => true), 'billing'=>array('firstName'=>$billing_fname, 'lastName'=>$billing_lname, 'locality'=>$billing_locality,'region'=>$billing_region,'countryCodeAlpha2'=>$billing_country_code_alpha2, 'postalCode'=>$biling_postal_code), 'shipping'=>array('firstName'=>$shipping_fname, 'lastName'=>$shipping_lname, 'locality'=>$shipping_locality,'region'=>$shipping_region,'countryCodeAlpha2'=>$shipping_country_code_alpha2, 'postalCode'=>$shipping_postal_code), 'amount' => $order_total, 'type' => $transaction_type, 'customer'=>array('id'=>$user_id),'creditCard'=>array('token'=>$user_vault_ref))));

        	
        	}   else {
        		$braintree_data = Braintree_TransparentRedirect::transactionData(
                    array('redirectUrl' => JURI::base()."index.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_braintree&orderid=".$data['order_id'],
                    'transaction' => array('options' => array('storeInVault' => true), 'shipping'=>array('firstName'=>$shipping_fname, 'lastName'=>$shipping_lname, 'locality'=>$shipping_locality,'region'=>$shipping_region,'countryCodeAlpha2'=>$shipping_country_code_alpha2, 'postalCode'=>$shipping_postal_code),'amount' => $order_total, 'type' => $transaction_type, 'customerId'=>$user_id,'paymentMethodToken'=>$user_vault_ref)));
    ...
    function onNotifyPaymentrs_payment_braintree($element, $request){
		...
		$query_string= "http_status=".$request['http_status']."&id=".$request['id']."&kind=".$request['kind']."&option=".$request['option']."&view=".$request['view']."&controller=".$request['controller']."&task=".$request['task']."&payment_plugin=".$request['payment_plugin']."&orderid=".$request['orderid']."&hash=".$request['hash'];
		...
		$braintree_parameters=$this->getparameters('rs_payment_braintree');
	...
	function getparameters($payment){
		...
		function onCapture_Paymentrs_payment_braintree($element, $data)
       {
				$braintree_parameters=$this->getparameters('rs_payment_braintree');
				$paymentinfo = $braintree_parameters[0];
				...
				// get the class
				$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$element.DS.'_environment.php';
				include($paymentpath);
    	...
    	function getUser_BraintreeVault_ref($user_id)
		{
			$db = JFactory::getDBO();
		...
		function generate_BraintreeVault_ref($user_id)
		{
    	...
    	function updateUsertovault_token($user_id, $user_vault_ref)
    	{
    		$db = JFactory::getDBO();
