 ...
 * @subpackage  PlgRedshop_PaymentRs_Payment_AmazonCheckout
 *
 ...
 * @since    1.7.0
 */
class PlgRedshop_PaymentRs_Payment_AmazonCheckoutInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->q('rs_payment_amazoncheckout'))
