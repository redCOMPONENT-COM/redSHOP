...
class plgRedshop_paymentrs_payment_braintree extends JPlugin
{
	...
	function getCredicardForm($element, $data)
	{
		...
		{
			$cart_data = '<form action="index.php?option=com_redshop&view=checkout&format=final&stap=2&oid=' . $data['order_id'] . '&Itemid=' . $Itemid . '" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data" >';
			$cart_data .= '<table height="100">
								<tr><td>' . JText::_('COM_REDSHOP_USER_IS_ALREADY_REGISTERED_IN_BRAINTREE_VAULT') . '</td></tr>
							 </table>';
		}
