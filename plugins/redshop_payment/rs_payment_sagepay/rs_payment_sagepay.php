...
/**
 * PlgRedshop_PaymentRs_Payment_SagePay class.
 *
...
class PlgRedshop_PaymentRs_Payment_SagePay extends JPlugin
{
	...
	/**
	 * [onNotifyPaymentrs_payment_sagepay description]
	 *
	 * @param   [string]  $element  [plugin name]
	...
	public function onNotifyPaymentrs_payment_sagepay($element, $request)
	{
		...
		if (($strStatus == "OK") || ($strStatus == "AUTHENTICATED") || ($strStatus == "REGISTERED"))
		{
			if ($debugMode == 1)
			{
				$paymentMessage = $strStatusDetail;
			}
			else
			{
				$paymentMessage = JText::_('COM_REDSHOP_ORDER_PLACED');
			}
		...
		else
		{
			...
			{
				$paymentMessage = JText::_('COM_REDSHOP_ORDER_NOT_PLACED');
			}
		...
		$values->log = $paymentMessage;
		$values->msg = $paymentMessage;

		return $values;
	}

	/**
	 * [onCapture_Paymentrs_payment_sagepay description]
	 *
	 * @param   [string]  $element  [plugin name]
	 * @param   [array]   $data     [data params]
	 *
	...
	public function requestPost($url, $data)
	{
		...
		$session->set('rawresponse', $rawresponse);
	 ...
	 *
	 * @param   [string]  $thisString  [token string]
	 *
	 * @return  [array]   $output
	 */
	public function getToken($thisString)
	{
		...
		for ($i = 0, $in = count($results); $i < $in; $i++)
		{
			// Get the start point of the value
			$valueStart = $results[$i]->start + strlen($results[$i]->token) + 1;

			// Get the length of the value
			if ($i == (count($results) - 1))
			{
				$output[$results[$i]->token] = substr($thisString, $valueStart);
			}
			else
			{
				$valueLength = $results[$i + 1]->start - $results[$i]->start - strlen($results[$i]->token) - 2;
				$output[$results[$i]->token] = substr($thisString, $valueStart, $valueLength);
			}
		}
	...
	/**
	 * [decryptAes  Decode a returned string from SagePay.]
	 *
	...
	/**
	 * [removePKCS5Padding Remove PKCS5 Padding from a string.]
	 *
	...
	/**
	 * [arrayToQueryString Convert a data array to a query string ready to post.]
	 *
	 * @param   array    $data        [The data array.]
	 * @param   string   $delimiter   [Delimiter used in query string]
