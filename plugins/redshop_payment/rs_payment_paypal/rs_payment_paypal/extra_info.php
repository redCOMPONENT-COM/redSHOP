...
$paypalPostData['discount_amount_cart'] = round(
    RedshopHelperCurrency::convert($data['order']->order_discount, '', $paymentCurrency),
    2
);
$paypalPostData['discount_amount_cart'] += round(
    RedshopHelperCurrency::convert($data['order']->special_discount, '', $paymentCurrency),
    2
);
...
switch ($this->params->get("payment_oprand")) {
    case '-':
        $paypalPostData['discount_amount_cart'] += round(
            RedshopHelperCurrency::convert($data['order']->payment_discount, '', $paymentCurrency),
            2
        );
        break;

    case '+':
        $paypalPostData['handling_cart'] = round(
            RedshopHelperCurrency::convert($data['order']->payment_discount, '', $paymentCurrency),
            2
        );
        break;
...
for ($i = 0, $countItems = count($items); $i < $countItems; $i++) {
    ...
    $paypalCartItems["item_name_" . $index] = strip_tags(
        str_replace('"', "'", $item->order_item_name) . ' (' . $item->order_item_sku . ')'
    );
