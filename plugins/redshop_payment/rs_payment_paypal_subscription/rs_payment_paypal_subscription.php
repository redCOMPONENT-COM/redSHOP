            ...
            else
            {
                ...
                if ($row->store())
                {
                    ...
                    for ($ri = 0; $ri < count($main_orderitemdetail); $ri++)
                    {
                        ...
                        if ($producthelper->checkProductDownload($main_orderitemdetail[$ri]->product_id))
                        {
                            ...
                            for ($j = 0; $j < count($medianame); $j++)
                            {
                                ...
                                $producthelper->insertProductDownload($main_orderitemdetail[$ri]->product_id, $row->user_id, $new_oid, $medianame[$j]->media_name, $product_serial_number->serial_number);
                            }
                        }
                        ...
                        if ($rowitem->store())
                        {
                            ...
                            if (count($OrderItemAttrDetail) > 0)
                            {
                                for ($at = 0; $at < count($OrderItemAttrDetail); $at++)
                                {
                                    ...
                                    $attdata->section_id        = $OrderItemAttrDetail[$at]->section_id;
                                    $attdata->section           = $OrderItemAttrDetail[$at]->section;
                                    $attdata->parent_section_id = $OrderItemAttrDetail[$at]->parent_section_id;
                                    $attdata->section_name      = $OrderItemAttrDetail[$at]->section_name;
                                    $attdata->section_price     = $OrderItemAttrDetail[$at]->section_price;
                                    $attdata->section_vat       = $OrderItemAttrDetail[$at]->section_vat;
                                    $attdata->section_oprand    = $OrderItemAttrDetail[$at]->section_oprand;
                                    $attdata->is_accessory_att  = $OrderItemAttrDetail[$at]->is_accessory_att;
                                    $attdata->store();
                                }
                            }
                        }
                    ...
                    if (ECONOMIC_INTEGRATION == 1 && ECONOMIC_INVOICE_DRAFT != 2)
                    {
                        ...
                        $economic->createInvoiceInEconomic($new_oid, $economicdata);

                        if (ECONOMIC_INVOICE_DRAFT == 0)
                        {
                            $bookinvoicepdf = $economic->bookInvoiceInEconomic($new_oid, 1);

                            if (is_file($bookinvoicepdf))
                            {
                                $redshopMail->sendEconomicBookInvoiceMail($new_oid, $bookinvoicepdf);
                            }
                        }
                    }
                ...
                if ($status == 'Completed')
                {
                    $this->updateFirstRecurringPayment($db, $row->order_id, $subscr_id);
                    $values->order_status_code         = $verify_status;
                    $values->order_payment_status_code = 'Paid';
                    $values->log                       = JTEXT::_('ORDER_PLACED');
                    $values->msg                       = JTEXT::_('ORDER_PLACED');
