 ...
 * @subpackage  PlgEconomicEconomic
 *
...
/**
 *  PlgEconomicEconomic installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.7.0
 */
class PlgEconomicEconomicInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->q('economic'))
					->where($db->qn('folder')) . ' = ' . $db->q('economic')
			)->loadResult();
