    ...
    public function processImport($table, $data)
    {
        ...
        if ($hasSubPropertyName) {
            ...
            $query         = $db->getQuery(true)
                ...
                ->where(
                    $db->quoteName('subattribute_color_name') . ' = ' . $db->quote($data['subattribute_color_name'])
                );
            ...
            $subPropertyTable->subattribute_color_main_image = isset($data['media_name']) ? basename(
                $data['media_name']
            ) : '';
