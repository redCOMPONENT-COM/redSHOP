...
class PlgRedshop_ImportProduct extends AbstractImportPlugin
{
    ...
    public function processImport($table, $data)
    {
        ...
        if (isset($data['product_s_desc'])) {
            $data['product_s_desc'] = html_entity_decode(
                str_replace(array("\r", "\n", '\r', '\n', "\\"), '', $data['product_s_desc'])
            );
        }

        if (isset($data['product_desc'])) {
            $data['product_desc'] = html_entity_decode(
                str_replace(array("\r", "\n", '\r', '\n', "\\"), '', $data['product_desc'])
            );
        }
    ...
    public function importAttributesData($data)
    {
        ...
        if ($hasPropertyName) {
            ...
            if (!$hasSubPropertyName) {
                ...
                $propertyTable->set(
                    'property_image',
                    isset($data['property_image']) ? basename($data['property_image']) : ''
                );
                $propertyTable->set(
                    'property_main_image',
                    isset($data['property_main_image']) ? basename($data['property_main_image']) : ''
                );
                ...
                // Property image
                if (!empty($data['property_image']) && getimagesize($data['property_image'])) {
        ...
        if ($hasSubPropertyName) {
            ...
            $oprand                                     = in_array(
                $data['subattribute_color_oprand'],
                array('+', '-', '*', '/', '=')
            ) ? $data['subattribute_color_oprand'] : '';
            $subPropertyTable->oprand                   = $oprand;
            $subPropertyTable->subattribute_color_image = isset($data['subattribute_color_image']) ? basename(
                $data['subattribute_color_image']
            ) : '';
            ...
            // Sub-property image
            if (!empty($data['subattribute_color_image']) && getimagesize($data['subattribute_color_image'])) {
    ...
    public function importAdditionalDocuments($productId = 0, $data = array())
    {
        ...
        $alternateTexts = !empty($data['document_alternattext']) ? explode(
            '#',
            $data['document_alternattext']
        ) : array();
    ...
    public function importAdditionalDownloads($productId = 0, $data = array())
    {
        ...
        $alternateTexts = !empty($data['download_alternattext']) ? explode(
            '#',
            $data['download_alternattext']
        ) : array();
