 ...
 * @since  1.0
 */
class PlgRedshop_ImportShopper_group_attribute_price extends AbstractImportPlugin
{
    /**
     * @var string
     */
    ...
    public function processImport($table, $data)
    {
        ...
        if (!empty($data['discount_start_date'])) {
            $data['discount_start_date'] = is_numeric(
                $data['discount_start_date']
            ) ? $data['discount_start_date'] : strtotime($data['discount_start_date']);
        }

        if (!empty($data['discount_end_date'])) {
            $data['discount_end_date'] = is_numeric(
                $data['discount_end_date']
            ) ? $data['discount_end_date'] : strtotime($data['discount_end_date']);
        }
