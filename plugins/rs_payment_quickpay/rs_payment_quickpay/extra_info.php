 ...
 */
	    ...
	    //  $rand = md5(rand());
		 // $randomPrefix = substr($rand, 0, 4);
          ...
          $ok_page= JURI::base()."index.php?option=com_redshop&view=order_detail&Itemid=$Itemid&oid=".$data['order_id'];
 	      $error_page  = JURI::base()."index.php?option=com_redshop&view=order_detail&controller=order_detail&Itemid=".$Itemid."&task=notify_payment&payment_plugin=rs_payment_quickpay&orderid=".$data['order_id'];
 	      $result_page = JURI::base()."index.php?option=com_redshop&view=order_detail&controller=order_detail&Itemid=".$Itemid."&task=notify_payment&payment_plugin=rs_payment_quickpay&orderid=".$data['order_id'];
	      ...
	      $ipaddress=$_SERVER['REMOTE_ADDR'];
	 	  $md5check = md5($protocol.$msgtype.$merchant_id.$quickpay_language.$qp_order_id.$order_amount.$currency_code.$ok_page.$error_page.$result_page.$autocapture.$cardtypelock.$description.$ipaddress.$testmode.$md5word);
	...
	<form action="https://secure.quickpay.dk/form/" method="post" name="frmQuickpay" id="frmQuickpay">
	   ...
	   <input name="continueurl" type="hidden" value="<?php echo $ok_page;?>" />
	    ...
	    <input type="hidden" name="md5word" value="<?php echo $md5word;?>">
