    ...
    */
 	function onPrePayment_rs_payment_firstdata($element, $data)
    {
		...
		// setting the path where cURL can find the certificate to verify the Info directly from the API Manual Below:
		 curl_setopt($ch, CURLOPT_SSLCERT, JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$plugin.DS.'certificates/WS'.$store_id.'._.1.pem');
		 curl_setopt($ch, CURLOPT_SSLKEY, JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$plugin.DS.'certificates/WS'.$store_id.'._.1.key');
