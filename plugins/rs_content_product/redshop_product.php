...
$mainframe->registerEvent( 'onPrepareContent', 'plgContentredshop_product' );
...
function plgContentredshop_product( &$row, &$params, $page=0 )
{
	if (preg_match_all("/{redshop:.+?}/", $row->text, $matches, PREG_PATTERN_ORDER) > 0)
	{
		...
		$red_params = new JParameter( $plugin->params );
		...
		for($i=0;$i<count($matches);$i++)
		{
			...
			$middlepath = "components/com_redshop/assets/images/product/";
		 	...
		 	$product_img = $url.$middlepath.$product_image;
		...
		$prtemplate = str_replace ( $pimg_tag, $thum_image.$hidden_thumb_image, $prtemplate );
	   				...
	   				if($product->product_on_sale && $product_price_discount > 0)
	   				{
	   					if($product_price > $product_price_discount)
	   					{
							...
							if($show_discountpricelayout)
							{
								...
								$pr_price =  "<div id='mod_redsavedprice' class='mod_redsavedprice'>".JText::_('PRODCUT_PRICE_YOU_SAVED').' '.$producthelper->getProductFormattedPrice($s_price)."</div>";
							}
							else
							{
								$product_price = $product_price_discount ;
			...
			{
				$read_more="<a href='".$link."'>".JText::_('TXT_READ_MORE')."</a>";
