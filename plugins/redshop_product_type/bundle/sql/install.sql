...
INSERT INTO `#__redshop_template` (`template_name`, `template_section`, `template_desc`, `order_status`, `payment_methods`, `published`, `shipping_methods`, `checked_out`, `checked_out_time`) VALUES
	('bundle', 'bundle_template', '<div class="attribute_listing table-responsive"><table class="table table-striped"><thead><tr><th>{property_number_lbl}</th><th>{property_name_lbl}</th><th></th><th> </th></tr></thead><tbody><tr><td>{bundle_number}</td><td>{bundle_name}</td><td align="center">{product_stock_amount_image}</td><td>{bundle_select}</td></tr><tr><td>{property_number}</td><td>{property_name}</td><td align="center">{property_stock_image}</td><td>{property_select}</td></tr></tbody></table></div>', '', '', 1, '', 0, '0000-00-00 00:00:00');