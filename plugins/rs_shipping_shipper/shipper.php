...
class plgredshop_shippingshipper extends JPlugin
{
	...
	function onListRates(&$d)
	{
		...
		for($i=0; $i<count($ratelist); $i++)
		{
			...
			$shipping_rate_id 			= $shippinghelper->encryptShipping( __CLASS__."|".$shipping->name."|".$rs->shipping_rate_name."|".number_format( $rs->shipping_rate_value, 2, '.', '' )."|".$rs->shipping_rate_id."|single|".$shippingVatRate) ;
