    ...
    */
 	function onPrePayment($element, $data)
    {
 	  ...
 	  function onNotifyPaymentrs_payment_sagepay($element, $request){
		...
		if ($strStatus=="OK")
			$strDBStatus=JText::_("SAGEPAY_AUTHORISED");//"AUTHORISED - The transaction was successfully authorised with the bank.";
		elseif ($strStatus=="MALFORMED")
			$strDBStatus=JText::_("SAGEPAY_MALFORMED"). mysql_real_escape_string(substr($strStatusDetail,0,255));//"MALFORMED - The StatusDetail was:" . mysql_real_escape_string(substr($strStatusDetail,0,255));
		elseif ($strStatus=="INVALID")
			$strDBStatus=JText::_("SAGEPAY_INVALID"). mysql_real_escape_string(substr($strStatusDetail,0,255));//"INVALID - The StatusDetail was:" . mysql_real_escape_string(substr($strStatusDetail,0,255));
		elseif ($strStatus=="NOTAUTHED")
			$strDBStatus=JText::_("SAGEPAY_DECLINED");//"DECLINED - The transaction was not authorised by the bank.";
		elseif ($strStatus=="REJECTED")
			$strDBStatus=JText::_("SAGEPAY_REJECTED");//"REJECTED - The transaction was failed by your 3D-Secure or AVS/CV2 rule-bases.";
		elseif ($strStatus=="AUTHENTICATED")
			$strDBStatus=JText::_("SAGEPAY_AUTHENTICATED");//"AUTHENTICATED - The transaction was successfully 3D-Secure Authenticated and can now be Authorised.";
		elseif ($strStatus=="REGISTERED")
			$strDBStatus=JText::_("SAGEPAY_REGISTERED");//"REGISTERED - The transaction was could not be 3D-Secure Authenticated, but has been registered to be Authorised.";
		elseif ($strStatus=="ERROR")
			$strDBStatus=JText::_("SAGEPAY_ERROR"). mysql_real_escape_string($strStatusDetail);//"ERROR - There was an error during the payment process.  The error details are: " . mysql_real_escape_string($strStatusDetail);
		else
			$strDBStatus=JText::_("SAGEPAY_UNKNOWN"). mysql_real_escape_string($strStatus) . ", with StatusDetail:" . mysql_real_escape_string($strStatusDetail);//"UNKNOWN - An unknown status was returned from Sage Pay.  The Status was: " . mysql_real_escape_string($strStatus) . ", with StatusDetail:" . mysql_real_escape_string($strStatusDetail);
		...
		if (($strStatus=="OK") || ($strStatus=="AUTHENTICATED") || ($strStatus=="REGISTERED"))
		{
			...
			if($debug_mode == 1)
			{
			  ...
			  $payment_message= JTEXT::_('ORDER_PLACED');
			}
		    ...
		    if($debug_mode == 1)
			{
			  ...
			  $payment_message= JTEXT::_('ORDER_NOT_PLACED');
			}
