...
/**
 *  PlgAjaxXmlCron installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.7.0
 */
class PlgAjaxXmlCronInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->q('xmlcron'))
					->where($db->qn('folder')) . ' = ' . $db->q('ajax')
			)->loadResult();
