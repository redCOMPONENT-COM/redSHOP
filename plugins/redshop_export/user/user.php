...
class PlgRedshop_ExportUser extends AbstractExportPlugin
{
    ...
    protected function getQuery()
    {
        return $this->db->getQuery(true)
            ...
            ->select(
                'GROUP_CONCAT(' . $this->db->qn('ug.group_id') . ' SEPARATOR ' . $this->db->quote(
                    ','
                ) . ') AS ' . $this->db->qn('usertype')
            )
            ->select(
                array(
                    $this->db->qn('u.block'),
            ...
            ->innerJoin(
                $this->db->qn('#__user_usergroup_map', 'ug') . ' ON ' . $this->db->qn(
                    'ug.user_id'
                ) . ' = ' . $this->db->qn('ui.user_id')
            )
            ->leftjoin(
