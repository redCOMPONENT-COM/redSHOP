...
class ProductsCheckoutDotpayCest
{
	...
	public function testDotpayPaymentPlugin(AcceptanceTester $I, $scenario)
	{
		...
		$pathToPlugin = $I->getConfig('repo folder') . 'plugins/redshop_payment/rs_payment_dotpay/';
		$I->installExtensionFromFolder($pathToPlugin, 'Plugin');

		$checkoutAccountInformation = array(
			'customerId' => 'javier.gomez',
			'password' => '2003ABC$$$a',
		);
	...
	private function updateDotpayPaymentPlugin(AcceptanceTester $I, $customerId, $password)
	{
		...
		$I->waitForElement(['id' => "jform_params_dotpay_customer_id"], 30);
		$I->fillField(['id' => "jform_params_dotpay_customer_id"], $customerId);
		$I->fillField(['id' => "jform_params_dotpay_pin"], $password);
...
$I->pauseExecution();
		...
		$I->click(['xpath' => "//div[@id='rs_payment_dotpay']//label//input"]);
		$I->click("Checkout");
