...
class newslettersubscr_detailModelnewslettersubscr_detail extends JModelLegacy
{
    ...
    function getuserfullname2($uid)
    {
        ...
        {
            $fullname = $this->_username->firstname . " " . $this->_username->lastname . ($this->_username->username != "" ? " (" . $this->_username->username . ")" : "");
        }
        else
        {
            $fullname = "";
    ...
    function getnewslettersbsc($subsc = array())
    {
        ...
        $query = 'SELECT ns.*,ns.name as subscribername,n.name' . ' FROM ' . $this->_table_prefix . 'newsletter_subscription as ns,' . $this->_table_prefix . 'newsletter as n WHERE ns.newsletter_id=n.newsletter_id ' . $where;
        $this->_db->setQuery($query);
        return $this->_db->loadObjectlist();
    }
