...
class RedshopModelRating_detail extends RedshopCoreModelDetail
{
    ...
    {
        $row = $this->getTable('product_rating');
    ...
    public function favoured($cid = array(), $publish = 1)
    {
        if (count($cid))
        {
            ...
            $query = 'UPDATE ' . $this->_table_prefix . 'product_rating' . ' SET favoured = ' . intval($publish) . ' WHERE rating_id IN ( ' . $cids . ' )';
            $this->_db->setQuery($query);
            if (!$this->_db->query())
            {
                $this->setError($this->_db->getErrorMsg());
