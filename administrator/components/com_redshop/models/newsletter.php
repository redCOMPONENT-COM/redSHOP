...
class newsletterModelnewsletter extends JModelLegacy
{
    ...
    function _buildQuery()
    {
        ...
        $query = 'SELECT distinct(n.newsletter_id),n.* FROM ' . $this->_table_prefix . 'newsletter AS n ' . 'WHERE 1=1 ' . $where . $orderby;
        return $query;
    }
    ...
    function _buildContentOrderBy()
    {
        ...
        $filter_order     = $mainframe->getUserStateFromRequest($this->_context . 'filter_order', 'filter_order', 'newsletter_id');
    ...
    function listallsubscribers($n = 0)
    {
        ...
        if ($cityfilter != "")
        {
            $query = 'SELECT uf.firstname,uf.lastname,u.username,ns.* FROM ' . $this->_table_prefix . 'newsletter_subscription AS ns ' . ', ' . $this->_table_prefix . 'users_info AS uf ' . ', #__users AS u ' . ', ' . $this->_table_prefix . 'fields_data AS f ' . 'WHERE ns.newsletter_id="' . $n . '" ' . 'AND ns.published=1 AND u.id=ns.user_id ' . 'AND ns.user_id=uf.user_id ' . 'AND uf.address_type LIKE "BT" ' . $where . $between . $country;
        }
        else
        {
            $query = "SELECT ns.*,u.username,uf.address_type,uf.firstname,uf.lastname FROM " . $this->_table_prefix . "newsletter_subscription AS ns " . "LEFT JOIN #__users AS u ON ns.user_id = u.id " . "LEFT JOIN " . $this->_table_prefix . "users_info AS uf ON uf.user_id = ns.user_id " . "WHERE (uf.address_type = 'BT' OR uf.address_type IS NULL) " . "AND ns.newsletter_id='" . $n . "' " . "AND published=1 " . $where . $between . $country . $shopper_group_ids;
        }
    ...
    {
        $query = 'SELECT IFNULL(u.email,s.email) AS email,IFNULL(u.username,s.name) AS username FROM ' . $this->_table_prefix . 'newsletter_subscription AS s ' . 'LEFT JOIN #__users as u ON  u.id=s.user_id ' . 'WHERE s.subscription_id="' . $subscriberid . '" ' . 'AND published=1 ';
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    function category($uid)
    {
        ...
        if (count($categories) > 0)
        {
            ...
            $query          = "SELECT * FROM " . $this->_table_prefix . "product_category_xref AS pcx " . "LEFT JOIN " . $this->_table_prefix . "order_item AS oi ON pcx.product_id = oi.product_id " . "LEFT JOIN " . $this->_table_prefix . "orders AS o ON o.order_id = oi.order_id " . "WHERE o.user_id='" . $uid . "' " . "AND category_id IN ('" . $categories_ids . "') ";
            $this->_db->setQuery($query);
    ...
    function product($user_id)
    {
        ...
        if (count($product) > 0)
        {
            ...
            $query       = "SELECT o.* FROM " . $this->_table_prefix . "orders AS o " . "LEFT JOIN " . $this->_table_prefix . "order_item AS oi ON o.order_id=oi.order_id " . "WHERE o.user_id='" . $user_id . "' " . "AND product_id IN ('" . $product_ids . "') ";
            $this->_db->setQuery($query);
            $result = $this->_db->loadObjectlist();
            if (count($result) <= 0)
            {
    ...
    function order_user($uid)
    {
        ...
        $query = "SELECT COUNT(*) AS total,order_total FROM " . $this->_table_prefix . "orders " . "GROUP BY user_id " . "HAVING total " . $cond . $order_total . " AND user_id =" . $uid;
        $this->_db->setQuery($query);
    ...
    function getProduct()
    {
        $query = "SELECT product_name as text, product_id as value from " . $this->_table_prefix . "product" . " ORDER BY product_id	";
        $this->_db->setQuery($query);
        return $this->_db->loadObjectlist();
    }
    ...
    {
        $query = "SELECT * FROM " . $this->_table_prefix . "newsletter_subscription " . "where newsletter_id='" . $newsletter_id . "' " . "AND subscription_id='" . $subscription_id . "' ";
        $this->_db->setQuery($query);
