...
class JInstallerShipping extends JObject
{
    ...
    public function install()
    {
        ...
        $query = 'SELECT `shipping_id`' . ' FROM `#__' . TABLE_PREFIX . '_shipping_method`' . ' WHERE plugin = "' . $pname . '" OR shipping_class = ' . $db->Quote($shipping_class);
    ...
    public function _rollback_plugin($arg)
    {
        ...
        $query = 'DELETE' . ' FROM `#__' . TABLE_PREFIX . '_shipping_method`' . ' WHERE shipping_id=' . (int)$arg['shipping_id'];
        $db->setQuery($query);
