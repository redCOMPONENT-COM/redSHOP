...
class xmlimport_detailModelxmlimport_detail extends JModel
{
    ...
    function updateFile()
    {
        ...
        if (array_key_exists("xmlimport_url", $post) && $post["xmlimport_url"] != "")
        {
            ...
            {
                if ($this->_data->filename != "" && is_file(JPATH_COMPONENT_SITE . DS . "assets/xmlfile/import" . DS . $this->_data->filename))
                {
    ...
    function auto_syncpublish($cid = array(), $publish = 1)
    {
        if (count($cid))
        {
            ...
            $query = 'UPDATE ' . $this->_table_prefix . 'xml_import ' . ' SET auto_sync = ' . intval($publish) . ' WHERE xmlimport_id IN ( ' . $cids . ' )';
            $this->_db->setQuery($query);
            if (!$this->_db->query())
            {
                $this->setError($this->_db->getErrorMsg());
