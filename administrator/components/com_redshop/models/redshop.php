...
class redshopModelredshop extends RedshopCoreModel
{
    ...
    public function demoContentInsert()
    {
        ...
        $this->_db->setQuery($q);
        $cols = $this->_db->loadObjectList('Key_name');
    ...
    {
        $query = 'SELECT o.*,CONCAT(u.firstname," ",u.lastname) AS name FROM #__redshop_order_users_info AS u ' . 'LEFT JOIN #__redshop_orders AS o ON u.order_id = o.order_id AND u.address_type="BT" ' . 'ORDER BY o.order_id desc limit 0, 5';
        $this->_db->setQuery($query);
    ...
    function gettotalOrder($id = 0)
    {
        ...
        $userquery           = "SELECT SUM(order_total) AS order_total, count(*) AS tot_order FROM " . $this->_table_prefix . "orders " . "WHERE `user_info_id`='" . $id . "' ";
        $this->_db->setQuery($userquery);
        return $this->_db->loadObject();
    }
    ...
    {
        $this->_table_prefix = '#__' . TABLE_PREFIX . '_';
        $query               = 'SELECT  SUM(o.order_total) AS order_total ' . 'FROM ' . $this->_table_prefix . 'orders AS o ' . 'LEFT JOIN ' . $this->_table_prefix . 'users_info as uf ON o.user_id =uf.user_id' . ' AND address_type LIKE "BT" ' . 'WHERE o.user_id = ' . $user_id . ' and  (o.order_status = "C" OR o.order_status = "PR" OR o.order_status = "S")';
        $this->_db->setQuery($query);
        return $this->_db->loadObject();
    }
    ...
    {
        $this->_table_prefix = '#__' . TABLE_PREFIX . '_';
        $query               = 'SELECT  (SUM(o.order_total)/ COUNT( DISTINCT o.user_id ) ) AS avg_order ' . 'FROM ' . $this->_table_prefix . 'orders AS o ' . 'WHERE o.user_id =' . $user_id . ' and (o.order_status = "C" OR o.order_status = "PR" OR o.order_status = "S") ';
        $this->_db->setQuery($query);
        return $this->_db->loadObject();
    }
