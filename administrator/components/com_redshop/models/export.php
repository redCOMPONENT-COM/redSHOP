...
class RedshopModelExport extends RedshopCoreModel
{
    public $_total = null;
    ...
    private function loadProducts()
    {
        ...
        if (count($cur) > 0)
        {
            for ($p = 0; $p < count($cur); $p++)
            {
                ...
                if ($fields + 1)
                {
                    $query = "SELECT pcx.category_id,c.category_name FROM " . $this->_table_prefix . "product_category_xref as pcx" . " LEFT JOIN " . $this->_table_prefix . "category c ON c.category_id = pcx.category_id" . " WHERE product_id ='" . $row['product_id'] . "' ";
                ...
                if ($fields + 2)
                {
                    $query = "SELECT CONCAT(`product_number`,'~',`accessory_price`) as accsdata  FROM `" . $this->_table_prefix . "product_accessory` as pa " . " LEFT JOIN " . $this->_table_prefix . "product p ON p.product_id = pa.child_product_id" . " WHERE pa.`product_id` = '" . $row['product_id'] . "' ";
                    $this->_db->setQuery($query);
    ...
    public function getProductExtrafield()
    {
        ...
        $query = "SELECT fieldid, data_txt, itemid FROM " . $this->_table_prefix . "fields_data " . "WHERE section=1 ";
        $this->_db->setQuery($query);
