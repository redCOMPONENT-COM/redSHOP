...
class RedshopModelStockroom_listing extends RedshopCoreModel
{
    public $_total = null;
    ...
    public function _buildQuery()
    {
        ...
        if ($stockroom_type == 'subproperty')
        {
            ...
            $leftjoin = "LEFT JOIN " . $this->_table_prefix . "product_attribute_property AS ap ON asp.subattribute_id = ap.property_id " . "LEFT JOIN " . $this->_table_prefix . "product_attribute AS a ON a.attribute_id = ap.attribute_id " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = a.product_id ";
        }
        else if ($stockroom_type == 'property')
        {
            $field    = ", ap.*, property_id AS section_id ";
            $table    = "product_attribute_property AS ap ";
            $leftjoin = "LEFT JOIN " . $this->_table_prefix . "product_attribute AS a ON a.attribute_id = ap.attribute_id " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = a.product_id ";
        }
        else
        {
            $table = "product AS p ";
        }
        $query = "SELECT distinct p.product_id, p . * " . $field . "FROM " . $this->_table_prefix . $table . $leftjoin . "LEFT JOIN " . $this->_table_prefix . "product_category_xref AS pcx ON pcx.product_id=p.product_id " . "WHERE p.product_id is not NULL " . $and . $orderby;
        return $query;
    }
    ...
    public function storeStockroomQuantity($stockroom_type, $sid, $pid, $quantity = "", $preorder_stock = 0, $ordered_preorder = 0)
    {
        ...
        if (count($list) > 0)
        {
            ...
            {
                $query = "DELETE FROM " . $this->_table_prefix . $table . "_stockroom_xref " . " WHERE stockroom_id='" . $sid . "' " . $product . $section;
            }
            else
            {
            ...
            else
            {
                ...
                if ($preorder_stock != "" || $quantity != "")
                {
                    ...
                    else
                    {
                        ...
                        if ($stockroom_type != 'product')
                        {
                            $query = "INSERT INTO " . $this->_table_prefix . $table . "_stockroom_xref " . "(section_id, stockroom_id, quantity, section , preorder_stock, ordered_preorder) " . "VALUES ('" . $pid . "', '" . $sid . "', '" . $quantity . "', '" . $stockroom_type . "', '" . $preorder_stock . "','0') ";
                        }
                        else
                        {
                            $query = "INSERT INTO " . $this->_table_prefix . $table . "_stockroom_xref " . "(product_id, stockroom_id, quantity, preorder_stock, ordered_preorder ) " . "VALUES ('" . $pid . "', '" . $sid . "', '" . $quantity . "', '" . $preorder_stock . "','0' ) ";
                        }
                    }
                }
            }
    ...
    public function getcontainerproducts($product_ids = 0)
    {
        ...
        {
            $query = "SELECT * FROM  " . $this->_table_prefix . "stockroom as s , " . $this->_table_prefix . "product_stockroom_xref AS ps " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON ps.product_id = p.product_id " . "WHERE ps.stockroom_id = s.stockroom_id " . $and;
            ;
        }
        $this->_db->setQuery($query);
