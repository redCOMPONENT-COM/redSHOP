...
class RedshopModelSearch extends RedshopCoreModel
{
    ...
    public function _buildQuery()
    {
        ...
        {
            $query = 'SELECT a.sectionid,a.catid, a.id AS value, a.title AS text ' . 'FROM #__content AS a ' . 'WHERE a.state = 1 ' . 'AND a.title LIKE "' . $this->_search . '%"';
            $this->_db->setQuery($query);
            ...
            for ($j = 0; $j < count($rows); $j++)
            {
                ...
                {
                    $query = 'SELECT a.id AS value, a.title AS text ' . 'FROM #__content AS a ' . 'LEFT JOIN #__categories AS cc ON cc.id = a.catid ' . 'LEFT JOIN #__sections AS s ON s.id = cc.section AND s.scope = "content" ' . 'LEFT JOIN #__groups AS g ON a.access = g.id ' . 'WHERE (cc.published = 1 AND s.published = 1) ' . 'AND a.state = 1 ' . 'AND a.title LIKE "' . $this->_search . '%"';
                    $this->_db->setQuery($query);
        ...
        else if ($this->_addreduser == 1)
        {
            ...
            $query = "SELECT uf.user_id AS id, CONCAT(uf.firstname,' ', uf.lastname, IF(u.username!='', CONCAT( ' (',u.username,')'), '' )) AS value, uf.user_email AS value_number " . "FROM " . $this->_table_prefix . "users_info AS uf " . "LEFT JOIN #__users AS u ON uf.user_id=u.id " . "WHERE (u.username LIKE '" . $this->_search . "%' " . "OR uf.firstname LIKE '" . $this->_search . "%' " . "OR uf.lastname LIKE '" . $this->_search . "%') " . "AND (uf.address_type LIKE 'BT')";
        }
        ...
        else if ($this->_related == 1)
        {
            ...
            {
                $query = "SELECT related_id " . "FROM " . $this->_table_prefix . "product_related " . "WHERE product_id='" . $this->_product_id . "' ";
                $this->_db->setQuery($query);
            ...
            $query = "SELECT p.product_id AS id,p.product_name AS value,p.product_number as value_number " . "FROM " . $this->_table_prefix . "product as p " . "WHERE (p.product_name LIKE '" . $this->_search . "%' or p.product_number LIKE '" . $this->_search . "%') " . $and . " LIMIT 0,50 ";
        }
        else if ($this->_parent == 1)
        {
            ...
            $query = "SELECT p.product_id AS id,p.product_name AS value " . "FROM " . $this->_table_prefix . "product as p " . "WHERE p.product_name LIKE '" . $this->_search . "%' " . $and . " LIMIT 0,50 ";
        }
