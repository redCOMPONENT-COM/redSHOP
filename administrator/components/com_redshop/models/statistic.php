...
class RedshopModelStatistic extends RedshopCoreModel
{
    ...
    public function getMostPopular()
    {
        ...
        $result  = array();
        $query   = 'SELECT pv.created_date ' . 'FROM ' . $this->_table_prefix . 'pageviewer AS pv ' . 'WHERE pv.section="product" ' . 'ORDER BY pv.created_date ASC ';
        $this->_db->setQuery($query);
        $mindate = $this->_db->loadResult();

        $query              = 'SELECT FROM_UNIXTIME(pv.created_date,"' . $formate . '") AS viewdate ' . ', p.product_id, p.product_name, p.product_price, count(*) AS visited ' . 'FROM ' . $this->_table_prefix . 'pageviewer AS pv ' . 'LEFT JOIN ' . $this->_table_prefix . 'product p ON p.product_id=pv.section_id ' . 'WHERE pv.section="product" ';
        $query1             = ' GROUP BY pv.section_id ' . 'ORDER BY visited desc ';
        ...
        if ($this->_filteroption && $mindate != "" && $mindate != 0)
        {
            while ($mindate < strtotime($today))
            {
                ...
                $q    = $query . " AND created_date > " . strtotime($list->preday) . " AND created_date <= " . strtotime($today) . $query1;
                $this->_db->setQuery($q);
    ...
    public function getBestSellers()
    {
        ...
        $formate = $this->getDateFormate();
        $result  = array();
        $query   = 'SELECT cdate ' . 'FROM ' . $this->_table_prefix . 'order_item ' //				.'WHERE order_status = "C" '
            . 'ORDER BY cdate ASC ';
        $this->_db->setQuery($query);
        $mindate = $this->_db->loadResult();
        ...
        if ($this->_filteroption && $mindate != "" && $mindate != 0)
        {
            ...
            {
                $list = $this->getNextInterval($today);
                $q    = $query . " WHERE oi.cdate > " . strtotime($list->preday) . " AND oi.cdate <= " . strtotime($today) . $query1;
                $this->_db->setQuery($q);
                $rs = $this->_db->loadObjectList();
                for ($i = 0; $i < count($rs); $i++)
                {
    ...
    public function getNewProducts()
    {
        ...
        $formate = $this->getDateFormate();
        $result  = array();
        $query   = 'SELECT publish_date ' . 'FROM ' . $this->_table_prefix . 'product ' . 'ORDER BY publish_date ASC ';
        $this->_db->setQuery($query);
        $mindate = $this->_db->loadResult();

        $query              = 'SELECT product_id,product_name,product_price ' . ', DATE_FORMAT(publish_date,"' . $formate . '") AS viewdate ' . 'FROM ' . $this->_table_prefix . 'product ';
        ...
        if ($this->_filteroption && $mindate != "" && $mindate != 0)
        {
            ...
            {
                $list = $this->getNextInterval($today);
                $q    = $query . " WHERE publish_date > '" . $list->preday . "' " . " AND publish_date <= '" . $today . "' " . $query1;
                $this->_db->setQuery($q);
                $rs = $this->_db->loadObjectList();
                for ($i = 0; $i < count($rs); $i++)
                {
    ...
    public function getNewOrders()
    {
        ...
        $formate = $this->getDateFormate();
        $result  = array();
        $query   = 'SELECT cdate ' . 'FROM ' . $this->_table_prefix . 'orders ' . 'ORDER BY cdate ASC ';
        $this->_db->setQuery($query);
        $mindate = $this->_db->loadResult();

        $query            = 'SELECT uf.firstname, uf.lastname,o.order_id, o.order_total ' . ', FROM_UNIXTIME(o.cdate,"' . $formate . '") AS viewdate ' . 'FROM ' . $this->_table_prefix . 'orders AS o ' . 'LEFT JOIN ' . $this->_table_prefix . 'users_info as uf ON o.user_id=uf.user_id ' . 'AND address_type LIKE "BT" ';
        ...
        if ($this->_filteroption && $mindate != "" && $mindate != 0)
        {
            ...
            {
                $list = $this->getNextInterval($today);
                $q    = $query . " WHERE o.cdate > " . strtotime($list->preday) . " AND o.cdate <= " . strtotime($today) . $query1;
                $this->_db->setQuery($q);
                $rs = $this->_db->loadObjectList();
                for ($i = 0; $i < count($rs); $i++)
                {
    ...
    public function getTotalTurnover()
    {
        ...
        $query = 'SELECT cdate ' . 'FROM ' . $this->_table_prefix . 'orders ' . 'WHERE order_status = "C" OR order_status = "PR" OR order_status = "S" ' . 'ORDER BY cdate ASC ';
        $this->_db->setQuery($query);
        $mindate = $this->_db->loadResult();

        $query   = 'SELECT FROM_UNIXTIME(o.cdate,"' . $formate . '") AS viewdate, SUM(o.order_total) AS turnover ' . 'FROM ' . $this->_table_prefix . 'orders AS o ' . 'LEFT JOIN ' . $this->_table_prefix . 'users_info as uf ON o.user_id=uf.user_id ' //.'AND address_type LIKE "BT" '
        ...
        if ($this->_filteroption && $mindate != "" && $mindate != 0)
        {
            ...
            {
                $list = $this->getNextInterval($today);
                $q    = $query . " AND cdate > " . strtotime($list->preday) . " AND cdate <= " . strtotime($today) . $quesry1;
                $this->_db->setQuery($q);
                $rs = $this->_db->loadObjectList();
    ...
    public function getAvgOrderAmount()
    {
        ...
        $query              = 'SELECT FROM_UNIXTIME(o.cdate,"' . $formate . '") AS viewdate ' . ', (SUM(o.order_total)/ COUNT( DISTINCT o.user_id ) ) AS avg_order ' . 'FROM ' . $this->_table_prefix . 'orders AS o ' . 'WHERE (o.order_status = "C" OR o.order_status = "PR" OR o.order_status = "S") ';
    ...
    public function getAmountPrice()
    {
        ...
        $query              = 'SELECT firstname,lastname, FROM_UNIXTIME(o.cdate,"' . $formate . '") AS viewdate, MAX(o.order_total) AS order_total ' . 'FROM ' . $this->_table_prefix . 'orders AS o ' . 'LEFT JOIN ' . $this->_table_prefix . 'users_info as uf ON o.user_id=uf.user_id ' . 'AND address_type LIKE "BT" ' . 'WHERE (o.order_status = "C" OR o.order_status = "PR" OR o.order_status = "S") ';
        $query1             = ' GROUP by o.user_id ' . 'ORDER BY order_total desc ';
        ...
        if ($this->_filteroption && $mindate != "" && $mindate != 0)
        {
            ...
            {
                $list = $this->getNextInterval($today);
                $q    = $query . " AND cdate > " . strtotime($list->preday) . " AND cdate <= " . strtotime($today) . $query1;
                $this->_db->setQuery($q);
                $rs = $this->_db->loadObjectList();
                for ($i = 0; $i < count($rs); $i++)
                {
    ...
    public function getAmountSpentInTotal()
    {
        ...
        $query              = 'SELECT firstname,lastname, FROM_UNIXTIME(o.cdate,"' . $formate . '") AS viewdate, SUM(o.order_total) AS order_total ' . 'FROM ' . $this->_table_prefix . 'orders AS o ' . 'LEFT JOIN ' . $this->_table_prefix . 'users_info as uf ON o.user_id=uf.user_id ' . 'AND address_type LIKE "BT" ' . 'WHERE (o.order_status = "C" OR o.order_status = "PR" OR o.order_status = "S") ';
        $query1             = ' GROUP by o.user_id ' . 'ORDER BY order_total desc ';
        $this->_amountprice = $this->_getList($query . $query1);
    ...
    public function getPageViewer()
    {
        ...
        $formate = $this->getDateFormate();
        $result  = array();
        $query   = 'SELECT created_date ' . 'FROM ' . $this->_table_prefix . 'pageviewer ' . 'ORDER BY created_date ASC ';
        $this->_db->setQuery($query);
        $mindate = $this->_db->loadResult();
    ...
    public function getRedshopViewer()
    {
        ...
        $formate = $this->getDateFormate();
        $result  = array();
        $query   = 'SELECT created_date ' . 'FROM ' . $this->_table_prefix . 'siteviewer ' . 'ORDER BY created_date ASC ';
        $this->_db->setQuery($query);
        $mindate = $this->_db->loadResult();

        $query            = 'SELECT COUNT(*) AS viewer ' . 'FROM ' . $this->_table_prefix . 'siteviewer ';
        $this->siteviewer = $this->_getList($query);

        if ($this->_filteroption && $mindate != "" && $mindate != 0)
        {
            $query = 'SELECT FROM_UNIXTIME(created_date,"' . $formate . '") AS viewdate ' . 'FROM ' . $this->_table_prefix . 'siteviewer ';
            while ($mindate < strtotime($today))
            {
                $list = $this->getNextInterval($today);
                $q    = $query . " WHERE created_date > " . strtotime($list->preday) . " AND created_date <= " . strtotime($today);
                $this->_db->setQuery($q);
                $rs = $this->_db->loadObjectList();
                // COUNT(*) AS viewer
    ...
    public function getSectionDetail($section, $sectionid)
    {
        ...
        {
            case "product":
                $query = 'SELECT product_name AS sname, product_id AS id FROM ' . $this->_table_prefix . 'product ' . 'WHERE product_id = ' . $sectionid;
                $this->_db->setQuery($query);
                $return = $this->_db->loadObject();
                break;
            case "category":
                $query = 'SELECT category_name AS sname, category_id AS id FROM ' . $this->_table_prefix . 'category ' . 'WHERE category_id = ' . $sectionid;
                $this->_db->setQuery($query);
                $return = $this->_db->loadObject();
                break;
            case "manufacturers":
                $query = 'SELECT manufacturer_name AS sname, manufacturer_id AS id FROM ' . $this->_table_prefix . 'manufacturer ' . 'WHERE manufacturer_id = ' . $sectionid;
                $this->_db->setQuery($query);
                $return = $this->_db->loadObject();
                break;
        }
