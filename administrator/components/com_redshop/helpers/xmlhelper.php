                ...
                switch ($childSection)
                {
                    ...
                    default:
                        ...
                        for ($i = 0; $i < count($cat); $i++)
                        {
                            ...
                            {
                                if ($cat[$i]->Field == "category_description")
                                {
                                    $catcol[] = $cat[$i];
                                }
                                else
                                {
                                    if ($cat[$i]->Field == "category_template") //Start Code for display product_url
                                    {
                                        $cat[$i]->Field = "link";
                                        $catcol[]       = $cat[$i];
                                    ...
                                    {
                                        if ($cat[$i]->Field == "category_thumb_image") //Start Code for display delivertime
                                        {
                                        ...
                                        {
                                            if ($cat[$i]->Field == "category_full_image") //Start Code for display pickup
                                            {
                                                $cat[$i]->Field = "pickup";
                                                $catcol[]       = $cat[$i];
                                            ...
                                            {
                                                if ($cat[$i]->Field == "category_back_full_image") //Start Code for display charges
                                                {
                                                    $cat[$i]->Field = "charge";
                                                    $catcol[]       = $cat[$i];
                                                ...
                                                {
                                                    if ($cat[$i]->Field == "category_pdate") //Start Code for display freight
                                                    {
                                                        $cat[$i]->Field = "freight";
                                                        $catcol[]       = $cat[$i];
                                                    }
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        if ($prop == "delivertime")
                                        {
                                            $query = "SELECT * FROM " . $this->_table_prefix . "stockroom AS s " . "LEFT JOIN " . $this->_table_prefix . "product_stockroom_xref AS sx ON s.stockroom_id=sx.stockroom_id " . "WHERE product_id='" . $product_id . "' " . "ORDER BY s.stockroom_id ASC ";
                                            $this->_db->setQuery($query);
                                            $list = $this->_db->loadObject();
                                            for ($k = 0; $k < count($list); $k++)
                                            {
                                                ...
                                                {
                                                    if ($list->max_del_time == 2 && $list->max_del_time <= 3)
                                                    {
                                                        $val = "2";
                                                    }
                                                    else
                                                    {
                                                        if ($list->max_del_time == 4)
                                                        {
                                                            $val = "4";
                                                        }
                                                        else
                                                        {
                                                            if ($list->max_del_time == 5)
                                                            {
                                                                $val = "5";
                                                            }
                                                            else
                                                            {
                                                                if ($list->max_del_time >= 6 && $list->max_del_time <= 10)
                                                                {
                                                                    $val = "6,7,8,9,10";
                                                                }
                                                                else
                                                                {
                                                                    if ($list->max_del_time == "")
                                                                    {
                                                                        $val = "";
                                                                    }
                                                                }
...
//		        	$val = '<a href="'.JURI::root().'index.php?option=com_redshop&view=product&pid='.$product_id.'" target="_black">'.JURI::root().'index.php?option=com_redshop&view=product&pid='.$product_id.'</a>';
            ...
            case "product":
                for ($i = 0; $i < count($datalist); $i++)
                {
                    ...
                    if ($xmlimportdata->override_existing && $update)
                    {
                        ...
                        $query                          = "SELECT product_id FROM " . $this->_table_prefix . "product " . "WHERE product_number='" . $oldproduct_number . "' ";
                        $this->_db->setQuery($query);
                                ...
                                for ($j = 0; $j < count($value); $j++)
                                {
                                    if ($key == $xmlimportdata->stock_element_name)
                                    {
                                        if (isset($value[$j]['stockroom_name']))
                                        {
                                            ...
                                            if (trim($stockstring) != "")
                                            {
                                                $query = "UPDATE " . $this->_table_prefix . "stockroom AS s " . ", " . $this->_table_prefix . "product_stockroom_xref AS sx " . ", " . $this->_table_prefix . "product AS p " . "SET $stockstring " . "WHERE sx.stockroom_id=s.stockroom_id " . "AND sx.product_id=p.product_id " . "AND p.product_number='" . $oldproduct_number . "' " . "AND s.stockroom_name='" . $value[$j]['stockroom_name'] . "' ";
                                                $this->_db->setQuery($query);
                                                $this->_db->Query();
                                                $affected_rows = $this->_db->getAffectedRows();
                                                if (!$affected_rows)
                                                {
                                                    $query = "SELECT stockroom_id FROM " . $this->_table_prefix . "stockroom " . "WHERE stockroom_name='" . $value[$j]['stockroom_name'] . "'";
                                                    $this->_db->setQuery($query);
                                                    $stockroom_id = $this->_db->loadResult();
                                                    if (!$stockroom_id)
                                                    {
                                                        $query = "INSERT IGNORE INTO " . $this->_table_prefix . "stockroom " . "(stockroom_name) VALUES ('" . $value[$j]['stockroom_name'] . "')";
                                                        $this->_db->setQuery($query);
                                                        $this->_db->Query();
                                                        $stockroom_id = $this->_db->insertid();
                                                    }

                                                    $query = "INSERT IGNORE INTO " . $this->_table_prefix . "product_stockroom_xref " . "(stockroom_id,product_id,quantity) VALUES ('" . $stockroom_id . "','" . $product_id . "',0)";
                                                    $this->_db->setQuery($query);
                                                    $this->_db->Query();
                                    ...
                                    elseif ($key == $xmlimportdata->prdextrafield_element_name)
                                    {
                                        if (isset($value[$j]['fieldid']))
                                        {
                                            ...
                                            {
                                                $query = "UPDATE " . $this->_table_prefix . "fields_data AS fa " . ", " . $this->_table_prefix . "product AS p " . "SET $prdextstring " . "WHERE p.product_id=fa.itemid " . "AND fa.section='1' " . "AND fa.fieldid='" . $value[$j]['fieldid'] . "' " . "AND p.product_number='" . $oldproduct_number . "' ";
                                                $this->_db->setQuery($query);
                                                $this->_db->Query();
                                                $affected_rows = $this->_db->getAffectedRows();
                                                if (!$affected_rows)
                                                {
                                                    $query = "INSERT IGNORE INTO " . $this->_table_prefix . "fields_data " . "(fieldid,itemid,section) VALUES ('" . $value[$j]['fieldid'] . "','" . $product_id . "',1)";
                                                    $this->_db->setQuery($query);
                                                    $this->_db->Query();
                        ...
                        if (count($prdarray) > 0)
                        {
                            ...
                            $query    = "UPDATE " . $this->_table_prefix . "product " . "SET $upstring " . "WHERE product_number='" . $oldproduct_number . "' ";
                            $this->_db->setQuery($query);
                            $this->_db->Query();
                        }
                        if (count($catarray) > 0)
                        {
                            ...
                            {
                                $query = "SELECT category_id FROM " . $this->_table_prefix . "category " . "WHERE category_name='" . $catarray['category_name'] . "' ";
                                $this->_db->setQuery($query);
                                $category_id = $this->_db->loadResult();
                            }
                            if ($category_id == 0 && isset($catarray['category_name']) && $catarray['category_name'] != "")
                            {
                                $query = "INSERT IGNORE INTO " . $this->_table_prefix . "category " . "(category_name) VALUES ('" . $catarray['category_name'] . "')";
                                $this->_db->setQuery($query);
                                $this->_db->Query();
                                $category_id = $this->_db->insertid();

                                $query = "INSERT IGNORE INTO " . $this->_table_prefix . "category_xref " . "(category_parent_id,category_child_id) " . "VALUES ('0', '" . $category_id . "')";
                                $this->_db->setQuery($query);
                                $this->_db->Query();
                            }
                            if ($category_id != 0)
                            {
                                $query = 'DELETE FROM ' . $this->_table_prefix . 'product_category_xref ' . "WHERE product_id='" . $product_id . "' " . "AND category_id='" . $category_id . "' ";
                                $this->_db->setQuery($query);
                                $this->_db->Query();
                    ...
                    else
                    {
                        if (!empty($datalist[$i]['product_number']) && trim($datalist[$i]['product_name']) != "")
                        {
                            ...
                            if (count($prdkeysarray) > 0)
                            {
                                ...
                                $query       = "INSERT IGNORE INTO " . $this->_table_prefix . "product " . "($fieldstring) VALUES ($valuestring)";
                                $this->_db->setQuery($query);
                                $this->_db->Query();
                                                ...
                                                if (isset($value[$j]['stockroom_name']))
                                                {
                                                    ...
                                                    if (trim($fieldstring) != "")
                                                    {
                                                        ...
                                                        if ($stockroom_id)
                                                        {
                                                            ...
                                                            $query = "INSERT IGNORE INTO " . $this->_table_prefix . "product_stockroom_xref " . "($fieldstring) VALUES ($valuestring)";
                                                            $this->_db->setQuery($query);
                                                            $this->_db->Query();
                                                        }
                                                    }
                                            ...
                                            elseif ($key == $xmlimportdata->prdextrafield_element_name)
                                            {
                                                if (isset($value[$j]['fieldid']))
                                                {
                                                    ...
                                                    if (trim($fieldstring) != "")
                                                    {
                                                        ...
                                                        $query = "INSERT IGNORE INTO " . $this->_table_prefix . "fields_data " . "($fieldstring) VALUES ($valuestring)";
                                                        $this->_db->setQuery($query);
                                                        $this->_db->Query();
                                                    }
                                                }
                ...
                for ($i = 0; $i < count($datalist); $i++)
                {
                    ...
                    if ($xmlimportdata->override_existing && $update)
                    {
                        ...
                        while (list($key, $value) = each($datalist[$i]))
                        {
                            ...
                            elseif (count($value) > 0)
                            {
                                if ($key == $xmlimportdata->orderitem_element_name)
                                {
                                    for ($j = 0; $j < count($value); $j++)
                                    {
                                        if (isset($value[$j]['order_item_sku']))
                                        {
                                            ...
                                            if (trim($oitemstring) != "")
                                            {
                                                $query = "UPDATE " . $this->_table_prefix . "order_item AS oi " . ", " . $this->_table_prefix . "orders AS o " . "SET $oitemstring " . "WHERE oi.order_id=o.order_id " . "AND o.order_number='" . $oldorder_number . "' " . "AND oi.order_item_sku='" . $value[$j]['order_item_sku'] . "' ";
                                                $this->_db->setQuery($query);
                                                $this->_db->Query();
                                            }
                                        }
                                ...
                                elseif ($key == $xmlimportdata->billing_element_name)
                                {
                                    ...
                                    {
                                        $query = "UPDATE " . $this->_table_prefix . "order_users_info AS ou " . ", " . $this->_table_prefix . "orders AS o " . "SET $billingstring " . "WHERE ou.order_id=o.order_id " . "AND o.order_number='" . $oldorder_number . "' " . "AND ou.address_type='BT' ";
                                        $this->_db->setQuery($query);
                                        $this->_db->Query();
                                    }
                                }
                                elseif ($key == $xmlimportdata->shipping_element_name)
                                {
                                    ...
                                    {
                                        $query = "UPDATE " . $this->_table_prefix . "order_users_info AS ou " . ", " . $this->_table_prefix . "orders AS o " . "SET $shippingstring " . "WHERE ou.order_id=o.order_id " . "AND o.order_number='" . $oldorder_number . "' " . "AND ou.address_type='ST' ";
                                        $this->_db->setQuery($query);
                                        $this->_db->Query();
                                    }
                                }
                        ...
                        if (count($ordarray) > 0)
                        {
                            ...
                            $query    = "UPDATE " . $this->_table_prefix . "orders " . "SET $upstring " . "WHERE order_number='" . $oldorder_number . "' ";
                            $this->_db->setQuery($query);
                            $this->_db->Query();
                        }
                    }
                    else
                    {
                        if (!empty($datalist[$i]['order_number']))
                        {
                            ...
                            if (count($ordkeysarray) > 0)
                            {
                                ...
                                $valuestring = "'" . $valuestring . "'";
                                $query       = "INSERT IGNORE INTO " . $this->_table_prefix . "orders " . "($fieldstring) VALUES ($valuestring)";
                                $this->_db->setQuery($query);
                                $this->_db->Query();
                                    ...
                                    if (is_array($value))
                                    {
                                            ...
                                            for ($j = 0; $j < count($value); $j++)
                                            {
                                                if (isset($value[$j]['order_item_sku']))
                                                {
                                                    ...
                                                    if (trim($fieldstring) != "")
                                                    {
                                                        ...
                                                        $query = "INSERT IGNORE INTO " . $this->_table_prefix . "order_item " . "($fieldstring) VALUES ($valuestring)";
                                                        $this->_db->setQuery($query);
                                                        $this->_db->Query();
                                                    }
                                                }
                                        ...
                                        elseif ($key == $xmlimportdata->billing_element_name)
                                        {
                                            ...
                                            if (trim($fieldstring) != "")
                                            {
                                                ...
                                                $query = "INSERT IGNORE INTO " . $this->_table_prefix . "order_users_info " . "($fieldstring) VALUES ($valuestring)";
                                                $this->_db->setQuery($query);
                                                $this->_db->Query();
                                            }
                                        }
    ...
    public function getProductList($xmlarray = array(), $xmlExport = array())
    {
        ...
        if (count($xmlarray) > 0)
        {
            foreach ($xmlarray AS $key=> $value)
            {
                ...
                {
                    if ($key == "freight") //Start Code for display freight
                    {
            ...
            if ($strfield != "")
            {
                /*$query = "SELECT ".$strfield.", p.product_id FROM ".$this->_table_prefix."product AS p "
						...
						;*/
                $query = "SELECT " . $strfield . ", p.product_id FROM " . $this->_table_prefix . "product AS p " . "LEFT JOIN " . $this->_table_prefix . "product_category_xref AS x ON x.product_id=p.product_id " . "LEFT JOIN " . $this->_table_prefix . "category AS c ON c.category_id=x.category_id " . "LEFT JOIN " . $this->_table_prefix . "manufacturer AS m ON m.manufacturer_id=p.manufacturer_id  " . "LEFT JOIN " . $this->_table_prefix . "product_stockroom_xref AS sx ON sx.product_id=p.product_id " . "LEFT JOIN " . $this->_table_prefix . "stockroom AS s ON s.stockroom_id =sx.stockroom_id  " . "WHERE p.published=1 " . $andcat . "GROUP BY p.product_id " . "ORDER BY p.product_id ASC ";
                $this->_db->setQuery($query);
    ...
    public function getOrderList($xmlarray = array())
    {
        ...
        if (count($xmlarray) > 0)
        {
            ...
            if ($strfield != "")
            {
                $query = "SELECT " . $strfield . ", order_id FROM " . $this->_table_prefix . "orders " . "ORDER BY order_id ASC ";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObjectlist();
            }
        }
    ...
    public function getOrderUserInfoList($xmlarray = array(), $order_id = 0, $addresstype = "BT")
    {
        ...
        if (count($xmlarray) > 0)
        {
            ...
            if ($strfield != "")
            {
                $query = "SELECT " . $strfield . " FROM " . $this->_table_prefix . "order_users_info " . "WHERE address_type='" . $addresstype . "' " . "AND order_id='" . $order_id . "' " . "ORDER BY order_id ASC ";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObject();
            }
        }
    ...
    public function getOrderItemList($xmlarray = array(), $order_id = 0)
    {
        ...
        if (count($xmlarray) > 0)
        {
            ...
            if ($strfield != "")
            {
                $query = "SELECT " . $strfield . " FROM " . $this->_table_prefix . "order_item " . "WHERE order_id='" . $order_id . "' " . "ORDER BY order_item_id ASC ";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObjectList();
            }
        }
    ...
    public function getStockroomList($xmlarray = array(), $product_id = 0)
    {
        ...
        if (count($xmlarray) > 0)
        {
            ...
            if ($strfield != "")
            {
                $query = "SELECT " . $strfield . " FROM " . $this->_table_prefix . "stockroom AS s " . "LEFT JOIN " . $this->_table_prefix . "product_stockroom_xref AS sx ON s.stockroom_id=sx.stockroom_id " . "WHERE product_id='" . $product_id . "' " . "ORDER BY s.stockroom_id ASC ";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObjectList();
            }
        }
    ...
    public function getExtraFieldList($xmlarray = array(), $section_id = 0, $fieldsection = 0)
    {
        ...
        if (count($xmlarray) > 0)
        {
            ...
            if ($strfield != "")
            {
                $query = "SELECT " . $strfield . " FROM " . $this->_table_prefix . "fields_data " . "WHERE itemid='" . $section_id . "' " . "AND section='" . $fieldsection . "' ";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObjectList();
            }
        }
