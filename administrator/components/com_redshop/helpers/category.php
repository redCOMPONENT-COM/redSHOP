...
class product_category
{
    ...
    public function list_tree($category_id = "", $cid = '0', $level = '0', $selected_categories = Array(), $disabledFields = Array(), $html = '')
    {
        ...
        $q = "SELECT category_id, category_child_id,category_name " . "FROM " . $this->_table_prefix . "category AS c, " . $this->_table_prefix . "category_xref AS cx " . "WHERE cx.category_parent_id='$cid' " . "AND c.category_id=cx.category_child_id " . "AND c.category_id != '$category_id' " . "ORDER BY c.category_name ASC";
        $db->setQuery($q);
        $cats = $db->loadObjectList();
    ...
    public function getCategoryListArray($category_id = "", $cid = '0', $level = '0')
    {
        ...
        $q = "SELECT c.category_id, cx.category_child_id, cx.category_parent_id " . ",c.category_name,c.category_description,c.published,ordering " . "FROM " . $this->_table_prefix . "category AS c " . " ," . $this->_table_prefix . "category_xref AS cx " . "WHERE c.category_id=cx.category_child_id " . $and . $orderby;
        $db->setQuery($q);
        $cats = $db->loadObjectList();
    ...
    public function getCategoryListReverceArray($cid = '0')
    {
        ...
        $db = jFactory::getDBO();
        $q  = "SELECT c.category_id,c.category_name " . ",cx.category_child_id,cx.category_parent_id " . "FROM " . $this->_table_prefix . "category_xref as cx, " . $this->_table_prefix . "category as c " . "WHERE cx.category_child_id='" . $cid . "' " . "AND c.category_id = cx.category_parent_id";
        $db->setQuery($q);
        $cats = $db->loadObjectList();
    ...
    public function getCategoryTree($cid = '0')
    {
        ...
        $db = jFactory::getDBO();
        $q  = "SELECT c.category_id,c.category_name " . ",cx.category_child_id,cx.category_parent_id " . "FROM " . $this->_table_prefix . "category_xref as cx, " . $this->_table_prefix . "category as c " . "WHERE cx.category_parent_id='" . $cid . "' " . "AND c.category_id = cx.category_child_id";
    ...
    public function CheckAccessoryExists($product_id, $accessory_id)
    {
        ...
        $query = "SELECT accessory_id,product_id " . "FROM " . $this->_table_prefix . "product_accessory  " . "WHERE 1=1 AND product_id = '" . $product_id . "' and child_product_id ='" . $accessory_id . "'";
        //.$orderby
