...
class shipping
{
    ...
    */
    function getDefaultShipping($d)
    {
        ...
        {
            $wherecountry = '(FIND_IN_SET( "' . DEFAULT_SHIPPING_COUNTRY . '", shipping_rate_country ) OR shipping_rate_country="0" OR shipping_rate_country="")';
        }
        ...
        if (empty($shippingArr))
        {
            ...
            if ($idx)
            {
                ...
                $sql       = "SELECT * FROM " . $this->_table_prefix . "shipping_rate as sr " . "LEFT JOIN #__extensions AS s ON sr.shipping_class = s.element
						 ...
						 AND ((shipping_rate_ordertotal_start <= '$order_subtotal' AND  shipping_rate_ordertotal_end >= '$order_subtotal')  OR (shipping_rate_ordertotal_end = 0))
						 AND ((shipping_rate_weight_start <= '$weighttotal' AND  shipping_rate_weight_end >= '$weighttotal')  OR (shipping_rate_weight_end = 0))
    ...
    function getShippingrate_calc()
    {
        ...
        for ($g = 0; $g < count($volumeShipping); $g++)
        {
            ...
            $whereShippingVolume .= "((shipping_rate_length_start <= '$length' AND  shipping_rate_length_end >= '$length') AND (shipping_rate_width_start <= '$width' AND  shipping_rate_width_end >= '$width') AND (shipping_rate_height_start <= '$height' AND  shipping_rate_height_end >= '$height')) ";
        ...
        $sql = "SELECT shipping_rate_value,shipping_rate_zip_start,shipping_rate_zip_end FROM " . $this->_table_prefix . "shipping_rate as sr
				...
				$zipCond
				AND ((shipping_rate_volume_start <= '$volume' AND  shipping_rate_volume_end >= '$volume') OR (shipping_rate_volume_end = 0) )
				AND ((shipping_rate_ordertotal_start <= '$ordertotal' AND  shipping_rate_ordertotal_end >= '$ordertotal')  OR (shipping_rate_ordertotal_end = 0))
				AND ((shipping_rate_weight_start <= '$weighttotal' AND  shipping_rate_weight_end >= '$weighttotal')  OR (shipping_rate_weight_end = 0))
				$whereShippingVolume
    ...
    function getShippingMethodById($id = 0)
    {
        ...
        $query = "SELECT * FROM #__extensions " . "WHERE LOWER(`folder`) = '{$folder}' " . "AND `extension_id`='" . $id . "' ";
        $this->_db->setQuery($query);
    ...
    function getShippingVatRates($shipping_tax_group_id, $user_id = 0)
    {
        ...
        $query = 'SELECT tr.* FROM ' . $this->_table_prefix . 'tax_rate as tr ' . $q2 . 'WHERE ( tr.tax_country="' . $userdata->country_code . '" or tr.tax_country = "") ' . 'AND ( tr.tax_state = "' . $userdata->state_code . '" or tr.tax_state = "")' . $and . ' ORDER BY `tax_rate` DESC';
        $this->_db->setQuery($query);
    ...
    function getShopperGroupDefaultShipping($user_id = 0)
    {
        ...
        if ($user_id)
        {
            ...
            if (count($result) > 0 && $result->default_shipping == 1)
            {
                ...
                //				if ($result->tax_exempt_on_shipping == 1)
                //				{
    ...
    function getCartItemDimention()
    {
        ...
        $ret = array("totalquantity"  => $totalQnt, "totalweight"    => $totalWeight, "totalvolume"    => $totalVolume, "totallength"    => $totalLength, "totalheight"    => $totalheight, "totalwidth"     => $totalwidth);
        return $ret;
    }
    ...
    function getShippingBox()
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "shipping_boxes " . "WHERE published = 1 " . $whereShippingVolume . " ORDER BY shipping_box_priority ASC ";
        $this->_db->setQuery($query);
    ...
    function getBoxDimensions($boxid = 0)
    {
        ...
        if ($boxid)
        {
            $query = "SELECT * FROM " . $this->_table_prefix . "shipping_boxes " . "WHERE published = 1 " . "AND shipping_box_id ='" . $boxid . "' ";
            $this->_db->setQuery($query);
    ...
    function isCartDimentionMatch(&$d)
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "shipping_rate " . "WHERE (shipping_class = 'default_shipping' OR shipping_class = 'shipper') " . "AND (( '$volume' BETWEEN shipping_rate_volume_start AND shipping_rate_volume_end) OR (shipping_rate_volume_end = 0) ) " . "AND (( '$order_subtotal' BETWEEN shipping_rate_ordertotal_start AND shipping_rate_ordertotal_end)  OR (shipping_rate_ordertotal_end = 0)) " . "AND (( '$weighttotal' BETWEEN shipping_rate_weight_start AND shipping_rate_weight_end)  OR (shipping_rate_weight_end = 0)) " . $whereShippingVolume . " ORDER BY shipping_rate_priority ";
        $this->_db->setQuery($query);
        $shippingrate = $this->_db->loadObjectList();
    ...
    function isUserInfoMatch(&$d)
    {
        ...
        if (strlen(str_replace($numbers, '', $zip)) == 0 && $zip != "")
        {
            $zipCond = "AND ( ( shipping_rate_zip_start <= '" . $zip . "' AND shipping_rate_zip_end >= '" . $zip . "' ) " . "OR (shipping_rate_zip_start='0' AND shipping_rate_zip_end='0') " . "OR (shipping_rate_zip_start='' AND shipping_rate_zip_end='') ) ";
        }
        $query = "SELECT * FROM " . $this->_table_prefix . "shipping_rate " . "WHERE (shipping_class = 'default_shipping' OR shipping_class = 'shipper') " . $wherecountry . $wherestate . $whereshopper . $zipCond . $where . " ORDER BY shipping_rate_priority ";
        $this->_db->setQuery($query);
        $shippingrate = $this->_db->loadObjectList();
        if (count($shippingrate) > 0)
        {
    ...
    function isProductDetailMatch(&$d)
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "shipping_rate " . "WHERE (shipping_class = 'default_shipping' OR shipping_class = 'shipper') " . "AND (shipping_rate_on_product = '' $pwhere) AND (shipping_rate_on_category = '' $cwhere ) " . "ORDER BY shipping_rate_priority ";
        $this->_db->setQuery($query);
        $shippingrate = $this->_db->loadObjectList();
        if (count($shippingrate) > 0)
        {
    ...
    function getfreeshippingRate($shipping_rate_id = 0)
    {
        ...
        {
            $wherecountry = 'AND (FIND_IN_SET( "' . DEFAULT_SHIPPING_COUNTRY . '", shipping_rate_country ) OR shipping_rate_country="0" OR shipping_rate_country="")';
        }
        ...
        if ($shippingrate)
        {
            ...
            {
                if ($shippingrate->shipping_rate_ordertotal_start < $order_subtotal && $shippingrate->shipping_rate_ordertotal_end > $order_subtotal)
                {
