...
class rsstockroomhelper
{
    ...
    public function getStockroomDetail($stockroom_id = 0)
    {
        ...
        if (USE_STOCKROOM == 1)
        {
            ...
            $query = "SELECT * FROM " . $this->_table_prefix . "stockroom " . "WHERE 1=1 " . $and;
            $db->setQuery($query);
    ...
    public function getStockAmountwithReserve($section_id = 0, $section = "product", $stockroom_id = 0)
    {
        ...
        if (USE_STOCKROOM == 1)
        {
            ...
            $query = "SELECT SUM(x.quantity)  FROM " . $this->_table_prefix . $table . "_stockroom_xref AS x " //					."LEFT JOIN ".$this->_table_prefix."stockroom AS s ON s.stockroom_id=x.stockroom_id "
                . ", " . $this->_table_prefix . "stockroom AS s " . "WHERE s.stockroom_id=x.stockroom_id " . "AND x.quantity>=0 " . $and . "ORDER BY s.min_del_time ";
            //echo "<br>";
            $db->setQuery($query);
    ...
    public function getPreorderStockAmountwithReserve($section_id = 0, $section = "product", $stockroom_id = 0)
    {
        $quantity = 1;
        if (USE_STOCKROOM == 1)
        {
            ...
            $query = "SELECT SUM(x.preorder_stock) as preorder_stock, SUM(x.ordered_preorder) as ordered_preorder FROM " . $this->_table_prefix . $table . "_stockroom_xref AS x " //					."LEFT JOIN ".$this->_table_prefix."stockroom AS s ON s.stockroom_id=x.stockroom_id "
                . ", " . $this->_table_prefix . "stockroom AS s " . "WHERE s.stockroom_id=x.stockroom_id " . "AND x.quantity>=0 " . $and . "ORDER BY s.min_del_time ";
            //die();
            //echo "<br>";
            $db->setQuery($query);
    ...
    public function getStockroomAmountDetailList($section_id = 0, $section = "product", $stockroom_id = 0)
    {
        ...
        if (USE_STOCKROOM == 1)
        {
            ...
            $query = "SELECT * FROM " . $this->_table_prefix . $table . "_stockroom_xref AS x " . "LEFT JOIN " . $this->_table_prefix . "stockroom AS s ON s.stockroom_id=x.stockroom_id " . "WHERE 1=1 " . "AND x.quantity>0 " . $and . "ORDER BY s.min_del_time ";
            $db->setQuery($query);
            $list = $db->loadObjectList();
        }
        return $list;
    ...
    public function getPreorderStockroomAmountDetailList($section_id = 0, $section = "product", $stockroom_id = 0)
    {
        ...
        if (USE_STOCKROOM == 1)
        {
            ...
            $query = "SELECT * FROM " . $this->_table_prefix . $table . "_stockroom_xref AS x " . "LEFT JOIN " . $this->_table_prefix . "stockroom AS s ON s.stockroom_id=x.stockroom_id " . "WHERE 1=1 " . "AND x.preorder_stock>= x.ordered_preorder " //."AND x.preordere>0 "
                . $and . "ORDER BY s.min_del_time ";
            $db->setQuery($query);
            $list = $db->loadObjectList();
        }
        return $list;
    ...
    public function updateStockAmount($section_id = 0, $quantity = 0, $stockroom_id = 0, $section = "product")
    {
        ...
        if (USE_STOCKROOM == 1)
        {
            ...
            if ($section_id != 0)
            {
                ...
                $query = 'UPDATE ' . $this->_table_prefix . $table . '_stockroom_xref ' . 'SET quantity=quantity - ' . $quantity . ' ' . 'WHERE stockroom_id="' . $stockroom_id . '" ' . 'AND quantity > 0 ' . $and;
                $db->setQuery($query);
                $db->query();
            }
        }
    ...
    public function updatePreorderStockAmount($section_id = 0, $quantity = 0, $stockroom_id = 0, $section = "product")
    {
        ...
        if (USE_STOCKROOM == 1)
        {
            ...
            if ($section_id != 0 && trim($section_id) != "")
            {
                ...
                $query = 'UPDATE ' . $this->_table_prefix . $table . '_stockroom_xref ' . 'SET ordered_preorder=ordered_preorder + ' . $quantity . ' ' . 'WHERE stockroom_id="' . $stockroom_id . '" ' //.'AND ordered_preorder <= preorder_stock '
                    . $and;
                $db->setQuery($query);
                $db->query();
            }
    ...
    public function manageStockAmount($section_id = 0, $quantity = 0, $stockroom_id = 0, $section = "product")
    {
        if (USE_STOCKROOM == 1)
        {
            ...
            for ($i = 0; $i < count($stockId); $i++)
            {
                ...
                {
                    $query = 'UPDATE ' . $this->_table_prefix . $table . '_stockroom_xref ' . 'SET quantity=quantity + ' . $stock_Qty[$i] . ' ' . 'WHERE stockroom_id = ' . $stockId[$i] . ' ' . $and;
                    $db->setQuery($query);
                    $db->query();
    ...
    public function getReservedStock($section_id, $section = "product")
    {
        if (IS_PRODUCT_RESERVE && USE_STOCKROOM)
        {
            ...
            $query = "SELECT SUM(qty) FROM " . $this->_table_prefix . "cart " . "WHERE product_id='" . $section_id . "' " . "AND section='" . $section . "' ";
            $db->setQuery($query);
    ...
    public function getCurrentUserReservedStock($section_id, $section = "product")
    {
        if (IS_PRODUCT_RESERVE && USE_STOCKROOM)
        {
            ...
            $query = "SELECT SUM(qty) FROM " . $this->_table_prefix . "cart " . "WHERE product_id='" . $section_id . "' " . "AND session_id='" . $session_id . "' " . "AND section='" . $section . "' ";
            $db->setQuery($query);
            $count = intval($db->loadResult());
            return $count;
        }
    ...
    public function deleteExpiredCartProduct()
    {
        if (IS_PRODUCT_RESERVE) //&& USE_STOCKROOM
        {
            ...
            $query = "DELETE FROM " . $this->_table_prefix . "cart " . "WHERE time < $time ";
            $db->setQuery($query);
            $db->query();
        }
        return true;
    ...
    public function deleteCartAfterEmpty($section_id = 0, $section = "product")
    {
        if (IS_PRODUCT_RESERVE) // && USE_STOCKROOM
        {
            ...
            $query = "DELETE FROM " . $this->_table_prefix . "cart " . "WHERE session_id='" . $session_id . "' " . $and;
            $db->setQuery($query);
            $db->query();
        }
        return true;
    ...
    public function addReservedStock($section_id, $quantity = 0, $section = "product")
    {
        if (IS_PRODUCT_RESERVE) // && USE_STOCKROOM
        {
            ...
            $time = time();
            $and  = "AND session_id='" . $session_id . "' " . "AND product_id='" . $section_id . "' " . "AND section='" . $section . "' ";

            $sql = "SELECT COUNT(*) FROM " . $this->_table_prefix . "cart " . "WHERE 1=1 " . $and;
            $db->setQuery($sql);
            $count = intval($db->loadResult());

            if ($count)
            {
                $query = "UPDATE " . $this->_table_prefix . "cart " . "SET qty='" . $quantity . "', time='" . $time . "' " . "WHERE 1=1 " . $and;
            }
            else
            {
                $query = "INSERT INTO " . $this->_table_prefix . "cart " . "(session_id, product_id, qty, time, section) " . "VALUES ('" . $session_id . "', '" . $section_id . "', '" . $quantity . "', '" . $time . "', '" . $section . "')";
            }
            $db->setQuery($query);
            $db->query();
        }
    ...
    // public function to get enabled Stockroom
    ...
    // public function to get min delivery time
