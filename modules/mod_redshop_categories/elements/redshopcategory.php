...
class JElementRedshopcategory extends JElement {
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		array_unshift($categories, JHTML::_('select.option', '0', '- '.JText::_('SELECT_CATEGORY').' -', 'category_id', 'category_name'));
		ob_start();
		$output = JHTML::_('select.genericlist',  $categories, ''.$control_name.'['.$name.']', 'class="inputbox"', 'category_id', 'category_name', $value, $control_name.$name );
		ob_end_clean();
		return $output;
	}
