...
{
	$row = $rows[$i];
	$path = $url.'/components/'.$option.'/assets/images/noimage.jpg';
	if(is_file ( JPATH_SITE.'/components/'.$option.'/assets/images/product/'.$row->product_full_image ))
	{
		$path = $url.'components/'.$option.'/assets/images/product/'.$row->product_full_image;
	}
