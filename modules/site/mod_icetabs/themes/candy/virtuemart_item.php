...
<div class="ice-description">
	<h3 class="ice-title"><a <?php echo $target;?>  href="<?php echo $row->link; ?>"
    ...
    <?php
		...
		<?php if (!trim(ps_product::product_has_attributes($row->product_id, true))) : ?>
			<div class="ice-addtocart">
				<form action="<?php echo $row->addtocart_link ?>" method="post" name="addtocart"
				      id="addtocart<?php echo $row->product_id; ?>" onsubmit="handleAddToCart( this.id );return false;">
					...
					<button type="submit" class="addtocart_button_module"><span
							class="round"><span><?php echo $VM_LANG->_('PHPSHOP_CART_ADD_TO')  ?></span></span></button>
				</form>
			</div>
		<?php endif; ?>
	</div>
	<div class="ice-vmproductdesc"> <?php echo $row->description;    ?> </div>
</div>
