...
/**
 * mod_redshop_shoppergrouplogo installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.7.0
 */
class Mod_Redshop_ShopperGroupLogoInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('mod_redshop_shoppergrouplogo'))
			)
				->loadResult();
