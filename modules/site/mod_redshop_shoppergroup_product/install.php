...
/**
 * mod_redshop_shoppergroup_product installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.7.0
 */
class Mod_Redshop_ShopperGroup_ProductInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('mod_redshop_shoppergroup_product'))
			)
				->loadResult();
