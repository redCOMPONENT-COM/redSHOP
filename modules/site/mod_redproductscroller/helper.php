 ...
 * @since  1.5
 */
abstract class ModRedProductScrollerrHelper
{
	/**
	 * getredProductSKU description
	 * 
	 ...
	 */
	public static function getredProductSKU($limit = 0, $how = null, $category_id = 0)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$categories = new product_category;

		$hierachy = $categories->getCategoryListArray($category_id, $category_id);
		...
		$query->select(
			...
			->innerJoin(
					$db->qn('#__redshop_product_category_xref', 'pc')
					. ' ON ' . $db->qn('p.product_id') . ' = ' . $db->qn('pc.product_id')
				)
			->innerJoin(
					$db->qn('#__redshop_category', 'c')
					. ' ON ' . $db->qn('pc.category_id') . ' = ' . $db->qn('c.category_id')
				)
			->where(
					$db->qn('p.published') . ' = ' . $db->q(1)
					. ' AND' .
					$db->qn('c.published') . ' = ' . $db->q(1)
					. ' AND ' .
					$db->qn('p.product_parent_id') . ' = ' . $db->q(0)
				);

		if (count($cid))
		{
			$query->where($db->qn('pc.category_id') . ' = ' . implode(' OR ' . $db->qn('pc.category_id') . ' = ', $cid));
		}
