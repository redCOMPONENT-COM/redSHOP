...
JHTML::Script('jquery', 'modules/mod_redshop_category_scroller/js/',false);
...
for($i=0;$i<count($rows);$i++)
{
	...
	if(SHOW_PRICE && !USE_AS_CATALOG && !DEFAULT_QUOTATION_MODE && $show_price && !$row->not_for_sale)
	{
		...
		if($show_discountpricelayout)
		{
			...
			echo "<div id='mod_redsavedprice' class='mod_redsavedprice'>".JText::_('PRODCUT_PRICE_YOU_SAVED').' '.$product_price_saving."</div>";
		} else {
	...
	if($show_readmore)
	{
		echo "<div class='mod_redshop_category_scroller_readmore'><a href='".$link."'>".JText::_('TXT_READ_MORE')."</a></div>";
	}
	if($show_image)
	{
		$prod_img="";
		if(is_file(JPATH_SITE."/components/com_redshop/assets/images/product/".$row->product_full_image))
		...
		else if(is_file(JPATH_SITE."/components/com_redshop/assets/images/product/".$row->product_thumb_image))
		...
		$prod_img=$url."components/com_redshop/assets/images/noimage.jpg";
