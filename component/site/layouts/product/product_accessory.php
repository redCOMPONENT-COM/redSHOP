        ...
        <?php if (!empty($accessory['accessory_childs'])) : ?>
            <?php foreach ($accessory['accessory_childs'] as $attributes): ?>
                <?php if (empty($attributes['attribute_childs'])) : ?>
                    <?php continue; ?>
                <?php endif; ?>
                <?php $attribute = \Redshop\Product\Attribute::getProductAttribute(
                    0,
                    0,
                ...
                <?php foreach ($attributes['attribute_childs'] as $properties): ?>
                    ...
                    <?php if ($propertyPrice > 0): ?>
                        <?php $propertyVat = RedshopHelperProduct::getProducttax(
                            $productId,
                            $propertyPrice,
                            $userId
                        ); ?>
                    ...
                    <?php endif; ?>
                    <?php $displayPrice = " (" . $properties['property_oprand'] . ' ' . RedshopHelperProductPrice::formattedPrice(
                            $propertyPrice
                        ) . ")"; ?>
                    <?php if (Redshop::getConfig()->get('DEFAULT_QUOTATION_MODE') && !Redshop::getConfig()->get(
                            'SHOW_QUOTATION_PRICE'
                    ...
                    <?php if (!empty($properties['property_childs'])) : ?>
                        <?php foreach ($properties['property_childs'] as $subProperties): ?>
                            ...
                            <?php if ($subPropertyPrice > 0): ?>
                                <?php $subPropertyVat = RedshopHelperProduct::getProducttax(
                                    $productId,
                                    $subPropertyPrice,
                                    $userId
                                ); ?>
                            ...
                            <?php endif; ?>
                            <?php $displayPrice = " (" . $subProperties['subproperty_oprand'] . ' ' . RedshopHelperProductPrice::formattedPrice(
                                    $subPropertyPrice
                                ) . ")"; ?>
                            <?php if (Redshop::getConfig()->get('DEFAULT_QUOTATION_MODE') && !Redshop::getConfig()->get(
                                    'SHOW_QUOTATION_PRICE'
                                ...
                                <?php $displayPrice = ""; ?>
                            <?php endif; ?>
                            <?php $subProperty = RedshopHelperProduct_Attribute::getAttributeProperties(
                                $subProperties['subproperty_id']
                            ); ?>
                            <?php $virtualNumber = ""; ?>
                            ...
                            <div class="checkout_subattribute_wrapper">
                                <div class="checkout_subattribute_price">
                                    <?php echo urldecode(
                                        $subProperties['subproperty_name']
