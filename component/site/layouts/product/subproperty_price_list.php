...
$cartTemplate  = \Redshop\Template\Helper::getAddToCart($subPropertyData);
$priceList     = array();
...
<?php if (!empty($subProperties)): ?>
    <table class="table price_table_override hidden property_table" border="1"
           property_id="property_table_<?php echo $propertyId; ?>"
        ...
        <tr class="quantity_inputs">
			...
			<?php foreach ($subProperties as $subProperty) : ?>
                <td>
					<?php
					echo \Redshop\Product\Property::replaceAddToCart(
						$productId,
						$propertyId,
						0,
						$commonId,
						RedshopHelperStockroom::getStockAmountWithReserve($subProperty->value, "subproperty"),
						$subPropertyData,
						$cartTemplate,
						$subPropertyData,
						$subProperty->value
