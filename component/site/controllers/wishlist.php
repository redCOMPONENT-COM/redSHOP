...
class wishlistController extends JController
{
	...
	function createsave()
	{
		...
		$post ['cdate'] = time();
		if($model->store($post)){
			echo "<div>".JText::_('PRODUCT_SAVED_IN_WISHLIST_SUCCESSFULLY')."</div>";
		}else{
			echo "<div>".JText::_('PRODUCT_NOT_SAVED_IN_WISHLIST')."</div>";
		}
		...
		<script language="javascript">
			var t = setTimeout("window.parent.document.getElementById('sbox-window').close();window.parent.location.reload();",2000);
		</script>
		<?php
		}
	}
	...
	function delwishlist()
	{
		...
		if($model->check_user_wishlist_authority($user->id,$post["wishlist_id"]))
		{
			if($model->delwishlist($user->id,$post["wishlist_id"]))
				$msg = JText::_('WISHLIST_DELETED_SUCCESSFULLY');
			else
				$msg = JText::_('ERROR_IN_DELETING_WISHLIST');
		}
		else
			$msg = JText::_('YOU_ARE_NOT_AUTHORIZE_TO_DELETE');
