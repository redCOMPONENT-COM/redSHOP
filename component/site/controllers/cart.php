...
class cartController extends JController
{
	...
	function add()
	{
		...
		{
			$errmsg = ($result) ? $result : JText::_("PRODUCT_NOT_ADDED_TO_CART");
...
//						$model->add($post);
		...
		if(!$userfiled)
		{
			...
			else
			{
				...
				else
				{
					...
					$msg .= JTEXT::_('PRODUCT_ADDED_TO_CART');
	...
	function coupon()
	{
		...
		$cart['total'] = $cart['total'] -  $couponDiscount;
		$cart['coupon_discount'] = $couponDiscount;
		...
		if($valid){
			...
			$msg=JText::_( 'COUPON_CODE_IS_NOT_VALID' );
...
//			$cart =$session->get('cart');
			...
			$cart['total'] = $cart['total'] -  $voucherDiscount;
			$cart['voucher_discount'] = $voucherDiscount;
			if (isset($cart ['free_shipping']) && $cart ['free_shipping'] > 0) {
					$cart['total'] -= $cart['shipping'];  	
					$cart['shipping'] = 0;
			...
			$msg=JText::_( 'VOUCHER_CODE_IS_NOT_VALID' );
	...
	function redmasscart(){
		...
		if($post["numbercart"] == "")
		{
			$msg = JText::_('PLEASE_ENTER_PRODUCT_NUMBER');
