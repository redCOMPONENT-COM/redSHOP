...
class searchController extends JController
{
	...
	function loadProducts(){
		...
		if (count($brands)>0){
			...
			$manufac = array();		
			$manufac[]   	= JHTML::_('select.option', '0',JText::_('SELECT_MANUFACTURE'));
			$manufacdata = @array_merge($manufac,$brands);
				
			echo JText::_('SELECT_MANUFACTURE').'<br/>'.JHTML::_('select.genericlist',$manufacdata,'manufacture_id','class="inputbox" size="1" onChange="'.$javaFun.'" ','value','text',$manufac_data);
		}			
		exit;
	}
	/**
