...
class checkoutController extends JController
{
	...
	function chkvalidation($users_info_id){
		...
		$return					= 0;
   		...
   		if(!$billingaddresses->is_company)
   		{
	   		...
	   		{
	   	 		$return = 1;
	   	 		$msg =  JText::_( 'PLEASE_ENTER_FIRST_NAME' );
			...
			{
			 	$return = 1;
			 	$msg =  JText::_( 'PLEASE_ENTER_LAST_NAME' );
				JError::raiseWarning ( '', $msg  );
				return $return;
			}
   		}
   		else
   		{
   			...
   			{
	   	 		$return = 1;
	   	 		$msg =  JText::_( 'PLEASE_ENTER_COMPANY_NAME' );
				JError::raiseWarning ( '', $msg  );
				return $return;
			}
   		    if($billingaddresses->firstname == '')
			...
			else if(ECONOMIC_INTEGRATION==1 && trim($billingaddresses->ean_number)!='')
			{
   			...
   			{
	   	 		$return = 1;
	   	 		$msg =  JText::_( 'PLEASE_ENTER_REQUISITION_NUMBER' );
				JError::raiseWarning ( '', $msg  );
				return $return;
			}
   			...
   			{
	   	 		$return = 1;
	   	 		$msg =  JText::_( 'PLEASE_ENTER_CONTACT_NAME' );
				JError::raiseWarning ( '', $msg  );
				return $return;
			}*/
   		}
   		...
   		{
		 	$return = 1;
		 	$msg =  JText::_( 'PLEASE_ENTER_ADDRESS' );
			JError::raiseWarning ( '', $msg  );
			return $return;
		}
		...
		{
		 	$return = 1;
		 	$msg =  JText::_( 'PLEASE_SELECT_COUNTRY' );
			JError::raiseWarning ( '', $msg  );
			return $return;
		}
		...
		{
		 	$return = 1;
		 	$msg =  JText::_( 'PLEASE_ENTER_ZIPCODE' );
			JError::raiseWarning ( '', $msg  );
			return $return;
		}
		...
		{
		 	$return = 1;
		 	$msg =  JText::_( 'PLEASE_ENTER_PHONE' );
			JError::raiseWarning ( '', $msg  );
			return $return;
		}
		...
		if ($billingaddresses->is_company == 1)
		{
			...
			{	$return = 1;
				$msg = $extrafield_name.JText::_('IS_REQUIRED');
				JError::raiseWarning ( '', $msg  );
				return $return;
			}
		} else {
	...
	function checkoutfinal()
	{
		...
		if(SHIPPING_METHOD_ENABLE)
		{
			...
			{
				$msg =  JText::_( 'SELECT_SHIP_METHOD' );
		...
		if($payment_method_id!='')
		{
			...
			if ($order_id)
			{
				...
				if($is_creditcard && !$is_redirected){
					...
					$msg = JText::_('ORDER_PLACED');
					$this->setRedirect($link,$msg);
				}else{
					//$link = JRoute::_('index.php?option=com_redshop&view=checkout&format=final&oid='.$order_id.'&Itemid='.$Itemid);
		...
		{
			$msg = JText::_('SELECT_PAYMENT_METHOD');
			$mainframe->Redirect( 'index.php?option='.$option.'&view=checkout&Itemid='.$Itemid,$msg );
		}
	}
	...
	function oneStepCheckoutProcess()
	{
		...
		if($objectname=="users_info_id" || $objectname=="shipping_box_id")
		{
			...
			{
				$rate_template_desc = JText::_( 'FILL_SHIPPING_ADDRESS' );
			}
		}
		if($shipping_rate_id!="")
		{
		...
		if($cart_template_id!=0)
		{
			...
			$onestep_template_desc = $model->displayShoppingCart($onestep_template_desc,$users_info_id,$shipping_rate_id,$payment_method_id,$Itemid,$customer_note,$req_number,'',$shop_id);
		}
