...
class quotationController extends JController  
{
	...
	function addquotation()
	{
		...
		if(!$post['user_email']) 
	   	{
	   		$msg = JText::_('PLEASE_ENTER_VALID_EMAIL_ADDRESS');
			$this->setRedirect( 'index2.php?option='.$option.'&view=quotation&return=1&Itemid='.$Itemid,$msg);
	   		die();
		} 
		...
		if($row)
		{
			...
			if($sent)
			{
				...
				$msg = JText::_('ERROR_SENDING_QUOTATION_MAIL');
			}
	...
	function usercreate(){
		...
		$msg = JText::_('QUOTATION_SENT_AND_USERNAME_PASSWORD_HAS_BEEN_MAILED');
		$this->setRedirect( 'index2.php?option='.$option.'&view=quotation&return=1&Itemid='.$Itemid,$msg);
