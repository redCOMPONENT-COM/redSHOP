	...
	public function build(&$query)
	{
		...
		switch ($view)
		{
			...
			case 'manufacturers':
				...
				{
					$segments[] = $mid;
					$queryManufacturer      = $db->getQuery(true);
					$queryManufacturer->select($db->quoteName(array('sef_url', 'manufacturer_name')))
						->from($db->quoteName('#__redshop_manufacturer'))
						->where($db->quoteName('manufacturer_id') . ' = ' . (int) $mid);
					$url = $db->setQuery($queryManufacturer)->loadObject();
			...
			case 'account':
				...
				if ($layout == 'mytags')
				{
					...
					if ($tagId)
					{
						...
						$queryAccount = $db->getQuery(true);
						$queryAccount->select($db->quoteName('tags_name'))
							->from($db->quoteName('#__redshop_product_tags'))
							->where($db->quoteName('tags_id') . ' = ' . (int) $tagId);
						$tagname = $db->setQuery($queryAccount)->loadResult();
					...
					else
					{
						...
						else
						{
							...
							if (isset($segments[$secondLast]))
							{
								...
								$vars['cid'] = $db->setQuery($queryCategory)->loadResult();
							}
						}
					}
				}
