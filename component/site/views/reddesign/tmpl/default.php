...
$url= JURI::base();
$pagetitle = JText::_('WelCome to redDESIGN');
...
rDesign(function() {
	...
	var alink = site_url+"index2.php?option=com_reddesign&view=getdesign&id="+imageId+"&task=loadimage";
	
	rDesign("#divDesignarea").load(site_url+'index2.php?option=com_reddesign&view=getdesign',
	...
	rDesign("#selimage").change(function(){
				...
				var alink = site_url+"index2.php?option=com_reddesign&tmpl=component&view=getdesign&id="+rDesign("#selimage").val()+"&task=loadimage";
				rDesign("#idimg").attr("src",alink);
	
		rDesign("#divDesignarea").load(site_url+'index2.php?option=com_reddesign&view=getdesign&tmpl=component',{id:rDesign(this).val(),task:'loadoptions'},
			function(){
				rDesign("#divDesignAreaInput").html(rDesign("#tblDesignAreaInput").html());
				rDesign("#tblDesignAreaInput").html("");
...
function saveDesign()
{
	...
	{
		rDesign.ajax({
			url : site_url+"index2.php?option=com_reddesign&view=getdesign&tmpl=component&task=blankpdf&id="+imageId,
			type:'post',
			data:{saveimage:1},
	...
	rDesign.ajax({
		...
		success:function(html){
				...
				else
				{
					...
					alert("<?php echo JText::_('DESIGN_AREA')." "; ?>"+area_name+"<?php echo " ".JText::_('EXCEEDS_THE_DESIGN_AREA');?>"+".\n"+"<?php echo JText::_('PLEASE_CORRECT_THIS');?>");
				}			
			}
		});
}
function setOption(id)
{
	...
	rDesign("#fonttype").load(site_url+"index2.php?option=com_reddesign&view=getdesign&tmpl=component&task=getfonttype&no_html=1&id="+id,{},function(){
	...
	rDesign("#selfontsize").load(site_url+"index2.php?option=com_reddesign&view=getdesign&tmpl=component&task=getfontsize&no_html=1&id="+id,{},function(){
		if(rDesign('#hdnArea_'+id).val()!="") {
	var getOpt = rDesign('#hdnArea_'+id).val();
	...
	rDesign("#tdColor").load(site_url+"index2.php?option=com_reddesign&view=getdesign&tmpl=component&task=getcolors&no_html=1&id="+id,{},function(){
		if(rDesign('#hdnArea_'+id).val()!="") {
	var getOpt = rDesign('#hdnArea_'+id).val();
	...
	else
	{
		...
		var url = site_url+'index2.php?option=com_reddesign&view=getdesign&tmpl=component&task=getfontalign&no_html=1&id='+id;
...
<table>
	...
	<tr>
		...
		<td valign="top">
			...
			<b><?php echo JText::_('DESIGN_QUANTITY');?> :</b><br/>
...
<?php 
	...
	$data_add =str_replace("{product_price_label}",JText::_('PRICE'),$data_add);
	...
	$data_add =str_replace("{design_quantity_label}", JText::_('DESIGN_QUANTITY'),$data_add);
