...
{
	if(!confirm("<?php echo Jtext::_('CONFIRM_CART_EMPTY');?>"))
	{
		return false;
	}
	return true;
...
<?php echo $this->escape ( JText::_ ( 'ORDER_DETAILS' ) );?></div>
<?php } ?>
...
for($d=0;$d<count($arr_discount);$d++){
	...
	if($arr_discount[$d]){
		...
		if($arr_discount_type[0] == 'c')
			$discount_type.= JText::_('COUPEN_CODE'). ' : '. $arr_discount_type[1].'<br>';
...
if($discount_type){
	...
	$search[] = "{discount_type}";
	$replace[] = JText::_('NO_DISCOUNT_AVAILABLE');
}
...
if ($issplit && ($split_amount > 0)) {
	$split_amounttext = "<br /><br />" . JText::_ ( 'RECEIPT_PARTIALLY_PAID_AMOUNT' ) . ": " . $producthelper->getProductFormattedPrice($split_amount);
	$payremaininglink = "<br />".JText::_ ('REMAINING_AMOUNT_TOBE_PAID_BEFORE_DEL').": ".$producthelper->getProductFormattedPrice($split_amount)."<a href='".JRoute::_('index.php?option=com_redshop&view=split_payment&oid='.$oid.'&Itemid='.$Itemid)."'>".JText::_ ('PAY_REMAINING')."</a>";
}
...
if($OrdersDetail->order_status!='C' && $OrdersDetail->order_status!='S' && $OrdersDetail->order_status!='PR' && $OrdersDetail->order_status!='APP' && $print!=1 && $OrdersDetail->order_payment_status != 'Paid')
{
	...
	<input type='submit' name='payment' value='".JText::_("PAY")."'>
	</form>";
}
else
{
	$reorder = "<form method='post' name='frmreorder' id='frmreorder'>";
	$reorder .= "<input type='submit' name='reorder' id='reorder' value='".JText::_('REORDER')."' onclick='return submitReorder();' />";
