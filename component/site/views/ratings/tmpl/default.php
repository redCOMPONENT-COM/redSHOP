	...
	for($i=0;$i<count($this->detail);$i++)
	{
		...
		if(strstr($product_data,"{review_loop_start}") && strstr($product_data,"{review_loop_end}"))
		{
			...
			if(count($reviews)>0)
			{
				...
				{
					$review_data .='<div style="clear:both;" class="show_reviews"><a href="javascript:showallreviews('.$this->detail[$i]->product_id.');"> <img src="'.$url.'components/'.$option.'/assets/images/reviewarrow.gif">&nbsp;'.JText::_('COM_REDSHOP_SHOW_ALL_REVIEWS').'</a></div>';
			    }
