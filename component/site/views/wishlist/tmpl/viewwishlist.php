...
if(!$user->id)
{
	...
	if( count($rows) > 0 )
	{
		...
		//echo $mail_link = '<div class="mod_wishlist_mail_icon"><a class="modal" href="'.$mlink.'" rel="{handler:\'iframe\',size:{x:450,y:400}}" ><img src="'.JURI::root().'administrator/components/com_redshop/assets/images/mailcenter16.png" ></a></div>';
...
<?php 
		//echo "<br /><div><a href=\"".$reglink."\"><input type='button' onClick="clickMe();"  value='".JText::_('SAVE_WISHLIST')."' onclick="" ></a></div>";
		...
		if($count_no_user_field>0)
		{
			...
			echo "<br /><div id='saveid' style='clear:both;'><input type='button' onClick='clickMe()'  value='".JText::_('SAVE_WISHLIST')."' ></div>";
		}
	}
	else
	{
...
else	// if user logged in than display this code.
{
	...
	//	echo $mail_link = '<div class="mod_wishlist_mail_icon"><a class="modal" href="'.$mlink.'" rel="{handler:\'iframe\',size:{x:450,y:400}}" ><img src="'.JURI::root().'administrator/components/com_redshop/assets/images/mailcenter16.png" ></a></div>';
		...
		echo "<br />";
		$mywishlist_link = "index2.php?option=com_redshop&view=wishlist&task=addtowishlist&tmpl=component";
		//echo "<div style=\"clear:both;\"><a class=\"modal\" href=\"".$mywishlist_link."\" rel=\"{handler:'iframe',size:{x:450,y:350}}\" ><input type='button' value='" . JText::_('SAVE_WISHLIST') . "'></a></div><br /><br />";
		if($count_no_user_field>0)
		{
			echo "<br /><div style='clear:both;' ><a class=\"modal\" href=\"".$mywishlist_link."\"  rel=\"{handler:'iframe',size:{x:450,y:350}}\"><form method='post' ><input type='hidden' name='product_id' value='".$myproductid."' ><input type='button' onClick='return productalladdprice(2)'  value='".JText::_('SAVE_WISHLIST')."' ></form></a></div>";
		}else{
			echo "<div style=\"clear:both;\" ><a class=\"modal\" href=\"".$mywishlist_link."\" rel=\"{handler:'iframe',size:{x:450,y:350}}\" ><input type='button'  value='" . JText::_('SAVE_WISHLIST') . "'></a></div><br /><br />";	
		}
	}
	if( count($wishlists) > 0 )
	{
		...
		for($j=0;$j<count($wishlists);$j++)
		{
			...
			."<td><a href=\"".$del_wishlist."\">".JText::_('DELETE')."</a></td></tr>";
		}
		echo "</table>";
	}
...
function display_products($rows)
{
	...
	if(count($template)<=0)
	{
		for($i=0;$i<count($rows);$i++)
		{
			...
			if($row->product_full_image)
			{
				...
				$thum_image = $producthelper->getProductImage($row->product_id,$link,"85","63")."</div></div>";
					...
					$maindefaultpath=JURI::root()."components/com_redshop/assets/images/product/".PRODUCT_DEFAULT_IMAGE;
				...
				if($row->product_on_sale && $product_price_discount > 0)
				{
					if($product_price > $product_price_discount)
					{
						...
						if($this->show_discountpricelayout)
						{
							...
							echo "<div id='mod_redsavedprice' class='mod_redsavedprice'>".JText::_('PRODCUT_PRICE_YOU_SAVED').' '.$producthelper->getProductFormattedPrice($s_price)."</div>";
						}
						else
						{
							$product_price = $product_price_discount ;
			...
			echo "<br><div class='wishlist_readmore'><a href='".$link."'>".JText::_('READ_MORE')."</a></div>&nbsp;</div> ";
		...
		$mail_link = '<a class="modal" href="'.$mlink.'" rel="{handler:\'iframe\',size:{x:450,y:400}}"  ><img src="'.JURI::root().'administrator/components/com_redshop/assets/images/mailcenter16.png" ></a>';
	...
	if(strstr($wishlist_data,"{child_products}"))
	{
		...
		if($parentproductid!=0)
		{
			...
			$childproducts = array_merge(array($productInfo),$childproducts);
				...
				$frmChild .= JText::_('CHILD_PRODUCTS').$lists ['product_child_id'];
					...
					if($row->product_on_sale && $product_price_discount > 0)
					{
						if($product_price > $product_price_discount)
						{
							...
							if($this->show_discountpricelayout)
							{
								...
								//$mainproduct_price .="<div id='mod_redsavedprice' class='mod_redsavedprice'>".JText::_('PRODCUT_PRICE_YOU_SAVED').' '.$producthelper->getProductFormattedPrice($s_price)."</div>";
		...
		/////////////////////////////////// Product accessory End /////////////////////////////////	
				...
				$rmore = "<a href='".$link."' title='".$row->product_name."'>" . JText::_ ( 'READ_MORE' ) . "</a>";
				...
				$mainregdellink="<div><a href=\"".$regdellink."\">".JText::_('REMOVE_PRODUCT_FROM_WISHLIST')."</a></div>";
			...
			<input type='button' name='submit' onclick='return productalladdprice();' value='".JText::_('ADD_TO_CART')."'>
			</form>";
