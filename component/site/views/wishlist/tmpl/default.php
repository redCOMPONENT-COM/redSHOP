...
$hasWishlist = ($productId && !empty($wishlists)) ? true : false;
...
<div class="divnewwishlist">
	...
	<?php if ($hasWishlist && Redshop::getConfig()->get('WISHLIST_LIST')): ?>
		<label>
			<input type="checkbox" name="chkNewwishlist" id="chkNewwishlist" onchange="changeDiv(this);"/>
		...
		<form name="newwishlistForm" method="post" action="">
			...
			<div class="row wishlistbtn">
				<div class="col-sm-12 span12">
					...
					/>&nbsp;
					<?php if ($input->getInt('loginwishlist') == 1) : ?>
			...
			<input type="hidden" name="attribute_id" value="<?php echo $input->getInt('attribute_id', '') ?>"/>
			<input type="hidden" name="property_id" value="<?php echo $input->getInt('property_id', '') ?>"/>
			<input type="hidden" name="subattribute_id" value="<?php echo $input->getInt('subattribute_id', '') ?>"/>
