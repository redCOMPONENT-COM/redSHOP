...
if ($gid != 0) {
    ...
    if (strstr($template, "{giftcard_image}")) {
        $product_img = RedshopHelperMedia::watermark(
            'giftcard',
            $detail->giftcard_image,
            Redshop::getConfig()->get('GIFTCARD_THUMB_WIDTH'),
            Redshop::getConfig()->get('GIFTCARD_THUMB_HEIGHT'),
        ...
        $linkimage   = RedshopHelperMedia::watermark(
            ...
            '',
            '',
            Redshop::getConfig()->get('WATERMARK_GIFTCART_IMAGE'),
            '0'
        );
    ...
    if ($detail->customer_amount != 1) {
        $template = str_replace(
            "{giftcard_price}",
            RedshopHelperProductPrice::formattedPrice($detail->giftcard_price),
            $template
        );
    } else {
    ...
    if (strstr($template, "{giftcard_loop_start}") && strstr($template, "{giftcard_loop_end}")) {
        ...
        for ($i = 0, $in = count($detail); $i < $in; $i++) {
            ...
            if (strstr($data_add, "{giftcard_image}")) {
                $product_img = RedshopHelperMedia::watermark(
                    'giftcard',
                    $detail[$i]->giftcard_image,
                    Redshop::getConfig()->get('GIFTCARD_LIST_THUMB_WIDTH'),
