...
if(count($this->template)>0 && $this->template[0]->template_desc!="")
{
...
if($gid!=0)
{
	...
	$template = str_replace ( "{giftcard_value_lbl}", JText::_('GIFTCARD_VALUE_LBL'), $template );



	if($detail->customer_amount!=1){
	$template = str_replace ( "{giftcard_price_lbl}", JText::_('GIFTCARD_PRICE_LBL'), $template );
	}else{
...
//	$customer_amountsection ='<input type="hidden" name="customer_amountsection" id="customer_amountsection" value="'.$detail->customer_amount.'" onkeyup="var f_value = this.value;addtocart_'.$gid.'.customer_amountsection.value = f_value;">';
	...
	$template = str_replace ( "{giftcard_reciver_name_lbl}", JText::_('GIFTCARD_RECIVER_NAME_LBL'), $template );
	$template = str_replace ( "{giftcard_reciver_email_lbl}", JText::_('GIFTCARD_RECIVER_EMAIL_LBL'), $template );
	}else{
	...
	$template = str_replace ( "{giftcard_reciver_name_lbl}", JText::_('GIFTCARD_RECIVER_NAME_LBL'), $template );
	$template = str_replace ( "{giftcard_reciver_email_lbl}", JText::_('GIFTCARD_RECIVER_EMAIL_LBL'), $template );
	$template = str_replace ( "{customer_quantity_lbl}", JText::_('CUSTOMER_QUANTITY_LBL'), $template );
	$template = str_replace ( "{customer_amount_lbl}", JText::_('CUSTOMER_AMOUNT_LBL'), $template );
	}else{
	$template = str_replace ( "{giftcard_reciver_name_lbl}", '', $template );
	$template = str_replace ( "{giftcard_reciver_email_lbl}",'', $template );
	...
	if(strstr($template,"{giftcard_loop_start}") && strstr($template,"{giftcard_loop_end}"))
	{
		...
		for($i=0;$i<count($detail);$i++)
		{
			...
			if(strstr($data_add,"{giftcard_image}"))
			{
				...
				if (is_file(JPATH_COMPONENT_SITE."/assets/images/giftcard/".$detail[$i]->giftcard_image))
				{
			...
			$giftcard_readmore = "<a href='".$link."'>".JText::_('READ_MORE')."</a>";
			...
			$data_add = str_replace ( "{giftcard_value_lbl}", JText::_('GIFTCARD_VALUE_LBL'), $data_add );
			$data_add = str_replace ( "{giftcard_value}", $producthelper->getProductFormattedPrice($detail[$i]->giftcard_value), $data_add );

			$data_add = str_replace ( "{giftcard_price_lbl}", JText::_('GIFTCARD_PRICE_LBL'), $data_add );
