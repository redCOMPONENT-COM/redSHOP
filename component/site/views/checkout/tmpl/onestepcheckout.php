...
{
	$msg =  JText::_( 'LOGIN_USER_IS_NOT_REDSHOP_USER' );
...
//		$order_subtotal =$cart['product_subtotal'];
	...
	else
	{
		...
		$onestep_template_desc = str_replace($shipping_template, JText::_( 'FILL_SHIPPING_ADDRESS' ), $onestep_template_desc);
	}
} else {
...
if(strstr($onestep_template_desc,"{edit_billing_address}"))
{
	...
	$edit_billing = '<a class="modal" href="'.$editbill.'" rel="{handler: \'iframe\', size: {x: 800, y: 550}}"> '.JText::_ ( 'EDIT' ).'</a>';
...
if(strstr($onestep_template_desc,"{shipping_address}"))
{
	if(SHIPPING_METHOD_ENABLE)
	{
		...
		if ($billingaddresses && OPTIONAL_SHIPPING_ADDRESS)
		{
			...
			$shipp .= '<div><input type="radio" onclick="javascript:onestepCheckoutProcess(this.name);" name="users_info_id" value="'.$billingaddresses->users_info_id.'" '.$ship_check.' />'.JText::_('DEFAULT_SHIPPING_ADDRESS').'</div>';
		}
		for($i=0;$i<count($shippingaddresses);$i++)
		{
			...
			$shipp .= '<a class="modal" href="'.$edit_addlink.'" rel="{handler: \'iframe\', size: {x: 570, y: 450}}">('.JText::_ ( 'EDIT_LBL' ).')</a> ';
			$shipp .= '<a href="'.$delete_addlink.'" title="">('.JText::_ ( 'DELETE_LBL' ).')</a></div>';
		}
		...
		$shipp .= '<a class="modal" href="'.$add_addlink.'" rel="{handler: \'iframe\', size: {x: 570, y: 450}}"> '.JText::_ ( 'ADD_ADDRESS' ).'</a>';
		$onestep_template_desc = str_replace('{shipping_address}',$shipp,$onestep_template_desc);
		$onestep_template_desc = str_replace('{shipping_address_information_lbl}',JText::_('SHIPPING_ADDRESS_INFO_LBL'),$onestep_template_desc);
	}
	else
	{
...
$payment_template_desc = $carthelper->replacePaymentTemplate($payment_template_desc,$payment_method_id,$is_company);
...
<?php
	...
	{	?>
		alert("<?php echo JText::_('MINIMUM_ORDER_TOTAL_HAS_TO_BE_MORE_THAN');?>");
		return false;
<?php
	}	?>
	if(document.getElementById('termscondition'))
	{
		...
		if(!termscondition)
		{
			alert("<?php echo JText::_('PLEASE_SELECT_TEMS_CONDITIONS')?>");
			return false;
		}
	}
	return true;
