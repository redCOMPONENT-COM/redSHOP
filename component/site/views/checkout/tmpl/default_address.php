...
<form action="<?php echo JRoute::_('index.php?option=com_redshop&view=checkout&Itemid=' . $Itemid . '') ?>"
	...
	<fieldset class="adminform">
		<legend><strong><?php echo JText::_('COM_REDSHOP_BILL_TO_INFORMATION');?></strong></legend>
		<?php
		if ($billingaddresses)
		{
	...
	?>
	<fieldset class="adminform">
		<legend><strong><?php echo JText::_('COM_REDSHOP_SHIPPING_ADDRESSES');?></strong></legend>
		<?php
		...
		for ($i = 0, $in = count($shippingaddresses); $i < $in; $i++)
		{
			...
			<div class="checkbox">
				<label for="users_info_id_<?php echo $i ?>">';
					...
					if (!$session->get('isredcrmuser')) { ?>
					<a class="modal btn btn-primary" href="<?php echo $edit_addlink; ?>"
