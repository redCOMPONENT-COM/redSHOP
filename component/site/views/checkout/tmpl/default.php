...
<tr><td width="33%" class="checkout-bar-1-active"><?php echo JText::_ ( 'ORDER_INFORMATION' );?></td>
	<td width="33%" class="checkout-bar-2"><?php echo JText::_ ( 'PAYMENT' );?></td>
...
else
{
	...
	if ($show_login)
	{	?>
	<h4><input class="mytogglermy" type="radio" name="mytogglermychecker" id="mytogglermycheckerlogin" onclick="rss( '#login_div' ).slideDown();rss( '#register_div' ).slideUp()" <?php if ($open_to_mystretchermy == 0) {	?> checked="checked" <?php	}	?> />
		<label for="mytogglermycheckerlogin"><?php echo JText::_ ( 'RETURNING_CUSTOMERS' );?></label></h4>
<?php	if(strstr($login_template_desc,"{rs_username}"))
		{
			$txtusername = '<input class="inputbox" type="text" id="username" name="username" value="" />';
			$login_template_desc = str_replace ( "{rs_username_lbl}", JText::_ ( 'USERNAME' ), $login_template_desc );
		...
		if(strstr($login_template_desc,"{rs_password}"))
		{
			...
			$login_template_desc = str_replace ( "{rs_password_lbl}", JText::_ ( 'PASSWORD' ), $login_template_desc );
			$login_template_desc = str_replace ( "{rs_password}", $txtpassword, $login_template_desc );
		}
		if(strstr($login_template_desc,"{rs_login_button}"))
		{
			$loginbutton = '<input type="submit" class="button" name="submitbtn" value="'.JText::_ ( 'LOGIN' ).'">';
		...
		$forgotpwd = '<a href="'.$forgotpwd_link.'">'.JText::_('FORGOT_PWD_LINK').'</a>';
	...
	<h4><input class="mytogglermy" type="radio" name="mytogglermychecker" id="mytogglermycheckerregister"  onclick="rss( '#register_div' ).slideDown();rss( '#login_div' ).slideUp()" <?php if ($open_to_mystretchermy == 1 || (isset($post['createaccount']) && $post['createaccount']==1)) {	?> checked="checked" <?php }?> />
		<label for="mytogglermycheckerregister"><?php echo JText::_ ( 'NEW_CUSTOMERS' );?></label></h4><?php
	}
	...
	<div><span id="customer_registrationintro" <?php echo ($is_company==1) ? 'style="display:none;"':'';?>><?php echo JText::_('REGISTRATION_INTROTEXT'); ?></span><span id="company_registrationintro" <?php echo ($is_company==1) ? '':'style="display:none;"';?>><?php echo JText::_('REGISTRATION_COMPANY_INTROTEXT'); ?></span></div>
	...
	<tr><td><span <?php echo $allowCustomer;?>><h4><input type="radio" name="togglerchecker" id="toggler1" class="toggler"
		...
		<td><span <?php echo $allowCompany;?>><h4><input type="radio" name="togglerchecker" id="toggler2" class="toggler"
		<?php if($is_company==1) { ?>	checked="checked" <?php	}?> onclick="showCompanyOrCustomer(this);" value="1" />
		<?php echo JText::_ ( 'COMPANY_REGISTRATION' );?></h4></span></td></tr>
<?php	if(count($telesearch)>0 && $telesearch[0]->published)
		{	?>
	<tr><td colspan="2"><?php echo JText::_('GET_ADDRESS_BY_PHONE').':<input name="searchphone" id="searchphone" type="text" value="" /><input type="button" name="searchaddbyphone" id="searchaddbyphone" value="'.JText::_('SEARCH').'" onclick="return searchByPhone();" />';?>
		<br/>
		<div id="divSearchPhonemsg" style="display:none"><?php echo JText::_('NO_RESULT_FOUND_BY_SEARCHPHONE');?></div></td></tr>
<?php 	}	?>
	</table>
	...
	?>
		<h4><input type="checkbox" name="createaccount" <?php echo $checked_style;?> id="createaccount" value="1" onclick="createUserAccount(this);" />
		<?php echo JText::_('CREATE_ACCOUNT');?></h4>
	<?php
	}	?>
	<div>
...
<?php
	...
	<tr class="subTable"><td>
		<fieldset class="adminform">
		<legend><?php echo JText::_ ( 'SHIPPING_ADDRESSES' );?></legend>
...
<?php 	}	?>
	...
	<tr><td align="right"><input type="button" class="blackbutton" name="back" value="<?php echo JText::_('BACK' );?>" onclick="javascript:window.history.go(-1);"></td>
		<td align="left">
			<input type="submit" class="greenbutton" name="submitbtn" onclick="return checkveisvalid();" value="<?php echo JText::_('PROCEED' );?>">
	    </td>
	</tr>	
	<?php 
	} else {
	...
	<tr><td align="right"><input type="button" class="blackbutton" name="back" value="<?php echo JText::_('BACK' );?>" onclick="javascript:window.history.go(-1);"></td>
		<td align="left">
			<input type="submit" class="greenbutton" name="submitbtn" id="submitbtn" value="<?php echo JText::_('PROCEED' );?>">
	    </td>
	</tr>	
	<?php 
	}
