...
if($is_creditcard==1 && $ccinfo!='1' && $cart['total']>0)
{
	...
	$cart_data .= '<input type="hidden" name="shop_id" value="'.$this->shop_id.'" />';
...
//	$cart_data=str_replace("{print}", '', $cart_data );//$cart_data=str_replace("{print}", $print_tag, $cart_data );
	...
	$cart_data = $model->displayShoppingCart($cart_data,$this->users_info_id,$this->shipping_rate_id,$this->payment_method_id,$Itemid,'','','',$this->shop_id);
...
<?php
	...
	{	?>
		alert("<?php echo JText::_('MINIMUM_ORDER_TOTAL_HAS_TO_BE_MORE_THAN'). ' ' . MINIMUM_ORDER_TOTAL . '';?>");
		return false;
<?php
	}	?>
