...
<fieldset>
    <legend>
        <?php
        echo Text::_('COM_REDSHOP_TOKEN_VARIFICATION');
        ?>
    </legend>
    <form name="downloadproduct" id="downloadproduct" method="post" action="<?php echo JURI::root(
    ) . 'index.php?option=com_redshop&view=product&layout=downloadproduct&tmpl=component&no_html=1'; ?>">
        <table cellpadding="0" cellspacing="0" width="100%">
            <tr>
                <td colspan="2">
                    <?php
                    echo Text::_('COM_REDSHOP_TOKEN_ID');
                    ?>
                </td>
                <td>
                    <input type="text" name="download_id" id="download_id" value="<?php echo $tid; ?>" size="35" />
                </td>
                <td>
                    <input type="submit" name="submit_token" id="submit_token"
                        value="<?php echo Text::_('COM_REDSHOP_SUBMIT_TOKEN'); ?>" />
                </td>
            </tr>
            <tr>
                <td>
...
if (isset($tid) && $tid != "") {
    ...
    <fieldset>
        <legend>
            <?php echo Text::_('COM_REDSHOP_DOWNLOAD_PRODUCTS'); ?>
        </legend>
        <?php
        if (count($downloaddata) > 0 && $downloaddata->media_id != null) {
            ...
            <div>
                <form name="downloadproduct" id="downloadproduct" method="post"
                    ...
                    <table cellpadding="0" cellspacing="0" width="100%">
                        ...
                        $filetype = strtolower(File::getExt($name));
                        ...
                        <tr>
                            <td>
                                <input type="radio" value="<?php echo $mid; ?>" name="additional" id="main"
                                    onchange="mainProductSet(this);" />
                                <?php echo $downloadname; ?>
                            </td>
                        </tr>
                        <?php
                        for ($i = 0, $in = count($addtional_downloaddata); $i < $in; $i++) {
                            ...
                            $additionalfiletype = strtolower(File::getExt($additionalname));
                            ...
                            <tr>
                                <td>
                                    <input type="radio" value="<?php echo $additionalid; ?>" name="additional" id="additional"
                                        onchange="mainProductSet(this);" />
                                    <?php echo $additionaldownloadname; ?>
                                </td>
                            </tr>
                            <?php
                        }
            ...
            echo Text::_('COM_REDSHOP_FILE_NOT_AVAILABLE_IN_DB');
        }
        ?>
    </fieldset>
    <?php
