...
$pagetitle = JText::_('COMPARE_PRODUCTS');
...
{
	$template = JText::_('ADD_ONE_MORE_PRODUCT_TO_COMPARE');
}
else if(isset($compare['idx']) && $compare['idx']>1)
{
	...
	$template = str_replace('{compare_product_heading}',$pagetitle,$template);
	$template = str_replace('{returntocategory_name}',JText::_("GO_BACK"),$template);
	...
	if(count($compare_template)>0)
	{
	 ...
	 for($i=0;$i<$compare['idx'];$i++)
	 {
		...
		$remove = "<a href='".$link_remove."'>".JText::_('REMOVE_PRODUCT_FROM_COMPARE_LIST')."</a>";
		...
		if(strstr($template,"{products_in_stock}") || strstr($template,"{product_stock_amount_image}"))
		{
			...
			if(count($stockamountList)>0)
			{
				...
				$stockamountImage .= '<img src="'.$url.'components/com_redshop/assets/images/stockroom'.DS.$stockamountList[0]->stock_amount_image.'" width="150px" height="90px" alt="'.$stockamountList[0]->stock_amount_image_tooltip.'" id="stockImage'.$compare[$i]["product_id"].'" /></a>';
			}
...
{
	$template = JText::_('NO_PRODUCTS_TO_COMPARE');
}
