...
if(!$user->id)
{
	...
	if( count($rows) > 0 )
	{
		...
		echo "<br /><div><a href=".$reglink."><input type='button' value='".JText::_('SAVE_WISHLIST')."></a></div>";
	}
	else
	{
...
else	// if user logged in than display this code.
{
	...
	if(count($this->wish_session)>0)
	{
		...
		echo "<a class=\"modal\" href=\"".$mywishlist_link."\" rel=\"{handler:'iframe',size:{x:450,y:350}}\" ><input type='submit' value='" . JText::_('SAVE_WISHLIST') . "'></a>";
		echo "<br /><br />";
	}
	if( count($wishlists) > 0 )
	{
		...
		for($j=0;$j<count($wishlists);$j++)
		{
			...
			echo $mail_link = '<div class="mod_wishlist_mail_icon"><a class="modal" href="'.$mlink.'" rel="{handler:\'iframe\',size:{x:450,y:400}}" ><img src="'.JURI::root().'administrator/components/com_redshop/assets/images/mailcenter16.png" >'.$wishlists[$j]->wishlist_name.'</a></div>';
...
function display_products($rows)
{
	...
	for($i=0;$i<count($rows);$i++)
	{
		...
		echo "<br><a href='".$link."'>".JText::_('READ_MORE')."</a>&nbsp;";
