...
//			$data	=& $this->get('data');
		} else if($layout == "sendimage")
		{
			$this->setLayout('sendimage');
//			$data	=& $this->get('data');
		}
		else if($layout == "sample")
		{
			$this->setLayout('sample');
			$data = $this->get('catalogtemplate');
		}
		...
		else
		{
			...
			if($data->product_on_sale == 1)
			{
				$meta_prodcut_price =1;
			}

			if($data->published ==0 ){
				JError::raiseError(404,sprintf(JText::_('PRODUCT_IS_NOT_PUBLISHED'),$data->product_name,$data->product_number));
			}
			...
			if ($data->product_thumb_image && file_exists ( JPATH_SITE."/components/com_redshop/assets/images/product/".$data->product_thumb_image ))
			{

				$meta = "<meta property=\"og:image\" content=\"$uri->_scheme://$uri->_host/components/com_redshop/assets/images/product/$data->product_thumb_image\"/>";
				$document->addCustomTag( $meta );

			}else if($data->product_full_image && file_exists ( JPATH_SITE."/components/com_redshop/assets/images/product/".$data->product_full_image))
			{
				$meta = "<meta property=\"og:image\" content=\"$uri->_scheme://$uri->_host/components/com_redshop/assets/images/product/$data->product_full_image\"/>";
				$document->addCustomTag( $meta );
			}
			...
			else
			{
				...
				else
				{
					...
					$document->setMetaData('description', JText::_('META_BUY')." ".$data->product_name." ".JText::_('META_AT_ONLY')." ".$prodhelperobj_array['product_price']." ".JText::_('META_SAVE')." ".$product_price_saving_main);
				}
			}
