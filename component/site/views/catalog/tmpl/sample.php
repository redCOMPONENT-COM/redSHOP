...
<form action="" method="post" name="frmcatalogsample" id="frmcatalogsample">
	<?php
	if (strstr($templateDesc, "{product_samples}"))
	{
		...
		$templateDesc = str_replace("{product_samples}", $saple_data, $templateDesc);
	}

	if (strstr($templateDesc, "{address_fields}"))
	{
		...
		$templateDesc = str_replace("{address_fields}", $myfield, $templateDesc);
	}

	$templateDesc = str_replace("{name_lbl}", JText::_('COM_REDSHOP_NAME_LBL'), $templateDesc);
	$templateDesc = str_replace("{name}", '<input type="text" name="name_2" id="name" />', $templateDesc);
	$templateDesc = str_replace("{email_lbl}", JText::_('COM_REDSHOP_EMAIL_LBL'), $templateDesc);
	$templateDesc = str_replace("{email_address}", '<input type="text" name="email_address" id="email_address" />', $templateDesc);

	$templateDesc = str_replace("{submit_button_sample}", '<input type="submit" name="samplesend" id="samplesend" onClick="return getCatalogSampleValidation();" value="' . JText::_('COM_REDSHOP_SAMPLE_SEND') . '" />', $templateDesc);
