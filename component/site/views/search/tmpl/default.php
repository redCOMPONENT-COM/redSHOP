...
if (count($this->search) > 0) {
    ...
    for ($i = 0, $countSearch = count($this->search); $i < $countSearch; $i++) {
        $this->search[$i]->stockroom             = RedshopHelperStockroom::getStockroomTotalAmount(
            $this->search[$i]->product_id
        );
        $this->search[$i]->product_related       = RedshopHelperProduct::getRelatedProduct(
            $this->search[$i]->product_id
        );
        ...
        $link = JRoute::_(
            'index.php?option=com_redshop&view=product&pid=' . $this->search[$i]->product_id . '&cid=' . $this->search[$i]->cat_in_sefurl . '&Itemid=' . $pItemid
        );
