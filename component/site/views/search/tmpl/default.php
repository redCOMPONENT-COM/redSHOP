...
if (count($this->search) > 0)
{
	...
	{
		$this->search[$i]->stockroom              =  RedshopHelperStockroom::getStockroomTotalAmount($this->search[$i]->product_id);
		$this->search[$i]->product_related        = $producthelper->getRelatedProduct($this->search[$i]->product_id);
		...
		$link = JRoute::_('index.php?option=com_redshop&view=product&pid=' . $this->search[$i]->product_id . '&cid=' . $this->search[$i]->cat_in_sefurl . '&Itemid=' . $pItemid);
		...
		/* count product related start */
		if(isset($this->search[$i]->count_product_related) && $this->search[$i]->count_product_related)
		{
			$data_add = str_replace("{countrelated}","<li class='count'><span>" . $this->search[$i]->count_product_related . ' '  . JText::_('COM_REDSHOP_COUNT_RELATED') . "</span>" . "</li>", $data_add);
		}
		else
		{
			$data_add = str_replace("{countrelated}",'', $data_add);
		}
		/* count product related end */

		/* stock room number start */
		$lang = JFactory::getLanguage();

		if(isset($this->search[$i]->stockroom) && $this->search[$i]->stockroom  == 0)
		{
			$data_add = str_replace("{stockroom}",'', $data_add);
		}
		else
		{
			$data_add = str_replace("{stockroom}","<li class='stocknumber'><span>" .
				JText::sprintf('COM_REDSHOP_STOCK_AMOUNT_LEFT', $this->search[$i]->stockroom) .
				"</span></li>" , $data_add);
		}
		/* stock room number end */

		/*custom enddate discount start*/
		if(isset($this->search[$i]->discount_enddate) && $this->search[$i]->discount_enddate)
		{
			$data_add = str_replace("{discount_end_date}", date('d/m',$this->search[$i]->discount_enddate), $data_add);
		}
		else
		{
			$data_add = str_replace("{discount_end_date}", '', $data_add);
		}
		/*custom enddate discount end*/
		...
		if (strpos($data_add, "{category_name}") !== false)
		{
			...
			$categoryName = '<a href="' . $linkCat . '" ' . $title . '>' . $category->name . '</a>';
			$data_add = str_replace('{category_name}', $categoryName, $data_add);
		}
