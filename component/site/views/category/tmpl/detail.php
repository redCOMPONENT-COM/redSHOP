...
<?php
	...
	$template_desc = str_replace ( "{total_product_lbl}", JText::_('TOTAL_PRODUCT'), $template_desc );
	...
	$cat_main_thumb = "";
	if ($this->maincat->category_full_image && file_exists ( 'components/'.$option.'/assets/images/category/'.$this->maincat->category_full_image ))
	{
	...
	if(strstr($template_desc,"{compare_product_div}"))
	{
		...
		if (PRODUCT_COMPARISON_TYPE != "")
		{
			...
			$compare_product_div .= "<a href='javascript:compare();' >".JText::_('COMPARE')."</a>";
	...
	if(strstr($template_desc,"{category_loop_start}") && strstr($template_desc,"{category_loop_end}"))
	{
		...
		for($i=0; $i<count($this->detail); $i++)
		{
			...
			$product_img = $url."/components/com_redshop/assets/images/noimage.jpg";
			...
			if (CAT_IS_LIGHTBOX)
			{
				$cat_thumb = "<a class='modal' href='".$url.$middlepath.$row->category_full_image."'  rel=\"{handler: 'image', size: {}}\"  ".$title.">";
			}
			else
			{
			...
			{
				$cat_name = '<a href="'.$link.'" '.$title.'>'.JText::_('READ_MORE').'</a>';
			...
			if (strstr ( $data_add, '{category_total_product}' )) {
				...
				$data_add = str_replace ( "{category_total_product_lbl}", JText::_('TOTAL_PRODUCT'), $data_add );
			}
	...
	if(strstr($template_desc,"{product_price_slider}"))
	{
		$price_slider = '<div id="pricefilter">
			    <div class="left" id="leftSlider">
			        <div id="range">'.JText::_('PRICE').': <span id="redcatamount"> </span></div>
				...
				<div class="left" id="productsWrap">
			        <div style="display: none;" id="ajaxcatMessage">'.JText::_('LOADING').'</div>
			    </div>
			</div>';
		$template_desc = str_replace("{product_price_slider}", $price_slider, $template_desc);
		$product_tmpl = JText::_('NO_PRODUCT_FOUND');
	}
}
...
if(strstr($template_desc,"{product_loop_start}") && strstr($template_desc,"{product_loop_end}"))
{
	...
	for($i=$start; $i <($start+$final_endlimit); $i++)
	{
		...
		{
			$rmore = "<a href='".$link."' title='".$product->product_name."'>" . JText::_ ( 'READ_MORE' ) . "</a>";
		...
		if (strstr ( $data_add, '{related_product_lightbox:' ))
		{
			...
			if(count($related_product) > 0){
				...
				$rtlna = '<a class="modal" href="'.$linktortln.'"  rel="{handler:\'iframe\',size:{x:'.$rtlnfwidth.',y:'.$rtlnfheight.'}}">'.JText::_('RELATED_PRODUCT_LIST_IN_LIGHTBOX').'</a>';
			}else{
				$rtlna = "";
			}
		...
		{
			$manufacturerPLink = "<a href='".JRoute::_( 'index.php?option=com_redshop&view=manufacturers&layout=products&mid='.$product->manufacturer_id.'&Itemid='.$Itemid )."'>".JText::_("VIEW_ALL_MANUFACTURER_PRODUCTS")." ".$product->manufacturer_name."</a>";
		...
		//front-back image tag...
			if(strstr($data_add,"{front_img_link}") || strstr($data_add,"{back_img_link}"))
			{
				...
				{
					$mainsrcPath = $url."components/com_redshop/assets/images/product/".$product->product_thumb_image;
				}
				else
				{
				...
				{
					$backsrcPath = $url."components/com_redshop/assets/images/product/".$product->product_back_thumb_image;
				}
				else
				{
				...
				$ahrefpath =$url."components/com_redshop/assets/images/product/".$product->product_full_image;
				$ahrefbackpath =$url."components/com_redshop/assets/images/product/".$product->product_back_full_image;

				$product_front_image_link = "<a href='#' onClick='javascript:changeproductImage(".$product->product_id.",\"".$mainsrcPath."\",\"".$ahrefpath."\");'>".JText::_('FRONT_IMAGE')."</a>";
				$product_back_image_link = "<a href='#' onClick='javascript:changeproductImage(".$product->product_id.",\"".$backsrcPath."\",\"".$ahrefbackpath."\");'>".JText::_('BACK_IMAGE')."</a>";
		...
		{
			if(is_file(JPATH_SITE.'/components/com_redshop/assets/images/product/'. $product->product_preview_image))
			{
		...
		if(strstr($data_add,"{front_preview_img_link}") || strstr($data_add,"{back_preview_img_link}"))
		{
			...
			{
				$mainpreviewsrcPath = $url."components/com_redshop/assets/images/product/".$product->product_preview_image."&newxsize=".CATEGORY_PRODUCT_PREVIEW_IMAGE_WIDTH."&newysize=".CATEGORY_PRODUCT_PREVIEW_IMAGE_HEIGHT."&swap=".USE_IMAGE_SIZE_SWAPPING;
			}

			if($product->product_preview_back_image)
			{
				$backpreviewsrcPath = $url."components/com_redshop/assets/images/product/".$product->product_preview_back_image."&newxsize=".CATEGORY_PRODUCT_PREVIEW_IMAGE_WIDTH."&newysize=".CATEGORY_PRODUCT_PREVIEW_IMAGE_HEIGHT."&swap=".USE_IMAGE_SIZE_SWAPPING;
			}

			$product_front_image_link = "<a href='#' onClick='javascript:changeproductPreviewImage(".$product->product_id.",\"".$mainpreviewsrcPath."\");'>".JText::_('FRONT_IMAGE')."</a>";
			$product_back_image_link = "<a href='#' onClick='javascript:changeproductPreviewImage(".$product->product_id.",\"".$backpreviewsrcPath."\");'>".JText::_('BACK_IMAGE')."</a>";
...
if(!$slide)
{
	if(strstr($template_desc,"{filter_by}"))
	{
		...
		$template_desc = str_replace("{filter_by_lbl}",JText::_ ( 'SELECT_FILTER_BY' ),$template_desc);
		else
	...
	if(strstr($template_desc,"{template_selector_category}"))
	{
		if($this->lists['category_template']!="")
		{
			...
			$template_desc =  str_replace("{template_selector_category_lbl}",JText::_('TEMPLATE_SELECTOR_CATEGORY_LBL'),$template_desc);
	...
	if(strstr($template_desc,"{order_by}"))
	{
		...
		$template_desc =  str_replace("{order_by_lbl}",JText::_ ( 'SELECT_ORDER_BY' ),$template_desc);
