...
if (strstr ( $cart_data, '{shipping_calculator}' ) && SHIPPING_METHOD_ENABLE)
{
	if (SHOW_SHIPPING_IN_CART) {
		...
		$cart_data = str_replace ( "{shipping_calculator_label}", JText::_ ( 'SHIPPING_CALCULATOR' ), $cart_data );
	} else {
...
if (DEFAULT_QUOTATION_MODE)
{
	...
	$checkout .= '<input type=button class="greenbutton" value="' . JText::_ ( 'CART_CHECKOUT' ) . '" ';
	if (MINIMUM_ORDER_TOTAL > 0 && $cart ['total'] < MINIMUM_ORDER_TOTAL) {
		$checkout .= ' onclick="alert(\'' . JText::_ ( 'MINIMUM_ORDER_TOTAL_HAS_TO_BE_MORE_THAN' ) . ' ' . MINIMUM_ORDER_TOTAL . '\');">';
	} else {
...
$update_all = '<form style="padding:0px;margin:0px;" name="update_cart" method="POST" >
		...
		<input type=button class="blackbutton" value="' . JText::_ ( 'UPDATE' ) . '" onclick="all_update(' . $i . ');">
		</form>';
...
$empty_cart = '<form style="padding:0px;margin:0px;" name="empty_cart" method="POST" >
		...
		<input type=button class="blackbutton" value="' . JText::_ ( 'EMPTY' ) . '" onclick="document.empty_cart.submit();">
		</form>';
...
if (count ( $discount ) > 0) {
	...
	if ($diff > 0)
		$text = sprintf ( JText::_ ( 'DISCOUNT_TEXT' ), $producthelper->getProductFormattedPrice ( $diff, true ), $producthelper->getProductFormattedPrice ( $discount_amount, true ), $price . $discount_sign );
...
if (COUPONS_ENABLE == 1 && VOUCHERS_ENABLE == 1)
{
	...
	$discount_form .= '<input type="submit" id="coupon_button"  class="blackbutton" value="' . JText::_ ( 'SUBMIT_CODE' ) . '" onclick="document.discount_form.task.value=\'coupon\';document.discount_form.submit();" />';
	$coupon_lableFLG = 1;
	...
	$discount_form .= '<input type="submit" id="coupon_button" class="blackbutton" value="' . JText::_ ( 'SUBMIT_CODE' ) . '" onclick="document.discount_form.task.value=\'voucher\';document.discount_form.submit();" />';
	$coupon_lableFLG = 1;
}
...
if($coupon_lableFLG){
	$coupon_lable = "<div id='coupon_label' class='coupon_label'>" . JText::_ ( 'CART_COUPON_CODE_TBL' ) . "</div>";
}
