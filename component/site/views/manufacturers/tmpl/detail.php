...
// Page Title Start
$pagetitle = JText::_('MANUFACTURER_DETAIL');
...
$row = &$this->detail[0];
$category = $model->getmanufacturercategory($row->manufacturer_id);
...
if(strstr($template_desc,"{manufacturer_image}"))
{
	...
	for($m=0; $m<count($media_image); $m++)
	{
		if ($media_image[$m]->media_name && file_exists(JPATH_BASE."/components/".$option."/assets/images/manufacturer/".$media_image[$m]->media_name))
		{
			...
			$thum_image ="<a title='".$altText."' class=\"modal\" href='".$url."/components/".$option."/assets/images/manufacturer/".$media_image[$m]->media_name."'   rel=\"{handler: 'image', size: {}}\">
				<img alt='".$altText."' title='".$altText."' src='".$url."/components/".$option."/helpers/thumb.php?filename=manufacturer/".$media_image[$m]->media_name."&newxsize=".$mw_thumb."&newysize=".$mh_thumb."&swap=".USE_IMAGE_SIZE_SWAPPING."'></a>";
		}
	}
...
$template_desc = str_replace("{manufacturer_allproductslink_lbl}",JText::_('MANUFACTURER_ALLPRODUCTSLINK_LBL'),$template_desc);
