...
if($print)
{
	$print_tag = "<a onclick='window.print();' title='".JText::_('PRINT_LBL')."' ><img src=".$url."images/M_images/printButton.png  alt='".JText::_('PRINT_LBL')."' title='".JText::_('PRINT_LBL')."' /></a>";
} else {
	$print_url = $url."index2.php?option=com_redshop&view=quotation_detail&quoid=".$quoid."&print=1";
	$print_tag = "<a href='#' onclick='window.open(\"$print_url\",\"mywindow\",\"scrollbars=1\",\"location=1\")' title='".JText::_('PRINT_LBL')."' ><img src=".$url."images/M_images/printButton.png  alt='".JText::_('PRINT_LBL')."' title='".JText::_('PRINT_LBL')."' /></a>";
}
...
if($quotationDetail->quotation_status=='2')
{
	...
	<input type='radio' name='quotation_status' checked value='3'>".JText::_('ACCEPT')."
	<input type='radio' name='quotation_status' value='4'>".JText::_('REJECT')."
	...
	<input type='submit' name='submit' value='".JText::_("SUBMIT")."' onclick='return confirm(\"".Jtext::_('CONFIRM_SEND_QUOTATION')."\")' />
	</form>";
}
else if($quotationDetail->quotation_status=='3')
{
	...
	<input type='hidden' name='view' value='quotation_detail'>
	<input type='submit' name='submit' value='".JText::_("CHECKOUT")."' />
	</form>";
} else if($quotationDetail->quotation_status=='5')
{
	$frm = " (".JText::_('ORDER_ID')."-".$quotationDetail->order_id." )";
} else {
	$frm = '';
}
...
if($quotationDetail->user_id!=0)
{
	...
	{
		$billadd .= JText::_("FIRSTNAME").' : '.$quotationDetail->firstname.'<br>';
	}
	if ($quotationDetail->lastname != "")
	{
		$billadd .= JText::_("LASTNAME").' : '.$quotationDetail->lastname.'<br>';
	}
	if ($quotationDetail->address != "")
	{
		$billadd .= JText::_("ADDRESS").' : '.$quotationDetail->address.'<br>';
	}
	if ($quotationDetail->zipcode != "")
	{
		$billadd .= JText::_("ZIP").' : '.$quotationDetail->zipcode.'<br>';
	}
	if ($quotationDetail->city != "")
	{
		$billadd .= JText::_("CITY").' : '.$quotationDetail->city.'<br>';
	}
	if ($order_functions->getCountryName( $quotationDetail->country_code ) != "")
	{
		$billadd .= JText::_("COUNTRY").' : '.JText::_( $order_functions->getCountryName( $quotationDetail->country_code )).'<br>';
	}
	if ($order_functions->getStateName( $quotationDetail->state_code, $quotationDetail->country_code ) != "")
	{
		$billadd .= JText::_("STATE").' : '.$order_functions->getStateName( $quotationDetail->state_code, $quotationDetail->country_code ).'<br>';
	}
	if ($quotationDetail->phone != "")
	{
		$billadd .= JText::_("PHONE").' : '.$quotationDetail->phone.'<br>';
	}
	if ($quotationDetail->user_email != "")
	{
		$billadd .= JText::_("EMAIL").' : '.$quotationDetail->user_email.'<br>';
	}
	if ($quotationDetail->is_company==1)
	{
		if ($quotationDetail->vat_number != "") {
			$billadd .= JText::_ ( "VAT_NUMBER" ).' : '.$quotationDetail->vat_number.'<br>';
		}
...
else
{
	...
	{
		$billadd .= JText::_("EMAIL").' : '.$quotationDetail->quotation_email.'<br>';
	}
}
if(strstr($quotation_template,"{quotation_custom_field_list}"))
{
	...
	$quotation_template("{quotation_custom_field_list}", "", $quotation_template);
}
else
{
...
for($i = 0; $i < count ( $quotationProducts ); $i ++)
{
	...
	if($quotationProducts[$i]->product_wrapperid)
	{
		...
		if(count($wrapper)>0)
		{
			$wrapper_name = JText::_('WRAPPER').":<br/>".$wrapper[0]->wrapper_name."(".$producthelper->getProductFormattedPrice($quotationProducts[$i]->wrapper_price).")";
		}
	}
	...
	$cart_mdata = str_replace ( "{product_number_lbl}", JText::_('PRODUCT_NUMBER_LBL'), $cart_mdata );
