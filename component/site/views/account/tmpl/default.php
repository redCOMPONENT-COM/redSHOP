...
$pagetitle = JText::_('ACCOUNT_MAINTAINANCE');
...
$logoutimg = '<img src="'.$url.'components/'. $option .'/assets/images/account/logout.jpg" align="absmiddle" />';
$logout = '<a href="'.$logout_link.'">'.JText::_('LOGOUT').'</a>';
$template_desc = str_replace('{logout_link}', $logoutimg.$logout , $template_desc);

$account_img = '<img src="'.$url.'components/'.$option.'/assets/images/account/home.jpg" align="absmiddle">';
$template_desc = str_replace('{account_image}',$account_img,$template_desc);
$template_desc = str_replace('{account_title}', JText::_('ACCOUNT_INFORMATION') ,$template_desc);

$customer_fullname_lbl = '';
$customer_fullname='';
if($this->userdata->firstname != "" )
{
	$customer_fullname_lbl 	= JText::_('CUSTOMER_FULLNAME');
...
if($this->userdata->is_company && $this->userdata->company_name != "" ) {
	$company_name_lbl 	= JText::_('COMPANY_NAME');
...
{
	$customer_state_lbl 	= JText::_('CUSTOMER_STATE');
...
{
	$customer_country_lbl 	= JText::_('CUSTOMER_COUNTRY');
...
if( ($this->userdata->is_company==1) && ($this->userdata->vat_number != "") ) {
	$customer_vatnumber_lbl 	= JText::_('CUSTOMER_VATNUMBER');
...
if( ($this->userdata->is_company==1) && ($this->userdata->ean_number != "") ) {
	$ean_number_lbl 	= JText::_('EAN_NUMBER');
...
if($this->userdata->is_company==1){
	$requesting_tax_exempt_lbl 		= JText::_('USER_TAX_EXEMPT_REQUEST_LBL');
	if( $this->userdata->requesting_tax_exempt == 1 )
	{
		$requesting_tax_exempt = JText::_ ( "YES" );
	}else
	{
		$requesting_tax_exempt = JText::_ ( "NO" );
	}
}
...
if($this->userdata->email) {
	$customer_email_lbl 	= JText::_('CUSTOMER_EMAIL');
...
if($this->userdata->city) {
	$customer_city_lbl = JText::_('CITY');
...
if($this->userdata->phone) {
	$customer_phone_lbl = JText::_('PHONE');
...
if($this->userdata->zipcode) {
	$customer_zipcode_lbl = JText::_('ZIP');
...
if($this->userdata->address) {
	$customer_add_lbl = JText::_('ADDRESS');
...
$edit_account_link = '<a href="'.$accountbillto_link.'">'.JText::_('EDIT_ACCOUNT_INFORMATION').'</a>';
...
{
	$shipping_image = '<img src="'.$url.'components/'.$option.'/assets/images/account/order.jpg" align="absmiddle">';
	$update_link = '<a href="'.$accountshipto_link.'">'.JText::_('UPDATE_SHIPPING_INFO').'</a>';
	$template_desc = str_replace('{shipping_image}', $shipping_image , $template_desc);
	$template_desc = str_replace('{shipping_title}', JText::_('SHIPPING_INFO') , $template_desc);
...
{
	$bulkorder_link = "<a href='".JRoute::_('index.php?option=com_redcrm&view=bulk_order&Itemid='.$Itemid)."'>".JText::_('BULK_ORDER_LINK')."</a>";
	...
	$template_desc = str_replace('{bulkorder_title}', JText::_('BULK_ORDER') , $template_desc);
...
if(strstr($template_desc,"{reserve_discount}"))
{
	...
	$template_desc = str_replace('{reserve_discount_lbl}', JTEXT::_('RESERVED_DISCOUNT_LBL') ,$template_desc);
}

if(strstr($template_desc,"{order_loop_start}") && strstr($template_desc,"{order_loop_end}"))
{
	$oder_image = '<img src="'.$url.'administrator/components/'.$option.'/assets/images/order16.png" align="absmiddle">';
	$template_desc = str_replace('{order_image}', $oder_image , $template_desc);
	$template_desc = str_replace('{order_title}', JText::_('ORDER_INFORMATION') , $template_desc);
	...
	if(count($orderslist)>0)
	{
		...
		$template_desc = str_replace('{more_orders}', "<a href='".$ordermoreurl."'>".JText::_('MORE')."</a>" , $template_desc);
	}
	else
	{
	...
	if(count($orderslist))
	{
		for($j=0;$j<count($orderslist);$j++)
		{
			...
			$order_detail ='<a href="'.$orderdetailurl.'">'.JText::_('DETAILS').'</a>';

			$order_data = str_replace('{order_index}', JText::_('ORDER_NUM') , $order_data);
	...
	else
	{
		...
		$order_data = str_replace('{order_detail_link}', JText::_('NO_ORDERS_PLACED_YET') , $order_data);
	}
...
if(strstr($template_desc,"{coupon_loop_start}") && strstr($template_desc,"{coupon_loop_end}"))
{
	...
	if(COUPONINFO)
	{
		$coupon_imagelbl = JText::_('COUPON_INFO');
		$coupon_image = '<img src="'.$url.'components/'.$option.'/assets/images/account/coupon.jpg" align="absmiddle">';
		...
		if(count($usercoupons))
		{
			for($i=0;$i<count($usercoupons);$i++)
			{
				...
				$coupon_data = str_replace('{coupon_code_lbl}', JText::_('COUPON_CODE') , $coupon_data);
				$coupon_data = str_replace('{coupon_code}', $usercoupons[$i]->coupon_code , $coupon_data);
				$coupon_data = str_replace('{coupon_value_lbl}', JText::_('COUPON_VALUE') , $coupon_data);
		...
		else
		{
			...
			$coupon_data = str_replace('{coupon_value}', JText::_('NO_COUPONS') , $coupon_data);
		}
	}
...
if(MY_TAGS)
{
	$tag_imagelbl = JText::_('MY_TAGS');
	$tag_image = '<img src="'.$url.'administrator/components/'.$option.'/assets/images/textlibrary16.png" align="absmiddle">';
 	$tag_link= JText::_('NO_TAGS_AVAILABLE');
 	$myTags = $model->countMyTags();
 	if ($myTags > 0)
 	{
 		$tag_link = '<a href="'.$mytags_link.'" style="text-decoration: none;">'.JText::_("SHOW_TAG").'</a>';
 	}
}
...
{
	$quotation_image = '<img src="'.$url.'administrator/components/'.$option.'/assets/images/quotation_16.jpg" align="absmiddle">';
	$template_desc = str_replace('{quotation_image}', $quotation_image , $template_desc);
	$template_desc = str_replace('{quotation_title}', JText::_('QUOTATION_INFORMATION') , $template_desc);
	...
	if(count($quotationlist)>0)
	{
		...
		$template_desc = str_replace('{more_quotations}', "<a href='".$quotationmoreurl."'>".JText::_('MORE')."</a>" , $template_desc);
	}
	...
	if(count($quotationlist))
	{
		for($j=0;$j<count($quotationlist);$j++)
		{
			...
			$quotation_detail = '<a href="'.$quotationurl.'" title="'.JText::_('VIEW_QUOTATION').'"  alt="'.JText::_('VIEW_QUOTATION').'">'.JText::_('DETAILS').'</a>';

			$quotation_data = str_replace('{quotation_index}', JText::_('QUOTATION')." #" , $quotation_data);
	...
	else
	{
		...
		$quotation_data = str_replace('{quotation_detail_link}', JText::_('NO_QUOTATION_PLACED_YET') , $quotation_data);
	}
...
if(MY_WISHLIST)
{
	$wishlist_imagelbl = JText::_('MY_WISHLIST');
	$wishlist_image = '<img src="'.$url.'administrator/components/'.$option.'/assets/images/textlibrary16.png" align="absmiddle">';
 	$edit_wishlist_link= JText::_('NO_PRODUCTS_IN_WISHLIST');
 	$myWishlist = $model->countMyWishlist();
 	if ($myWishlist > 0)
 	{
 		$edit_wishlist_link = '<a href="'.$wishlist_link.'" style="text-decoration: none;">'.JText::_("SHOW_WISHLIST_PRODUCTS").'</a>';
 	}
}
...
{
	$product_serial_image = '<img src="'.$url.'administrator/components/'.$option.'/assets/images/products16.png" align="absmiddle">';
	$template_desc = str_replace('{product_serial_image}', $product_serial_image , $template_desc);
	$template_desc = str_replace('{product_serial_title}', JText::_('MY_SERIALS') , $template_desc);
...
if(COMARE_PRODUCTS)
{
	$cmp_imagelbl = JText::_('COMPARE_PRODUCTS');
	$cmp_image = '<img src="'.$url.'administrator/components/'.$option.'/assets/images/textlibrary16.png" align="absmiddle">';
 	$cmp_link= JText::_('NO_PRODUCTS_TO_COMPARE');
 	$compare	= $producthelper->getcompare();
 	if (isset($compare['idx']) && $compare['idx']>0)
 	{
 		$cmp_link = '<a href="'.$compare_link.'" style="text-decoration: none;">'.JText::_("SHOW_PRODUCTS_TO_COMPARE").'</a>';
 	}
}
