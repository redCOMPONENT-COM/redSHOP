...
if ($user->id != 0)
{
	if (isset($tagid))
	{
		...
		<table border="0" cellpadding="5" cellspacing="0" width="100%" class="adminlist">
			<tr>
				<td width="10%"><?php echo JText::_('TAG_NAME');?></td>
				<td width="70%"><input type="text" name="tags_name" value="<?php echo $model->getMytag($tagid);?>" id="tags_name" size="50" /></td>
				<td width="20%">
					<input type="submit" class="button" name="tags_submit" value="<?php echo JText::_('EDIT_TAG');?>" />
		...
		<div>&nbsp;</div>
		<div><a href="<?php echo 'index.php?option=com_redshop&view=account&layout=mytags&tagid='.$tagid.'&Itemid='.$Itemid;?>" title="<?php echo JText::_('BACK_TO_TAG_LIST');?>"><?php echo JText::_('BACK_TO_TAG_LIST');?></a></div>
		<?php			
		}else {
		...
		<table border="0" cellpadding="5" cellspacing="0" width="100%">
			<tr>				
				<td align="right">			
					<span><a href="<?php echo $link_edit;?>" title=""  style="text-decoration: none;"><?php echo JText::_('EDIT_TAG');?></a>&nbsp;|</span>
					<span><a href="<?php echo $link_remove;?>" title=""  style="text-decoration: none;"><?php echo JText::_('REMOVE_TAG');?></a></span>				
				</td>
			</tr>
		</table>		
		<div>&nbsp;</div>
				...
				if (count($MyTags) > 0){
					foreach ($MyTags as $row){
						...
						$data_add .="<div><form name='addtocartscroll".$i."' id='addtocartscroll".$i."' action='' method='post' >
										...
										<span onclick='document.addtocartscroll".$i.".submit();' align='center' style='background-image:url(".$url."/components/com_redshop/assets/images/".ADDTOCART_BACKGROUND.");border:1px solid #183a5c;background-position:bottom;background-repeat:no-repeat;cursor:pointer;'><span style='cursor: pointer;' >".JText::_('ADD_TO_CART')."</span></span>
										</form>
										</div>";
						$i++;
						$data_add .= '</div>';
					...
					echo "<div>".JText::_('NO_PRODUCTS_IN_TAGS')."</div>";
				}
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>        
		        <td>
		        	<div><a href="<?php echo 'index.php?option=com_redshop&view=account&layout=mytags&Itemid='.$Itemid;?>" title="<?php echo JText::_('BACK_TO_TAG_LIST');?>"><?php echo JText::_('BACK_TO_TAG_LIST');?></a></div>
		    	</td>
	...
	<legend><?php echo JText::_('AVAILABLE_TAGS')?></legend>
			...
			<td><?php
					if (count($MyTags) != 0){ 
						foreach ($MyTags as $MyTag){
							?>
							<a href="<?php echo 'index.php?option='.$option.'&view=account&layout=mytags&tagid='.$MyTag->tags_id.'&Itemid='.$Itemid;?>" style="text-decoration: none;"><span style="font-size: <?php echo $MyTag->tags_counter+15;?>px;"><?php echo $MyTag->tags_name; ?></span></a>
							<?php 			
						}
					}
				?>
	...
	<div>&nbsp;</div>
	<div ><a href="<?php echo 'index.php?option=com_redshop&view=account&Itemid='.$Itemid;?>" title="<?php echo JText::_('BACK_TO_MYACCOUNT');?>"><?php echo JText::_('BACK_TO_MYACCOUNT');?></a></div>
	<?php 
	}
}
?>
