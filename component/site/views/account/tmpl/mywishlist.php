...
if ($mail == 0)
{
	...
	if(strstr($wishlist_data,"{child_products}"))
	{
		...
		// Product User Field End	
			...
			$remove = '<a href="'.$link_remove.'" title="" style="text-decoration:none;">'.JText::_("REMOVE_PRODUCT_FROM_WISHLIST").'</a>';
	...
	$back_link = '<a href="'.JRoute::_( 'index.php?option=com_redshop&view=account&Itemid='.$Itemid).'" title="'.JText::_('BACK_TO_MYACCOUNT').'">'.JText::_('BACK_TO_MYACCOUNT').'</a>';
...
else
{
	...
	$data = str_replace("{email_to_friend}",JText::_("EMAIL_TO_FRIEND"),$data);
	$data = str_replace("{emailto_lbl}",JText::_("EMAIL_TO"),$data);
	$email_to_field = '<input type="text" name="emailto" value="" />';
	$data = str_replace("{emailto}",$email_to_field,$data);
	$data = str_replace("{sender_lbl}", JText::_('SENDER') ,$data);
	$sender_field = '<input type="text" name="sender" value="'.$user->name.'" />';
	$data = str_replace("{sender}", $sender_field ,$data);
	$data = str_replace("{mail_lbl}", JText::_('YOUR_EMAIL') ,$data);
	$email_field = '<input type="text" name="email" value="'.$user->email.'" />';
	$data = str_replace("{mail}", $email_field ,$data);
	$data = str_replace("{subject_lbl}", JText::_('SUBJECT') ,$data);
	$subject = '<input type="text" name="subject" value="" >';
	$data = str_replace("{subject}", $subject ,$data);

	$cancel_btn = '<input type="button" name="cancel" class="button" onclick="parent.location.reload();" value="'.JText::_('CANCEL').'" />';
	$data = str_replace("{cancel_button}", $cancel_btn ,$data);

	$send_btn = '<input type="submit" name="send" class="button" value="'.JText::_('SEND').'" />';
