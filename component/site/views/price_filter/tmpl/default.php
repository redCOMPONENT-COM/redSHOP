					...
					if($row->product_on_sale && $product_price_discount > 0)
					{
						if($product_price > $product_price_discount)
						{
							...
							if($show_discountpricelayout)
							{
								...
								$pricetext .="<div id='mod_redsavedprice' class='mod_redsavedprice'>".JText::_('PRODCUT_PRICE_YOU_SAVED').' '.$producthelper->getProductFormattedPrice($s_price)."</div>";
							} else {
...
<?php	}
	...
	if(!$k){
	echo "<tr><td>".JText::_('NO_PRODUCT_FOUND')."</td></tr>";
	}	?>
</table>