...
class searchModelsearch extends JModel
{
	...
	function __construct()
	{
		...
		$productlimit = $params->get('productlimit');
	...
	function _buildQuery($manudata=0)
	{
		...
		# changes by Gunjan for more accuracy
...
//		}
		...
		{
			$defaultSearchField = "name_number_desc";
		...
		{
			$qand = ($manufacture_id!=0) ? " AND p.manufacturer_id='".$manufacture_id."' " : "";
		...
		else if($layout == 'featuredproduct')
		{
			...
			$catid =  $params->get( 'categorytemplate' );
	...
	function getCategoryTemplet()
	{
		...
		$templateid	= $mainframe->getUserStateFromRequest( $context.'templateid', 'templateid',   '');
	  	$params = &JComponentHelper::getParams( 'com_redshop' );
	    $cid = ($layout=='newproduct' || $layout=='productonsale') ? $params->get('categorytemplate') : 0;
				...
				if (count($tags) > 0) {
					...
					for ($t=0;$t<count($tags);$t++){
						...
						if ($tags[$t]->tagid == $tag_id){
							...
							$tagname .= "<span style='float:left;'>&nbsp;&nbsp;".$tags[$t]->tag_name."</span><span style='float:right;'><a href='javascript:deleteTag(\"$type->id\",\"$Itemid\");' title='".JText::_('DELETE')."' >".JText::_('DELETE')."</a></span><br/>";
...
<?php
	...
	?>
<div><a href="<?php echo JRoute::_('index.php?option=com_redshop&view=search&layout=redfilter&remove=1&Itemid='.$Itemid); ?>" title="<?php echo JText::_('Clear All'); ?>" ><?php echo JText::_('Clear All'); ?></a></div>
<div id="spacer">&nbsp;_________________________</div>
<?php
}
if (count($types)>0){
?>
<div id="pfsearchheader"><?php echo JText::_('SEARCH_CRITERIA');?></div>
...
//	$ptotal = getProducttotal($type->id);
	...
	if (@!array_key_exists($type->id,$getredfilter) && @array_key_exists('type'.$key,$lists)){
		...
		?>
		<div id="<?php echo $type->id;?>" class="typename <?php echo $type->type_name_css; ?>" ><?php echo $type->type_name; ?>
		<?php if (strlen($type->tooltip) > 0) {
			echo ' '.JHTML::tooltip($type->tooltip, $type->type_name, 'tooltip.png', '', '', false);
		} 	?>
		</div>
