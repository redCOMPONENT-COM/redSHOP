...
class RedshopModelSearch extends RedshopModel
{
    ...
    public function _buildQuery($manudata = 0, $getTotal = false)
    {
        ...
        if ($layout == 'productonsale') {
            ...
            $query->leftJoin(
                $db->qn('#__redshop_fields_data', 'fds') . ' ON p.product_id = fds.itemid'
                . $joinSearchable
            );
                ...
                $query->leftJoin($db->qn('#__redshop_product_attribute', 'a') . ' ON a.product_id = p.product_id')
                    ->leftJoin(
                        $db->qn(
                            ...
                            'pap'
                        ) . ' ON pap.attribute_id = a.attribute_id'
                    )
                    ->leftJoin(
                        $db->qn(
                            '#__redshop_product_subattribute_color',
                            'ps'
                        ) . ' ON ps.subattribute_id = pap.property_id'
                    );
            }
    ...
    public function mod_redProductfilter($Itemid)
    {
        ...
        foreach ($types as $key => $type) {
            if (@!array_key_exists($type->id, $getredfilter)) {
                ...
                $str                        = preg_replace(
                    '/&([a-zA-Z])(uml|acute|grave|circ|tilde|elig|slash|ring);/',
                    '$1',
                    $str
                );
                $str                        = str_replace(' ', '', $str);
        ...
        if (count($getredfilter) != 0) {
            ...
            <div id="pfsearchheader">
                <?php echo Text::_('COM_REDSHOP_SEARCH_RESULT'); ?>
            </div>
            ...
            foreach ($getredfilter as $typeid => $tag_id) {
                foreach ($types as $key => $type) {
                    if ($typeid == $type->id) {
                        ?>
                        <div id="typename_<?php echo $type->id; ?>" class="typename <?php echo $type->type_name_css; ?>">
                        ...
                        <div id="typevalue_<?php echo $type->id; ?>" class="typevalue <?php echo $type->type_name_css; ?>">
                            <?php echo $filteredlists['type' . $key]; ?>
                        </div>
            ...
            <div>
                <a href="<?php echo Redshop\IO\Route::_(
                    ...
                    <?php echo Text::_('COM_REDSHOP_CLEAR_ALL'); ?>
                </a>
            </div>
        ...
        if (count($types) > 0) {
            ?>
            <div id="pfsearchheader">
                <?php echo Text::_('COM_REDSHOP_SEARCH_CRITERIA'); ?>
            </div>
            ...
            foreach ($types as $key => $type) {
                if (@!array_key_exists($type->id, $getredfilter) && @array_key_exists('type' . $key, $lists)) {
                    ?>
                    <div id="<?php echo $type->id; ?>" class="typename <?php echo $type->type_name_css; ?>">
                        <?php echo $type->type_name; ?>
                        <?php
                        if (strlen($type->tooltip) > 0) {
                            HTMLHelper::_('bootstrap.tooltip', '.hasTooltip');
    ...
    public function getajaxData()
    {
        ...
        if ($search_type == "name_number_desc" || $search_type == "virtual_product_num") {
            $query->leftJoin($db->qn('#__redshop_product_attribute', 'a') . ' ON a.product_id = p.product_id')
                ->leftJoin(
                    $db->qn('#__redshop_product_attribute_property', 'pap') . ' ON pap.attribute_id = a.attribute_id'
                )
                ->leftJoin(
                    $db->qn('#__redshop_product_subattribute_color', 'ps') . ' ON ps.subattribute_id = pap.property_id'
                );
        }
        ...
        if ($data) {
            foreach ($data as &$row) {
                ...
                //get product price
                $row->product_price = RedshopHelperProductPrice::formattedPrice(
                    Redshop\Product\Price::getPrice($row->id, false, 0)
                );
            }
        }
    ...
    public function getListQuery()
    {
        ...
        if (!empty($keyword)) {
            ...
            $query->leftJoin(
                ...
                ->where(
                    '(' . $db->qn('p.product_name') . ' LIKE ' . $search . ' OR ' . $db->qn(
                        'm.name'
                    ) . ' LIKE ' . $search . ')'
                );
        }
        ...
        if (!empty($customField)) {
            ...
            foreach ($customField as $fieldId => $fieldValues) {
                ...
                $query->leftJoin(
                    $db->qn('#__redshop_fields_data', 'fd' . $key) . ' ON ' . $db->qn('p.product_id') . ' = ' . $db->qn(
                        'fd' . $key . '.itemid'
                    )
                )
                    ->where($db->qn('fd' . $key . '.fieldid') . ' = ' . $db->q((int) $fieldId));
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        if (
            !strstr($templateDesc, "{show_all_products_in_category}")
        ...
        $productlimit = $app->getUserStateFromRequest(
            $this->context . '.productlimit',
            'productlimit',
            $productlimit,
            8
        );
