...
class accountModelaccount extends JModel
{
	...
	function removeWishlistProduct()
	{
		...
		if(count($list)>0)
		{
			...
			if ($this->_db->Query())
			{
				$mainframe->enqueueMessage(JText::_('WISHLIST_PRODUCT_DELETED_SUCCESSFULLY'));
			}else {
				$mainframe->enqueueMessage(JText::_('ERROR_DELETING_WISHLIST_PRODUCT'));
			}
		}
		else
		{
			$mainframe->enqueueMessage(JText::_('YOU_DONT_HAVE_ACCESS_TO_DELETE_THIS_PRODUCT'));
		}
	...
	function removeTag()
	{
		...
		{
			$mainframe->enqueueMessage(JText::_('TAG_DELETED_SUCCESSFULLY'));
		}else {
			$mainframe->enqueueMessage(JText::_('ERROR_DELETING_TAG'));
		}
	...
	function removeCompare()
	{
		...
		if ($this->_db->Query())
		{
			$mainframe->enqueueMessage(JText::_('PRODUCT_DELETED_FROM_COMPARE_SUCCESSFULLY'));
		}else {
			$mainframe->enqueueMessage(JText::_('ERROR_DELETING_PRODUCT_FROM_COMPARE'));
		}
	...
	function sendWishlist($post)
	{
		...
		else
		{
			if (count($MyWishlist))
			{
				$link = JURI::root()."index2.php?option=com_redshop&view=account&layout=mywishlist&mail=1";
				foreach ($MyWishlist as $row)
				{
		...
		if (JUtility::sendMail($email,$sender,$emailto,$subject,$data_add,true,NULL,$mailbcc))
			return true;
		else
			return false;
