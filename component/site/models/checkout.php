...
class RedshopModelCheckout extends RedshopModel
{
	...
	public function orderplace()
	{
		...
		// Redirect if cart is empty without any process
		if ($cart['idx'] < 1)
		{
			$app->redirect(JRoute::_('index.php?option=com_redshop&Itemid=' . $itemId), JText::_('COM_REDSHOP_EMPTY_CART'));
		}

		$shopId  = RedshopHelperCheckoutGls::getShopId($shopId);
		$user    = JFactory::getUser();
		$session = JFactory::getSession();
		...
		$shippingAddresses  = $this->shipaddress($userInforId);
		...
		$orderDiscount        = $cart['coupon_discount'] + $cart['voucher_discount'] + $cart['cart_discount'];
		$orderDiscountVat = $cart['discount_vat'];

		$d["order_payment_trans_id"] = '';
		$d['discount']               = $orderDiscount;
		...
		{
			$paymentOprand = $cart['payment_oprand'];
		}

		$isCreditCard = $paymentMethod->params->get('is_creditcard', '');
		$isRedirected = $paymentMethod->params->get('is_redirected', 0);
		...
		if ($isCreditCard == 1 && $isRedirected == 0 && $cart['total'] > 0)
		{
			...
			if ($paymentResponse->responsestatus == "Success")
			{
				...
				$orderStatusLog             = $paymentResponse->message;
		...
		$orderDetailTable = $this->getTable('order_detail');

		if (!$orderDetailTable->bind($post))
		{
		...
		$orderDetailTable->analytics_status   = 0;
		$orderDetailTable->order_shipping     = $orderShipping [3];
		$orderDetailTable->order_shipping_tax = $shippingVatRate;
		$orderDetailTable->coupon_discount    = $cdiscount;
		$orderDetailTable->shop_id            = $shopId;
		$orderDetailTable->customer_message   = $customerMessage;
		$orderDetailTable->referral_code      = $referralCode;
		$db                                   = JFactory::getDbo();
		...
		$orderDetailTable->order_discount       = $orderDiscount;
		$orderDetailTable->order_discount_vat   = $orderDiscountVat;
		$orderDetailTable->payment_discount     = $paymentAmount;
		$orderDetailTable->payment_oprand       = $paymentOprand;
		$orderDetailTable->order_status         = $orderStatus;
		$orderDetailTable->order_payment_status = $orderPaymentStatus;
		$orderDetailTable->cdate                = $timestamp;
		$orderDetailTable->mdate                = $timestamp;
		$orderDetailTable->ship_method_id       = $shippingRateId;
		$orderDetailTable->customer_note        = $post['customer_note'];
		$orderDetailTable->requisition_number   = $post['requisition_number'];
		$orderDetailTable->ip_address           = $ip;
		$orderDetailTable->encr_key             = $random_gen_enc_key;
		$orderDetailTable->discount_type        = $this->discount_type;
		$orderDetailTable->order_id             = $app->input->getInt('order_id', 0);
		// @deprecated  2.0.3
		$orderDetailTable->barcode = null;

		if (!$orderDetailTable->store())
		{
			$this->setError($this->_db->getErrorMsg());
		...
		if (((boolean) Redshop::getConfig()->get('INVOICE_NUMBER_FOR_FREE_ORDER') || $isCreditCard)
			&& ('C' == $orderDetailTable->order_status && 'Paid' == $orderDetailTable->order_payment_status)
		)
		{
			RedshopHelperOrder::generateInvoiceNumber($orderDetailTable->order_id);
		}

		$order_id = $orderDetailTable->order_id;

		$this->coupon($cart, $order_id);
		$this->voucher($cart, $order_id);

		// @TODO Consider use JTable for loading and storing
		...
		{
			RedshopHelperUser::updateUserTermsCondition($userInforId, 1);
		}

		// Place order id in quotation table if it Quotation
		if (array_key_exists("quotation_id", $cart) && $cart['quotation_id'])
		{
			RedshopHelperQuotation::updateQuotationWithOrder($cart['quotation_id'], $orderDetailTable->order_id);
		}

		if ($orderDetailTable->order_status == Redshop::getConfig()->get('CLICKATELL_ORDER_STATUS'))
		{
		...
		$rowOrderStatus->customer_note = $orderStatusLog;
		$rowOrderStatus->store();

		$input->set('order_id', $orderDetailTable->order_id);
		$input->set('order_number', $orderDetailTable->order_number);
		...
		// Storing order item detail
		for ($i = 0; $i < $idx; $i++)
		{
			$is_giftcard          = 0;
			$product_id           = $cart [$i] ['product_id'];
			$product              = RedshopHelperProduct::getProductById($product_id);
			$orderItemDetailTable = $this->getTable('order_item_detail');

			if (!$orderItemDetailTable->bind($post))
			{
				$this->setError($this->_db->getErrorMsg());
			...
			$orderItemDetailTable->delivery_time = '';
			...
			if (!$is_giftcard)
			{
				...
				$orderItemDetailTable->stockroom_id       = $stockroom_id_list;
				$orderItemDetailTable->stockroom_quantity = $stockroom_quantity_list;
			}
			...
			{
				$orderItemDetailTable->attribute_image = $order_id . $cart[$i]['attributeImage'];
				$old_media                             = JPATH_ROOT . '/components/com_redshop/assets/images/mergeImages/' . $cart[$i]['attributeImage'];
				$new_media                             = JPATH_ROOT . '/components/com_redshop/assets/images/orderMergeImages/' . $orderItemDetailTable->attribute_image;
				JFile::copy($old_media, $new_media);
			}
			elseif (!empty($vals[1]))
			{
				$orderItemDetailTable->attribute_image = $vals[1];
			}

			$wrapper_price = 0;

			if (isset($cart[$i]['wrapper_id']) && $cart[$i]['wrapper_id'])
			{
			...
			if ($is_giftcard == 1)
			{
				...
				$orderItemDetailTable->product_id             = $cart [$i] ['giftcard_id'];
				$orderItemDetailTable->order_item_name        = $giftcardData->giftcard_name;
				$orderItemDetailTable->product_item_old_price = $cart [$i] ['product_price'];
			}
			else
			{
				$orderItemDetailTable->product_id             = $product_id;
				$orderItemDetailTable->product_item_old_price = $cart [$i] ['product_old_price'];
				$orderItemDetailTable->supplier_id            = $product->manufacturer_id;
				$orderItemDetailTable->order_item_sku         = $product->product_number;
				$orderItemDetailTable->order_item_name        = $product->product_name;
			}

			$orderItemDetailTable->product_item_price          = $cart [$i] ['product_price'];
			$orderItemDetailTable->product_quantity            = $cart [$i] ['quantity'];
			$orderItemDetailTable->product_item_price_excl_vat = $cart [$i] ['product_price_excl_vat'];
			$orderItemDetailTable->product_final_price         = ($cart [$i] ['product_price'] * $cart [$i] ['quantity']);
			$orderItemDetailTable->is_giftcard                 = $is_giftcard;
			...
			$orderItemDetailTable->order_id            = $order_id;
			$orderItemDetailTable->user_info_id        = $userInforId;
			$orderItemDetailTable->order_item_currency = Redshop::getConfig()->get('REDCURRENCY_SYMBOL');
			$orderItemDetailTable->order_status        = $orderStatus;
			$orderItemDetailTable->cdate               = $timestamp;
			$orderItemDetailTable->mdate               = $timestamp;
			$orderItemDetailTable->product_attribute   = $cart_attribute;
			$orderItemDetailTable->discount_calc_data  = $cart_calc_data;
			$orderItemDetailTable->product_accessory   = $cart_accessory;
			$orderItemDetailTable->wrapper_price       = $wrapper_price;

			if (!empty($cart[$i]['wrapper_id']))
			{
				$orderItemDetailTable->wrapper_id = $cart[$i]['wrapper_id'];
			}

			if (!empty($cart[$i]['reciver_email']))
			{
				$orderItemDetailTable->giftcard_user_email = $cart[$i]['reciver_email'];
			}

			if (!empty($cart[$i]['reciver_name']))
			{
				$orderItemDetailTable->giftcard_user_name = $cart[$i]['reciver_name'];
			}

			if ($this->_producthelper->checkProductDownload($orderItemDetailTable->product_id))
			{
				$medianame = $this->_producthelper->getProductMediaName($orderItemDetailTable->product_id);

				for ($j = 0, $jn = count($medianame); $j < $jn; $j++)
				{
					$product_serial_number = $this->_producthelper->getProdcutSerialNumber($orderItemDetailTable->product_id);
					$this->_producthelper->insertProductDownload($orderItemDetailTable->product_id, $user->id, $orderItemDetailTable->order_id, $medianame[$j]->media_name, $product_serial_number->serial_number);
				}
			}
			...
			// Add plugin support
			$dispatcher->trigger('afterOrderItemSave', array($cart, $orderItemDetailTable, $i));
			...
			$this->_producthelper->insertProdcutUserfield($i, $cart, $orderItemDetailTable->order_item_id, $section_id);
			...
			if (count($cart [$i] ['cart_accessory']) > 0)
			{
				...
				for ($a = 0, $an = count($attArr); $a < $an; $a++)
				{
					...
					{
						$accessory_vat_price = $this->_producthelper->getProductTax($orderItemDetailTable->product_id, $accessory_price);
					}
					...
					for ($j = 0, $jn = count($attchildArr); $j < $jn; $j++)
					{
						...
						if ($totalProperty)
						{
							...
							$orderAttributeItemTable                    = $this->getTable('order_attribute_item');
							$orderAttributeItemTable->order_att_item_id = 0;
							$orderAttributeItemTable->order_item_id     = $orderItemDetailTable->order_item_id;
							$orderAttributeItemTable->section_id        = $attribute_id;
							$orderAttributeItemTable->section           = "attribute";
							$orderAttributeItemTable->parent_section_id = $accessory_id;
							$orderAttributeItemTable->section_name      = $attchildArr[$j]['attribute_name'];
							$orderAttributeItemTable->is_accessory_att  = 1;

							if ($attribute_id > 0)
							{
								if (!$orderAttributeItemTable->store())
								{
									$this->setError($this->_db->getErrorMsg());
						...
						for ($k = 0; $k < $totalProperty; $k++)
						{
							...
							{
								$section_vat = RedshopHelperProduct::getProductTax($orderItemDetailTable->product_id, $propArr[$k]['property_price']);
							}
							...
							$orderAttributeItemTable->order_att_item_id = 0;
							$orderAttributeItemTable->order_item_id     = $orderItemDetailTable->order_item_id;
							$orderAttributeItemTable->section_id        = $property_id;
							$orderAttributeItemTable->section           = "property";
							$orderAttributeItemTable->parent_section_id = $attribute_id;
							$orderAttributeItemTable->section_name      = $propArr[$k]['property_name'];
							$orderAttributeItemTable->section_price     = $propArr[$k]['property_price'];
							$orderAttributeItemTable->section_vat       = $section_vat;
							$orderAttributeItemTable->section_oprand    = $propArr[$k]['property_oprand'];
							$orderAttributeItemTable->is_accessory_att  = 1;
							...
							for ($l = 0, $nl = count($subpropArr); $l < $nl; $l++)
							{
								...
								{
									$section_vat = RedshopHelperProduct::getProductTax($orderItemDetailTable->product_id, $subpropArr[$l]['subproperty_price']);
								}
								...
								$orderAttributeItemTable->order_att_item_id = 0;
								$orderAttributeItemTable->order_item_id     = $orderItemDetailTable->order_item_id;
								$orderAttributeItemTable->section_id        = $subproperty_id;
								$orderAttributeItemTable->section           = "subproperty";
								$orderAttributeItemTable->parent_section_id = $property_id;
								$orderAttributeItemTable->section_name      = $subpropArr[$l]['subproperty_name'];
								$orderAttributeItemTable->section_price     = $subpropArr[$l]['subproperty_price'];
								$orderAttributeItemTable->section_vat       = $section_vat;
								$orderAttributeItemTable->section_oprand    = $subpropArr[$l]['subproperty_oprand'];
								$orderAttributeItemTable->is_accessory_att  = 1;
					...
					$rowaccitem->order_item_id           = $orderItemDetailTable->order_item_id;
			...
			if (count($cart [$i] ['cart_attribute']) > 0)
			{
				...
				for ($j = 0, $jn = count($attchildArr); $j < $jn; $j++)
				{
					...
					if ($totalProperty > 0)
					{
						...
						$orderAttributeItemTable->section_id        = $attribute_id;
						$orderAttributeItemTable->section           = "attribute";
						$orderAttributeItemTable->parent_section_id = $orderItemDetailTable->product_id;
						$orderAttributeItemTable->section_name      = $attchildArr[$j]['attribute_name'];
						$orderAttributeItemTable->is_accessory_att  = 0;
						...
						for ($k = 0; $k < $totalProperty; $k++)
						{
							...
							$orderAttributeItemTable->section_oprand     = $propArr[$k]['property_oprand'];
							$orderAttributeItemTable->is_accessory_att   = 0;
							$orderAttributeItemTable->stockroom_id       = $stockroom_att_id_list;
							$orderAttributeItemTable->stockroom_quantity = $stockroom_att_quantity_list;
							...
							for ($l = 0, $nl = count($subpropArr); $l < $nl; $l++)
							{
								...
								$orderAttributeItemTable->section_oprand     = $subpropArr[$l]['subproperty_oprand'];
								$orderAttributeItemTable->is_accessory_att   = 0;
								$orderAttributeItemTable->stockroom_id       = $stockroom_subatt_id_list;
								$orderAttributeItemTable->stockroom_quantity = $stockroom_subatt_quantity_list;
			...
			if ($product->product_type == 'subscription')
			{
				...
				$subscribe->order_item_id   = $orderItemDetailTable->order_item_id;
		...
		$orderPaymentTable = $this->getTable('order_payment');

		if (!$orderPaymentTable->bind($post))
		{
			$this->setError($this->_db->getErrorMsg());
		...
		$orderPaymentTable->order_id          = $order_id;
		$orderPaymentTable->payment_method_id = $paymentMethodId;

		$ccData = RedshopHelperCheckout::getCcData();

		$orderPaymentTable->order_payment_code     = $ccData['creditcard_code'];
		$orderPaymentTable->order_payment_cardname = base64_encode($ccData['order_payment_name']);
		$orderPaymentTable->order_payment_number   = base64_encode($ccData['order_payment_number']);

		// This is ccv code
		$orderPaymentTable->order_payment_ccv      = base64_encode($ccData['credit_card_code']);
		$orderPaymentTable->order_payment_amount   = $orderTotal;
		$orderPaymentTable->order_payment_expire   = $ccData['order_payment_expire_month'] . $ccData['order_payment_expire_year'];
		$orderPaymentTable->order_payment_name     = $paymentMethod->name;
		$orderPaymentTable->payment_method_class   = $paymentMethod->element;
		$orderPaymentTable->order_payment_trans_id = $d ["order_payment_trans_id"];
		$orderPaymentTable->authorize_status       = "";

		if (!$orderPaymentTable->store())
		{
			$this->setError($this->_db->getErrorMsg());
		...
		// Add billing Info
		$userDetailTable = $this->getTable('user_detail');
		$userDetailTable->load($billingAddresses->users_info_id);
		$userDetailTable->thirdparty_email = $post['thirdparty_email'];
		$orderUserRow                      = $this->getTable('order_user_detail');

		if (!$orderUserRow->bind($userDetailTable))
		{
			$this->setError($this->_db->getErrorMsg());
		...
		{
			$userDetailTable->load($shippingAddresses->users_info_id);
		}
		...
		RedshopHelperCheckoutEconomic::integrate($orderDetailTable, $paymentMethod);
		RedshopHelperCheckout::sendOrderMail($orderDetailTable);
		RedshopHelperCheckout::sendDownload($orderDetailTable);
