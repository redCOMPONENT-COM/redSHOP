...
class checkoutModelcheckout extends JModel {
	...
	function orderplace()
	{
    	...
    	$params 					= new JParameter($paymentmethod->params,$xmlpath);
		 ...
		 * page will redirect to order detail page from plugin
		 */
		...
		for($i = 0; $i < $idx; $i ++)
		{
			...
			if(!is_null($cart[$i]['attributeImage']) && $cart[$i]['attributeImage']){
				$rowitem->attribute_image = $cart[$i]['attributeImage'];
			}else{
			...
			if(count($cart [$i] ['cart_attribute'])>0)
			{
				...
				for($j=0;$j<count($attchildArr);$j++)
				{
					...
					for($k=0;$k<count($propArr);$k++)
					{
						...
						//$stockroomhelper->updateStockroomQuantity($property_id,$cart [$i] ['quantity'],"property", $product_id);
						$rowattitem 					= & $this->getTable ( 'order_attribute_item' );
						$rowattitem->order_att_item_id 	= 0;
						$rowattitem->order_item_id 		= $rowitem->order_item_id;
						$rowattitem->section_id 		= $property_id;
						...
						for($l=0;$l<count($subpropArr);$l++)
						{
							...
							//$stockroomhelper->updateStockroomQuantity($subproperty_id,$cart [$i] ['quantity'],"subproperty", $product_id);
		...
		if($session->get('isredcrmuser') && count($crmDebitorHelper_contact_values)>0)
		{
			$orderuserrow->user_id = $crmDebitorHelper_contact_values[0]->cp_user_id;
			$orderuserrow->user_email = $crmDebitorHelper_contact_values[0]->person_email;
			$name_sp = preg_split(" ",$crmDebitorHelper_contact_values[0]->person_name,2);
			$orderuserrow->firstname = $name_sp[0];
			$orderuserrow->lastname = $name_sp[1];
		}
...
function sendGiftCard($order_id)
	{
				...
				foreach($orders as $eachorders)
				{
						...
						$thum_image 		= "<img src='" . $url . "/components/com_redshop/assets/images/giftcard/" . $giftcardData->giftcard_image."'  title='" . $giftcardData->giftcard_name . "' alt='" . $giftcardData->giftcard_name . "'></a>";
						...
						if(file_exists( JPATH_SITE . '/components/com_redshop/assets/images/giftcard/' . $giftcardData->giftcard_bgimage ) && $giftcardData->giftcard_bgimage)
						$pdf->img_file = JPATH_SITE . '/components/com_redshop/assets/images/giftcard/' . $giftcardData->giftcard_bgimage;
						...
						if(file_exists ( JPATH_SITE . '/components/com_redshop/assets/images/giftcard/' . $giftcardData->giftcard_image )  && $giftcardData->giftcard_image)
					{
						$pdfImage = '<img src="'. JPATH_SITE . '/components/com_redshop/assets/images/giftcard/' . $giftcardData->giftcard_image.'" alt="test alt attribute" width="150px" height="150px" border="0" />';
					}
	...
	function validatepaymentccinfo()
	{
		...
		{ //Not? Then Error
			$validpayment [0] = 0;
			$validpayment [1] = JText::_ ( 'CHECKOUT_ERR_NO_CCDATA' );
			return $validpayment;
		}
		if(isset($ccdata['order_payment_name']))
		{
			...
			{
				$validpayment [0] = 0;
				$validpayment [1] = JText::_ ( 'CHECKOUT_ERR_NO_CCNM_FOUND' );
				return $validpayment;
			}
		}
		...
		{
			$validpayment [0] = 0;
			$validpayment [1] = JText::_ ( 'CHECKOUT_ERR_NO_CCNR_FOUND' );
			return $validpayment;
		}
		if ($ccdata['order_payment_number'])
		{
			...
			{
				$validpayment [0] = 0;
				$validpayment [1] = JText::_ ( 'CHECKOUT_ERR_NO_CCNR_NUM_FOUND' );
				return $validpayment;
			}
		}
		if (!$ccdata['order_payment_expire_month']) {
			$validpayment [0] = 0;
			$validpayment [1] = JText::_ ( 'CHECKOUT_ERR_NO_MON_FOUND' );
			return $validpayment;
		}
		$ccerror 		= '';
		$ccerrortext 	= '';
...
'length' => '16', 'prefixes' => '51,52,53,54,55', 'checkdigit' => true ), array ('name' => 'Solo', 'length' => '16,18,19', 'prefixes' => '6334,6767', 'checkdigit' => true ), array ('name' => 'Switch', 'length' => '16,18,19', 'prefixes' => '4903,4905,4911,4936,564182,633110,6333,6759', 'checkdigit' => true ), array ('name' => 'Visa', 'length' => '13,16', 'prefixes' => '4', 'checkdigit' => true ), array ('name' => 'Visa Electron', 'length' => '16', 'prefixes' => '417500,4917,4913,4508,4844', 'checkdigit' => true ), array ('name' => 'LaserCard', 'length' => '16,17,18,19', 'prefixes' => '6304,6706,6771,6709', 'checkdigit' => true ) );
		...
		$ccErrors [0] = JText::_ ( 'CHECKOUT_ERR_NO_UNKNOWN_CCTYPE' );
		$ccErrors [1] = JText::_ ( 'CHECKOUT_ERR_NO_CARD_PROVIDED' );
		$ccErrors [2] = JText::_ ( 'CHECKOUT_ERR_NO_CARD_INVALIDFORMAT' );
		$ccErrors [3] = JText::_ ( 'CHECKOUT_ERR_NO_CARD_INVALIDNUMBER' );
		$ccErrors [4] = JText::_ ( 'CHECKOUT_ERR_NO_CARD_WRONGLENGTH' );
	...
	function voucher()
	{
		...
		if(isset($cart['voucher']))
		{
			...
			for($i=0;$i<count($cart['voucher']);$i++)
			{
				...
				if($cart[$i]['remaining_voucher_discount']>0)
				{
					...
					$rowvoucher->amount 					= $cart['voucher'][$vi]['remaining_voucher_discount'];
					$rowvoucher->voucher_code 				= $cart['voucher'][$vi]['voucher_code'];
					$rowvoucher->user_id 					= $user->id;
					$rowvoucher->trancation_date			= mktime( 0, 0, 0, date('m'), date('d'),date('Y'));
					$rowvoucher->product_id					= $cart['voucher'][$vi]['product_id'];
	...
	function coupon()
	{
		...
		if(isset($cart['coupon']))
		{
			...
			for($i=0;$i<count($cart['coupon']);$i++)
			{
				...
				if($cart[$i]['remaining_coupon_discount']>0)
				{
					...
					$rowcoupon->amount 					= $cart['coupon'][$vi]['remaining_coupon_discount'];
					$rowcoupon->coupon_code 			= $cart['coupon'][$vi]['coupon_code'];
					$rowcoupon->user_id 				= $user->id;
	...
	function displayShoppingCart($template_desc="",$users_info_id,$shipping_rate_id=0,$payment_method_id,$Itemid,$customer_note="",$req_number="",$thirdparty_email="",$shop_id="")
	{
		$session=& JFactory::getSession();
		$cart	= $session->get( 'cart');
		$user = JFactory::getUser();
...
//		$subtotal 			= $cart ['subtotal'];
		...
		if(strstr($template_desc,"{thirdparty_email}"))
		{
			...
			$template_desc = str_replace("{thirdparty_email_lbl}",JText::_('THIRDPARTY_EMAIL_LBL'),$template_desc);
		}
		...
		if(strstr($template_desc,"{customer_note}"))
		{
			...
			$template_desc = str_replace("{customer_note_lbl}",JText::_('CUSTOMER_NOTE_LBL'),$template_desc);
		}
...
//			{
				$req_number_lbl = JText::_('REQUISITION_NUMBER');
...
//			}
		...
		if(strstr($template_desc,"{shop_more}"))
		{
			...
			$shop_more ='<input type=button class="blackbutton" value="'.JText::_( 'SHOP_MORE' ).'" onclick="javascript:document.location=\''.$shopmorelink.'\'">';
			$template_desc=str_replace("{shop_more}",$shop_more,$template_desc);
		}
		if(strstr($template_desc,"{checkout_back_button}"))
		{
			$checkout_back ='<input type=button class="blackbutton" value="'.JText::_( 'BACK_BUTTON' ).'" onclick="javascript: history.go(-1);">';
...
*/
	    ...
	    $checkout .='<input type="button" id="checkout_final" name="checkout_final" class="greenbutton" value="'.JText::_("BTN_CHECKOUTFINAL").'" onclick="if(chkvalidaion()){checkout_disable(\'checkout_final\');}"/>';
		...
		$quotation_request = '<a href="'.$qlink.'" class="modal" rel="{handler: \'iframe\', size: {x: 570, y: 550}}"><input type=button class="greenbutton" value= "'.JText::_('REQUEST_QUOTATION').'" /></a>';
		...
		if(strstr($template_desc,"{coupon_code_lbl}"))
		{
			...
			if(isset($cart["coupon_code"]))
			{
				...
				$coupon = '<span>'.JText::_('CART_COUPON_CODE_TBL').' <br>'.$cart['coupon_code'].' <span class="discount">  ' . $coupon_price.'</span></span>';
			}
