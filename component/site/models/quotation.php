...
//							$producthelper->updateAttributeStockRoom($subproperty_id,"subproperty",$data [$i] ['quantity']);
	...
	function usercreate($data)
	{
		...
		$mailbody = '<table>';
		$mailbody .= '<tr><td>'.JText::_('USERNAME').'</td><td> : </td><td>'.$data['username'].'</td></tr>';
		$mailbody .= '<tr><td>'.JText::_('PASSWORD').'</td><td> : </td><td>'.$password.'</td></tr>';
		$mailbody .= '<tr><td>'.JText::_('QUOTATION_DETAILS').'</td><td> : </td><td><a href="'.$quotationdetailurl.'">'.JText::_("QUOTATION_DETAILS").'</a></td></tr>';
		$mailbody .= '</table>';
		...
		$link = "<a href='".$quotationdetailurl."'>".JText::_("QUOTATION_DETAILS")."</a>";
		...
		JUtility::sendMail ( $MailFrom , $FromName, $email,$mailsubject,$mailbody,1,NULL,$mailbcc );
