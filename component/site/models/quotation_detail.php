...
class RedshopModelQuotation_detail extends RedshopModel
{
    ...
    public function addtocart($data = array())
    {
        ...
        $quotation_att_data = RedshopHelperQuotation::getQuotationItemAttributeDetail(
            $data->quotation_item_id,
            0,
            "attribute",
            $data->product_id
        );
        ...
        if ($data->product_excl_price) {
            $getprotax                      = RedshopHelperProduct::getProductTax(
                $cart[$idx]['product_id'],
                $data->product_excl_price
            );
        ...
        for ($i = 0, $in = count($quotation_acc_data); $i < $in; $i++) {
            ...
            $acc_att_data = RedshopHelperQuotation::getQuotationItemAttributeDetail(
                $data->quotation_item_id,
                1,
                "attribute",
                $quotation_acc_data[$i]->accessory_id
            );
            ...
            for ($ia = 0, $countAccessoryAttribute = count($acc_att_data); $ia < $countAccessoryAttribute; $ia++) {
                ...
                $acc_prop_data = RedshopHelperQuotation::getQuotationItemAttributeDetail(
                    $data->quotation_item_id,
                    1,
                    "property",
                    $acc_att_data[$ia]->section_id
                );
                ...
                for ($ip = 0, $countAccessoryProperty = count($acc_prop_data); $ip < $countAccessoryProperty; $ip++) {
                    ...
                    $acc_subpro_data          = RedshopHelperQuotation::getQuotationItemAttributeDetail(
                        $data->quotation_item_id,
                        1,
                        "subproperty",
                        $acc_prop_data[$ip]->section_id
                    );
        ...
        for ($ia = 0, $countQuotationAtrribute = count($quotation_att_data); $ia < $countQuotationAtrribute; $ia++) {
            ...
            $acc_prop_data          = RedshopHelperQuotation::getQuotationItemAttributeDetail(
                ...
                0,
                "property",
                $quotation_att_data[$ia]->section_id
            );
