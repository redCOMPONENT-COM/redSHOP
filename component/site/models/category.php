...
class categoryModelcategory extends JModel
{
	...
	function _buildQuery()
	{
		...
		$manufacturer_id = (int)$redconfig->get('manufacturer_id');
	...
	function getCategorylistProduct($category_id=0)
	{
		...
		$order_by = "order by ".$redconfig->get ( 'order_by');
	...
	function getCategoryProduct($minmax=0,$isSlider=false)
	{
		...
		if(strstr($order_by,"p.product_price DESC"))
		{
			$priceSort = true;
			$sort = "DESC";
			for($i=0;$i<count($this->_product);$i++)
	...
	function _buildProductOrderBy()
	{
		...
		if ($order_by=='')
		{
			$order_by = $params->get ( 'order_by', DEFAULT_PRODUCT_ORDERING_METHOD );
		}
	...
	function getData()
	{
		...
		{
			$redconfig = &$mainframe->getParams ();
			$endlimit = intval ( $redconfig->get ( 'maxcategory', 5 ) );
			$limit	= $mainframe->getUserStateFromRequest( $context.'limit', 'limit', $mainframe->getCfg('list_limit'), $endlimit);
	...
	function getCategoryTemplate()
	{
		global $mainframe;
		$params = &$mainframe->getParams ( 'com_redshop' );
		$category_template = $params->get ( 'category_template' );
	...
	function getredproductfindertags()
	{
		...
		if($setproductfinder)
		{
			...
			$productfinderconfig = new JParameter( $findercomponent->params );
