...
class split_paymentModelsplit_payment extends JModel
{
	...
	function orderplace() {
		...
		{
			$order_status = 'ABT';
			$order_paymentstatus = JText::_('PAYMENT_STA_PAID');
		...
		if ($paymentmethod->is_creditcard == 1) 
		{
			...
			else 
			{
				...
				if($d ["order_payment_log"]=='SUCCESS')
				{
					...
					$order_paymentstatus = JText::_('PAYMENT_STA_PAID');
					$msg = JText::_('PARTIAL_PAYMENT_DONE');
				...
				{
					$order_paymentstatus = JText::_('PAYMENT_STA_PARTIAL_PAID');
					$msg = JText::_('PARTIAL_PAYMENT_FAILURE');
					$return=JRoute::_('index.php?option='.$option.'&view=order_detail&oid='.$oid.'&Itemid='.$Itemid);
				}				
			}
