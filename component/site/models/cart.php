...
class cartModelcart extends JModel
{
	...
	function update($data)
	{
		...
		if($newQuantity!=$oldQuantity)
		{
			...
			{
				$cart['notice_message'] = $cart[$cartElement]['quantity']." ".JTEXT::_('AVAILABLE_STOCK_MESSAGE');
			} else {
				$cart['notice_message'] ="";
			}
			...
			if($cart[$cartElement]['wrapper_id'])
				...
				$wrapper_vat 	= $wrapperArr['wrapper_vat'] ;
	   	  	...
	   	  	$product =  $this->_producthelper->getProductById($cart[$cartElement]['product_id']);
...
//					$product_price_excl_vat += $subscription_price;
	...
	function redmasscart($post)
	{
		...
		for($i=0;$i<count($products_number);$i++)
		{
			...
			if($product->published == 0){
				$msg = sprintf(JText::_('PRODUCT_IS_NOT_PUBLISHED'),$product->product_name,$product_id);
				JError::raiseWarning(20,$msg);
				continue;
			}

			if($product->not_for_sale == 1){
				$msg = sprintf(JText::_('PRODUCT_IS_NOT_FOR_SALE'),$product->product_name,$product_id);
				JError::raiseWarning(20,$msg);
				continue;
			}

			if($product->expired == 1){
				$msg = sprintf(JText::_('PRODUCT_IS_EXPIRED'),$product->product_name,$product_id);
				JError::raiseWarning(20,$msg);
				continue;
			}
	...
	function shippingrate_calc(){
		...
		$shipping_calc .= "<label>".JText::_('COUNTRY')."</label><br />";
		$shipping_calc .= $conutry;
		$shipping_calc .= "<div id='div_state_lbl'><label>".JText::_('STATE')."</label></div>";
		$shipping_calc .= "<div id='div_state_txt'>".$state."</div>";
		$shipping_calc .= "<br />";
		$shipping_calc .= "<label>".JText::_('ZIPCODE')."</label><br />";
		$shipping_calc .= "<input type='text' name='zipcode' id='zip_code' />";
		$shipping_calc .= "<br />";
		$shipping_calc .= "<input class='blackbutton' type='button' name='shippingcalc' id='shippingcalc' value='".JText::_('UPDATE')."' onClick='javascript:getShippingrate();' />";
		$shipping_calc .= "</form>";
...
//		elseif(isset($data['hidden_attribute_cartimage']))
//		{
