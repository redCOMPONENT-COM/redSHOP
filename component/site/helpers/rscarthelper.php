...
class rsCarthelper
{
     ...
     * @return  array
     */
    public function generateAttributeFromOrder(
        $order_item_id = 0,
        $is_accessory = 0,
        $parent_section_id = 0,
        $quantity = 1
    ...
    public function generateAccessoryFromOrder($order_item_id = 0, $product_id = 0, $quantity = 1)
    {
        ...
        foreach ($orderItemdata as $index => $orderItem) {
            ...
            $generateAccessoryCart[$index]['accessory_childs'] = $this->generateAttributeFromOrder(
                $order_item_id,
                1,
    ...
    public function discountCalculator($get)
    {
        ...
        if (!$use_range) {
            $product_unit = \Redshop\Helper\Utility::getUnitConversation(
                $globalUnit,
     ...
     * @throws  Exception
     */
    public function handleRequiredSelectedAttributeCartMessage(
        $data,
        $attributeTemplate,
        $selectedAttrId,
        $selectedPropId,
