 ...
 */
	...
	function list_all_field($field_section="",$section_id=0,$uclass='')
	{
		...
		for($i=0;$i<count($row_data);$i++)
		{
			...
			$ex_field .= '<tr><td valign="top" align="right"><label>'.JText::_($row_data[$i]->field_title).': </label></td><td>';
			...
			switch ($type)
			{
				case 1: // 1 :- Text Field
						...
						$ex_field .='<input '.$class.' type="text" maxlength="'.$row_data[$i]->field_maxlength.'" name="'.$row_data[$i]->field_name.'" id="'.$row_data[$i]->field_name.'" value="'.$text_value.'" size="32" />';			        				break;
				case 2: // 2 :- Text Area
			 			$textarea_value='';
				...
				case 3: // 3 :- Check Box
						...
						for($c=0;$c<count($field_chk);$c++)
						{
							...
							$ex_field .='<input class="'.$row_data[$i]->field_class.' '.$class.'"   type="checkbox"  '.$checked.' name="'.$row_data[$i]->field_name.'[]" id="'.$row_data[$i]->field_name."_".$field_chk[$c]->value_id.'" value="'.htmlentities($field_chk[$c]->field_value).'" />'.$field_chk[$c]->field_name.'<br />';
						}
							$ex_field .='<label for="'.$row_data[$i]->field_name.'[]" class="error">'.JTEXT::_('PLEASE_SELECT_YOUR').'&nbsp;'.$row_data[$i]->field_title.'</label>';
				        break;
				case 4: // 4 :- Radio Button
 						...
 						for($c=0;$c<count($field_chk);$c++)
						{
				        	...
				        	$ex_field .='<input class="'.$row_data[$i]->field_class.' '.$class.'"   type="radio" '.$checked.'  name="'.$row_data[$i]->field_name.'"  id="'.$row_data[$i]->field_name."_".$field_chk[$c]->value_id.'" value="'.htmlentities($field_chk[$c]->field_value).'" />'.$field_chk[$c]->field_name.'<br />';
						}
						$ex_field .='<label for="'.$row_data[$i]->field_name.'" class="error">'.JTEXT::_('PLEASE_SELECT_YOUR').'&nbsp;'.$row_data[$i]->field_title.'</label>';
				        break;
				case 5: // 5 :-Select Box (Single select)
						...
						for($c=0;$c<count($field_chk);$c++)
						{
				        	...
				        	$ex_field .='<option value="'.htmlentities($field_chk[$c]->field_value).'" '.$selected.' >'.$field_chk[$c]->field_value.'</option>';
						}
						$ex_field .='</select>';
				        break;
...
//		}
			...
			if($field_type == 'hidden')
			{
				...
				switch ($type)
				{
					...
					case 10 : /// File Upload
						...
						{
							$ex_field .='<div class="userfield_input"><input class="'.$row_data[$i]->field_class.'" type="button" value="'.JText::_('UPLOAD_FILE').'" name="file'.$row_data[$i]->field_name.'_'.$product_id.'"  id="file'.$row_data[$i]->field_name.'_'.$product_id.'" '.$req.' userfieldlbl="'.$row_data[$i]->field_title.'" size="'.$row_data[$i]->field_size.'" /><p>'.JText::_('UPLOADED_FILE').':<ol id="ol_'.$row_data[$i]->field_name.'"></ol></p></div>';
			        ...
			        case 11: // 11 :- Image select
						...
						for($c=0;$c<count($field_chk);$c++)
						{
							$ex_field .='<td><div class="userfield_input"><img id="'.$row_data[$i]->field_name."_".$field_chk[$c]->value_id.'" class="pointer imgClass_'.$product_id.'" src="'.$url.'/components/com_redshop/assets/images/extrafield/'.$field_chk[$c]->field_name.'" title="'.$field_chk[$c]->field_value.'" alt="'.$field_chk[$c]->field_value.'" onclick="javascript:setProductUserFieldImage(\''.$row_data[$i]->field_name.'\',\''.$product_id.'\',\''.$field_chk[$c]->field_value.'\',this);"/></div></td>';
						}
						$ex_field .='</tr></table>';
						$ajax = '';
						if(AJAX_CART_BOX && $isatt>0)
					...
					case 15:
						...
						{
							$mainsplit_date_total=explode(" ",$field_chk->data_txt);
							...
							if($dateStart<= $todayStart  && $dateEnd>=$todayEnd)
							{
								...
								{
										if($mainsplit_date_extra[$c]!="" && $mainsplit_date_extra[$c] >= $currentTime)
										{
			...
			if(count($data_value)!=0 && $published	&& $field_show_in_front)
			{
				...
				case 7: // 7 :-Select Country box
				  ...
				  case 9 : /// Media
						...
						$link = $url."components/com_redshop/assets/images/media/".$data_value->data_txt;
						$link_phy = JPATH_SITE."/components/com_redshop/assets/images/media/".$data_value->data_txt;
						$displayvalue = "";
						if(is_file($link_phy)){
				  ...
				  case 10 : /// Document
						...
						$link = $url."components/com_redshop/assets/document/extrafields/".$filename;
						$link_phy = JPATH_SITE."/components/com_redshop/assets/document/extrafields/".$filename;
						$displayvalue = "";
						if(is_file($link_phy)){
			      ...
			      case 13 :
					...
					for($c=0;$c<count($document_value);$c++)
					{
						if(@in_array($document_value[$c]->value_id,$chk_data))
						{
				        	...
				        	$link = $url."components/com_redshop/assets/images/extrafield/".$filename;
				...
				$template_data 	= str_replace($search_lbl,$displaytitle,$template_data);
	...
	{
		$q = "SELECT * "
			."FROM ".$this->_table_prefix."fields_value "
			."WHERE field_id='".$id."' "
