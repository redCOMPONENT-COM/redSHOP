...
class rsCarthelper
{
	...
	public function replaceCartItem($data, $cart = array(), $replace_button, $quotation_mode = 0)
	{
		...
		for ($i = 0; $i < $idx; $i++)
		{
			...
			else
			{
				...
				if (is_file(REDSHOP_FRONT_IMAGES_RELPATH . "mergeImages/" . $cart[$i]['attributeImage']) && isset($cart[$i]['attributeImage']))
				{
				...
				if ($product_image_path)
				{
					...
					{
						$product_cart_img = $redhelper->watermark($type,
							$prd_image,
							CART_THUMB_WIDTH,
							CART_THUMB_HEIGHT,
	...
	public function replaceOrderTemplate($row, $ReceiptTemplate)
	{
		...
		if ($paymentmethod_detail->element == "rs_payment_banktransfer"
			|| $paymentmethod_detail->element == "rs_payment_banktransfer_discount"
	...
	public function GetCartParameters()
	{
		...
		for ($i = 0; $i < count($allparams); $i++)
		{
			...
			if (!empty($cart_param))
			{
				if (strstr($cart_param[0], 'cart_output')
					|| strstr($cart_param[0], 'show_with_shipping')
					|| strstr($cart_param[0], 'show_with_discount')
					|| strstr($cart_param[0], 'show_with_vat')
					|| strstr($cart_param[0], 'show_shipping_line')
				)
				{
	...
	public function replacePaymentTemplate($template_desc = "", $payment_method_id = 0, $is_company = 0, $ean_number = 0)
	{
		...
		if (strstr($template_desc, "{payment_loop_start}") && strstr($template_desc, "{payment_loop_end}"))
		{
			...
			for ($p = 0; $p < count($paymentmethod); $p++)
			{
				...
				if (file_exists(JPATH_SITE . '/plugins/redshop_payment/' . $paymentmethod[$p]->element . '/' . $paymentmethod[$p]->element . '.php'))
				{
