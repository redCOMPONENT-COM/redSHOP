	...
	function replaceTax($data='',$amount=0,$discount=0,$check=0,$quotation_mode=0)
	{
		if(strstr($data,'{if vat}') && strstr($data,'{vat end if}'))
		{
			...
			else
			{
				...
				$data=str_replace("{vat_lbl}",JText::_('CHECKOUT_VAT_LBL'),$data);
	...
	function replaceDiscount($data='',$discount=0,$subtotal=0, $quotation_mode = 0)
	{
		if(strstr($data,'{if discount}') && strstr($data,'{discount end if}'))
		{
			...
			if($discount<=0)
			{
				...
				$data=str_replace("{discount_lbl}",JText::_('CHECKOUT_DISCOUNT_LBL'),$data);
	...
	function replacePayment($data='',$amount=0,$cart=0,$payment_oprand='-')
	{
		if(strstr($data,'{if payment_discount}') && strstr($data,'{payment_discount end if}'))
		{
			...
			else
			{
				...
				$payText = ($payment_oprand=='+') ? JText::_('PAYMENT_CHARGES_LBL') : JText::_('PAYMENT_DISCOUNT_LBL');
		...
		if(strstr($data,'{billing_address_start}') && strstr($data,'{billing_address_end}'))
		{
			...
			if(isset($billingaddresses))
			{
				...
				if ($cname != "")
				{
					...
					$billingdata=str_replace("{country_lbl}",JText::_('COUNTRY'),$billingdata);
				}
				...
				if ($billingaddresses->is_company == 1)
				{
					if ($billingaddresses->vat_number != "")
					{
						...
						$billingdata=str_replace("{vatnumber_lbl}",JText::_('VAT_NUMBER'),$billingdata);
					}
					if ($billingaddresses->ean_number != "")
					{
						$billingdata=str_replace("{ean_number}",$billingaddresses->ean_number,$billingdata);
						$billingdata=str_replace("{ean_number_lbl}",JText::_('EAN_NUMBER'),$billingdata);
					}
...
//						$billingdata=str_replace("{requisition_number_lbl}",JText::_('REQUISITION_NUMBER'),$billingdata);
//					}
					if(SHOW_TAX_EXEMPT_INFRONT)
					{
						if ($billingaddresses->tax_exempt == 1 ) {
							$taxexe = JText::_ ( "TAX_YES" );
						} else {
							$taxexe = JText::_ ( "TAX_NO" );
						}
						$billingdata=str_replace("{taxexempt}",$taxexe,$billingdata);
						$billingdata=str_replace("{taxexempt_lbl}",JText::_('TAX_EXEMPT'),$billingdata);

						if ($billingaddresses->requesting_tax_exempt == 1) {
							$taxexereq = JText::_ ( "YES" );
						} else {
							$taxexereq = JText::_ ( "NO" );
						}
						$billingdata=str_replace("{user_taxexempt_request}",$taxexereq,$billingdata);
						$billingdata=str_replace("{user_taxexempt_request_lbl}",JText::_('USER_TAX_EXEMPT_REQUEST_LBL'),$billingdata);
					}
				}
...
//			$billingdata=str_replace("{requisition_number_lbl}","",$billingdata);
		...
		elseif(strstr($data,'{billing_address}'))
		{
			...
			if(isset($billingaddresses))
			{
				...
				if ($billingaddresses->is_company == 1 && $billingaddresses->company_name != "")
				{
					$billadd .=  JText::_('COMPANY_NAME').' : '.$billingaddresses->company_name.'<br />';
				}
				if ($billingaddresses->firstname != "") {
					$billadd .= JText::_ ( "FIRSTNAME" ) . ' : ' . $billingaddresses->firstname.'<br />' ;
				}
				if ($billingaddresses->lastname != "") {
					$billadd .= JText::_ ( "LASTNAME" ) . ' : ' . $billingaddresses->lastname . '<br />';
				}
				if ($billingaddresses->address != "") {
					$billadd .=  JText::_ ( "ADDRESS" ) . ' : ' . $billingaddresses->address . '<br /> ';
				}
				if ($billingaddresses->zipcode != "") {
					$billadd .=  JText::_ ( "ZIP" ) . ' : ' . $billingaddresses->zipcode . '<br />';
				}
				if ($billingaddresses->city != "") {
					$billadd .=  JText::_ ( "CITY" ) . ' : ' . $billingaddresses->city . '<br /> ';
				}
				$cname = $this->_order_functions->getCountryName( $billingaddresses->country_code );
				if ($cname != "")
				{
					$billadd .=  JText::_ ( "COUNTRY" ) . ' : ' . JText::_($cname) . '<br />';
				}
				$sname = $this->_order_functions->getStateName ( $billingaddresses->state_code, $billingaddresses->country_code );
				if ($sname != "") {
					$billadd .= JText::_ ( "STATE" ).' : '.$sname.'<br />';
				}
				if ($billingaddresses->phone != "") {
					$billadd .= JText::_ ( "PHONE" ) . ' : ' . $billingaddresses->phone . '<br/>';
				}
				if ($billingaddresses->user_email != "") {
					$billadd .= JText::_ ( "EMAIL" ) . ' : ' . $billingaddresses->user_email. '<br />';
				}
				else if($user->email!='')
				{
					$billadd .= JText::_ ( "EMAIL" ) . ' : ' .  $user->email . '<br />';
				}
				if ($billingaddresses->is_company == 1)
				{
					if ($billingaddresses->vat_number != "")
					{
						$billadd .= JText::_ ( "VAT_NUMBER" ).' : '.$billingaddresses->vat_number.'<br />';
					}
					if ($billingaddresses->ean_number != "")
					{
						$billadd .= JText::_ ( "EAN_NUMBER" ).' : '.$billingaddresses->ean_number.'<br />';
					}
//					if ($billingaddresses->requisition_number != "")
//					{
//						$billadd .= JText::_ ( "REQUISITION_NUMBER" ).' : '.$billingaddresses->requisition_number.'<br />';
//					}
					if(SHOW_TAX_EXEMPT_INFRONT)
					{
		...
		$data=str_replace("{billing_address_information_lbl}",JText::_('BILLING_ADDRESS_INFORMATION_LBL'),$data);
		return $data;
	}
	...
	function replaceShippingAddress($data,$shippingaddresses)
	{
		if(strstr($data,'{shipping_address_start}') && strstr($data,'{shipping_address_end}'))
		{
			...
			if(isset($shippingaddresses) && SHIPPING_METHOD_ENABLE)
			{
				...
				if ($cname != "")
				{
					...
					$shippingdata=str_replace("{country_lbl}",JText::_('COUNTRY'),$shippingdata);
				}
				...
				if ($sname != "") {
					...
					$shippingdata=str_replace("{state_lbl}",JText::_('STATE'),$shippingdata);
				}
				if ($shippingaddresses->phone != "") {
					$shippingdata=str_replace("{phone}",$shippingaddresses->phone,$shippingdata);
					$shippingdata=str_replace("{phone_lbl}",JText::_('PHONE'),$shippingdata);
				}
		...
		else if(strstr($data,'{shipping_address}'))
		{
			...
			if(isset($shippingaddresses) && SHIPPING_METHOD_ENABLE)
			{
				if ($shippingaddresses->is_company == 1 && $shippingaddresses->company_name != "")
				{
					$shipadd .=  JText::_('COMPANY_NAME').' : '.$shippingaddresses->company_name.'<br />';
				}
				if ($shippingaddresses->firstname != "") {
					$shipadd .= JText::_ ( "FIRSTNAME" ) . ' : ' . $shippingaddresses->firstname.'<br />' ;
				}
				if ($shippingaddresses->lastname != "") {
					$shipadd .= JText::_ ( "LASTNAME" ) . ' : ' . $shippingaddresses->lastname . '<br />';
				}
				if ($shippingaddresses->address != "") {
					$shipadd .= JText::_ ( "ADDRESS" ) . ' : ' . $shippingaddresses->address . '<br />';
				}
				if ($shippingaddresses->zipcode != "") {
					$shipadd .= JText::_ ( "ZIP" ) . ' : ' . $shippingaddresses->zipcode . '<br />';
				}
				if ($shippingaddresses->city != "") {
					$shipadd .=  JText::_ ( "CITY" ) . ' : ' . $shippingaddresses->city . '<br />';
				}
				$cname = $this->_order_functions->getCountryName( $shippingaddresses->country_code );
				if ($cname != "") {
					$shipadd .= JText::_ ( "COUNTRY" ) . ' : ' . JText::_( $cname) . '<br />';
				}
				$sname = $this->_order_functions->getStateName( $shippingaddresses->state_code, $shippingaddresses->country_code );
				if ($sname != "") {
					$shipadd .= JText::_ ( "STATE" ) . ' : ' . $sname. '<br />';
				}
				if ($shippingaddresses->phone != "") {
					$shipadd .= JText::_ ( "PHONE" ) . ' : ' . $shippingaddresses->phone . '<br />';
				}
		...
		$shippingtext = (SHIPPING_METHOD_ENABLE) ? JText::_('SHIPPING_ADDRESS_INFO_LBL') : '';
...
//			$data=$shipping_start[0].$shipping_end[0].$shipping_end[1];
			...
			if(SHIPPING_METHOD_ENABLE)
			{
				...
				$replace[] = $shopLocation.' '.$mobilearr;//str_replace('###',' ',str_replace("|"," ",$shopLocation));
...
//		}
	...
	function replaceCartItem($data,$cart=array(),$replace_button,$quotation_mode=0)
	{
		...
		if (is_file(JPATH_ROOT."/components/com_redshop/assets/images/".ADDTOCART_DELETE)) {
		...
		for( $i=0 ; $i < $idx; $i++ )
		{
			if(isset($cart[$i]['giftcard_id']) && $cart[$i]['giftcard_id'])
			{
				...
				<img class="delete_cart" src="'.$url.'/components/com_redshop/assets/images/'.$delete_img.'" title="'.JText::_('DELETE_PRODUCT_FROM_CART_LBL').'" alt="'.JText::_('DELETE_PRODUCT_FROM_CART_LBL').'" onclick="document.delete_cart'.$i.'.task.value=\'delete\';document.delete_cart'.$i.'.submit();"></form>';
			...
			else
			{
				...
				{
					$image_path = JURI::base()."components/com_redshop/assets/images/";
					$product_image = str_replace($image_path,'',$cart[$i]['hidden_attribute_cartimage']);
				}

				if($product_image && is_file(JPATH_COMPONENT_SITE."/assets/images/".$product_image))
				{
					$product_image_path = $url."/components/com_redshop/helpers/thumb.php?filename=".$product_image;
					$val=explode("/",$product_image);
					$prd_image=$val[1];
					$type=$val[0];
				}
				else if($product->product_full_image && is_file(JPATH_COMPONENT_SITE."/assets/images/product/".$product->product_full_image))
				{
				...
				else if(is_file(JPATH_COMPONENT_SITE."/assets/images/product/".PRODUCT_DEFAULT_IMAGE))
				{
				...
				else if(is_file(JPATH_COMPONENT_SITE."/assets/images/mergeImages/".$cart[$i]['attributeImage']) && isset($cart[$i]['attributeImage']))
				{
				...
				if($product_image_path)
				{
					...
					if(WATERMARK_CART_THUMB_IMAGE && file_exists(JPATH_BASE."/components/".$option."/assets/images/product/".WATERMARK_IMAGE))
					{
				...
				if((array_key_exists('wrapper_id', $cart[$i])) && $cart[$i]['wrapper_id'])
				{
					...
					if(count($wrapper)>0)
					{
						$wrapper_name = JText::_('WRAPPER').": ".$wrapper[0]->wrapper_name;
						if(!$quotation_mode || ($quotation_mode && SHOW_QUOTATION_PRICE))
						{
				...
				$cart_mdata=str_replace("{product_customfields_lbl}",JText::_("PRODUCT_CUSTOM_FIELD"),$cart_mdata);
				...
				if($product->product_type=='subscription')
				{
					...
					$cart_mdata=str_replace("{product_subscription_lbl}",JText::_('SUBSCRIPTION'),$cart_mdata);
				...
				if(isset($cart[$i]["reddesign"]))
				{
					...
					$designPreview .= "<a class='modal' href='".$designPreviewlink."'>".JText::_('PREVIEW')."</a>";

					$redbg_image = $this->_redhelper->getImagePath($cart[$i]['reddesign_image_id']);
					$bgPreview .= "<span>".JText::_('BACKGROUND_IMAGE')."</span><br>".$redbg_image;
				...
				if($replace_button)
				{
					...
					if($mainview=='cart')
					{
						...
						$update_attribute = '<a class="modal" rel="{handler: \'iframe\', size: {x: 550, y: 400}}" href="'.$attchange.'">'.JText::_('CHANGE_ATTRIBUTE').'</a>';
					}
					...
					else
					{
						...
						<input type="hidden" name="Itemid" value="'.$Itemid.'">
						<input type="hidden" name="task" value="">';
							if (is_file(JPATH_ROOT."/components/com_redshop/assets/images/".ADDTOCART_UPDATE)) {
							$update_img=ADDTOCART_UPDATE;
						}else{
							$update_img="defaultupdate.jpg";
						}
						$update_cart .='<img class="update_cart" src="'.$url.'/components/com_redshop/assets/images/'.$update_img.'" title="'.JText::_('UPDATE_PRODUCT_FROM_CART_LBL').'" alt="'.JText::_('UPDATE_PRODUCT_FROM_CART_LBL').'" onclick="document.update_cart'.$i.'.task.value=\'update\';document.update_cart'.$i.'.submit();">';
						$update_cart .='</form>';
					}
					$update_cart_minus_plus ='<form name="update_cart'.$i.'" method="POST">';
					$update_cart_minus_plus .='<input type="text" id="quantitybox'.$i.'" name="quantity"  size="1"  value="'.$quantity.'" />';
					$update_cart_minus_plus .='<input type="button" id="minus" value="-" onClick="quantity.value = (quantity.value) ; var qty1 = quantity.value; if( !isNaN( qty1 ) &amp;&amp; qty1 > 1 ) quantity.value--;return false;">';
				    $update_cart_minus_plus .='<input type="button" value="+" onClick="quantity.value = (+quantity.value+1)">';
				    $update_cart_minus_plus .='<input type="hidden" name="product_id" value="'.$product_id.'">';
				    $update_cart_minus_plus .='<input type="hidden" name="cart_index" value="'.$i.'">';
				    $update_cart_minus_plus .='<input type="hidden" name="Itemid" value="'.$Itemid.'">';
				    $update_cart_minus_plus .='<input type="hidden" name="task" value=""><img class="update_cart" src="'.$url.'/components/com_redshop/assets/images/'.$update_img.'" title="'.JText::_('UPDATE_PRODUCT_FROM_CART_LBL').'" alt="'.JText::_('UPDATE_PRODUCT_FROM_CART_LBL').'" onclick="document.update_cart'.$i.'.task.value=\'update\';document.update_cart'.$i.'.submit();">
								</form>';
					if (is_file(JPATH_ROOT."/components/com_redshop/assets/images/".ADDTOCART_DELETE)) {
						$delete_img=ADDTOCART_DELETE;
					}else{
					...
					$empty_cart ='<form style="padding:0px;margin:0px;" name="delete_cart'.$i.'" method="POST" >
								...
								<input type="hidden" name="task" value="">
								<input type="hidden" name="Itemid" value="'.$Itemid.'">
								<img class="delete_cart" src="'.$url.'/components/com_redshop/assets/images/'.$delete_img.'"  onclick="document.delete_cart'.$i.'.task.value=\'delete\';document.delete_cart'.$i.'.submit();"></form>';
					if($mainview=='checkout')
					{
						$remove_product ='';
					}
	...
	function repalceOrderItems($data,$rowitem=array())
	{
		...
		{
		    $dirname = JPATH_COMPONENT_SITE.DS."assets/images/mergeImages/".$rowitem [$i]->attribute_image;
		    ...
		    {
		    	$attribute_image_path=$url."components/com_redshop/helpers/thumb.php?filename=mergeImages/".$rowitem [$i]->attribute_image."&newxsize=".CART_THUMB_WIDTH."&newysize=".CART_THUMB_HEIGHT."&swap=".USE_IMAGE_SIZE_SWAPPING;
		    }
			...
			if ($rowitem [$i]->wrapper_id)
			{
				...
				$wrapper_name = JText::_ ( 'WRAPPER' ) . ": " . $wrapper_name . "(" . $wrapper_price . ")";
			}
			...
			if ($product->product_full_image)
			{
				...
				{
					if (is_file ( JPATH_COMPONENT_SITE . "/assets/images/product/" . PRODUCT_DEFAULT_IMAGE ))
					{
						$product_image_path = $url . "/components/com_redshop/helpers/thumb.php?filename=product/" . PRODUCT_DEFAULT_IMAGE;
					}
				}
			...
			$cart_mdata = str_replace ( "{product_number_lbl}", JText::_ ( 'PRODUCT_NUMBER' ), $cart_mdata );
			...
			if($mainview=="order_detail")
			{
				...
				$copytocart .= "<img src='".$url."/administrator/components/com_redshop/assets/images/add.jpg' title='".JText::_("COPY_TO_CART")."' alt='".JText::_("COPY_TO_CART")."' /></a>";
			...
			if(isset($dproducts[$product_id]) && count($dproducts[$product_id])>0 && $OrdersDetail->order_status == "C"  && $OrdersDetail->order_payment_status == "Paid")
			{
				...
				$limit .= "</table>";
				$enddate .= "</table>";
				$cart_mdata = str_replace ( "{download_token_lbl}", JText::_('DOWNLOAD_TOKEN'), $cart_mdata );
				$cart_mdata = str_replace ( "{download_token}", $dpData, $cart_mdata );
				$cart_mdata = str_replace ( "{download_counter_lbl}", JText::_('DOWNLOAD_LEFT'), $cart_mdata );
				$cart_mdata = str_replace ( "{download_counter}", $limit, $cart_mdata );
				$cart_mdata = str_replace ( "{download_date_lbl}", JText::_('DOWNLOAD_ENDDATE'), $cart_mdata );
			...
			if(isset($dproductslog[$product_id]) && count($dproductslog[$product_id])>0 && $OrdersDetail->order_status == "C")
			{
				...
				foreach($downloadarraylog as $downloads)
				{
					...
					$dpData .= "</tr>";
					$dpData .= "<td>(".$g.") ".$mailtoken." ".JText::_('ON')." ".$download_date." ".JText::_('FROM')." ".$ip."</td>";
					$dpData .= "</tr>";

					$g++;
				}
				$dpData .= "</table>";
				$cart_mdata = str_replace ( "{download_date_list_lbl}", JText::_('DOWNLOAD_LOG'), $cart_mdata );
	...
	function replaceLabel($data)
	{
		...
		if(strstr($data,'{order_status_payment_only_lbl}'))
		{
			...
			$replace[] = JText::_('PAYMENT_STAUS_LBL');
		}
		if(SHIPPING_METHOD_ENABLE)
		{
			...
			{
				$search[] = "{shipping_lbl}";
				$replace[] = JText::_('CHECKOUT_SHIPPING_LBL');
			}
		...
		if(strstr($data,'{order_information_lbl}'))
		{
			...
			$replace[] = JText::_('ORDER_INFORMATION_LBL');
		}

		if(strstr($data,'{order_detail_lbl}'))
		{
			$search[] = "{order_detail_lbl}";
			$replace[] = JText::_('ORDER_DETAIL_LBL');
		}
		...
		if(SHIPPING_METHOD_ENABLE)
		{
			if(strstr($data,'{shipping_method_lbl}'))
			{
				...
				$replace[] = JText::_('SHIPPING_METHOD_LBL');
			}
		}else
		{
			if(strstr($data,'{shipping_method_lbl}'))
		...
		if(strstr($data,'{product_number_lbl}'))
		{
			...
			$replace[] =  JText::_ ( 'PRODUCT_NUMBER' );
		}
	...
	function replaceTemplate($cart,$cart_data,$checkout=1)
	{
		...
		if (! DEFAULT_QUOTATION_MODE || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE))
		{
			...
			{
				$cart_data = str_replace ( "{product_subtotal_lbl}", JText::_('PRODUCT_SUBTOTAL_LBL'), $cart_data );
			}
			if(strstr($cart_data,'{product_subtotal_excl_vat_lbl}'))
			{
				$cart_data = str_replace ( "{product_subtotal_excl_vat_lbl}", JText::_('PRODUCT_SUBTOTAL_EXCL_LBL'), $cart_data );
			}
			if(strstr($cart_data,'{shipping_with_vat_lbl}'))
			{
				$cart_data = str_replace ( "{shipping_with_vat_lbl}", JText::_('SHIPPING_WITH_VAT_LBL'), $cart_data );
			}
			if(strstr($cart_data,'{shipping_excl_vat_lbl}'))
			{
				$cart_data = str_replace ( "{shipping_excl_vat_lbl}", JText::_('SHIPPING_EXCL_VAT_LBL'), $cart_data );
			}

			if(strstr($cart_data,'{product_price_excl_lbl}'))
			{
				$cart_data = str_replace ( "{product_price_excl_lbl}", JText::_('PRODUCT_PRICE_EXCL_LBL'), $cart_data );
			}
			...
			{
				$cart_data = str_replace ( "{denotation_label}", JText::_('DENOTATION_TXT'), $cart_data );
			} else {
...
//				$cart_data=$shipping_start[0].$shipping_end[0].$shipping_end[1];
				...
				if (!empty($rep))
				{
					...
					$cart_data = str_replace ( "{shipping_lbl}", JText::_ ( 'CHECKOUT_SHIPPING_LBL' ), $cart_data );
					$cart_data = str_replace ( "{shipping}", $this->_producthelper->getProductFormattedPrice ( $shipping, true ), $cart_data );
					$cart_data = str_replace ( "{tax_with_shipping_lbl}", JText::_ ( 'CHECKOUT_SHIPPING_LBL' ), $cart_data );
...
//			}
	...
	function replaceOrderTemplate($row,$ReceiptTemplate)
	{
		...
		$img_url			= $url."components/com_redshop/assets/images/barcode/".$barcode_code.".png";
		...
		$replace[] = "<a href='" . $orderdetailurl . "'>" . JText::_ ( "ORDER_MAIL" ) . "</a>";
		...
		if ($row->order_status == "C" && $row->order_payment_status == "Paid")
		{
			...
			if($dpData != "")
				$replace [] = JText::_('DOWNLOAD_TOKEN');
			else
				$replace [] = "";
		}else{
			$search  [] = "{download_token}";
		...
		if( (strstr($ReceiptTemplate,"{discount_denotation}") || strstr($ReceiptTemplate,"{shipping_denotation}") ) && ($Total_discount != 0 || $row->order_shipping !=0))
		{
			...
			$replace [] = JText::_('DENOTATION_TXT');
		}
		else {
			$search  [] = "{denotation_label}";
			$replace [] = "";
		...
		{
			$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$paymentmethod_detail->element.'.xml';
			$paymentparams = new JParameter( $paymentmethod_detail->params );
...
//		{
			...
			$replace[] = JText::_('REQUISITION_NUMBER');
//		}
//		else
//		{
//			$search [] = "{requisition_number}";
...
//		}
		...
		if(strstr($ReceiptTemplate,'{redcrm_debitornumber_lbl}'))
		{
			if($session->get('isredcrmuser_debitor'))
			{
				...
				$replace[] = JText::_('DEBITOR_NUMBER');
			} else {
				$search [] = "{redcrm_debitornumber_lbl}";
				$replace[] = "";
			}
...
//			$orderdetailurl 	= JURI::root ().'index.php?option=com_redshop&view=order_detail&oid=' . $order_id . '&encr=' . $row->encr_key;
	...
	function replaceredDesignOrderItem($data,$rowitem,&$reddesign_attachment){
		...
		for($i = 0; $i < count ( $rowitem ); $i ++)
		{
				...
				$datamessage 	= str_replace ( "{product_name_lbl}", JText::_ ( 'PRODUCT_NAME' ), $data );
				$datamessage 	= str_replace ( "{product_quantity_lbl}", JText::_ ( 'PRODUCT_QUANTITY' ), $datamessage );
				$datamessage 	= str_replace ( "{product_number_lbl}", JText::_ ( 'PRODUCT_NUMBER' ), $datamessage );
				$datamessage 	= str_replace ( "{product_price_lbl}", JText::_ ( 'PRODUCT_PRICE' ), $datamessage );
				$datamessage 	= str_replace ( "{design_lbl}", JText::_ ( 'DESIGN' ), $datamessage );
				...
				$datamessage 	= str_replace ( "{background_image_label}", JText::_ ( 'ORIGINAL_IMAGE' ), $datamessage );
	...
	function makeCart_output($cart)
	{
		...
		for($i = 0; $i < $idx; $i++)
		{
			...
			{
				if ($show_with_vat)
					$output .= JText::_ ( 'PRICE_CART_LBL' ) . " " . $this->_producthelper->getProductFormattedPrice ( $cart [$i] ['product_price'], true ) ;
				else
					$output .= JText::_ ( 'PRICE_CART_LBL' ) . " " . $this->_producthelper->getProductFormattedPrice ( $cart [$i] ['product_price_excl_vat'], true );
			}
		}
		...
		if($cart_output=='simple')
		{
			...
			$output .= JTEXT::_ ( 'TOTAL_PRODUCT' ). ':' . ' ' . $totalQuntity . ' ' . JTEXT::_ ( 'PRODUCTS_IN_CART' );
			$output .='</div>';
		}
		if (! DEFAULT_QUOTATION_MODE || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE))
		{
			$output .= '<div class="mod_cart_total_txt" id="mod_cart_total_txt_ajax" >'.JTEXT::_ ( 'TOTAL' ).':'.'</div>';
			...
			if($show_shipping_line){
				$output .= '<div class="mod_cart_shipping_txt" id="mod_cart_shipping_txt_ajax" >'.JTEXT::_ ( 'SHIPPING_LBL' ).':'.'</div>';
		...
		for($i=0;$i<count($allparams);$i++)
		{
			...
			if(!empty($cart_param)){
				if(strstr($cart_param[0],'cart_output') || strstr($cart_param[0],'show_with_shipping') || strstr($cart_param[0],'show_with_discount')
				  	...
				  	$cartparamArr[$cart_param[0]] = $cart_param[1];
			 	}
			}
		}
		return $cartparamArr;
	...
	function replaceShippingBoxTemplate($box_template_desc="",$shipping_box_post_id=0)
	{
		...
		$box_template_desc = str_replace ( "{shipping_box_heading}", JText::_( 'SHIPPING_BOXES' ), $box_template_desc );
		...
		$shipping_box_list = JText::_('NO_SHIPPING_BOX');
	...
	function replaceShippingTemplate($template_desc="",$shipping_rate_id=0,$shipping_box_post_id=0,$user_id=0,$users_info_id=0,$ordertotal=0,$order_subtotal=0)
	{
		...
		$template_desc = str_replace ( "{shipping_heading}", JText::_( 'SHIPPING_METHOD' ), $template_desc );
					...
					if(count($rate)>0)
					{
						...
						if($template_rate_middle!="")
						{
							...
							for($i=0;$i<count($rate);$i++)
							{
								...
								$shipping_rate_name = '<input type="radio" name="shipping_rate_id" value="'.$rate[$i]->value.'" '.$checked.' onclick="javascript:onestepCheckoutProcess(this.name);">'.html_entity_decode($rate[$i]->text);
	...
	function replaceCreditCardInformation($payment_method_id=0)
	{
		...
		$montharr = array();
		$montharr[]   = JHTML::_('select.option', '0',JText::_('MONTH'));
	  	$montharr[]   = JHTML::_('select.option', '01',1);//JText::_('JAN'));
	  	$montharr[]   = JHTML::_('select.option', '02',2);//JText::_('FEB'));
	  	$montharr[]   = JHTML::_('select.option', '03',3);//JText::_('MAR'));
	  	$montharr[]   = JHTML::_('select.option', '04',4);//JText::_('APR'));
	  	$montharr[]   = JHTML::_('select.option', '05',5);//JText::_('MAY'));
	  	$montharr[]   = JHTML::_('select.option', '06',6);//JText::_('JUN'));
	  	$montharr[]   = JHTML::_('select.option', '07',7);//JText::_('JUL'));
	  	$montharr[]   = JHTML::_('select.option', '08',8);//JText::_('AUG'));
	  	$montharr[]   = JHTML::_('select.option', '09',9);//JText::_('SEp'));
	  	$montharr[]   = JHTML::_('select.option', '10',10);//JText::_('OCT'));
	  	$montharr[]   = JHTML::_('select.option', '11',11);//JText::_('NOV'));
	  	$montharr[]   = JHTML::_('select.option', '12',12);//JText::_('DEC'));
		...
		$cardinfo = "";
		if( file_exists (JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$paymentmethod->element.'.php'))
		{
//			$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$paymentmethod->element.'.php';
			$paymentparams = new JParameter( $paymentmethod->params );
...
//			$business = $paymentparams->get('business','');
			...
			if($is_creditcard)
			{
				...
				$cardinfo .='<fieldset class="adminform"><legend>'.JText::_( 'CARD_INFORMATION' ).'</legend>';
				...
				{
			  		$cardinfo .='<td align="center"><img src="'.$url.'/components/com_redshop/assets/images/checkout/'.$cc_list[$credict_card[$ic]]->img.'" alt="" border="0" /></td>';
			  	}
			  	$cardinfo .='</tr>';
			  	$cardinfo .='<tr>';
			  	for($ic=0;$ic<count($credict_card);$ic++)
			  	...
			  	$cardinfo .='<td align="right" nowrap="nowrap" width="10%"><label for="order_payment_name">'.JText::_('NAME_ON_CARD').'</label></td>';
			  	...
			  	$cardinfo .='</tr>';
			  	$cardinfo .='<tr valign="top">';
			  	$cardinfo .='<td align="right" nowrap="nowrap" width="10%"><label for="order_payment_number">'.JText::_('CARD_NUM').'</label></td>';
			  	...
			  	$cardinfo .='</tr>';
			  	$cardinfo .='<tr><td align="right" nowrap="nowrap" width="10%">'.JText::_('EXPIRY_DATE').'</td>';
			  	$cardinfo .='<td>';
				...
				$cardinfo .='<tr valign="top"><td align="right" nowrap="nowrap" width="10%"><label for="credit_card_code">'.JText::_( 'CARD_SECURITY_CODE').'</label></td>';
...
//				$cardinfo .='<tr valign="top"><td align="right" nowrap="nowrap" width="10%"><label for="order_total">'.JText::_( 'ORDERTOTAL' ).'</label></td>';
//				$cardinfo .='<td>'.$this->_producthelper->getProductFormattedPrice($order->order_total).'</td></tr>';
	...
	function replacePaymentTemplate($template_desc="",$payment_method_id=0,$is_company=0)
	{
		$ccdata	= $this->_session->get('ccdata');
		...
		$montharr = array();
		$montharr[]   = JHTML::_('select.option', '0',JText::_('MONTH'));
	  	$montharr[]   = JHTML::_('select.option', '01',JText::_('JAN'));
	  	$montharr[]   = JHTML::_('select.option', '02',JText::_('FEB'));
	  	$montharr[]   = JHTML::_('select.option', '03',JText::_('MAR'));
	  	$montharr[]   = JHTML::_('select.option', '04',JText::_('APR'));
	  	$montharr[]   = JHTML::_('select.option', '05',JText::_('MAY'));
	  	$montharr[]   = JHTML::_('select.option', '06',JText::_('JUN'));
	  	$montharr[]   = JHTML::_('select.option', '07',JText::_('JUL'));
	  	$montharr[]   = JHTML::_('select.option', '08',JText::_('AUG'));
	  	$montharr[]   = JHTML::_('select.option', '09',JText::_('SEP'));
	  	$montharr[]   = JHTML::_('select.option', '10',JText::_('OCT'));
	  	$montharr[]   = JHTML::_('select.option', '11',JText::_('NOV'));
	  	$montharr[]   = JHTML::_('select.option', '12',JText::_('DEC'));
		...
		$template_desc = str_replace ( "{payment_heading}", JText::_( 'PAYMENT_METHOD' ), $template_desc );
		if(strstr($template_desc,"{split_payment}"))
		{
			if (SPLITABLE_PAYMENT == 1)
			{
				$splitpayment = '<input type="checkbox" name="issplit" value="1">'.JText::_( 'SPLIT_PAYMENT' ).'?';
		...
		if(strstr($template_desc,"{payment_loop_start}") && strstr($template_desc,"{payment_loop_end}"))
		{
			...
			for($p=0;$p<count($paymentmethod);$p++)
			{
				$cardinfo = "";
				$display_payment = "";
				if( file_exists (JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$paymentmethod[$p]->element.'.php'))
				{
					$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$paymentmethod[$p]->element.'.php';
...
//					$checked = ($payment_method_id==$paymentmethod[$p]->element) ? "checked" : "";
					...
					$paymentparams = new JParameter( $paymentmethod[$p]->params );
					$private_person = $paymentparams->get('private_person','');
					$business = $paymentparams->get('business','');
					$is_creditcard = $paymentparams->get('is_creditcard',0);
					...
					if(in_array($shopperGroupId,$shopper_groupArr) || $shopper_groupArr[0]==0)
					{
						...
						}else if($paymentmethod[$p]->element == 'rs_payment_googlecheckout'){

							$display_payment = '';
						} else if($is_subscription)
						{
							$display_payment = '<input  type="radio" name="payment_method_id" value="'.$paymentmethod[$p]->element.'" '.$checked.' onclick="javascript:onestepCheckoutProcess(this.name);" />'.$paymentmethod[$p]->name.'<br>';
							$display_payment .='<table><tr><td>'.JText::_( 'SUBSCRIPTION_PLAN' ).'</td><td>'.$this->getSubscriptionPlans().'<td></tr><table>';
	...
	function replaceTermsConditions($template_desc="",$Itemid=1)
	{
		if (strstr ( $template_desc,"{terms_and_conditions"))
		{
			...
			if(SHOW_TERMS_AND_CONDITIONS==0 || (SHOW_TERMS_AND_CONDITIONS==1 && ((count($list)>0 && $list->accept_terms_conditions==0) || count($list)==0)))
			{
				...
				$termscondition .= JText::_( 'TERMS_AND_CONDITIONS_LBL' );
				$termscondition .= ' <a class="modal" href="'.$article_link.'" rel="{handler: \'iframe\', size: {x: '.$finalwidth.', y: '.$finalheight.'}}">'.JText::_('TERMS_AND_CONDITIONS_FOR_LBL').'</a>';
			}
			...
			if (DEFAULT_NEWSLETTER != 0)
			{
				...
				/*if($subscribe)
				{
					if(strstr($template_desc,"{newsletter_unsubscribe}"))
					{
						...
						$newslettersignup = "<input type='checkbox' name='newsletter_signoff' value='1' '$link'>";
						$newslettersignup_lbl = JText::_( 'SIGN_OFF_FOR_NEWSLETTER' );
					}
				}*/
				if($subscribe==0)
				{
					...
					$newslettersignup = "<input type='checkbox' name='newsletter_signup' value='1' $link'>";
					$newslettersignup_lbl = JText::_( 'SIGN_UP_FOR_NEWSLETTER' );
				}
			}
	...
	function checkQuantityInStock($data=array(),$newquantity=1,$minQuantity=0)
	{
		...
		if(USE_STOCKROOM==1)
		{
			...
			if(($product_preorder == "global" && ALLOW_PRE_ORDER) || ($product_preorder == "yes") || ($product_preorder == "" && ALLOW_PRE_ORDER))
			{
				 ...
				 $currentStock = $regular_currentStock + $preorder_currentStock;//die();
		...
		{
			$msg = $productData->product_name. " " . JText::_('WARNING_MSG_MAXIMUM_QUANTITY');
	...
	function dbtocart($userId=0)
	{
		...
		if(count($cart_items)>0)
		{
			...
			for($i=0;$i<count($cart_items);$i++)
			{
				...
				{
					$setCartItem = false;
					$msg = JText::_('PRODUCT_OUTOFSTOCK_MESSAGE');
				...
				if($product_data->product_type=='subscription')
				{
					...
					{
						$setCartItem = false;
						$msg = JText::_('SELECT_PRODUCT_SUBSCRIPTION');
					}
				}
				...
				if(count($req_attribute)>0)
				{
					...
					$msg = $requied_attribute_name." ".JText::_('IS_REQUIRED'); // give error as first attribute is required
					$setCartItem = false;
				}
...
//				return false;
        ...
        if(!$isStock)
        {
			...
			{
				$msg = urldecode(JText::_('PRODUCT_OUTOFSTOCK_MESSAGE'));
...
//				return false;
			...
			{
				$msg = urldecode(JText::_('PREORDER_PRODUCT_OUTOFSTOCK_MESSAGE'));
//				JError::raiseWarning(21, $msg);
//				return false;
				return $msg;
			}
		...
		if($product_data->product_type=='subscription')
		{
			...
			{
				$msg = urldecode(JText::_('PLEASE_SELECT_YOUR_SUBSCRIPTION_PLAN'));
//				JError::raiseWarning(21, $msg);
//				return false;
				return $msg;
			}
...
//					return false;
				...
				if(count($req_property)>0)
				{
					...
					$msg = urldecode($requied_subattribute_name)." ".JText::_('SUBATTRIBUTE_IS_REQUIRED'); // give error as second attribute is required
...
//					return false;
		...
		for( $i=0 ; $i< $idx ; $i++ )
		{
		   	if($cart[$i]['product_id'] == $data['product_id'])
		   	{
				...
				//$sameProduct = !in_array(false,$results,true);
				# End
				...
				if(!empty($row_data))
	        	{
	        		...
	        		for($r=0;$r<count($row_data);$r++)
	        		{
	        			...
	        			$cart[$i][$produser_field];
	        			if($added_userfield != $cart[$i][$produser_field])
	        			{
	        				$puf = 0;
	        			}
	        		}
		   		...
		   		if($sameProduct)
				{
					...
					{
						$cart['notice_message'] = $newcartquantity." ".JTEXT::_('AVAILABLE_STOCK_MESSAGE');
					}else {
						$cart['notice_message']="";
					}
					...
					{
						$msg = (CART_RESERVATION_MESSAGE != '' && IS_PRODUCT_RESERVE) ? CART_RESERVATION_MESSAGE : urldecode(JText::_ ( 'PRODUCT_OUTOFSTOCK_MESSAGE' ));
//						JError::raiseWarning(21, $msg);
//						return false;
						return $msg;
					}
	    ...
	    if(!$sameProduct)
		{
			...
			{
				$cart['notice_message'] = $cart[$idx]['quantity']." ".JTEXT::_('AVAILABLE_STOCK_MESSAGE');
			}else {
				$cart['notice_message']="";
			}
     	 	if($cart[$idx]['quantity']<=0)
     	 	{
     	 		$msg = (CART_RESERVATION_MESSAGE != '' && IS_PRODUCT_RESERVE) ? CART_RESERVATION_MESSAGE : JText::_ ( 'PRODUCT_OUTOFSTOCK_MESSAGE' );
//     	 		JError::raiseWarning(21, $msg);
//				return false;
				return $msg;
     	 	}
		...
		if(count($userfieldArr)>0)
		{
			...
			for($i=0;$i<count($req_fields);$i++)
			{
				if(in_array($req_fields[$i]->field_name,$userfieldArr))
				{
					...
					{
						$msg .= $req_fields[$i]->field_title." ".JText::_('IS_REQUIRED')."<br/>";
					}
				}
			}
		}
   	 		...
   	 		if($calculator_price)
			{
    	 		...
    	 		switch ($discount_calc_method)
				{
					case "volume":
						...
						$calc_output .= JText::_('DISCOUNT_CALC_HEIGHT')." ".$calcHeight."<br />";
						$calc_output_array['calcHeight'] = $calcHeight;
						$calc_output .= JText::_('DISCOUNT_CALC_WIDTH')." ".$calcWidth."<br />";
						$calc_output_array['calcWidth'] = $calcWidth;
						$calc_output .= JText::_('DISCOUNT_CALC_LENGTH')." ".$calcDepth."<br />";
					...
					case "area":
						...
						$calc_output .= JText::_('DISCOUNT_CALC_DEPTH')." ".$calcDepth."<br />";
						$calc_output_array['calcDepth'] = $calcDepth;
						$calc_output .= JText::_('DISCOUNT_CALC_WIDTH')." ".$calcWidth."<br />";
						$calc_output_array['calcWidth'] = $calcWidth;
						if($calcPricePerPiece!="")
					...
					case "circumference":
						...
						$calc_output .= JText::_('DISCOUNT_CALC_RADIUS')." ".$calcRadius."<br />";
				...
				$calc_output .= JText::_('DISCOUNT_CALC_UNIT')." ".$data['calcUnit'];
	...
	function discountCalculator($get)
	{
		...
		if(count($discount_calc_data)){
			...
			if($use_range){
				...
				echo JText::_('TOTAL_AREA')."\n";

				echo JText::_('PRICE_PER_AREA')."\n";

				echo JText::_('PRICE_PER_PIECE')."\n";

				echo JText::_('PRICE_TOTAL')."\n";
...
//				$finalArea = $Area;
				...
				$price_per_piece = $this->getdiscountcalcArea($product_id,$Area,$price_per_piece);

				$price_per_piece_tax = $this->_producthelper->getProductTax($product_id,$price_per_piece,0,1);

				echo $Area."<br />".JText::_('TOTAL_PIECE').$total_sheet."\n";
				...
				echo JText::_('PRICE_OF_ALL_PIECE')."\n";
	...
	function getdiscountcalcArea($product_id,$totalArea,$product_price)
	{
		...
		if ($categorydata)
		{
			...
			{
				$acwhere[] = " FIND_IN_SET('" . $categorydata [$c]->category_id . "', category_id) ";
			}
		}
		...
		$query = "SELECT amount,discount_on FROM `".$this->_table_prefix."discountArea` "
				."WHERE published = 1 "
				."AND (product_id = '' OR FIND_IN_SET('". $product_id . "', product_id)) "
				."AND (category_id = '' $cwhere) "
				."AND (	( ('$totalArea' BETWEEN area_start AND area_end) "
				."OR (area_start = '0' AND area_end = '0'))) "
				."AND (	( ('".time()."' BETWEEN discountstart_date AND discountend_date) "
				."OR (discountstart_date = '0' AND discountend_date = '0'))) "
				."ORDER BY discountAreaid DESC Limit 0,1";

		$this->_db->setQuery($query);
		$list = $this->_db->loadObject();

		switch($list->discount_on){
			case 0:
				$product_price = $product_price - $list->amount;
				break;
			case 1:

				$discountAmount = $product_price/$list->amount;
				$product_price  = $product_price - $discountAmount;
				break;
	...
	function getSubscriptionPlans()
	{
		...
		$optionsection = array();
		$optionsection[]   = JHTML::_('select.option', '6','6 '.JText::_('MONTH'));
		$optionsection[]   = JHTML::_('select.option', '12', '12 '.JText::_('MONTH'));
		$optionsection[]   = JHTML::_('select.option', '18', '18 '.JText::_('MONTH'));
		$optionsection[]   = JHTML::_('select.option', '21', '21 '.JText::_('MONTH'));
		$optionsection[]   = JHTML::_('select.option', '24', '24 '.JText::_('MONTH'));
		...
		if($sectionvalue!="")
		{
			...
			$optiontype = $sectionname;
		}
		else
		{
			$optiontype = $optionsection;
		}
		$option_types 		= JHTML::_('select.genericlist',$optiontype,  'subscription_id', 'class="inputbox" size="1"', 'value', 'text');
		return $option_types;
	}
}?>
