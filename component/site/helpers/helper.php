...
class redhelper
{
    ...
    */
	function removeShippingRate()
	{
		...
		if(count($data)>0)
		{
			$query_plg = "SELECT element FROM #__plugins WHERE folder='redshop_shipping'" ;
	...
	function getItemid($product_id='',$cat_id=0)
	{
		...
		for($i=0;$i<count($cats);$i++)
		{
		    ...
		    $sql = "SELECT id FROM #__menu "
		    		...
		    		."AND `link` LIKE '%com_redshop%' "
		    		."AND `link` LIKE '%view=category%' "
		    		."AND (`link` LIKE '%id=".$cat->category_id."' OR `link` LIKE '%cid=".$cat->category_id."') "
		    		."ORDER BY 'ordering'";
			$this->_db->setQuery($sql);
			if($Itemid = $this->_db->loadResult())
			{
		...
		if($option!='com_redshop')
		{
			if(!$catDetailmenu)
			{
				...
				$this->_db->setQuery($sql);
				if($Itemid = $this->_db->loadResult())	return $Itemid;
			}
		...
		$Itemid = intval( JRequest::getVar('Itemid'));//,$Itemid) );
		return $Itemid;
	}
	
	function getManufacturerItemid($mid)
	{
		$and="";
		if($mid)
		{
			$and="AND `params` LIKE '%manufacturerid=".$mid."%'";
		}
		$sql = "SELECT id FROM #__menu "
				."WHERE published=1 "
				."AND `link` LIKE '%com_redshop%' "
				."AND `link` LIKE '%view=manufacturers%' "
				.$and
				."ORDER BY 'ordering'";
		$this->_db->setQuery($sql);
	...
	function getShopperGroupProductCategory($pid=0)
	{
		...
		{
			$shoppercatdata = $this->getShopperGroupCategory($prodctcat[$i]->category_id);
	...
	function getOrderByList()
	{
		...
		$order_data[0]->text=JText::_( 'PRODUCT_NAME' );
		$order_data[1]->value="p.product_price ASC";
		$order_data[1]->text=JText::_( 'PRODUCT_PRICE_ASC' );
		$order_data[2]->value="p.product_price DESC";
		$order_data[2]->text=JText::_( 'PRODUCT_PRICE_DESC' );
		$order_data[3]->value="p.product_number ASC";
		$order_data[3]->text=JText::_( 'PRODUCT_NUMBER' );
		$order_data[4]->value="p.product_id DESC";
		$order_data[4]->text=JText::_( 'NEWEST' );
		$order_data[5]->value="pc.ordering ASC";
		$order_data[5]->text=JText::_( 'ORDERING' );
		$order_data[6]->value="m.manufacturer_name ASC";
		$order_data[6]->text=JText::_('MANUFACTURER_NAME' );
		return $order_data;
	}
	...
	function getManufacturerOrderByList()
	{
		...
		$order_data[0]->text=JText::_( 'ALPHABETICALLY' );
	...
	function getRelatedOrderByList()
	{
		$order_data = array();
		$order_data[0]->value="p.product_name ASC";
		$order_data[0]->text=JText::_( 'PRODUCT_NAME_ASC' );
		$order_data[1]->value="p.product_name DESC";
		$order_data[1]->text=JText::_( 'PRODUCT_NAME_DESC' );
		$order_data[2]->value="p.product_price ASC";
		$order_data[2]->text=JText::_( 'PRODUCT_PRICE_ASC' );
		$order_data[3]->value="p.product_price DESC";
		$order_data[3]->text=JText::_( 'PRODUCT_PRICE_DESC' );
		$order_data[4]->value="p.product_number ASC";
		$order_data[4]->text=JText::_( 'PRODUCT_NUMBER_ASC' );
		$order_data[5]->value="p.product_number DESC";
		$order_data[5]->text=JText::_( 'PRODUCT_NUMBER_DESC' );

		$order_data[6]->value="r.ordering ASC";
		$order_data[6]->text=JText::_( 'ORDERING_ASC' );
		$order_data[7]->value="r.ordering DESC";
		$order_data[7]->text=JText::_( 'ORDERING_DESC' );
		$order_data[8]->value="e.data_txt ASC";
		$order_data[8]->text=JText::_( 'DATEPICKER_ASC' );
		$order_data[9]->value="e.data_txt DESC";
		$order_data[9]->text=JText::_( 'DATEPICKER_DESC' );
		return $order_data;
	}
	...
	function getAccessoryOrderByList()
	{
		...
		$order_data[0]->text=JText::_( 'PRODUCT_ID_ASC' );
		$order_data[1]->value="child_product_id DESC";
		$order_data[1]->text=JText::_( 'PRODUCT_ID_DESC' );
		$order_data[2]->value="accessory_id ASC";
		$order_data[2]->text=JText::_( 'ACCESSORY_ID_ASC' );
		$order_data[3]->value="accessory_id DESC";
		$order_data[3]->text=JText::_( 'ACCESSORY_ID_DESC' );
		$order_data[4]->value="newaccessory_price ASC";
		$order_data[4]->text=JText::_( 'ACCESSORY_PRICE_ASC' );
		$order_data[5]->value="newaccessory_price DESC";
		$order_data[5]->text=JText::_( 'ACCESSORY_PRICE_DESC' );
    ...
    function getPreOrderByList()
	{
		...
		$preorder_data[0]->text=JText::_( 'GLOBAL' );
		$preorder_data[1]->value="yes";
		$preorder_data[1]->text=JText::_( 'YES' );
		$preorder_data[2]->value="no";
		$preorder_data[2]->text=JText::_( 'NO' );
    ...
    function getChildProductOption()
	{
		...
		$childproduct_data[0]->text=JText::_( 'CHILD_PRODUCT_NAME' );
		$childproduct_data[1]->value="product_number";
		$childproduct_data[1]->text=JText::_( 'CHILD_PRODUCT_NUMBER' );
   ...
   function getStateAbbrivationByList()
	{
		...
		$state_data[0]->text=JText::_( 'TWO_LETTER_ABBRIVATION' );
		$state_data[1]->value="3";
		$state_data[1]->text=JText::_( 'THREE_LETTER_ABBRIVATION' );
	...
	*/
	...
	function watermark($mtype,$Imagename='',$thumb_width='',$thumb_height='',$enable_watermart=WATERMARK_PRODUCT_IMAGE,$add_img=0)
	{
		...
		if($enable_watermart<=0)
		{
			if(($thumb_width != '' || $thumb_width != 0) && ($thumb_height != '' || $thumb_width != 0))
			{
				...
				$filename = $url."components/".$option."/assets/images/".$mtype."/".$Imagename;
			}
			return $filename;
		}

		if($Imagename && file_exists(JPATH_BASE."/components/".$option."/assets/images/".$mtype."/".$Imagename) && ( WATERMARK_IMAGE && file_exists(JPATH_BASE."/components/".$option."/assets/images/product/".WATERMARK_IMAGE)))
		{
			if($thumb_width != '' && $thumb_height != '')
			{
				...
				$watermark = JPATH_BASE."/components/".$option."/assets/images/product/".WATERMARK_IMAGE;
				$filename = JPATH_BASE."/components/".$option."/assets/images/".$mtype."/".$Imagename;
				$gnImagename = 'main'.$Imagename;
				if(file_exists(JPATH_BASE."/components/".$option."/assets/images/watermarked/".$gnImagename))
					return $DestinationFile = $url."components/".$option."/assets/images/watermarked/".$gnImagename;
			}
			$DestinationFile = JPATH_BASE."/components/".$option."/assets/images/watermarked/".$gnImagename;
			...
			switch($filetype)
			{
				case "gif":
					...
					$DestinationFile = $url."components/".$option."/assets/images/watermarked/".$gnImagename;
	...
	function sendmessage($text,$to)
	{
		...
		$api_id = CLICKATELL_API_ID; 			//  clickatell_api_id
		$baseurl ="http://api.clickatell.com";

		$url = "$baseurl/http/auth?user=$user&password=$password&api_id=$api_id"; 			// auth call
		$ret = file($url);																	// do auth call
	...
	/**
	 * logic for get super admin and Administrator
	 *
	 * @access public
	 * @return array
	 * @since 0.1
	 */
	function getSuperAdminList($id=0)
	{
		$and = "";
		if($id!=0)
		{
		...
		$query = "SELECT u.*, CONCAT(u.username,' (',u.name,')') AS title "
				."FROM #__users AS u "
	...
	function isredProductfinder()
	{
		...
		$query = "SELECT enabled FROM `#__components` WHERE `link` LIKE '%com_redproductfinder%'";
		$this->_db->setQuery($query);
		$redproductfinder = $this->_db->loadobject();
		$redproductfinder_path = JPATH_ADMINISTRATOR. DS .'components'. DS .'com_redproductfinder';
	...
	{
		// get redshop from joomla component table
		$isredCRM 	= $this->_session->get('isredCRM');
		...
		{	$user =& JFactory::getUser();
			...
			if ($crmHelper->isredCRMUser($user->id)){
				$this->_session->set('isredcrmuser', true);
			}else {
				$this->_session->set('isredcrmuser', false);
			}

			$this->_session->set('isredcrmuser_debitor', $crmHelper->isredCRMUserdebitor($user->id));

			$this->_isredCRM = true;
		}
			$this->_session->set('isredCRM',$this->_isredCRM);
