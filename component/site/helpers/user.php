...
class rsUserhelper
{
	 ...
	 */
	function getShopperGroup($user_id=0)
	{
	...
	function getShoppergroupData($user_id=0)
	{
		$list = array();
	...
	function updateUserTermsCondition($users_info_id=0,$isSet=0)
	{
		$and = '';
		if($users_info_id!=0)
		{
	...
	function getShopperGroupList($shopper_group_id=0)
	{
		$and = '';
	...
	function validate_user($username,$id=0)
	{
...
//			echo JText::_('EMPTY_EMAIL');
		...
		$original_usertype = $user->get('gid');
		if(!$original_gid)
		{
			$user->set('gid', 18);
		}
		if(!$original_usertype)
		{
		...
		{
			$msg = JText::_( 'YOU_CANNOT_BLOCK_YOURSELF!' );
...
//			echo JText::_('EMPTY_EMAIL');
		...
		if($registeruser==1)
		{
			...
			{
	 			JError::raiseWarning ( '', JText::_('EMPTY_PASSWORD'));
//				echo JText::_('EMPTY_PASSWORD');
				return false;
			}
			if(strlen($data['password']) || strlen($data['password2']))
			{
				...
				{
					JError::raiseWarning ( '', JText::_('PASSWORDS_DO_NOT_MATCH'));
//					echo JText::_('PASSWORDS_DO_NOT_MATCH');
					return false;
				}
			}
...
//				echo $user->getError();
	...
	function checkCaptcha($data)
	{
		if(SHOW_CAPTCHA)
		{
			...
			{
				JError::raiseWarning(21, JText::_('INVALID_SECURITY'));
				return false;
			}
		}
		return true;
...
//			return false;
	...
	function getBillingTable($post=array(),$is_company=0,$lists,$show_shipping=0,$show_newsletter=0, $create_account)
	{
		...
		$template_desc = str_replace("{required_lbl}", JText::_ ( 'REQUIRED' ), $template_desc);
		if($show_shipping && SHIPPING_METHOD_ENABLE)
		{
			$template_desc = str_replace("{sipping_same_as_billing_lbl}", JText::_ ( 'SHIPPING_SAME_AS_BILLING' ), $template_desc);
		...
		if(strstr($template_desc,"{account_creation_start}") && strstr($template_desc,"{account_creation_end}"))
		{
			...
			if (REGISTER_METHOD != 1 && REGISTER_METHOD != 3)
			{
				...
				$template_middle = str_replace("{username_lbl}", JText::_ ( 'USERNAME_REGISTER' ), $template_middle);
				...
				$template_middle = str_replace("{password_lbl}", JText::_ ( 'PASSWORD_REGISTER' ), $template_middle);
				...
				$template_middle = str_replace("{confirm_password_lbl}", JText::_ ( 'CONFIRM_PASSWORD' ), $template_middle);
				...
				{
					$newsletter_signup_lbl = JText::_ ( 'SIGN_UP_FOR_NEWSLETTER' );
	...
	function replaceBillingCommonFields($template_desc,$post=array(),$lists)
	{
		...
		$template_desc = str_replace("{email_lbl}", JText::_ ( 'EMAIL' ), $template_desc);
		$template_desc = str_replace("{email}", '<input class="inputbox required" type="text" title="'.JTEXT::_('PROVIDE_CORRECT_EMAIL_ADDRESS').'" name="email1" id="email1" size="32" maxlength="250" value="'.@$post ["email1"].'" />', $template_desc);
		if(strstr($template_desc,"{retype_email_start}") && strstr($template_desc,"{retype_email_end}"))
		{
			...
			{
				$template_middle = $template_pd_edata[0];
				$template_middle = str_replace("{retype_email_lbl}", JText::_ ( 'RETYPE_CUSTOMER_EMAIL' ), $template_middle);
		...
		$template_desc = str_replace("{firstname_lbl}", JText::_ ( 'FIRSTNAME' ), $template_desc);
		...
		$template_desc = str_replace("{lastname_lbl}", JText::_ ( 'LASTNAME' ), $template_desc);
		...
		$template_desc = str_replace("{address_lbl}", JText::_ ( 'ADDRESS' ), $template_desc);
		...
		$template_desc = str_replace("{zipcode_lbl}", JText::_ ( 'ZIP' ), $template_desc);
		...
		$template_desc = str_replace("{city_lbl}", JText::_ ( 'CITY' ), $template_desc);
		...
		$template_desc = str_replace("{phone_lbl}", JText::_ ( 'PHONE' ), $template_desc);
		$template_desc = str_replace("{phone}", '<input class="inputbox required" type="text" name="phone" id="phone" size="32" maxlength="250" value="'.@$post ["phone"].'" />', $template_desc);
		...
		$template_desc = str_replace("{country_lbl}", JText::_('COUNTRY'), $template_desc);
		$template_desc = str_replace("{country}", $lists['country_code'], $template_desc);
		$template_desc = str_replace("{state_lbl}", JText::_ ( 'STATE' ), $template_desc);
	...
	function replaceCompanyCustomer($template_desc,$post=array(),$lists)
	{
		...
		$template_desc = str_replace("{company_name_lbl}", JText::_ ( 'COMPANY_NAME' ), $template_desc);
		...
		$template_desc = str_replace("{ean_number_lbl}", JText::_ ( 'EAN_NUMBER' ), $template_desc);
		...
		if(strstr($template_desc,"{vat_number_start}") && strstr($template_desc,"{vat_number_end}"))
		{
			...
			if(USE_TAX_EXEMPT==1)
			{
				...
				$template_middle = str_replace("{vat_number_lbl}", JText::_ ( 'BUSINESS_NUMBER' ), $template_middle);
		...
		{
			$tax_exempt = JHTML::_('select.booleanlist',  'tax_exempt', 'class="inputbox" ', @$post["tax_exempt"], JText::_('COMPANY_IS_VAT_EXEMPTED'), JText::_('COMPANY_IS_NOT_VAT_EXEMPTED'));
			$template_desc = str_replace("{tax_exempt_lbl}", JText::_ ( 'TAX_EXEMPT' ), $template_desc);
			$template_desc = str_replace("{tax_exempt}", $tax_exempt, $template_desc);
		}
		else
		{
	...
	function getShippingTable($post=array(),$is_company=0,$lists)
	{
		...
		if(!isset($post ["phone_ST"]) || (isset($post ["phone_ST"]) && $post ["phone_ST"]==0))
		{
			$post ["phone_ST"] ='';
		}
		$allowCustomer = '';
		...
		$template_desc = str_replace("{firstname_st_lbl}", JText::_ ( 'FIRSTNAME' ), $template_desc);
		...
		$template_desc = str_replace("{lastname_st_lbl}", JText::_ ( 'LASTNAME' ), $template_desc);
		...
		$template_desc = str_replace("{address_st_lbl}", JText::_ ( 'ADDRESS' ), $template_desc);
		...
		$template_desc = str_replace("{zipcode_st_lbl}", JText::_ ( 'ZIP' ), $template_desc);
		...
		$template_desc = str_replace("{city_st_lbl}", JText::_ ( 'CITY' ), $template_desc);
		...
		$template_desc = str_replace("{phone_st_lbl}", JText::_ ( 'PHONE' ), $template_desc);
		$template_desc = str_replace("{phone_st}", '<input class="inputbox billingRequired valid phone" type="text" name="phone_ST" id="phone_ST" size="32" maxlength="250" value="'.@$post ["phone_ST"].'" />', $template_desc);
		...
		$template_desc = str_replace("{country_st_lbl}", JText::_('COUNTRY'), $template_desc);
		$template_desc = str_replace("{country_st}", $lists['country_code_ST'], $template_desc);
		$template_desc = str_replace("{state_st_lbl}", JText::_ ( 'STATE' ), $template_desc);
	...
	function getCaptchaTable()
	{
		...
		if(SHOW_CAPTCHA)
		{
			...
			$html .= '<tr><td>&nbsp;</td>
						<td align="left"><img src="'.JURI::base(true).'/index.php?option=com_redshop&view=registration&task=captcha&tmpl=component&captcha=security_code&width=100&height=40&characters=5" /></td></tr>';
			$html .= '<tr><td width="100" align="right"><label for="security_code">'.JTEXT::_('SECURITY_CODE').'</label></td>
