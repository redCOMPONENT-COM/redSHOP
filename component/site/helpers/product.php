 ...
 *
 * redSHOP can be downlodisplayAdditionalImageaded from www.redcomponent.com
...
class producthelper
{
	...
	var $_session		= null;
	var $_cartTemplateData			= null;
	var $_ajaxdetail_templatedata	= null;
	var $_vatCountry 	= null;
	var $_vatState 		= null;
	var $_vatGroup 		= null;
	var $_taxData 		= array();
	var $_cart_template = null;
	var $_acc_template  = null;
	var $_attribute_template = null;
	var $_attributewithcart_template= null;
	var $_shopper_group_id = null;
	var $_discount_product_data = null;
...
//	  	$this->_carthelper 		= new rsCarthelper();
	...
	function getwishlistuserfieldata($wishlistid,$productid)
	{
		$query = 'SELECT * FROM '.$this->_table_prefix.'wishlist_userfielddata  WHERE wishlist_id = "'. $wishlistid.'"  and product_id="'.$productid.'" order by fieldid ASC';
		$this->_db->setQuery($query);
		$result = $this->_db->loadObjectList();

		return $result;
	}
	function getWishlistmodule($menu_id)
	{
		$query = 'SELECT * FROM #__modules WHERE module = "'. $menu_id.'" ';
		$this->_db->setQuery($query);
	...
	function getProductById($product_id,$field_name="",$test='')
	{
		if($this->_id != $product_id )
		{
			$query = 'SELECT * FROM '.$this->_table_prefix.'product WHERE product_id = "'. $product_id.'" ';
			$this->_db->setQuery($query);
	...
	function getProductPrices($product_id,$userid,$quantity=1)
	{
		$leftjoin = "";
...
//		$shopperGroupId = $this->_userhelper->getShopperGroup($userid);
		if($userid)
		{
			...
			$and = " AND u.user_id='".$userid."' AND u.address_type='BT' ";
		...
		$query = "SELECT p.price_id,p.product_price,p.product_currency,p.discount_price, p.discount_start_date, p.discount_end_date "
				...
				. $and
				. " AND (( p.price_quantity_start <= '".$quantity."' AND p.price_quantity_end >= '".$quantity."' ) OR (p.price_quantity_start = '0' AND p.price_quantity_end = '0'))"
	...
	function getProductSpecialPrice($product_price,$discount_product_id)
	{
		$result = array();
		...
		if ($discount_product_id)
		{
			...
			$where .= " AND discount_product_id IN (".$discount_product_id.") ";
	...
	function getProductTax($product_id=0,$product_price=0,$user_id=0,$tax_exempt=0)
	{
		$vat_rate_id = 0;
		$userArr 	= $this->_session->get('rs_user');
		if($user_id==0)
		...
		# redCRM contact person handle
		$redhelper   = new redhelper();
		if($redhelper->isredCRM()){

		$crmDebitorHelper = new crmDebitorHelper();
		$debitor_info = $crmDebitorHelper->getDebitor(0,0,$user_id);
		$vat_rate_id = $debitor_info[0]->debitor_vat_rate;
		}
		# End
...
//			unset($userArr['rs_tax_rate']);
		...
		$vatrates_data = $this->getVatRates($product_id,$user_id,$vat_rate_id);
		if($vatrates_data){
			$tax_rate 						= $vatrates_data->tax_rate;
		}
...
//            $protax = $this->productPriceRound($protax);
	...
	function replaceVatinfo($data_add){
	...
	function getVatUserinfo($user_id=0)
	{
		...
		{
			$userArr 			= $this->_session->get('rs_user');
			$rs_user_info_id = (array_key_exists('rs_user_info_id',$userArr)) ? $userArr['rs_user_info_id'] : 0;
	...
	function getVatRates($product_id=0,$user_id=0,$vat_rate_id=0)
	{
		...
		if(!empty($userArr))
		{
			if(array_key_exists('vatCountry',$userArr) && !empty($userArr['taxData']))
			{
				...
				if($userArr['vatCountry'] == $userdata->country_code && $userArr['vatState'] == $userdata->state_code && @$userArr['vatGroup'] == $proinfo->product_tax_group_id)
				{
...
//			$and .= ' AND tr.is_eu_country=0 ';
		...
		if($vat_rate_id==0)
		{
			...
			$where = 'WHERE tr.tax_rate_id = "'.$vat_rate_id.'"';
		}
	...
	function getGoogleVatRates($product_id=0,$product_price=0,$tax_exempt=0)
	{
		$proinfo = $this->getProductById($product_id);
		$tax_group_id 		= 0;
		$rs_user_info_id = 0;
...
//			$and .= ' AND tr.is_eu_country=0 ';
	 ...
	 *
	 */
	function getExtraSectionTag($filedname=array(),$product_id,$section,$template_data,$categorypage=0)
	{
	...
	function getPriceReplacement($product_price)
	{
		...
		else
		{
			...
			{
				$return = 	PRICE_REPLACE_URL ?  "<a href='http://".PRICE_REPLACE_URL."' target='_blank'>".PRICE_REPLACE."</a>" : PRICE_REPLACE;
			}
     ...
     * @since    1.5
     */
	function getProductFormattedPrice($product_price,$convert=true,$currency_symbol=REDCURRENCY_SYMBOL)
	{
		...
		if($convert && $session->get('product_currency'))
		{
			$convertPrice = new convertPrice();
	   		...
	   		if($session->get('product_currency') == CURRENCY_CODE)
	...
	function productPriceRound($product_price)
	{
		$cal_no = 4;
	...
	function getProductparentImage($product_parent_id)
	{
	...
	function getProductImage($product_id = 0, $link = '', $width, $height, $Product_detail_is_light = 2, $enableHover=0,$suffixid=0)
	{
		$thum_image = '';
		...
		{
			if ($result->product_thumb_image && file_exists ( JPATH_SITE.$middlepath.$result->product_thumb_image ))
			{
		...
		{
			if (PRODUCT_OUTOFSTOCK_IMAGE && file_exists (JPATH_SITE.$middlepath.PRODUCT_OUTOFSTOCK_IMAGE))
			{
			...
			else if ($product_image && file_exists ( JPATH_SITE.$middlepath.$product_image ))
			{
				if ($result->product_full_image && file_exists ( JPATH_SITE.$middlepath.$result->product_full_image ) && ($result->product_thumb_image && file_exists ( JPATH_SITE.$middlepath.$result->product_thumb_image )))
				{
				...
				}else if ($result->product_thumb_image && file_exists ( JPATH_SITE.$middlepath.$result->product_thumb_image ))
				{
				...
				}else if($result->product_full_image && file_exists ( JPATH_SITE.$middlepath.$result->product_full_image))
				{
					$thum_image = $this->replaceProductImage($result,$product_image,"",$link,$width,$height,$Product_detail_is_light,$enableHover,array(),$suffixid);
				}
			} else {
//				$noimage = "/components/com_redshop/assets/images/noimage.jpg";
		...
		else if($product_image && file_exists ( JPATH_SITE.$middlepath.$product_image))
		{
			...
			if (PRODUCT_DEFAULT_IMAGE && file_exists ( JPATH_SITE.$middlepath.PRODUCT_DEFAULT_IMAGE ))
			{
				$thum_image = $this->replaceProductImage($result,PRODUCT_DEFAULT_IMAGE,"",$link,$width,$height,$Product_detail_is_light,$enableHover,array(),$suffixid);
			}
		}
	...
	function replaceProductImage($product,$imagename="",$linkimagename="",$link="",$width,$height,$Product_detail_is_light=2,$enableHover=0,$preselectedResult=array(),$suffixid=0)
	{
		...
		$parseArr['product'] =	$product;
		$parseArr['imagename'] =	$imagename;
		$parseArr['linkimagename'] =	$linkimagename;
		$parseArr['link'] =	$link;
		$parseArr['width'] =	$width;
		$parseArr['height'] =	$height;
		$parseArr['Product_detail_is_light'] =	$Product_detail_is_light;
		$parseArr['enableHover'] =	$enableHover;
		$parseArr['preselectedResult'] =	$preselectedResult;
		$parseArr['suffixid'] =	$suffixid;

		$pluginResults = $dispatcher->trigger('onAfterImageLoad', array ($parseArr));
		...
		if($Product_detail_is_light != 2 )
		{
			if ($product->product_thumb_image && file_exists ( JPATH_SITE.$middlepath.$product->product_thumb_image ))
			{
		...
		$title = ' title="'.htmlspecialchars($product->product_name).'" ';
		...
		$product_img = $url.$noimage;

		$product_hover_img = $url.$noimage;
		$linkimage = $url.$noimage;
		if($imagename!="")
		{
			$product_img = $redhelper->watermark('product',$imagename,$width,$height,WATERMARK_PRODUCT_THUMB_IMAGE,'0');
			if($cat_product_hover)
			{
				$product_hover_img = $redhelper->watermark('product',$imagename,PRODUCT_HOVER_IMAGE_WIDTH,PRODUCT_HOVER_IMAGE_HEIGHT,WATERMARK_PRODUCT_THUMB_IMAGE,'0');
			}
			if($linkimagename!="")
			{
		...
		if(count($preselectedResult)>0)
		{
			...
			$title = ' title="'.htmlspecialchars($preselectedResult['aTitleImageResponse']).'" ';
		...
		{
			$thum_image = "<img id='main_image".$commonid."' src='".$product_img."' ".$title.$alt." rel=\"lightbox[product7]\" />";
		}
		else
		{
			...
			{
				$thum_image = "<a id='a_main_image".$commonid."' ".$title." href='".$linkimage."' rel=\"lightbox[product7]\">";
			}
			...
			$thum_image .= "<img id='main_image".$commonid."' src='" . $product_img . "' ".$title.$alt." class='redproductimage' />";
			if($cat_product_hover)
			{
				$thum_image .= "<img id='main_image".$commonid."' src='".$product_hover_img."' ".$title.$alt." class='redImagepreview' />";
			}
			$thum_image .= "</a>";
		}
	...
	function getProductCategoryImage($product_id=0,$category_img = '',$link='', $width, $height)
	{
		$redhelper   = new redhelper();
		$result = $this->getProductById( $product_id );
		$middlepath = "/components/com_redshop/assets/images/category/";
		$thum_image= "";
		$title = " title='".$result->product_name."' ";
		$alt = " alt='".$result->product_name."' ";
		if($category_img && file_exists ( JPATH_SITE.$middlepath.$category_img))
		{
			 if (PRODUCT_IS_LIGHTBOX == 1) {
				...
				$thum_image = "<a id='a_main_image".$product_id."' href='" . $linkimage . "' ".$title."  rel=\"lightbox[product7]\">";
	...
	function getProductMinDeliveryTime($product_id=0,$section_id=0,$section='',$loadDiv=1)
	{
		$helper = new redhelper();
		...
		if(!$section_id && !$section)
		{
			...
			.   $this->_table_prefix."stockroom as s "
			. " WHERE "
			. " pas.section_id = '".$section_id."' AND pas.section = '".$section."' AND pas.stockroom_id = s.stockroom_id and pas.quantity >0 ORDER BY min_del_time ASC LIMIT 0,1";
	 	...
	 	if($result){
	 		if(!$section_id && !$section)
	 		{
	 			$sql = "SELECT  min_del_time as deltime, s.max_del_time,s.delivery_time "
						...
						. " WHERE "
						. " ps.product_id = '".$product_id."' AND ps.stockroom_id = s.stockroom_id AND s.min_del_time = ".$result->deltime." and ps.quantity >=0 ORDER BY max_del_time ASC LIMIT 0,1";
	 		}else{
	 			$sql = "SELECT  min_del_time as deltime, s.max_del_time,s.delivery_time "
						. " FROM ".  $this->_table_prefix."stockroom as s, "
						.$this->_table_prefix."product_attribute_stockroom_xref AS pas  "
						. " WHERE "
						. " pas.section_id = '".$section_id."' AND pas.section = '".$section."' AND pas.stockroom_id = s.stockroom_id AND s.min_del_time = ".$result->deltime." and pas.quantity >=0 ORDER BY max_del_time ASC LIMIT 0,1";
		...
		*/
		if ($helper->isredCRM()){
			...
			if(ENABLE_ITEM_TRACKING_SYSTEM){
				...
				$dayLanguage = (strlen($product_delivery_time) == 1 && $product_delivery_time == 1) ? JText::_( 'DAY' ) : JText::_( 'DAYS' );
	...
	function GetDefaultQuantity($product_id=0,$data_add="")
	{
		$cart_template = $this->getAddtoCartTemplate($data_add);
		$cartform = (count($cart_template)>0 && $cart_template->template_desc!="") ? $cart_template->template_desc : "";
		$qunselect= 1;
	...
	function GetProductShowPrice($product_id,$data_add,$seoTemplate="",$user_id=0,$isrel=0,$attributes=array())
	{
		$product_price = '';
		$price_excluding_vat = '';
		...
		$stockroomhelper = new rsstockroomhelper();
		//echo DEFAULT_QUOTATION_MODE;die();
		if(SHOW_PRICE  && (!DEFAULT_QUOTATION_MODE || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE)))
		{
			...
			$isStockExists = $stockroomhelper->isStockExists($product_id);//&& $user->id
			...
			$price_excluding_vat 		= $ProductPriceArr['price_excluding_vat'];//*$qunselect;
			...
			{
				 $display_product_price_incl_vat = '<span id="display_product_price_incl_vat'.$product_id.'">'.$product_price_incl_vat.'</span>';
			}
		}
	...
	function getProductNetPrice($product_id,$user_id=0,$quantity=1,$data_add='',$attributes=array())
	{
		$user 			= &JFactory::getUser();
		if($user_id==0)
		{
		...
		$price_text 				= JText::_( 'REGULAR_PRICE' )."";
		...
		if(!empty($res))
		{
			...
			$price_text = $price_text."<span class='redPriceLineThrough'>".$formatted_price."</span><br />".JText::_( 'SPECIAL_PRICE' );
		}
		else
		{
		...
		if(SHOW_PRICE)// && !DEFAULT_QUOTATION_MODE)
		{
			...
			if($row->product_on_sale && $product_discount_price_tmp > 0)
			{
				...
				if($product_price < $product_discount_price_tmp )
				{
					...
					$product_price_saving_lbl       = JText::_('PRODUCT_PRICE_SAVING_LBL');
					$product_old_price_lbl			= JText::_('PRODUCT_OLD_PRICE_LBL');
				}
			} else {
				$product_main_price 			= $product_price;
				$product_price 					= $this->defaultAttributeDataPrice($product_id,$product_price,$data_add,$user_id,intval($applytax),$attributes);
			...
			{
				$product_vat_lbl = ' '.JText::_('PRICE_INCLUDING_TAX');
			}else
			{
				$product_vat_lbl = ' '.JText::_('PRICE_EXCLUDING_TAX');
			}
			$product_price_lbl		= JText::_('PRODUCT_PRICE');
		} else {
	...
	function getProductQuantityPrice($product_id,$userid)
	{
		$userArr 	= $this->_session->get('rs_user');
		if(empty($userArr)){
			$userArr = $this->_userhelper->createUserSession($userid);
		...
		{
			$quantitytable = "<table>";
			$quantitytable .= "<tr><th>".JText::_('QUANTITY')."</th><th>".JText::_('PRICE')."</th></tr>";
			foreach($result as $r)
			{
				if($r->discount_price!=0 && $r->discount_start_date!=0 && $r->discount_end_date!=0 && $r->discount_start_date<=time() && $r->discount_end_date>=time())
				{
					$r->product_price = $r->discount_price;
				}
				$price = $this->getProductFormattedPrice($r->product_price);
				$quantitytable .= "<tr><td>".$r->price_quantity_start." - ".$r->price_quantity_end."</td><td>".$price."</td></tr>";
			}
			$quantitytable .= "</table>";
		}
	...
	function getDiscountId($subtotal=0,$user_id=0)
	{
		$user = &JFactory::getUser();
		if($user_id==0)
		{
...
// 			}
			...
			if(APPLY_VAT_ON_DISCOUNT)
			{
				...
				// Temp fix to show right VAT of cart content subtotal
	 			//$cart['tax'] = $cart['tax'] - $discount_vat;
				$cart['tax'] = $cart['tax'];
				// Temp fix to not have VAT on discount as the value is allready set to either discount with or without vat as a result of the config setting for VAT on or off discount
				//$discount_amount += $discount_vat;
				// Temp fix to solve issues with difference in amount of digits between e-conomic and redSHOP make them calculate on the same
				$discount_amount = round($discount_amount, 2);
				// Temp fix to add in new var with discount vat to fix e-conomic discount and calculated based on the actual VAT rate for the user retracted from the discount amount incl. VAT
				$discount_vat_fix = 0;
				if($discount_amount)
				{
	 ...
	 * second argument is used in
	 * content_product plugin
	 *****************************************/

	function getProductPrice($product_id,$show_price_with_vat=1,$user_id=0)
	{
		$user = &JFactory::getUser();
		if($user_id==0)
		{
	...
	function getAdditionMediaImage($section_id=0,$section="",$mediaType="images")
	{
		$left = "";
		if($section=="product")
		{
	...
	function getAltText($media_section,$section_id,$media_name='',$media_id=0,$mediaType="images")
	{
		$and = '';
		if($media_name != ''){
	...
	function getUserInformation($userid=0,$address_type='BT',$rs_user_info_id=0)
	{
		$list = array();
		$user = &JFactory::getUser();
		$and	= '';
	...
	function GetdefaultshopperGroupData()
	{
		$list = array();
	...
	function checkDiscountDate($productid=0)
	{
		$discountprice = 0;
		$today = time();
	...
	function getPropertyPrice($section_id='',$quantity='',$section='',$user_id=0)
	{
		$leftjoin = "";
		$and = "";
		$user = &JFactory::getUser();
		...
		$this->_db->setQuery($query);
		$result = $this->_db->loadObject();
		if(count($result)>0 && $result->discount_price!=0 && $result->discount_start_date!=0 && $result->discount_end_date!=0 && $result->discount_start_date<=time() && $result->discount_end_date>=time() && $result->discount_price < $result->product_price)
		{
			$result->product_price = $result->discount_price;
		}
		return $result;
	}
	function getProperty($section_id,$section)
	{
		if($section=='property'){
			$query = "SELECT p.*,property_price as product_price  "
				. " FROM ".$this->_table_prefix."product_attribute_property AS p "
				. " WHERE "
				. " p.property_id = '".$section_id."' ";
		}
		if($section=='subproperty'){
			$query = "SELECT p.*,subattribute_color_price as product_price  "
				...
				. " WHERE "
				. " p.subattribute_color_id = '".$section_id."' " ;
		}
		$this->_db->setQuery($query);
		$result = $this->_db->loadObject();
		return $result;
	...
	function getWrapper($product_id,$wrapper_id=0,$default=1)
	{
		$usetoall = "";
		$and = "";
		if($wrapper_id!=0)
	...
	function getBreadcrumbPathway($category=array())
	{
		$pathway_items = array();
	...
	function getCategoryNavigationlist($category_id)
	{
		$redhelper   = new redhelper();
		static $i=0;
	...
	function generateBreadcrumb($sectionid=0)
	{
		global $mainframe;
...
//				$refererurl = $_SERVER['HTTP_REFERER'];
			...
			case "account":
				...
				{
					$main = new stdClass();
					$main->name = JText::_('ACCOUNT_MAINTAINANCE');
					$main->link = "";
				}
				$custompathway[] = $main;
				break;
			case "order_detail":
				...
				$main = new stdClass();
				$main->name = JText::_('ORDER_DETAILS');
				$main->link = "";
				$custompathway[] = $main;
				break;
			case "orders":
			...
			case "account_shipto":
				...
				if($view=='orders')
				{
					$lastlink = JText::_('ORDER_LIST');
				}
				elseif($view=='account_billto')
				{
					$lastlink = JText::_('BILLING_ADDRESS_INFORMATION_LBL');
				}
				elseif($view=='account_shipto')
				{
					$lastlink = JText::_('SHIPPING_ADDRESS_INFO_LBL');
				}
				$main = new stdClass();
				$main->name = $lastlink;
				$main->link = "";
	...
	function getSection($section="",$id=0)
	{
		$and = "";
		if($id!=0)
		{
	...
	function getMenuInformation($Itemid=0,$sectionid=0,$sectioname="",$menuview="",$isRedshop=true)
	{
	...
	function getCategoryProduct($id=0)
	{
	...
	function getProductCategory($id=0)
	{
	 ...
	 *
	 */
	function checkProductDownload($pid,$return=false)
	{
	  	$query = 'SELECT product_download,product_download_days,product_download_limit,product_download_clock,product_download_clock_min,product_download_infinite FROM '.$this->_table_prefix.'product '
	...
	function getProductMediaName($product_id)
	{
	...
	function updateContainerStock($product_id,$quantity,$container_id)
	{
	...
	function getValidityDate($period,$data){
		...
		$data = str_replace ( "{giftcard_validity_from}", JText::_('FROM')." ".$fromdate , $data );
		$data = str_replace ( "{giftcard_validity_to}", JText::_('TO')." ". $todate , $data );

		return $data;
		//return JText::_('TO').' '.$fromdate.' '.JText::_('FROM')." ". $todate;
		//$date = strtotime(date());

	}
	function getAccessoryPrice($product_id=0,$accessory_price=0,$accessory_main_price=0,$vat=0,$user_id=0)
	{
		$return = array();
		$saved = 0;
		/*
	...
	function getuserfield($orderitemid=0,$section_id=12)
	{
				...
				for($j=0;$j<count($userfield);$j++)
				{
					if($userfield[$j]->field_name==$userFieldTag[$i])
					{
						if($userfield[$j]->field_type==10)
						{
							...
							{
								$u_link = $live_site."components/com_redshop/assets/document/product/".$files[$f];
		...
		{
			$resultstr = "<div>".JText::_("PRODUCT_USERFIELD")."</div><div>".implode("<br/>",$resultArr)."</div>";
		}
		return $resultstr;
	}
	...
	function GetProdcutUserfield($id='NULL',$section_id=12)
	{
		$extraField = new extraField();
		$redTemplate = new Redtemplate();
		$cart = $this->_session->get( 'cart');
	...
	function GetProdcutfield($id='NULL',$section_id=1)
	{
		$extraField = new extraField();
		$cart = $this->_session->get( 'cart');
		$product_id = $cart[$id]['product_id'];
	...
	function GetProdcutfield_order($orderitemid='NULL',$section_id=1)
	{
		$extraField = new extraField();
		$order_functions = new order_functions();
		$orderItem = $order_functions->getOrderItemDetail(0,0,$orderitemid);
			...
			if(array_key_exists($row_data[$i]->field_name,$cart[$id]) && $cart[$id][$row_data[$i]->field_name]){
				...
				if(trim($user_fields)!='')
				{
					$sql="INSERT INTO ".$this->_table_prefix."fields_data "
						...
						."value ('".$row_data[$i]->field_id."','".$user_fields."','".$order_item_id."','".$section_id."')";
					$this->_db->setQuery($sql);
					$this->_db->query();
				}
			}
	...
	function getProductAttribute($product_id=0,$attribute_set_id=0,$attribute_id=0,$published=0,$attribute_required=0,$notAttributeId=0)
	{
		$and = "";
		$astpublished = "";
		if($product_id!=0)
		...
		$query = "SELECT a.attribute_id AS value,a.attribute_name AS text,a.*,ast.attribute_set_name "
				...
				."LEFT JOIN ".$this->_table_prefix."attribute_set AS ast ON ast.attribute_set_id=a.attribute_set_id ".$astpublished
	...
	function getAttibuteProperty($property_id=0,$attribute_id=0,$product_id=0,$attribute_set_id=0,$required=0,$notPropertyId=0)
	{
		$and = "";
		if($attribute_id!=0)
		{
	...
	function getAttibutePropertyWithStock($property)
	{
		$stockroomhelper = new rsstockroomhelper();
	...
	function getAttibuteSubPropertyWithStock($subproperty)
	{
		$stockroomhelper = new rsstockroomhelper();
	...
	function getAttibuteSubProperty($subproperty_id=0,$property_id=0)
	{
		$and = "";
		if($subproperty_id!=0)
		{
		...
		$query = "SELECT sp.subattribute_color_id AS value, sp.subattribute_color_name AS text"
				.",sp.*,p.property_name,p.setrequire_selected,p.setmulti_selected,p.setdisplay_type "
				."FROM ".$this->_table_prefix."product_subattribute_color AS sp "
	...
	function getAttributeTemplate($data_add="",$display=true)
	{
	...
	function getProductAccessory($accessory_id=0,$product_id=0,$child_product_id=0,$cid=0)
	{
		...
		{
			$switchquery = ", IF ( (p.product_on_sale>0 && ((p.discount_enddate='' AND p.discount_stratdate='') OR ( p.discount_enddate>='".time()."' AND p.discount_stratdate<='".time()."'))), p.discount_price, p.product_price ) AS newaccessory_price ";//",'+' AS oprand ";
		}
		else
		{
			$switchquery =   ", CASE a.oprand "
							."WHEN '+' THEN IF ( (p.product_on_sale>0 && ((p.discount_enddate='' AND p.discount_stratdate='') OR ( p.discount_enddate>='".time()."' AND p.discount_stratdate<='".time()."'))), p.discount_price, p.product_price ) + accessory_price "
							."WHEN '-' THEN IF ( (p.product_on_sale>0 && ((p.discount_enddate='' AND p.discount_stratdate='') OR ( p.discount_enddate>='".time()."' AND p.discount_stratdate<='".time()."'))), p.discount_price, p.product_price ) - accessory_price "
							."WHEN '=' THEN accessory_price "
							."END AS newaccessory_price ";
		}
		$mainpricequery = "IF ( (p.product_on_sale>0 && ((p.discount_enddate='' AND p.discount_stratdate='') OR ( p.discount_enddate>='".time()."' AND p.discount_stratdate<='".time()."'))), p.discount_price, p.product_price ) AS accessory_main_price ";

		$query = "SELECT a.*,p.product_number, p.product_name, ".$mainpricequery.", p.product_s_desc, p.product_full_image, p.product_on_sale "
				.$switchquery
	...
	function getAddtoCartTemplate($data_add="")
	{
		$redTemplate = new Redtemplate();
	...
	function getAccessoryTemplate($data_add="")
	{
		$redTemplate = new Redtemplate();
	...
	function getRelatedProductTemplate($data_add="")
	{
		$redTemplate = new Redtemplate();
	...
	function getRelatedProduct($product_id=0,$related_id=0)
	{
		$helper = new redhelper();
		$and = "";
		...
		if($related_id!=0)
		{
	 	...
	 	if(count($finaltypetype_result)>0 && $finaltypetype_result->extrafield!='' && (DEFAULT_RELATED_ORDERING_METHOD=='e.data_txt ASC' || DEFAULT_RELATED_ORDERING_METHOD=='e.data_txt DESC'))
		{
			$add_e=",e.*";
		}else{
			$add_e=" ";
		...
		if((DEFAULT_RELATED_ORDERING_METHOD=='e.data_txt ASC' || DEFAULT_RELATED_ORDERING_METHOD=='e.data_txt DESC')){
		...
		$this->_db->setQuery($query);
		//echo $this->_db->getQuery($query);
		$list = $this->_db->loadObjectlist();
		return $list;
	}
	function makeTotalPriceByOprand($price=0,$oprandArr=array(),$priceArr=array())
	{
		$setEqual = true;
	...
	function getProductOnSaleComment($product=array(),$data_add="")
	{
...
//				if(strstr($data_add,"{product_old_price}"))
//					$data_add = str_replace("{product_old_price}", $productoldprice, $data_add );
	...
	function getProductNotForSaleComment($product=array(),$data_add="",$attributes=array(),$is_relatedproduct=0,$seoTemplate="")
	{
	...
	function getSpecialProductComment($product=array(),$data_add="")
	{
	...
	function getAjaxDetailboxTemplate($product=array())
	{
		if(!AJAX_CART_BOX)
				return array();
	...
	function replaceAccessoryData($product_id=0,$relproduct_id=0,$accessory=array(),$data_add,$isChilds=false,$selectAcc=array())
	{
		$user_id=0;
		$url = JURI::base();
		$redconfig = new Redconfiguration();
...
//		}
//		$accessory_template_data = $this->replaceCartTemplate($product_id, 0,0,0,$accessory_template_data,$isChilds,array(),count($attributes),$totalAccessory);
		...
		if($totalAccessory>0)
		{
			...
			if(strstr($acctemplate_data,"{if accessory_main}") && strstr($acctemplate_data,"{accessory_main end if}"))
			{
				...
				if (strstr ( $accessory_middle, "{accessory_main_short_desc}" )){
					$accessory_main_short_description = $redconfig->maxchar ( $product->product_s_desc, ACCESSORY_PRODUCT_DESC_MAX_CHARS, ACCESSORY_PRODUCT_DESC_END_SUFFIX );
					$accessory_middle = str_replace ( "{accessory_main_short_desc}", $accessory_main_short_description, $accessory_middle );
				}
				if (strstr ( $accessory_middle, "{accessory_main_title}" )){
					$accessory_main_product_name = $redconfig->maxchar ( $product->product_name, ACCESSORY_PRODUCT_TITLE_MAX_CHARS, ACCESSORY_PRODUCT_TITLE_END_SUFFIX );
					$accessory_middle = str_replace ( "{accessory_main_title}", $accessory_main_product_name, $accessory_middle );
				}
				$accessory_productdetail = "<a href='#' title='".$product->product_name."'>".JText::_('READ_MORE')."</a>";
				...
				if (is_file(JPATH_ROOT.DS."components/com_redshop/assets/images/product/".$accessory_main_image))
				{
					if(ACCESSORY_PRODUCT_IN_LIGHTBOX==1)
					$accessorymainimage = "<a id='a_main_image' href='" . $url . "components/com_redshop/assets/images/product/" . $accessory_main_image . "' title='' rel=\"lightbox[product10]\"><img id='main_image' class='redAttributeImage' src='" . $url . "components/com_redshop/helpers/thumb.php?filename=product/" . $accessory_main_image . "&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
					else
				    $accessorymainimage = "<img id='main_image' class='redAttributeImage' src='" . $url . "components/com_redshop/helpers/thumb.php?filename=product/" . $accessory_main_image . "&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' />";
				...
				if (strstr ( $accessory_middle, "{accessory_mainproduct_price}" ))
				{
					...
					if(SHOW_PRICE  && (!DEFAULT_QUOTATION_MODE || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE)))
					{
						...
						$accessory_middle 				 = str_replace ( "{accessory_mainproduct_price}", $accessory_mainproduct_price, $accessory_middle );
					}
				}
			...
			if(strstr($acctemplate_data,"{accessory_product_start}") && strstr($acctemplate_data,"{accessory_product_end}"))
			{
				...
				for($a = 0; $a < count ( $accessory ); $a ++)
				{
					...
					$accessory_product_name = $redconfig->maxchar ( $accessory [$a]->product_name, ACCESSORY_PRODUCT_TITLE_MAX_CHARS, ACCESSORY_PRODUCT_TITLE_END_SUFFIX );
					...
					$hidden_thumb_image = "<input type='hidden' name='acc_main_imgwidth' id='acc_main_imgwidth' value='".$aw_thumb."'><input type='hidden' name='acc_main_imgheight' id='acc_main_imgheight' value='".$ah_thumb."'>";
					$acc_prod_link="";
					$acc_prod_link=JRoute::_('index.php?option=com_redshop&view=product&pid='.$ac_id.'&Itemid='.$Itemid);
					$acc_noimage = "components/com_redshop/assets/images/noimage.jpg";
					$acc_noimage = $url.$acc_noimage;
						if(ACCESSORY_PRODUCT_IN_LIGHTBOX==1)
						{
							if (is_file(JPATH_ROOT.DS."components/com_redshop/assets/images/product/".$accessory_image))
							$accessoryimage = "<a id='a_main_image".$accessory [$a]->accessory_id."' href='" . $url . "/components/com_redshop/assets/images/product/" . $accessory_image . "' title='' rel=\"lightbox[product10]\"><img id='main_image".$accessory [$a]->accessory_id."' class='redAttributeImage' src='" . $url . "/components/com_redshop/helpers/thumb.php?filename=product/" . $accessory_image . "&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
							else
							$accessoryimage = "<a id='a_main_image".$accessory [$a]->accessory_id."' href='" . $acc_noimage . "' title='' rel=\"lightbox[product10]\"><img id='main_image".$accessory [$a]->accessory_id."' class='redAttributeImage' src='" . $url . "/components/com_redshop/helpers/thumb.php?filename=noimage.jpg&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
						}
						else
						{
							if (is_file(JPATH_ROOT.DS."components/com_redshop/assets/images/product/".$accessory_image))
							$accessoryimage = "<a href='".$acc_prod_link."'><img id='main_image".$accessory [$a]->accessory_id."' class='redAttributeImage' src='" . $url . "/components/com_redshop/helpers/thumb.php?filename=product/" . $accessory_image . "&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
							else
							$accessoryimage = "<a href='".$acc_prod_link."'><img id='main_image".$accessory [$a]->accessory_id."' class='redAttributeImage' src='" . $url . "/components/com_redshop/helpers/thumb.php?filename=noimage.jpg&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
					...
					$accessoryprice_withoutvat = $this->getAccessoryPrice($product_id, $accessory[$a]->newaccessory_price,$accessory[$a]->accessory_main_price, 1);
					if(!strstr($data_add,"{without_vat}"))
					{
						$accessorypricelist = $this->getAccessoryPrice($product_id, $accessory[$a]->newaccessory_price,$accessory[$a]->accessory_main_price);
					} else {
					...
					$readmorelink = JRoute::_('index.php?option=com_redshop&view=product&pid='.$ac_id.'&Itemid='.$Itemid);
					$accessory_productdetail = "<a href='".$readmorelink."' title='".$accessory [$a]->product_name."'>".JText::_('READ_MORE')."</a>";
	//				$accessory_productdetail = "<a href='#' onclick='window.open(\"".$url. "index.php?option=com_redshop&view=product&pid=".$ac_id."&tmpl=component\",\"accessoryproduct\",\"scrollbars=1\",\"location=1\")' title='".JText::_('READ_MORE')."'>".JText::_('READ_MORE')."</a>";
					...
					$accessory_div = $this->replaceAttributeData($product_id,$accessory [$a]->accessory_id,$relproduct_id,$attributes,$accessory_div,$attribute_template,$isChilds,$selectAtt);
					$accessory_div = $this->replaceProductInStock($accessory [$a]->child_product_id,$accessory_div);
					/////////////////////////////////// Accessory attribute  End /////////////////////////////////

					$accchecked = "";
					if(($isAjax==1 && in_array($accessory [$a]->accessory_id,$selectedAccessory)) || ($isAjax==0 && $accessory [$a]->setdefault_selected))
					{
						$accchecked = "checked";
					}
					$accessory_checkbox = "<input type='checkbox' name='accessory_id_".$prefix.$product_id."[]' onClick='calculateTotalPrice(\"".$product_id."\",\"".$relproduct_id."\");' totalattributs='".count($attributes)."' accessoryprice='".$accessory_price."' accessorywithoutvatprice='".$accessory_price_withoutvat."' id='accessory_id_".$commonid."' value='".$accessory [$a]->accessory_id."' ".$accchecked." />";
					$accessory_div = str_replace ( "{accessory_add_chkbox}", $accessory_checkbox, $accessory_div );
					$accessory_div = str_replace ( "{accessory_add_chkbox_lbl}", JText::_ ( 'ACCESSORY_ADD_CHKBOX_LBL' ) . '&nbsp;' . $accessory [$a]->product_name, $accessory_div );
					...
					if(strstr($accessory_div, "{accessory_quantity}"))
					{
						if(ACCESSORY_AS_PRODUCT_IN_CART_ENABLE)
						{
							...
							$accessory_div = str_replace ( "{accessory_quantity_lbl}", JText::_ ( 'QUANTITY' ) , $accessory_div );
						}
						else
						{
...
//					if(strstr($accessory_div, "{form_addtocart:") && strstr($accessory_div, "}"))
//					{
//						$accessory_div = $this->replaceCartTemplate($accessory[$a]->child_product_id, 0,$accessory[$a]->accessory_id,0,$accessory_div,false,array(),$totalatt,0,0);
//					}
				}
			...
			if(strstr($data_add, "{selected_accessory_price}") && $isAjax == 0)
			{
				...
				$data_add = str_replace ( "{selected_accessory_price}", "<div id='rs_selected_accessory_price' class='rs_selected_accessory_price'>".$selected_accessory_price."</div>", $data_add );
			}else{
	...
	function replaceAttributewithCartData($product_id=0,$accessory_id=0,$relproduct_id=0,$attributes=array(),$data_add,$attribute_template=array(),$isChilds=false)
	{
		$user_id=0;
		$url = JURI::base();
		$stockroomhelper = new rsstockroomhelper();
		...
		if(!$isChilds && count($attributes)>0)
		{
			...
			for($a=0;$a<count($attributes);$a++)
			{
				...
				$attribute_table = str_replace ( "{property_image_lbl}", JText::_('PROPERTY_IMAGE_LBL'), $attribute_table);
				$attribute_table = str_replace ( "{virtual_number_lbl}", JText::_('VIRTUAL_NUMBER_LBL'), $attribute_table);
				$attribute_table = str_replace ( "{property_name_lbl}", JText::_('PROPERTY_NAME_LBL'), $attribute_table);
				$attribute_table = str_replace ( "{property_price_lbl}", JText::_('PROPERTY_PRICE_LBL'), $attribute_table);
				$attribute_table = str_replace ( "{property_stock_lbl}", JText::_('PROPERTY_STOCK_LBL'), $attribute_table);
				$attribute_table = str_replace ( "{add_to_cart_lbl}", JText::_('ADD_TO_CART_LBL'), $attribute_table);

				$property = $this->getAttibuteProperty(0,$attributes[$a]->attribute_id);
				if($attributes[$a]->text!="" && count($property)>0 && strstr($attribute_table,"{property_start}") && strstr($attribute_table,"{property_end}"))
				{
					...
					for($i=0;$i<count($property);$i++)
					{
						...
						$display_stock = ($property_stock) ? JText::_('IN_STOCK') : JText::_('NOT_IN_STOCK');
						$property_data = str_replace ( "{property_stock}", $display_stock, $property_data);

						$property_image = "";
						if($property[$i]->property_image && is_file(JPATH_COMPONENT_SITE."/assets/images/product_attributes/".$property[$i]->property_image))
						{
							$property_image = "<img title='".urldecode($property[$i]->property_name)."' src='".$url."components/com_redshop/helpers/thumb.php?filename=product_attributes/".$property[$i]->property_image."&newxsize=".$mpw_thumb."&newysize=".$mph_thumb."&swap=".USE_IMAGE_SIZE_SWAPPING."'>";
						}
						...
						if($property[$i]->property_price > 0)
						{
							...
							if(SHOW_PRICE && (!DEFAULT_QUOTATION_MODE || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE)) && (!$attributes[$a]->hide_attribute_price))
							{
						...
						{
							$property_data = $this->replacePropertyAddtoCart($product_id,$property[$i]->value,0,$propertyid,$property_stock,$property_data,$cart_template,$data_add);
						}

						$property_data .= '<input type="hidden" id="'.$propertyid.'_oprand'.$property[$i]->value.'" value="'.$property[$i]->oprand.'" />';
						$property_data .= '<input type="hidden" id="'.$propertyid.'_proprice'.$property[$i]->value.'" value="'.$property_withvat.'" />';
						$property_data .= '<input type="hidden" id="'.$propertyid.'_proprice_withoutvat'.$property[$i]->value.'" value="'.$property_withoutvat.'" />';
						$property_data .= '<input type="hidden" id="'.$propertyid.'_stock'.$property[$i]->value.'" value="'.$property_stock.'" />';
						$property_data .= '<input type="hidden" id="'.$propertyid.'_preorderstock'.$property[$i]->value.'" value="'.$preorder_property_stock.'" />';
					}
	...
	function get_hidden_attribute_cartimage($product_id,$property_id,$subproperty_id)
	{
			$url 		= JURI::base();
			$attrbimg="";
			if($property_id>0)
	...
	function replaceAttributeData($product_id=0,$accessory_id=0,$relproduct_id=0,$attributes=array(),$data_add,$attribute_template=array(),$isChilds=false,$selectAtt=array(),$displayIndCart=1,$category_id=0)
	{
		$user_id=0;
		$url = JURI::base();
		$redTemplate = new Redtemplate ();
		...
		$data_add .= '<span id="att_lebl" style="display:none;">'.JText::_('ATTRIBUTE_IS_REQUIRED').'</span>';
		if(count($attributes) > 0)
		{
			...
			for($a=0;$a<count($attributes);$a++)
			{
				...
				if($attributes[$a]->text!="" && count($property)>0)
				{
					...
					$property_scrollerdiv .= "<td><a id=\"FirstButton\" href=\"javascript:isFlowers".$commonid.".scrollReverse();\" onmouseover=\"isFlowers".$commonid.".smoothScrollReverse();\" onmouseout=\"isFlowers".$commonid.".stopSmoothScroll();\"><img src=\"".$url."components/com_redshop/assets/images/leftarrow.jpg\" style=\"margin-top: 12px;margin-right: 6px;\" style=\"margin-top: 3px;\" border=\"0\"  /></a></td>";
					$property_scrollerdiv .= "<td><div id=\"isFlowersFrame".$commonid."\" name=\"isFlowersFrame".$commonid."\" style=\"margin: 0px; padding: 0px;position: relative; overflow: hidden;\"><div id=\"isFlowersImageRow".$commonid."\" name=\"isFlowersImageRow".$commonid."\" style=\"position: absolute; top: 0px;left: 0px;\">";
					$property_scrollerdiv .= "<script type=\"text/javascript\">var isFlowers".$commonid." = new ImageScroller(\"isFlowersFrame".$commonid."\", \"isFlowersImageRow".$commonid."\");";
					for($i=0;$i<count($property);$i++)
					{
						...
						if($property[$i]->property_image && is_file ( JPATH_COMPONENT_SITE . "/assets/images/product_attributes/".$property[$i]->property_image ))
						{
							...
							//$property_scrollerdiv .= "<td id='".$propertyid."_propimg_".$property[$i]->value."'><a onclick='setPropImage(\"".$product_id."\",\"".$propertyid."\",\"".$property[$i]->value."\");changePropertyDropdown(\"" .$product_id. "\",\"" .$accessory_id. "\",\"" .$relproduct_id. "\",\"" . $attributes [$a]->value . "\",\"".$property[$i]->value."\");'><img class='redAttributeImage' width='50' height='50' src='".$url."/components/com_redshop/assets/images/product_attributes/thumb/".$property[$i]->property_image."'></a></td>";

							$property_woscrollerdiv .= "<div class='property_image_inner' id='".$propertyid."_propimg_".$property[$i]->value."'><a onclick='setPropImage(\"".$product_id."\",\"".$propertyid."\",\"".$property[$i]->value."\");changePropertyDropdown(\"".$product_id."\",\"".$accessory_id."\",\"".$relproduct_id."\",\"".$attributes [$a]->value."\",\"".$property[$i]->value."\",\"".$mpw_thumb."\",\"".$mph_thumb."\");'><img class='redAttributeImage' width='50' height='50' src='".$url."components/com_redshop/helpers/thumb.php?filename=product_attributes/".$property[$i]->property_image."&newxsize=".$mpw_thumb."&newysize=".$mph_thumb."&swap=".USE_IMAGE_SIZE_SWAPPING."'></a></div>";
							$property_scrollerdiv .= "isFlowers".$commonid.".addThumbnail(\"".$url."components/com_redshop/helpers/thumb.php?filename=product_attributes/".$property[$i]->property_image."&newxsize=".ATTRIBUTE_SCROLLER_THUMB_WIDTH."&newysize=".ATTRIBUTE_SCROLLER_THUMB_HEIGHT."&swap=".USE_IMAGE_SIZE_SWAPPING."\",\"javascript:isFlowers".$commonid.".scrollImageCenter('".$i."');setPropImage('".$product_id."','".$propertyid."','".$property[$i]->value."');changePropertyDropdown('".$product_id."','".$accessory_id."','".$relproduct_id."','".$attributes [$a]->value."','".$property[$i]->value."','".$mpw_thumb."','".$mph_thumb."');\",\"".$property[$i]->text."\",\"\",\"".$propertyid."_propimg_".$property[$i]->value."\",\"".$borderstyle."\");";
							$imgAdded++;
						}
						...
						if($property [$i]->property_price > 0)
						{
							...
							if(SHOW_PRICE && (!DEFAULT_QUOTATION_MODE || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE)) && (!$attributes[$a]->hide_attribute_price))
							{
								$property[$i]->text = urldecode($property[$i]->property_name)." (".$property [$i]->oprand. $this->getProductFormattedPrice ( $attributes_property_vat_show ) . ")";
							} else {
						...
						$property[$i]->text = $property[$i]->text;//."=".$property_stock;

						$attribute_table .= '<input type="hidden" id="'.$propertyid.'_oprand'.$property [$i]->value.'" value="'.$property [$i]->oprand.'" />';
						$attribute_table .= '<input type="hidden" id="'.$propertyid.'_proprice'.$property [$i]->value.'" value="'.$attributes_property_vat_show.'" applyvat="'.$chktag.'" />';
						$attribute_table .= '<input type="hidden" id="'.$propertyid.'_proprice_withoutvat'.$property [$i]->value.'" value="'.$attributes_property_withoutvat.'" />';
						$attribute_table .= '<input type="hidden" id="'.$propertyid.'_stock'.$property [$i]->value.'" value="'.$property_stock.'" />';
						$attribute_table .= '<input type="hidden" id="'.$propertyid.'_preorderstock'.$property [$i]->value.'" value="'.$preorder_property_stock.'" />';
					}
					if(!$mph_thumb)
					{
						$mph_thumb = 50;
					...
					$property_scrollerdiv .= "<td><a id=\"FirstButton\" href=\"javascript:isFlowers".$commonid.".scrollForward();\" onmouseover=\"isFlowers".$commonid.".smoothScrollForward();\" onmouseout=\"isFlowers".$commonid.".stopSmoothScroll();\"><img src=\"".$url."components/com_redshop/assets/images/rightarrow.jpg\" style=\"margin-top: 12px;margin-right: 6px;\" style=\"margin-top: 3px;\" border=\"0\" /></a></td>";
					...
					$tmp_array = array ();
					$tmp_array [0]->value = 0;
					$tmp_array [0]->text = JText::_('SELECT')." ".urldecode($attributes[$a]->text);
					...
					if($display_type=='checkbox' || $display_type=='radio')
					{
						for($chk=0;$chk<count($property);$chk++)
						{
							...
							$chklist .= "<div class='attribute_multiselect_single'><input type='".$display_type."' ".$checked." value='".$property[$chk]->value."' name='".$propertyid."[]' id='".$propertyid."' class='inputbox' attribute_name='".urldecode($attributes [$a]->attribute_name)."' required='".$attributes[$a]->attribute_required."' onClick='javascript:".$scrollerFunction."changePropertyDropdown(\"".$product_id."\",\"".$accessory_id."\",\"".$relproduct_id."\",\"".$attributes [$a]->value."\",\"".$property[$chk]->value."\",\"".$mpw_thumb."\",\"".$mph_thumb."\");'  />&nbsp;".$property[$chk]->text."</div>";
						}
					} else {
						$scrollerFunction = "";
						if($imgAdded>0 && strstr($attribute_table,"{property_image_scroller}"))
	...
	function replaceSubPropertyData($product_id=0,$accessory_id=0,$relatedprd_id=0,$attribute_id=0,$property_id=0,$subatthtml="",$layout="",$selectSubproperty=array())
	{
		$redTemplate = new Redtemplate();
		$stockroomhelper = new rsstockroomhelper();
		$url = JURI::base();
...
//		$chktag = $this->chktag;//$this->getApplyattributeVatOrNot($data_add);
		...
		if($subatthtml!="")
		{
			if (count($subproperty)>0)
			{
				...
				$attribute_table .= '<span id="subprop_lbl" style="display:none;">'.JText::_('SUBATTRIBUTE_IS_REQUIRED').'</span>';
			    ...
			    if (strstr ( $subatthtml, "{subproperty_image_without_scroller}" ))
				{
					...
					//$subproperty_woscrollerdiv .= "<table cellpadding='0' cellspacing='0' border='0'>";
				}
				...
				$subproperty_scrollerdiv .= "<td><div id=\"isFlowersFrame".$commonid."\" name=\"isFlowersFrame".$commonid."\" style=\"margin: 0px; padding: 0px;position: relative; overflow: hidden;\"><div id=\"isFlowersImageRow".$commonid."\" name=\"isFlowersImageRow".$commonid."\" style=\"position: absolute; top: 0px;\">";
				...
				for($i = 0; $i < count ( $subproperty ); $i ++)
				{
					...
					if($subproperty[$i]->subattribute_color_image && is_file ( JPATH_COMPONENT_SITE . "/assets/images/subcolor/".$subproperty[$i]->subattribute_color_image ))
					{
						$borderstyle = ($selectedsubproperty==$subproperty[$i]->value) ? " 1px solid " : "";
						$subprop_Arry[] = $url."components/com_redshop/helpers/thumb.php?filename=subcolor/".$subproperty[$i]->subattribute_color_image."&newxsize=".ATTRIBUTE_SCROLLER_THUMB_WIDTH."&newysize=".ATTRIBUTE_SCROLLER_THUMB_HEIGHT."&swap=".USE_IMAGE_SIZE_SWAPPING."`_`".$subproperty[$i]->value;
						$subproperty_woscrollerdiv .= "<div id='".$subpropertyid."_subpropimg_".$subproperty[$i]->value."' class='subproperty_image_inner'><a onclick='setSubpropImage(\"".$product_id."\",\"".$subpropertyid."\",\"".$subproperty[$i]->value."\");calculateTotalPrice(\"".$product_id."\",\"".$relatedprd_id."\");displayAdditionalImage(\"".$product_id."\",\"".$accessory_id."\",\"".$relatedprd_id."\",\"".$property_id."\",\"".$subproperty[$i]->value."\");'><img class='redAttributeImage'  src='".$url."/components/com_redshop/helpers/thumb.php?filename=subcolor/".$subproperty[$i]->subattribute_color_image."&newxsize=".ATTRIBUTE_SCROLLER_THUMB_WIDTH."&newysize=".ATTRIBUTE_SCROLLER_THUMB_HEIGHT."&swap=".USE_IMAGE_SIZE_SWAPPING."'></a></div>";
						$subproperty_scrollerdiv .= "isFlowers".$commonid.".addThumbnail(\"".$url."components/com_redshop/helpers/thumb.php?filename=subcolor/".$subproperty[$i]->subattribute_color_image."&newxsize=".$mpw_thumb."&newysize=".$mph_thumb."&swap=".USE_IMAGE_SIZE_SWAPPING."\",\"javascript:isFlowers".$commonid.".scrollImageCenter('".$i."');setSubpropImage('".$product_id."','".$subpropertyid."','".$subproperty[$i]->value."');calculateTotalPrice('".$product_id."','".$relatedprd_id."');displayAdditionalImage('".$product_id."','".$accessory_id."','".$relatedprd_id."','".$property_id."','".$subproperty[$i]->value."');\",\"".$subproperty[$i]->text."\",\"\",\"".$subpropertyid."_subpropimg_".$subproperty[$i]->value."\",\"".$borderstyle."\");";
...
//				$subproperty_scrollerdiv .= "</tr></table></marquee>";
				$tmp_array = array ();
				$tmp_array[0]->value = 0;
				$tmp_array[0]->text = JText::_('SELECT')." ".$displayPropertyName;
...
//				}
	...
	function defaultAttributeDataPrice($product_id=0,$product_showprice=0,$data_add,$user_id=0,$applyTax=0,$attributes=array())
	{
					...
					for($sp=0;$sp<count($subproperty);$sp++)
					{
						if($subproperty[$sp]->setdefault_selected)
						{
							if ($subproperty[$sp]->subattribute_color_price > 0)
							{
								...
								if($applyTax)
								{
									$attributes_subproperty_vat = $this->getProducttax ( $product_id, $subproperty[$sp]->subattribute_color_price,$user_id );
								}
	...
	function replacePropertyAddtoCart($product_id=0,$property_id=0,$category_id=0,$commonid="",$property_stock=0,$property_data="",$cart_template=array(),$data_add="")
	{
		$user_id=0;
		$url = JURI::base();
		$stockroomhelper = new rsstockroomhelper();
...
//		}
		if($property_stock<=0)
		{
			$property_data = str_replace ( "{form_addtocart:$cart_template->template_name}", JText::_ ( 'PRODUCT_OUTOFSTOCK_MESSAGE' ), $property_data );
		...
		$product_price_incl_vat= $productArr['product_price_incl_vat']*$qunselect;
		...
		$cartform .= "
			...
			<input type='hidden' name='requiedaccAttribute' id='requiedaccAttribute' value=''>
			<input type='hidden' name='requiedaccProperty' id='requiedaccProperty' value=''>
			...
			<input type='hidden' name='product_price_incl_vat' id='product_price_incl_vat".$product_id."' value='".$product_price_incl_vat."' />
			<input type='hidden' name='productprice_inclvat' id='productprice_inclvat' value='0'>

			<input type='hidden' name='min_quantity' id='min_quantity' value='".$min_quantity."' requiredtext='".JText::_('MINIMUM_QUANTITY_SHOULD_BE')."'>
			<input type='hidden' name='max_quantity' id='max_quantity' value='".$max_quantity."' requiredtext='".JText::_('MAXIMUM_QUANTITY_SHOULD_BE')."'>
			...
			<input type='hidden' name='hidden_attribute_cartimage' id='hidden_attribute_cartimage".$product_id."' value='' />";
		...
		if (strstr ( $cartform, "{addtocart_quantity}" ))
		{
			...
			$cartform = str_replace ( "{quantity_lbl}", JText::_('QUANTITY_LBL'), $cartform );
		}
		elseif(strstr ( $cartform, "{addtocart_quantity_selectbox}" ))
		{
			...
			if((DEFAULT_QUANTITY_SELECTBOX_VALUE!=""  && $product->quantity_selectbox_value=='') || $product->quantity_selectbox_value!='')
			{
				...
				$quaboxarr = array_merge(array(),array_unique($quaboxarr));
				sort($quaboxarr);
				$qselect = "<select name='quantity' id='quantity".$product_id."'  OnChange='calculateTotalPrice(".$product_id.",0);'>";
				for($q=0;$q<count($quaboxarr);$q++)
				{
					if(intVal($quaboxarr[$q]) && intVal($quaboxarr[$q])!=0)
					{
			...
			$cartform = str_replace ( "{quantity_lbl}", '', $cartform );
		}

		$tooltip = (DEFAULT_QUOTATION_MODE) ? JText::_ ( 'REQUEST_A_QUOTE_TOOLTIP' ) : JText::_ ( 'ADD_TO_CART_TOOLTIP' );
		$ADD_OR_LBL = (DEFAULT_QUOTATION_MODE) ? JText::_ ( 'REQUEST_A_QUOTE' ) : JText::_ ( 'ADD_TO_CART' );
		...
		$onclick = 'onclick="if(displayAddtocartProperty(\''.$addtocartFormName.'\',\''.$product_id.'\',\''.$attribute_id.'\',\''.$property_id.'\')){checkAddtocartValidation(\''.$addtocartFormName.'\',\''.$product_id.'\',0,0,\'\',0,0,0);}" ';
		$class='';
		$title='';
		...
		if (strstr ( $cartform, "{addtocart_button}" ))
		{
			...
			$cartIcon = '<span id="pdaddtocart'.$stockId.'" '.$class.' '.$title.'"><input type="button" '.$onclick.$cartTitle.' name="addtocart_button" value="'.$ADD_OR_LBL.'" /></span>';
		}
		if (strstr ( $cartform, "{addtocart_link}" ))
		{
			$cartTag = "{addtocart_link}";
			$cartIcon = '<span '.$class.' '.$title.' id="pdaddtocart'.$stockId.'" '.$onclick.$cartTitle.' style="cursor: pointer;">'.$ADD_OR_LBL.'</span>';
		}
		if (strstr ( $cartform, "{addtocart_image_aslink}" ))
		{
			$cartTag = "{addtocart_image_aslink}";
			$cartIcon = '<span '.$class.' '.$title.' id="pdaddtocart'.$stockId.'"><img '.$onclick.$cartTitle.' alt="'.$ADD_OR_LBL.'" style="cursor: pointer;" src="'.$url.'components/'.$option.'/assets/images/'.$ADD_CART_IMAGE.'" /></span>';
		}
		if (strstr ( $cartform, "{addtocart_image}" ))
		{
			$cartTag = "{addtocart_image}";
			$cartIcon = '<span id="pdaddtocart'.$stockId.'" '.$class.' '.$title.'><div '.$onclick.$cartTitle.' align="center" style="cursor:pointer;background:url('.$url.'components/'.$option.'/assets/images/'.$ADD_CART_BACKGROUND.');background-position:bottom;background-repeat:no-repeat;">'.$ADD_OR_LBL.'</div></span>';
		}
	...
	function replaceCartTemplate($product_id=0, $category_id=0, $accessory_id=0, $relproduct_id=0, $data_add="", $isChilds=false, $userfieldArr=array(), $totalatt=0, $totalAccessory=0, $count_no_user_field=0, $module_id=0, $giftcard_id=0)
	{
		$user_id=0;
		$url = JURI::root();
		$redconfig = new Redconfiguration();
...
//			return $data_add;
		...
		else
		{
			...
			if ($isChilds) {
				$data_add = str_replace ( "{form_addtocart:$cart_template->template_name}", "", $data_add );
				return $data_add;
			}else if ($this->isProductDateRange($userfieldArr,$product_id)) {	# new type custome field - Selection based on selected conditions
				$data_add = str_replace ( "{form_addtocart:$cart_template->template_name}", JText::_('PRODUCT_DATE_FIELD_EXPIRED'), $data_add );
				return $data_add;
		...
		$display_text = JText::_ ( 'PRODUCT_OUTOFSTOCK_MESSAGE' );
		if (!$isStockExists)
		{
			...
			// get preorder stock for Product
			 ...
			 $isPreorderStockExists = $stockroomhelper->isPreorderStockExists($product_id);
				...
				if (!$isPreorderStockExists)
				{
					...
					$stockdisplay = true;
			    	$add_cart_flag = true;
				    $display_text = JText::_ ( 'PREORDER_PRODUCT_OUTOFSTOCK_MESSAGE' );
...
//				$data_add = str_replace ( "{form_addtocart:$cart_template->template_name}", JText::_ ( 'PRODUCT_OUTOFSTOCK_MESSAGE' ), $data_add );
			}
		} else {
		    $cartdisplay=true;
			$add_cart_flag = true;
		...
		if($totalatt>0)
		{
			...
			{
				$totrequiredatt .= JText::_('ATTRIBUTE_IS_REQUIRED')." ".urldecode($requiredattribute[$i]->attribute_name)."\n";
			}
			$requiredproperty = $this->getAttibuteProperty(0,0,$product_id,0,1);
			for($y=0;$y<count($requiredproperty);$y++)
			{
				$totrequiredprop .= JText::_('SUBATTRIBUTE_IS_REQUIRED')." ".urldecode($requiredproperty[$y]->property_name)."\n";
			}
		}
		...
		if ($add_cart_flag)
		{
			...
			//$cartstyle = ($stockdisplay) ? 'style="display:none" ': '';
			//$stockstyle = ($stockdisplay) ? '': 'style="display:none" ';
			$cartTag = '';
			$cartIcon = '';
			$cartTitle = ' title="'.$ADD_OR_TOOLTIP.'" ';

			if($giftcard_id)
				$onclick = ' onclick="if(validateEmail()){if(displayAddtocartForm(\''.$addtocartFormName.'\',\''.$product_id.'\',\''.$relproduct_id.'\',\''.$giftcard_id.'\', \'user_fields_form\')){checkAddtocartValidation(\''.$addtocartFormName.'\',\''.$product_id.'\',\''.$relproduct_id.'\',\''.$giftcard_id.'\', \'user_fields_form\',\''.$totalatt.'\',\''.$totalAccessory.'\',\''.$count_no_user_field.'\');}}" ';
			else{
				$onclick = ' onclick="if(displayAddtocartForm(\''.$addtocartFormName.'\',\''.$product_id.'\',\''.$relproduct_id.'\',\''.$giftcard_id.'\', \'user_fields_form\')){checkAddtocartValidation(\''.$addtocartFormName.'\',\''.$product_id.'\',\''.$relproduct_id.'\',\''.$giftcard_id.'\', \'user_fields_form\',\''.$totalatt.'\',\''.$totalAccessory.'\',\''.$count_no_user_field.'\');}" ';
			}
			$class='class=""';
			$title='title=""';
			if (strstr ( $cartform, "{addtocart_tooltip}" ))
			...
			{
				$cartTag = "{addtocart_image_aslink}";
				if(is_file(JPATH_SITE."/components/com_redshop/assets/images/".$ADD_CART_IMAGE))
				$cartIcon = '<span '.$class.' '.$title.' '.$cartstyle.' id="pdaddtocart'.$stockId.'" class="'.$class.'" title="'.$title.'"><img '.$onclick.$cartTitle.' alt="'.$ADD_OR_LBL.'" style="cursor: pointer;" src="'.$url.'components/'.$option.'/assets/images/'.$ADD_CART_IMAGE.'" /></span>';
				else
				$cartIcon = '<span '.$class.' '.$title.' '.$cartstyle.' id="pdaddtocart'.$stockId.'" '.$onclick.$cartTitle.' style="cursor: pointer;">'.$ADD_OR_LBL.'</span>';
			}
			if (strstr ( $cartform, "{addtocart_image}" ))
			{
				$cartTag = "{addtocart_image}";
				if(is_file(JPATH_SITE."/components/com_redshop/assets/images/".$ADD_CART_BACKGROUND))
				$cartIcon = '<span '.$class.' '.$title.' '.$cartstyle.' id="pdaddtocart'.$stockId.'"><div '.$onclick.$cartTitle.' align="center" style="cursor:pointer;background:url('.$url.'components/'.$option.'/assets/images/'.$ADD_CART_BACKGROUND.');background-position:bottom;background-repeat:no-repeat;">'.$ADD_OR_LBL.'</div></span>';
				else
				$cartIcon = '<span '.$class.' '.$title.' '.$cartstyle.' id="pdaddtocart'.$stockId.'" '.$onclick.$cartTitle.' style="cursor: pointer;">'.$ADD_OR_LBL.'</span>';
			}
			// pre-Order
			if(is_file(JPATH_SITE."/components/com_redshop/assets/images/".$ADD_OR_PRE_BTN))
			$cartIconPreorder = '<span id="preordercart'.$stockId.'" '.$preorderstyle.'><img '.$onclick.$cartTitle.' alt="'.$ADD_OR_PRE_LBL.'" style="cursor: pointer;" src="'.$url.'components/'.$option.'/assets/images/'.$ADD_OR_PRE_BTN.'" /></span>';
           	else
			$cartIconPreorder = '<span id="preordercart'.$stockId.'" '.$preorderstyle.'><a href="javascript:;" '.$onclick.'>'.JTEXT::_('PREORDER_BTN').'</a></span>';
	...
	function replaceWishlistButton($product_id=0,$data_add="")
	{
		...
		if (MY_WISHLIST != 0)
		{
			...
			{	//allow user to send wishlist while user is not logged in
				$mywishlist_link = JURI::root().'index2.php?option=com_redshop&view=wishlist&task=addtowishlist&tmpl=component&product_id='.$product_id;
				if($wishtag == '{wishlist_button}')
				{
					$wishlist = "<input type='button' value='" . JText::_ ( "ADD_TO_WISHLIST" ) . "'>";
				}
				if($wishtag == '{wishlist_link}')
				{
					$wishlist = JText::_ ( "ADD_TO_WISHLIST" );
				}
				$my_wishlist = "<a class=\"modal\" href=\"" . $mywishlist_link . "\" rel=\"{handler:'iframe',size:{x:450,y:350}}\"  >".$wishlist."</a>";
			}
			else
			{
				...
				if($wishtag == '{wishlist_button}')
				{
					$wishlist = "<input type='submit' name='btnwishlist' id='btnwishlist' value='" . JText::_ ( "ADD_TO_WISHLIST" ) . "'>";
				}
				if($wishtag == '{wishlist_link}')
				{
					...
					$wishlist = "<a href='javascript:document.form_wishlist_".$product_id.".submit();' class='wishlistlink'>".JText::_( "ADD_TO_WISHLIST" ) . "</a>";
				...
				if(WISHLIST_LOGIN_REQUIRED!=0)
				{
					...
					$my_wishlist = "<form method='post' action='' id='form_wishlist_".$product_id."' name='form_wishlist_".$product_id."'>
								<input type='hidden' name='task' value='addtowishlist' />
							    <input type='hidden' name='product_id' value='" . $product_id . "' />
								<input type='hidden' name='view' value='product' />
								<input type='hidden' name='rurl' value='".base64_encode($u->toString())."' />".$wishlist."</form>";
				}
			}
		}
	...
	function replaceCompareProductsButton($product_id=0,$category_id=0,$data_add="",$is_relatedproduct=0)
	{
		$Itemid = JRequest::getInt( 'Itemid' );
		...
		if (PRODUCT_COMPARISON_TYPE != "")
		{
			if(strstr($data_add,'{'.$prefix.'compare_product_div}'))
			{
				...
				$compare_product_div .= "<a href='javascript:compare();' >".JText::_('COMPARE')."</a><br />";
			...
			if(strstr($data_add,'{'.$prefix.'compare_products_button}'))
			{
				...
				$compare_product = "<input type='checkbox' id='chk".$category_id.$product_id."' ".$chked." onClick='add_to_compare(".$product_id.",".$category_id.")'>".JText::_ ( "ADD_TO_COMPARE" );
	...
	function makeAccessoryCart($attArr=array(),$product_id=0,$user_id=0,$data='')
	{
		$user = JFactory::getUser();
		if($user_id==0)
		{
...
//		$data 					= $this->getcartTemplate();
		...
		if(count($attArr)>0)
		{
			$displayaccessory .= "<div class='checkout_accessory_static'>".JText::_("ACCESSORY")."</div>";
			for($i=0;$i<count($attArr);$i++)
			{
				...
				$displayaccessory .= "<div class='checkout_accessory_title'>".urldecode($attArr[$i]['accessory_name']).$displayPrice."</div>";
				...
				for($j=0;$j<count($attchildArr);$j++)
				{
					...
					if(count($propArr)>0)
					{
						$displayaccessory .= "<div class='checkout_attribute_title'>".urldecode($attchildArr[$j]['attribute_name']).":</div>";
					}
...
//						}
						...
						$displayaccessory .= "<div class='checkout_attribute_wrapper'><div class='checkout_attribute_price'>".urldecode($propArr[$k]['property_name']).$displayPrice."</div>".$virtualNumber."</div>";
						...
						for($l=0;$l<count($subpropArr);$l++)
						{
							...
							$displayPrice = " (".$subpropArr[$l]['subproperty_oprand']." ".$this->getProductFormattedPrice($subproperty_price).")";
...
//						$subElementArr = $propArr[$t]['property_childs'];
						...
						{
							$accessory_priceArr	= $this->makeTotalPriceByOprand($accessory_price,$subprooprand[$t],$subprovatprice[$t]);
							$accessory_vatArr	= $this->makeTotalPriceByOprand($accessory_vat_price,$subprooprand[$t],$subprovat[$t]);
	...
	function makeAttributeCart($attArr=array(),$product_id=0,$user_id=0,$new_product_price=0,$quantity=1,$data='')
	{
		...
		for($i=0;$i<count($attArr);$i++)
		{
			...
			if(count($propArr)>0)
			{
				$displayattribute .= "<div class='checkout_attribute_title'>".urldecode($attArr[$i]['attribute_name']).":</div>";
			}
			for($k=0;$k<count($propArr);$k++)
			{
				$att_vat = 0;
				...
				$displayattribute .= "<div class='checkout_attribute_wrapper'><div class='checkout_attribute_price'>".urldecode($propArr[$k]['property_name']).$displayPrice."</div>".$virtualNumber."</div>";
				$prooprand[$k] = $propArr[$k]['property_oprand'];
				$proprice[$k] = $propArr[$k]['property_price'];
				$provatprice[$k] = $property_price;
				$provat[$k] = $att_vat;
				...
				{
					$displayattribute .= "<div class='checkout_subattribute_title'>".urldecode($subpropArr[0]['subattribute_color_title'])."</div>";
				}
				for($l=0;$l<count($subpropArr);$l++)
				{
					if($l==0)
					...
					$isStock = $stockroomhelper->isStockExists($subpropArr[$l]['subproperty_id'],"subproperty");
					$isPreorderStock = $stockroomhelper->isPreorderStockExists($subpropArr[$l]['subproperty_id'],"subproperty");
					if(!strstr($data,'{product_attribute_price}'))
					{
						$displayPrice = '';
					}
...
//				$subElementArr = $propArr[$t]['property_childs'];
				...
				if($setPropEqual && $setSubpropEqual && isset($subprovatprice[$t]))
				{
					$accessory_priceArr=$this->makeTotalPriceByOprand($product_price,$subprooprand[$t],$subprovatprice[$t]);
					$accessory_vatArr		= $this->makeTotalPriceByOprand($product_vat_price,$subprooprand[$t],$subprovat[$t]);
					$accessory_oldpriceArr=$this->makeTotalPriceByOprand($productOldprice,$subprooprand[$t],$subproprice[$t]);
					//$setSubpropEqual = $accessory_priceArr[0];
					$product_price = $accessory_priceArr[1];
					$product_vat_price	= $accessory_vatArr[1];
					$productOldprice = $accessory_oldpriceArr[1];
		...
		if($displayattribute!="")
		{
			$displayattribute = "<div class='checkout_attribute_static'>".JText::_("ATTRIBUTE")."</div>".$displayattribute;
		}
		...
		return array($displayattribute,$product_price,$product_vat_price,$selectedAttributs,$isStock,$productOldprice,$product_vat_Oldprice,$isPreorderStock,$selectedProperty);
	}

	function makeAccessoryOrder($order_item_id=0)
	{
		$order_functions = new order_functions();
		$displayaccessory = "";
		...
		if(count($orderItemdata)>0)
		{
			...
			{
				$accessory_quantity = " [".JText::_('ACCESSORY_QUANTITY_LBL')." ".$orderItemdata[$i]->product_quantity."] ";
	...
	function makeAttributeOrder($order_item_id=0,$is_accessory=0,$parent_section_id=0,$stock=0,$export=0,$data='')
	{
		$stockroomhelper 	= new rsstockroomhelper();
		$order_functions 	= new order_functions();
		$displayattribute 	= "";
	...
	function makeAccessoryQuotation($quotation_item_id=0,$quotation_status=2)
	{
	...
	function makeAttributeQuotation($quotation_item_id=0,$is_accessory=0,$parent_section_id=0,$quotation_status=2,$stock=0)
	{
//		$stockroomhelper = new rsstockroomhelper();
		$quotationHelper = new quotationHelper();
		$displayattribute = "";
...
//			$stockroom_id = $Itemdata[0]->stockroom_id;
		}
...
//					if($stock==1)
//					{
//						$stockroomhelper->manageStockAmount($propdata[$p]->section_id,$quntity,$stockroom_id,"property");
//					}
					$displayattribute .= "<div class='checkout_attribute_price'>".urldecode($propdata[$p]->section_name)." ";
					if($quotation_status!=1 || ($quotation_status==1 && SHOW_QUOTATION_PRICE ==1))
					{
						$displayattribute .= "(".$propdata[$p]->section_oprand.$this->getProductFormattedPrice($propdata[$p]->section_price+$propdata[$p]->section_vat).")";
					}
...
//						if($stock==1)
//						{
//							$stockroomhelper->manageStockAmount($subpropdata[$sp]->section_id,$quntity,$stockroom_id,"subproperty");
//						}
						$displayattribute .= "<div class='checkout_subattribute_price'>".urldecode($subpropdata[$sp]->section_name)." ";
						if($quotation_status!=1 || ($quotation_status==1 && SHOW_QUOTATION_PRICE ==1))
						{
							$displayattribute .= "(".$subpropdata[$sp]->section_oprand.$this->getProductFormattedPrice($subpropdata[$sp]->section_price+$subpropdata[$sp]->section_vat).")";
						}
						$displayattribute .= "</div>";
					}
				}
	...
	function loadAclProducts()
	{
		...
		if ($user->id >0)
			$catquery = "SELECT sg.shopper_group_categories FROM `".$this->_table_prefix."shopper_group` as sg LEFT JOIN ".$this->_table_prefix."users_info as uf ON sg.`shopper_group_id` = uf.shopper_group_id WHERE uf.user_id = '".$user->id."' AND sg.shopper_group_portal=1 ";
		else
			$catquery = "SELECT sg.shopper_group_categories FROM `".$this->_table_prefix."shopper_group` as sg WHERE  sg.`shopper_group_id` = '".$shopperGroupId."' AND sg.shopper_group_portal=1";


		$query = "SELECT product_id
						FROM `".$this->_table_prefix."product_category_xref` WHERE category_id IN (".$catquery.")";
	...
	function getUnitConversation($globalUnit,$calcUnit){
		/*
	...
	function insertProductDownload($product_id,$user_id,$order_id,$media_name,$serial_number)
	{
		// download data
	 ...
	 */
	 ...
	 function getProdcutSerialNumber($product_id,$is_used=0)
	 {
	 	$query = "SELECT * FROM ".$this->_table_prefix."product_serial_number "
	  	...
	  	if(count($rs)>0)
	  		$this->updateProdcutSerialNumber($rs->serial_id);	// update serial number...
	  	else
	  		$rs->serial_number= "";
	 ...
	 */
	function updateProdcutSerialNumber($serial_id)
	{
	...
	function getSubscription($product_id=0)
	{
	...
	function getQuestionAnswer($questionid=0,$productid=0,$faq=0,$front=0)
	{
		$and = "";
		if($questionid!=0)
		{
	...
	function getProductRating($product_id)
	{
		...
		if ($avgratings > 0) {
			$final_avgreview_data = '<img src="'.$url.'administrator/components/com_redshop/assets/images/star_rating/'.$avgratings.'.gif" />';
			$final_avgreview_data .= JText::_( 'AVG_RATINGS_1' )." ".$totalreviews." ".JText::_ ( 'AVG_RATINGS_2' );
		}
		return $final_avgreview_data;
	}

	function getProductReviewList($product_id)
	{
	...
	function calOprandPrice($price_1,$oprand,$price_2)
	{
		switch ($oprand)
		{
			case "+" :
	...
	function checkcompareproduct($product_id)
	{
	...
	function makeCompareProductDiv()
	{
		$Itemid = JRequest::getVar('Itemid');
		...
		for($i=0;$i<$idx;$i++)
		{
			...
			$product_link = JRoute::_( 'index.php?option=com_redshop&view=product&pid='.$product_id.'&Itemid='.$Itemid);

			$div .= "<li>".$product->product_name." <a onClick='javascript:add_to_compare(".$product_id.",".$category_id.",\"remove\")' href='javascript:void(0)'>".JText::_('DELETE')."</a></li>";
			$moddiv .='<tr valign="top"><td width="95%"><span><a href="'.$product_link.'">'.$product->product_name.'</a></span></td>';
			$moddiv .='<td width="5%"><span><a href="javascript:void(0);" onClick="javascript:remove_compare('.$product_id.','.$category_id.')">'.JText::_('DELETE').'</a></span></td></tr>';
		}
		$moddiv .= "</table>";
	 ...
	 *
	 */
	function product_tag($template_id,$section,$template_data)
	{
	...
	function getJcommentEditor($product=array(),$data_add="")
	{
		global $mainframe;
		$product_reviews="";
		$product_id = $product->product_id;
	...
	function getSelectedAccessoryArray($data=array())
	{
		$selectedAccessory = array();
		$selectedAccessoryQua = array();
		$selectedproperty = array();
	...
	function getSelectedAttributeArray($data=array())
	{
		$selectedproperty = array();
		$selectedsubproperty = array();
	...
	function replaceProductInStock($product_id=0,$data_add,$attributes=array(),$attribute_template=array())
	{
		if(count($attribute_template)<=0)
		{
			$attributes = array();
		...
		{
            $data_add = str_replace ( "{products_in_stock}", JText::_('PRODUCT_IN_STOCK_LBL').' <span id="displayProductInStock'.$product_id.'">'.$productinstock.'</span>', $data_add);
		}
		if(strstr($data_add,"{product_stock_amount_image}"))
		{
			...
			if(count($stockamountList)>0)
			{
				...
				$stockamountImage .= '<img src="'.JURI::base().'components/com_redshop/helpers/thumb.php?filename=stockroom'.DS.$stockamountList[0]->stock_amount_image.'&newxsize='.DEFAULT_STOCKAMOUNT_THUMB_WIDTH.'&newysize='.DEFAULT_STOCKAMOUNT_THUMB_HEIGHT.'&swap='.USE_IMAGE_SIZE_SWAPPING.'" alt="'.$stockamountList[0]->stock_amount_image_tooltip.'" id="stockImage'.$product_id.'" /></a>';
			}
		...
		if(strstr($data_add,"{stock_status"))
		{
			...
			{
				$isStockExists = $productinstock;
				$isPreorderStockExists = $productinpreorderstock;
		   ...
		   // for product stock
			...
			if(!$isStockExists)
			{
					if(($product_preorder == "global" && ALLOW_PRE_ORDER) || ($product_preorder == "yes") || ($product_preorder == "" && ALLOW_PRE_ORDER))
					{
						 if (!$isPreorderStockExists)
						 {
								$stock_status = "<span id='stock_status_div".$product_id."'><div id='".$out_stock_class."' class='".$out_stock_class."'>".JText::_('OUT_OF_STOCK')."</div></span>";
						 } else {
						 		$stock_status = "<span id='stock_status_div".$product_id."'><div id='".$pre_order_class."' class='".$pre_order_class."'>".JText::_('PRE_ORDER')."</div></span>";
						 }
					}else {
					...
					$stock_status = "<span id='stock_status_div".$product_id."'><div id='".$avail_class."' class='".$avail_class."'>".JText::_('AVAILABLE_STOCK')."</div></span>";
			}

			// End
			/*if($productinstock == 0)
			{
		    	if(($product_detail->preorder == "global" && ALLOW_PRE_ORDER) ||($product_detail->preorder == "" && ALLOW_PRE_ORDER))
				{

					$stock_status = "<span id='stock_status_div".$product_id."'><div id='".$pre_order_class."' class='".$pre_order_class."'>".JText::_('PRE_ORDER')."</div></span>";

				}
				else if($product_detail->preorder == "yes")
				{
                     $stock_status = "<span id='stock_status_div".$product_id."'><div id='".$pre_order_class."' class='".$pre_order_class."'>".JText::_('PRE_ORDER')."</div></span>";
			...
			$data_add = str_replace ( $realstocktag , $stock_status, $data_add);    // end for stock status
		}
		return $data_add;
	}
	...
	function getChildProduct($product_id=0)
	{
		$query = "SELECT product_parent_id,product_id,product_name,product_number FROM ".$this->_table_prefix."product "
	 ...
	 * @return: int
	 */
	function getMainParentProduct($parent_id)
	{
	...
	function redpriceDecimal($price)
	{
		return number_format($price,PRICE_DECIMAL,'.','');
	}

	function redunitDecimal($price)
	{
	...
	function isProductDateRange($userfieldArr,$product_id)
	{
		$isEnable = true;
		if (count($userfieldArr)<=0)
		{
		...
		if(is_array($userfieldArr))
		{
			if(in_array($field_name,$userfieldArr))
			{
				...
				if(count($datedata)>0)
				{
					...
					$mainsplit_date_total=explode(" ",$data_txt);
					$mainsplit_date=explode(":",$mainsplit_date_total[0]);
					...
					/*echo "Start date: ".date('Y-m-d H:i:s',$dateStart)."  Start Time : ".$mainsplit_date[0]."<br />";
					echo "Today date : ".date('Y-m-d H:i:s',mktime(23,59,59,date('m'),date('d'),date('Y')))." Time :".time()."<br />";
					echo "End date: ".date('Y-m-d H:i:s',$dateEnd)."  End Time : ".$mainsplit_date[1]."<br />";*/

					if($dateStart<= $todayStart  && $dateEnd>=$todayEnd)
					{
						$isEnable = false;	# show add to cart button
					}
				} else {
						$isEnable = false;	# show add to cart button
				}
	...
	function getassociatetag($product_id=0)
	{
	...
	function getCategoryCompareTemplate($cid)
	{
	...
	function getProductCaterories($product_id,$displaylink=0)
	{
	...
	function getdisplaymainImage($product_id=0,  $property_id=0, $subproperty_id=0, $pw_thumb=0, $ph_thumb=0, $redview="")
	{
		...
		if($subproperty_id>0)
		{
			...
			if (count($subproperty)>0 && is_file ( JPATH_COMPONENT_SITE."/assets/images/subproperty/".$subproperty[0]->subattribute_color_main_image ))
			{
				$type = 'subproperty';
				$imagename			= $subproperty[0]->subattribute_color_main_image;
				$aTitleImageResponse = $subproperty[0]->text;
				//Subproperty image in cart
				//$attrbimg=$url."components/com_redshop/assets/images/subproperty/".$subproperty[0]->subattribute_color_main_image ;
			}
			//Subproperty image in cart
			if (count($subproperty)>0 && is_file (JPATH_COMPONENT_SITE."/assets/images/subcolor/".$subproperty[0]->subattribute_color_image ))
			{
		...
		if(!empty($imagename) && !empty($type))
		{
			if((WATERMARK_PRODUCT_THUMB_IMAGE) && $type=='product')
			{
					...
					//$aHrefImageResponse = $redhelper->watermark('product',$imagename,'','',WATERMARK_PRODUCT_THUMB_IMAGE,'0');
			}
			else
			{
			...
			$mainImageResponse = "<img id='main_image".$product_id."' src='".$productmainimg."' alt='".$product->product_name."' title='".$product->product_name."'>";
	...
	function displayAdditionalImage($product_id=0, $accessory_id=0, $relatedprd_id=0, $property_id=0, $subproperty_id=0, $main_imgwidth=0, $main_imgheight=0, $redview="", $redlayout="")
	{
		...
		//$refererpath=explode("layout=",$_SERVER['HTTP_REFERER']);
		//$getlayout=explode("&",$refererpath[1]);
...
//		$ImageAttributes['type']		= $type;
			...
			if ($media_image [$m]->media_name != $media_image [$m]->product_full_image)
			{
				if ($media_image [$m]->media_name && is_file ( JPATH_COMPONENT_SITE . "/assets/images/product/".$media_image[$m]->media_name ))
				{
						...
						if(PRODUCT_ADDIMG_IS_LIGHTBOX)
						{
							...
							$prodadditionImg_div_start= "<div class='additional_image'><a href='".$linkimage."' title='".$alttext."'  rel='lightbox[product7]'>";
		...
		if($property_id>0)
		{
           ...
           else
           {
          	   	for($m = 0; $m < count ( $media_image ); $m ++)
				{
					...
					// Main Image
					if ($thumb!=$media_image [$m]->property_image && ($thumb!=$media_image [$m]->property_main_image))
					{
						if ($thumb && is_file ( JPATH_COMPONENT_SITE . "/assets/images/property/".$thumb))
						{
							if(PRODUCT_ADDIMG_IS_LIGHTBOX)
							{
								$propadditionImg_div_start= "<div class='additional_image'><a href='".$url."components/com_redshop/assets/images/property/".$thumb."' title='".$alttext."' rel=\"lightbox[product7]\">";
							...
							else
							{
						    	...
						    	$propadditionImg .= $propadditionImg_div_start;
								$propadditionImg .= "<a href='javascript:void(0)'>"."<img src='".$url."components/com_redshop/helpers/thumb.php?filename=property/".$thumb."&newxsize=" . $mpw_thumb . "&newysize=" . $mph_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' alt='" . $alttext . "' title='" . $alttext . "' style='cursor: auto;'>";
								$prophrefend="</a>";
							}
							if(ADDITIONAL_HOVER_IMAGE_ENABLE)
							{
								$propadditionImg .= "<img src='".$url."components/com_redshop/helpers/thumb.php?filename=property/".$thumb."&newxsize=" . ADDITIONAL_HOVER_IMAGE_WIDTH . "&newysize=" . ADDITIONAL_HOVER_IMAGE_HEIGHT . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' alt='" . $alttext . "' title='" . $alttext . "' class='redImagepreview'>";
							}
		...
		if($subproperty_id>0)
		{
			...
			for($m = 0; $m < count ( $media_image ); $m ++)
			{
				...
				//Main Image
				if ($thumb && ($thumb!=$media_image [$m]->subattribute_color_main_image))
				{
					$filedir = (is_file ( JPATH_COMPONENT_SITE . "/assets/images/subproperty/".$thumb)) ? 'subproperty' : 'property';
					if ($thumb && is_file ( JPATH_COMPONENT_SITE . "/assets/images/".$filedir."/".$thumb))
					{
						if(PRODUCT_ADDIMG_IS_LIGHTBOX)
						{
							$subpropadditionImg_div_start= "<div class='additional_image'><a href='".$url."components/com_redshop/assets/images/".$filedir."/".$thumb."' title='".$alttext."' rel=\"lightbox[product7]\">";
		                	$subpropadditionImg_div_end="</a></div>";
		                	$subpropadditionImg .= $subpropadditionImg_div_start;
							$subpropadditionImg .= "<img src='".$url."components/com_redshop/helpers/thumb.php?filename=".$filedir."/".$thumb."&newxsize=" . $mpw_thumb . "&newysize=" . $mph_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' alt='" . $alttext . "' title='" . $alttext . "'>";
							$subprophrefend="";
						}
						else
						{
							$imgs_path=$url."components/com_redshop/helpers/thumb.php?filename=".$filedir."/".$thumb."&newxsize=" . $pw_thumb . "&newysize=" . $ph_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING;
							//Main Image
							$subproperty_filename_org=JPATH_COMPONENT_SITE . "/assets/images/subproperty/".$imagename;
							if(is_file($subproperty_filename_org))
							{
								//Main Image
								$subproperty_thumb_original=$media_image [$m]->subattribute_color_main_image;
								$subproperty_img_path_org=$url."components/".$option."/helpers/thumb.php?filename=subcolor/".$media_image [$m]->subattribute_color_image."&newxsize=".$pw_thumb."&newysize=".$ph_thumb."&swap=".USE_IMAGE_SIZE_SWAPPING;
							}
							else
							{
					    	...
					    	$subpropadditionImg .= $subpropadditionImg_div_start;
							$subpropadditionImg .= "<a href='javascript:void(0)'>"."<img src='".$url."components/com_redshop/helpers/thumb.php?filename=".$filedir."/".$thumb."&newxsize=" . $mpw_thumb . "&newysize=" . $mph_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' alt='" . $alttext . "' title='" . $alttext . "' style='cursor: auto;'>";
							$subprophrefend="</a>";
						}
						if(ADDITIONAL_HOVER_IMAGE_ENABLE)
						{
							$subpropadditionImg .= "<img src='".$url."components/com_redshop/helpers/thumb.php?filename=".$filedir."/".$thumb."&newxsize=" . ADDITIONAL_HOVER_IMAGE_WIDTH . "&newysize=" . ADDITIONAL_HOVER_IMAGE_HEIGHT . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' alt='" . $alttext . "' title='" . $alttext . "' class='redImagepreview'>";
						}
...
//				}
	...
	function getProductFinderDatepickerValue($templatedata="",$productid=0,$fieldArray=array(),$giftcard=0)
	{
		$extraField = new extraField();
		if(count($fieldArray)>0)
		{
			for($i=0;$i<count($fieldArray);$i++)
			{
				...
				if($fieldValueArray->data_txt!="" && $fieldArray[$i]->field_show_in_front==1 && $fieldArray[$i]->published==1 && $giftcard==0)
				{
...
// function for related product layout
	...
	function getRelatedtemplateView($template_desc, $product_id)
	{
		...
		if(count($related_template)>0)
		{
			if(count($related_product)>0 && strstr($related_template->template_desc,"{related_product_start}") && strstr($related_template->template_desc,"{related_product_end}"))
			{
				...
				for($r=0; $r<count($related_product); $r++)
				{
					...
					$related_template_data = str_replace ( "{relproduct_number_lbl}", JText::_ ( 'PRODUCT_NUMBER_LBL' ), $related_template_data );
...
//
					...
					$rmore = "<a href='".$relmorelink."' title='".$related_product [$r]->product_name."'>".JText::_('READ_MORE')."</a>";
	 ...
	 */
	function replaceAttributePriceList($id,$templatedata){
		...
		for($i=0;$i<count($attributes);$i++){
			...
			#$output .= (count($propertys)) > 0 ? '<div class="related_plist_attribute_name'.$i.$id.'">'.$attribute_name.'</div>':'';
			...
			for($p=0;$p<count($propertys);$p++){
				...
				#$output .= '<div class="related_plist_property_name'.$i.'">'.$property_name.'&nbsp;:&nbsp;'.$proprty_oprand.' '.$property_formated_price.'</div>';
				...
				for($s=0;$s<count($subpropertys);$s++){
					...
					#$output .= '<div class="related_plist_subproperty_name'.$i.'">'.$subproperty_name.'&nbsp;:&nbsp;'.$subproprty_oprand.' '.$subproperty_formated_price.'</div>';
					#$output .= '<div class="related_plist_subproperty_name'.$plistnum.'">'.$subproperty_formated_price.'</div>';
				}
	...
	function removeOutofstockProduct($products)
	{
		$stockroomhelper = new rsstockroomhelper();
