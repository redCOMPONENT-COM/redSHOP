...
//	alert(accessory_data + " " + acc_quantity_data + " " + acc_attribute_data + " " + acc_property_data + " " + acc_subproperty_data);
	...
	if(document.getElementById('quantity'+prefix))
 	{
 ...
 //	alert("123");
 	var url = site_url+"index.php?option=com_redshop&view=product&tmpl=component&task=displayProductaddprice&qunatity="+qty;
...
function changePropertyDropdown(product_id,accessory_id,relatedprd_id,attribute_id,selectedproperty_id,mpw_thumb,mph_thumb)
{
	...
	var url = site_url+"index.php?option=com_redshop&view=product&tmpl=component&task=displaySubProperty&isAjaxBox="+layout;
	url = url + suburl;
	//alert(url);
	request = getHTTPObject();
	request.onreadystatechange=function() {
...
//					alert('divsubimgscroll'+commonid+'_'+property_id);
			...
			/*
			 * override javascript function via plugin or non-redSHOP page
			 */
			onchangePropertyDropdown(onorgargs);
		}
	};
	request.open("GET", url, true);
	request.send(null);
 ...
 * as a base function
 */
function onchangePropertyDropdown(onorgargs){
	return true;
}
...
//	alert("attrArr = " + attrArr + "allpropArr = " + allpropArr + "tolallsubpropArr = " + tolallsubpropArr);
	...
	if(accessory_id!=0)
	{
		...
		if(document.getElementById("productprice_inclvat")){
			document.getElementById("productprice_inclvat").value = mainprice;
		}
	...
	if(accessory_id!=0)	
	{
		if(document.getElementById("requiedaccAttribute"))
		{
			document.getElementById("requiedaccAttribute").value = acc_error;
		}
		if(document.getElementById("requiedaccProperty"))
		{
			document.getElementById("requiedaccProperty").value = subacc_error;
		}
	}
	else
	{
			...
			if(stockAmount == 0 )
			{
				if( (preorder == 'global' && ALLOW_PRE_ORDER!=1) || (preorder == '' && ALLOW_PRE_ORDER!=1) || (preorder == 'no'))
				{
					...
					if(preorder_stock == 0 )
					{
							...
							if(document.getElementById('stockaddtocart'+commonstockid)){
							document.getElementById('stockaddtocart'+commonstockid).innerHTML = PREORDER_PRODUCT_OUTOFSTOCK_MESSAGE;
							}
...
//		alert("Product ID is missing");
	...
	if(document.getElementById('productprice_inclvat'))
	{
		price_incl_vat = parseFloat(document.getElementById('productprice_inclvat').value);
	}
	// end
...
//    alert("main price = " + mainprice + " : aacc = " + accfinalprice + " : wrapp = " + wprice);
 ...
 // product dropdown qunty change multiple by original product price start
    ...
    price_incl_vat = parseFloat(price_incl_vat) + parseFloat(accfinalprice) + parseFloat(wprice);
	...
	if(SHOW_PRICE=='1' && ( DEFAULT_QUOTATION_MODE!='1' || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE)))
	{
		...
		if(document.getElementById('display_product_price_incl_vat'+product_id))
		{
			if(!price_incl_vat)
			{
				price_incl_vat = getPriceReplacement(price_incl_vat);
			} else {
				price_incl_vat = number_format(price_incl_vat,PRICE_DECIMAL,PRICE_SEPERATOR,THOUSAND_SEPERATOR);
			}
			document.getElementById('display_product_price_incl_vat'+product_id).innerHTML = 	price_incl_vat;
		}
...
{  
	...
	//var url = site_url+"index.php?option=com_redshop&view=product&tmpl=component&task=displayAdditionImage";
	var url = site_url+"index.php?option=com_redshop&view=product&tmpl=component&task=displayAdditionImage&redview="+REDSHOP_VIEW+"&redlayout="+REDSHOP_LAYOUT;
	url = url + suburl;
		...
		if(request.readyState == 4 )
		{
			...
			{  
				...
				if(arrResponse[4]!="")
				{
					...
					{	
						if(document.getElementById('rsZoom_image'+product_id) && arrResponse[12])
						{
							
							document.getElementById('rsZoom_image'+product_id).href=arrResponse[12];
						}
						...
						if(arrResponse[13] && document.getElementById('main_image'+product_id))
							document.getElementById('main_image'+product_id).title = arrResponse[13]; 
					}
				}			
			}
			else
...
function preloadSlimbox(){
	...
	if(moopatt.test(MooTools.version) == true){
			...
			if(areaPrice == "fail"){
				...
				if(document.getElementById('main_price'+proid))
				{
					...
					if(SHOW_PRICE=='1' && ( DEFAULT_QUOTATION_MODE!='1' || (DEFAULT_QUOTATION_MODE && SHOW_QUOTATION_PRICE))){
						 ...
						 if(document.getElementById('display_product_price_incl_vat'+proid))
						 {
							document.getElementById('display_product_price_incl_vat'+proid).innerHTML = formatted_product_price_excl_vat;
						 }
...
{
	var imgLength = document.RedgetElementsByClassName('imgClass_'+prodid);
...
//		alert("requiedProperty= " + frm.requiedProperty.value);
	...
	if(document.getElementById('requiedaccAttribute'))
	{
		frm.requiedaccAttribute.value = document.getElementById('requiedaccAttribute').value;
//		alert("requiedaccAttribute= " + frm.requiedaccAttribute.value);
	}
	if(document.getElementById('requiedaccProperty'))
	{
		frm.requiedaccProperty.value = document.getElementById('requiedaccProperty').value;
//		alert("requiedaccProperty= " + frm.requiedaccProperty.value);
	}
	
	
	var product_quantity = 1;
	if(document.getElementById('quantity'+product_id) && document.getElementById('quantity'+product_id).value)
	{
	...
	if(document.getElementById('hidden_subscription_id')){
		...
		if(subId==0){
			alert(SELECT_SUBSCRIPTION_PLAN);
			return false;
		}else{
...
//		alert("Product ID is missing");
	...
	if(AJAX_CART_BOX==0)
	{
		...
		var requiedaccAttribute = document.getElementById(frmCartName).requiedaccAttribute.value;
		var requiedaccProperty = document.getElementById(frmCartName).requiedaccProperty.value;
		
		if(requiedaccAttribute!="")
		{
			alert(requiedaccAttribute);
			return false;
		}
		if(requiedaccProperty!="")
		{
			alert(requiedaccProperty);
			return false;
		}
...
function submitAjaxCartdetail(frmCartName,product_id,relatedprd_id,giftcard_id,totAttribute,totAccessory,totUserfield)
{
	...
	var requiedaccAttribute = document.getElementById(frmCartName).requiedaccAttribute.value;
	var requiedaccProperty = document.getElementById(frmCartName).requiedaccProperty.value;
	if(requiedaccAttribute!=0 && requiedaccAttribute!="")
	{
		alert(requiedaccAttribute);
		return false;
	}
	if(requiedaccProperty!=0 && requiedaccProperty!="")
	{
		alert(requiedaccProperty);
		return false;
	}
		...
		{		
			...
			{ 
				if(extrafields[ex].checked)// || rdo_flag== true)
				{
					...
					alert(extrafieldpost);
					//alert(extrafields[ex].id + ' ' + extrafields[ex].checked);
					continue; 
				}
			}	
		}
...
//	params = params + "&attribute_price="+frm.attribute_price.value;
	...
	params = params + "&requiedaccAttribute="+frm.requiedaccAttribute.value;
	params = params + "&requiedaccProperty="+frm.requiedaccProperty.value;
		...
		if(request.readyState == 4 ) 
		{
			...
			request_inner.onreadystatechange = function()
			{
				if(request_inner.readyState == 4 && request_inner.status==200 && aj_flag ) 
				{
					...
					document.ajaxbox = new MooPrompt( '',responcebox,{buttons: 0,width:500,height:150,overlay: false,delay:AJAX_CART_DISPLAY_TIME,showCloseBtn: true,effect: 'grow',boxId:'cartBox'});
