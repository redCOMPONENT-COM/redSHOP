 ...
 */
    ...
    $order->order_subtotal = $currencyClass->convert ( $order_details[0]->order_total, '', $currency_main );
     ...
     $returnUrl = JURI::base()."index.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_paypal&Itemid=$Itemid&orderid=".$data['order_id'];
	...
	"return" => $returnUrl,
	"notify_url" => JURI::base()."index.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_paypal&Itemid=$Itemid&orderid=".$data['order_id'],
    "night_phone_b" =>substr($data['billinginfo']->phone, 0, 25),
	"cancel_return" => JURI::base()."index.php?option=com_redshop&view=order_detail&controller=order_detail&task=notify_payment&payment_plugin=rs_payment_paypal&Itemid=$Itemid&orderid=".$data['order_id'],
    ...
    for($i=0;$i<count($items);$i++) {
     ...
     // $supp_var["tax_". ($i+1)] = round($tax,2);
      ...
      $shipping2 = $item->product_quantity*$shipping;
		...
		echo "<form action='$paypalurl' method='post' name='paypalfrm'>";
