...
class RedshopTableField extends RedshopTable
{
    ...
    protected function saveFieldValues($id)
    {
        ...
        for ($j = 0; $j < $total; $j++)
        {
            ...
            {
                $extra_name_file_check = $post->get('extra_name_file_check', array(), 'array');
                $set = " field_name='" . $extra_name_file_check[$j] . "', ";

                if (isset($extra_name_file_check[$j]) && $extra_name_file_check[$j] && $valueIds[$j] != "")
                {
                    $query = $db->getQuery(true)
                        ->update($db->qn('#__redshop_fields_value'))
                        ->set($set . ' ' . $db->qn('field_value') . ' = ' . $db->q($extraValues[$j]))
