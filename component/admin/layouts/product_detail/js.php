...
<script>
    ...
    const ajaxSaveElement = (t) => {
        ...
        jQuery.ajax({
            method: 'POST',
            url: '<?php echo JUri::root() ?>administrator/index.php?option=com_ajax&plugin=SaveElement&group=redshop_product&format=raw',
            data: dataAjax,
            beforeSend: function() {
                jQuery('#loader').show();
            }
    ...
    const createSampleColumns = (papa, t, res) => {
        ...
        switch (t) {
            case 'property':
                ...
                inner.html('<img src="<?php echo \JUri::root() ?>media/com_redshop/images/media16.png" alt="" />');
                c[i].append(inner.clone());
                inner.html('<img src="<?php echo \JUri::root() ?>media/com_redshop/images/discountmanagmenet16.png" alt="" />');
                c[i].append(inner.clone());
                papa.append(c[i]);
                i++;
