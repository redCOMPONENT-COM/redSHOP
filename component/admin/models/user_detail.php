...
class user_detailModeluser_detail extends JModel
{
	...
	function _initData()
	{
		if (empty($this->_data))
		{
			...
			$detail->override_vat_zone			= 0;
			$detail->vat_zone					= null;
...
//				$detail->requisition_number			= $bill_data->requisition_number;
	...
	function storeUser($post){
		...
		if (!$user->bind($post))
		{
			$mainframe->enqueueMessage(JText::_('CANNOT_SAVE_THE_USER_INFORMATION'), 'message');
		...
		if (!$isNew)
		{
			...
			if ( $user->get('gid') != $original_gid && $original_gid == 25 )
			{
				...
				if ( $count <= 1 )
				{
					...
					$this->setRedirect( 'index.php?option='.$option.'&view=user', JText::_('WARN_ONLY_SUPER') );
					return false;
				}
			}
		}
