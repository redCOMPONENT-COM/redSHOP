...
class exportModelexport extends JModel
{
	...
	if(JRequest::getVar('text_qul'))
		{
			$this->_text_qul = trim(JRequest::getVar('text_qul'));
		}
		if(!$exportname)
			$mainframe->Redirect("index.php?option=com_redshop&view=export",JText::_("PLEASE_SELECT_SECTION"));
	...
	private function loadProducts() {
		...
		{
			$q = "SELECT p.*,pc.product_id FROM `".$this->_table_prefix."product` p left outer join `".$this->_table_prefix."product_category_xref` pc on p.product_id = pc.product_id ";
			$q.=" where ";
		} else {
		...
		if(count($cur)>0)
		{
			...
			if ($i == 0) {
					foreach ($row as $id => $value) {
						if($id=="discount_stratdate")
						{
							$id="discount_startdate";
						}
					...
					$headerArray=array("sitepath","category_id","category_name","accessory_products","product_stock","images","images_order","images_alternattext","video","video_order","video_alternattext","document","document_order","document_alternattext","download","download_order","download_alternattext");
					$implodedstring= implode($this->_text_qul.','.$this->_text_qul,$headerArray);
					//echo ','.$this->_text_qul.$implodedstring;//substr($implodedstring,0,-1);
					//echo ','.$this->_text_qul.$implodedstring;//substr($implodedstring,0,-1);
					echo ','.$this->_text_qul.$implodedstring.$this->_text_qul.','.$this->_text_qul;//substr($implodedstring,0,-1);

					if($export_product_extra_field)
					{

					 echo 	$implodedstring1=implode($this->_text_qul.','.$this->_text_qul,$extrafheader).$this->_text_qul;
				...
				//	$value= preg_replace('/[^a-z0-9&\s\.\,]/i', '',$value);
					...
					if($id == 'product_full_image' && $value != "" ){
						if(!is_file(JPATH_COMPONENT_SITE.'/assets/images/product/'.$value))
					  		$value = "";
					}
					...
					if(trim($this->_text_qul)=='"')
					{
						echo $this->_text_qul.str_replace('"',"'", $value).$this->_text_qul;
					}
					else
					{
						echo $this->_text_qul.$value.$this->_text_qul;
					}
				...
				if($fields+3){
					$query = 'SELECT quantity FROM `'.$this->_table_prefix.'product_stockroom_xref` WHERE `product_id` = '.$row['product_id'].' AND 	stockroom_id = '.DEFAULT_STOCKROOM ;
					$this->_db->setQuery($query);
				...
				if($export_product_extra_field){
					...
					if(count($extrafheader)>0){
						...
						{
							$fieldoutdata = $this->_text_qul.$fieldoutdata.$this->_text_qul;
						}
							echo $fieldoutdata;

							$fd++;

							if($fd < count($extrafheader))	echo ",";
						}
					}
				}
				# End
			...
			for($c=0;$c<count($cur);$c++)
			{
				...
				foreach ($row as $id => $value) {
					if($id == 'category_full_image' && $value != "" ){
						if(is_file(JPATH_COMPONENT_SITE.'/assets/images/category/'.$value))
					  		$value = JUri::root().'components/com_redshop/assets/images/category/'.$value;
					  	else
					  		$value = "";
					}
					$value = str_replace("\n", "" , $value);
		...
		if(count($cur) > 0)
		{
				...
				for($i=0;$i<count($cur);$i++){
					...
					if ($i == 0) {
						...
						if($isrecrm) echo ",property_stock_placement";
						...
						if($isrecrm) echo ",subattribute_stock_placement";
						...
						if ($attribute[$att]->attribute_name != "")
						{
							...
							for($prop=0;$prop<count($att_property);$prop++)
							{
								...
								if($att_property[$prop]->property_image!="")
									$property_image = JURI::root().'components/com_redshop/assets/images/product_attributes/'.$att_property[$prop]->property_image;
								if($att_property[$prop]->property_main_image != "")
									$property_main_image = JURI::root().'components/com_redshop/assets/images/property/'.$att_property[$prop]->property_main_image;
								echo $cur[$i]->product_number.",".$attribute[$att]->attribute_name.",,,,,,".$att_property[$prop]->property_name.",".$main_attribute_stock;
								if($isrecrm) echo ",".$main_attribute_stock_placement;
								...
								for($subprop=0;$subprop<count($subatt_property);$subprop++)
								{
									...
									$sel_arrtibute_stock_sub="select * from `".$this->_table_prefix."product_attribute_stockroom_xref` where section_id='".$subatt_property[$subprop]->subattribute_color_id ."'";
									...
									if($subatt_property[$subprop]->subattribute_color_image!="")
										$subattribute_color_image = JURI::root().'components/com_redshop/assets/images/subcolor/'.$subatt_property[$subprop]->subattribute_color_image;
									...
									if($isrecrm) echo ",".$main_attribute_stock_sub_placement;
	...
	private function loadFields()
	{
		...
		for($i=0;$i<=count($cur);$i++){
			...
			$query = 'SELECT data_id,`data_txt`,`itemid`,`section` FROM `'.$this->_table_prefix.'fields_data` WHERE `fieldid` = '.$cur[$i]->field_id.' and section!=""';
			$this->_db->setQuery($query);
	...
	private function loadUsers()
	{
		...
		$db = JFactory::getDBO();
		$query = "SELECT ui.`users_info_id` , sg.shopper_group_name, IFNULL( u.id, ui.user_id ) as id , IFNULL( u.email, ui.user_email ) as email , u.username,u.name, u.password, u.usertype, u.block, u.sendEmail, u.gid, ui.company_name, ui.firstname, ui.lastname, ui.vat_number, ui.tax_exempt, ui.shopper_group_id, ui.country_code, ui.address, ui.city, ui.state_code, ui.zipcode, ui.tax_exempt_approved, ui.approved, ui.is_company, ui.phone
			FROM (
...
//				mysql_free_result( $cur );
		...
		$query = "SELECT IFNULL( p.property_number, sp.subattribute_color_number ) AS product_number, ap.section, s.shopper_group_id, s.shopper_group_name, ap.product_price, price_quantity_start, price_quantity_end, ap.discount_price, ap.discount_start_date, ap.discount_end_date "
				...
				."LEFT JOIN `".$this->_table_prefix."product_attribute_property` AS p ON p.property_id=ap.section_id AND ap.section='property' AND p.property_number != '' "
				."LEFT JOIN `".$this->_table_prefix."product_subattribute_color` AS sp ON sp.subattribute_color_id=ap.section_id AND ap.section='subproperty' AND sp.subattribute_color_number != '' "
				;
		$db->setQuery($query);
