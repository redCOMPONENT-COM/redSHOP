	...
	function newsletterEntry($cid=array(),$userid=array(),$username=array())
	{
		...
		for($i=0;$i<count($product_id_list);$i++)
		{
			...
			if(strstr($content,'{Newsletter Products:'.$product_id.'}'))
			{
				...
				{
					$thum_image = "<a id='a_main_image' href='".$url."/components/com_redshop/assets/images/product/".$product_id_list[$i]->product_full_image."' title='' rel=\"lightbox[product7]\">";
		...
		for($j=0;$j<count($cid);$j++)
		{
			...
			if(count($subscriberinfo)>0)
			{
				...
				$unsubscriberlink = "<a href='".$unsub_link."'>".JText::_('UNSUBSCRIBE')."</a>";
