...
class category_detailModelcategory_detail extends JModel
{
  	...
  	function store($data)
	{
		...
		if(isset($data['image_delete']))
		{
			...
			unlink(JPATH_ROOT.DS.'components'.DS.$option.DS.'assets/images/category/thumb'.DS.$data['old_image']);
			unlink(JPATH_ROOT.DS.'components'.DS.$option.DS.'assets/images/category'.DS.$data['old_image']);
		...
		if(count($_FILES)>0 && $_FILES['category_full_image']['name']!="")
		{
			...
			$src=$file['tmp_name'];
			$dest = JPATH_ROOT.DS.'components'.DS.$option.DS.'assets/images/category'.DS.$filename; //specific path of the file
			...
			if(isset($data['category_image']) && $data['category_image'] !=null){
				...
				$dest = JPATH_COMPONENT_SITE.DS.'assets/images/category'.DS.$filename;
...
// upload back image
		...
		if(isset($data['image_back_delete']))
		{
			...
			unlink(JPATH_ROOT.DS.'components'.DS.$option.DS.'assets/images/category/thumb'.DS.$data['old_back_image']);
			unlink(JPATH_ROOT.DS.'components'.DS.$option.DS.'assets/images/category'.DS.$data['old_back_image']);
...
// upload back image end
		...
		else
		{
			...
			{
				@unlink(JPATH_ROOT.DS.'components'.DS.$option.DS.'assets/images/category/thumb'.DS.$_POST['old_image']);
				@unlink(JPATH_ROOT.DS.'components'.DS.$option.DS.'assets/images/category'.DS.$_POST['old_image']);
			}
	...
	function delete($cid = array())
	{
		for($i=0;$i<count($cid);$i++){
			...
			if ($childs->ctotal >0) {
				$errorMSG = sprintf(JText::_('CATEGORY_PARENT_ERROR_MSG'),$childs->category_name,$cid[$i]);
			...
			$thumb_path = JPATH_ROOT.DS.'components/com_redshop/assets/images/category/thumb'.DS.$cat_thumb_image;
			$full_image_path = JPATH_ROOT.DS.'components/com_redshop/assets/images/category'.DS.$cat_full_image;
	...
	function copy($cid = array())
	{
		if (count( $cid ))
		{
			...
			for($i=0;$i<count($copydata);$i++)
			{
				...
				$src = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/category'.DS.$copydata[$i]->category_full_image;
				$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/category'.DS.$post['category_full_image'];
