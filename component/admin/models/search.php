    ...
    public function search()
    {
        ...
        if ($jInput->getCmd('media_section', '') != '') {
            switch ($jInput->getCmd('media_section', '')) {
                ...
                default:
                    $query->select(
                        array(
                            $db->qn('product_id', 'id'),
                            'CONCAT(' . $db->qn('product_name') . ', " (", ' . $db->qn(
                                'product_number'
                            ) . ', ")") as text'
                        )
                    )
            ...
            $query->select(
                ...
                ->where('(' . $subQuery . ') = 0')
                ->where($db->qn('ui.address_type') . ' =  ' . $db->q('ST') . ' OR ' . $db->qn('ui.address_type'). ' =  ' . $db->q('BT'));
            ...
            $query->select(
                ...
                ->leftJoin(
                    $db->qn('#__redshop_stockroom_container_xref', 'cp') . ' ON cp.container_id = p.container_id'
                )
            ...
            $query->select(
                array(
                    $db->qn('u.id'),
                    'CONCAT (' . $db->qn('uf.firstname') . ', ' . $db->quote(' ') . ', ' . $db->qn(
                        'uf.lastname'
                    ) . ', ' . $db->quote(' (')
            ...
            if ($productId = $jInput->getInt('product_id', 0)) {
                $query->leftJoin(
                    $db->qn(
                        ...
                        'pa'
                    ) . ' ON pa.child_product_id = p.product_id AND pa.product_id = ' . $productId
                )
