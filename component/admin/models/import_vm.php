...
class RedshopModelImport_Vm extends RedshopModel
{
    ...
    protected function getCategoryIdSynced($vmCategoryId)
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->leftJoin(
                $db->qn('#__virtuemart_categories_en_gb', 'vmc') . ' ON ' . $db->qn(
                    'vmc.category_name'
                ) . ' = ' . $db->qn('c.name')
            )
    ...
    public function syncUser($index)
    {
        ...
        if ($useDefault) {
            $userTable->shopper_group_id = $isPrivate ?
                Redshop::getConfig()->get('SHOPPER_GROUP_DEFAULT_PRIVATE') : Redshop::getConfig()->get(
    ...
    public function syncManufacturer($index)
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->leftJoin(
                $db->qn('#__virtuemart_medias', 'm') . ' ON ' . $db->qn('cm.virtuemart_media_id') . ' = ' . $db->qn(
        ...
        if (!empty($prices)) {
            ...
            foreach ($prices as $price) {
                if ($price->virtuemart_product_id) {
                    $shopperGroupName = RedshopHelperVirtuemart::getVirtuemartShopperGroups(
    ...
    public function syncOrder($index)
    {
        ...
        $query->clear()
            ...
            ->leftJoin(
                $db->qn('#__redshop_order_item', 'rdoi') . ' ON ' . $db->qn(
                    'rdoi.order_id'
                ) . ' = ' . $orderTable->order_id
            )
            ->leftJoin(
                $db->qn('#__redshop_product', 'rdp') . ' ON ' . $db->qn('rdp.product_number') . ' = ' . $db->qn(
                    'vmoi.order_item_sku'
                )
            )
