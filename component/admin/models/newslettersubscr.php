...
class newslettersubscrModelnewslettersubscr extends JModel
{
	...
	function _buildQuerycount()
	{
		...
		if($where=='')
		{
			$query = ' SELECT  count(*) FROM '.$this->_table_prefix.'newsletter_subscription as ns,'.$this->_table_prefix.'newsletter as n WHERE ns.newsletter_id=n.newsletter_id';
		}
		else
		{
			 $query = ' SELECT  count(*) FROM '.$this->_table_prefix.'newsletter_subscription as ns,'.$this->_table_prefix.'newsletter as n WHERE ns.newsletter_id=n.newsletter_id'.$where;
		}
	...
	function _buildQuery()
	{
		...
		if($where=='')
		{
			$query = ' SELECT  distinct(ns.subscription_id),ns.*,n.name as n_name'
			. ' FROM '.$this->_table_prefix.'newsletter_subscription as ns,'.$this->_table_prefix.'newsletter as n WHERE ns.newsletter_id=n.newsletter_id'.$orderby.$limit;
		}
		else
		{
			 $query = ' SELECT  distinct(ns.subscription_id),ns.*,n.name as n_name'
			. ' FROM '.$this->_table_prefix.'newsletter_subscription as ns,'.$this->_table_prefix.'newsletter as n WHERE ns.newsletter_id=n.newsletter_id'.$where.$orderby.$limit;
		}
