...
class newslettersubscr_detailModelnewslettersubscr_detail extends JModel
{
	...
	function __construct()
	{
		...
		/*$query = 'SELECT * FROM '.$this->_table_prefix.'newsletter_subscription WHERE user_id=0';
		$this->_db->setQuery($query);
		$list =$this->_db->loadObjectlist();
		for($i=0;$i<count($list);$i++)
		{
			$rs = $this->getUserFromEmail($list[$i]->email);
			if(count($rs)>0)
			{
				$query = 'update `'.$this->_table_prefix.'newsletter_subscription` '
						.'set `user_id`="'.$rs->user_id.'" '
						.'WHERE subscription_id="'.$list[$i]->subscription_id.'" ';
				$this->_db->setQuery( $query );
				$this->_db->query();
			}
		}*/
