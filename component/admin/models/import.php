...
class importModelimport extends JModel
{
	...
	var $_text_qul='"';
	var $_Overwrite_Blank_Fields=null;
	function __construct()
	{
		parent::__construct();
		$this->_table_prefix = '#__redshop_';
		JTable::addIncludePath(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_redproductfinder'.DS.'tables');
	...
	function importdata()
	{
		...
		if ($post['text_qul'] != "")
		{
			$this->_text_qul = $post['text_qul'];
		}

		if ($post['Overwrite_Blank_Fields'] != "")
		{
			$this->_Overwrite_Blank_Fields = $post['Overwrite_Blank_Fields'];
			define("OVERWRITE_BLANK_FIELDS_VAL",     $post['Overwrite_Blank_Fields']);
		}
		...
		while (($data = fgetcsv($handle, 0, $separator , $this->_text_qul)) !== FALSE)
		{
			...
			else
			{
				...
				foreach ($data as $key => $name)
				{
					...
					if ($headers[$key] == 'category_full_image' && $post['import'] == 'categories')
					{
						...
						{
							@fopen($name,"r");
							$dest = JPATH_COMPONENT_SITE.'/assets/images/category/'.$image_name;
					...
					if($this->_Overwrite_Blank_Fields==1 && empty($name))
					{
						//	unset($rawdata[$headers[$key]]);
					}
				}
				$rawdata['_Overwrite_Blank_Fields']= $post['Overwrite_Blank_Fields'];
				$rawdata=$this->removeNullVal($rawdata);
				// import Categories
				if ($post['import'] == 'categories')
				{
					...
					$row->category_template 	= 	$rawdata['category_more_template'];
					...
					else
					{
						...
						$rows->category_name = $rawdata['category_name'];
						$rows->category_short_description = $rawdata['category_short_description'];
						$rows->category_description = $rawdata['category_description'];
						$rows->category_thumb_image = $rawdata['category_thumb_image'];
						$rows->category_full_image = $rawdata['category_full_image'];
						$rows->published = $rawdata['published'];
						$rows->category_pdate = $rawdata['category_pdate'];
						$rows->products_per_page = $rawdata['products_per_page'];
						$rows->category_template = $rawdata['category_template'];
						$rows->category_more_template = $rawdata['category_more_template'];
						...
						$rows->set('category_id',$category_id);
						$ret = $this->_db->insertObject( $this->_table_prefix.'category', $rows, 'category_id' );
				...
				if ($post['import'] == 'products' && isset($rawdata['product_number']))
				{
					...
					$rawdata['Overwrite_Blank_Fields']= $post['Overwrite_Blank_Fields'];
					...
					if (!$row->store())
					{
						echo JText::_('ERROR_DURING_IMPORT');
					}
					else
					{
						...
						if(isset($rawdata['product_full_image']))
						{
							# product Full Image
							$product_full_image = trim($rawdata['product_full_image']);
							if ($product_full_image != "")
							{
								$src = trim($this->sitepath)."components/com_redshop/assets/images/product/".$product_full_image;
								@fopen($src,"r");
								$dest = JPATH_COMPONENT_SITE.'/assets/images/product/'.$product_full_image;
								# Copy If file is not already exist
								if(!file_exists($dest))
								{
									@copy($name,$dest);
						...
						if(isset($rawdata['images']))
						{
							...
							if (is_array($image_name))
							{
								for ($i=0;$i<count($image_name);$i++)
								{
									...
									{
										$src = trim($this->sitepath)."components/com_redshop/assets/images/product/".trim($image_name[$i]);
										@fopen($src,"r");
										$dest = JPATH_COMPONENT_SITE.'/assets/images/product/'.trim($image_name[$i]);
										# Copy If file is not already exist
										if(!file_exists($dest))
										{
											@copy($src,$dest);
						...
						if(isset($rawdata['images_order']))
						{
							...
							if (is_array($image_name))
							{
								for ($i=0;$i<count($image_name);$i++)
								{
									if (trim($image_name[$i]) != "")
									{
										$src = trim($this->sitepath)."components/com_redshop/assets/video/product/".trim($image_name[$i]);
										@fopen($src,"r");
						...
						if(isset($rawdata['video_order']))
						{
							...
							if (is_array($image_name))
							{
								for ($i=0;$i<count($image_name);$i++)
								{
									if (trim($image_name[$i]) != "")
									{
										$src = trim($this->sitepath)."components/com_redshop/assets/document/product/".trim($image_name[$i]);
										@fopen($src,"r");
										$dest = JPATH_COMPONENT_SITE.'/assets/document/product/'.trim($image_name[$i]);
										# Copy If file is not already exist
										if(!file_exists($dest))
										{
											@copy($src,$dest);
						...
						if(isset($rawdata['download']))
						{
							...
							if (is_array($image_name))
							{
								for ($i=0;$i<count($image_name);$i++)
								{
									if (trim($image_name[$i]) != "")
									{
										$src = trim($this->sitepath)."components/com_redshop/assets/download/product/".trim($image_name[$i]);
										@fopen($src,"r");
						...
						# End
						if(isset($rawdata['download_order']))
						{
							...
							."WHERE `media_name` LIKE '".trim($product_full_image)."' "
							...
							if ($count <= 0 )
							{
								...
								$rows->media_id = 0;
								$rows->media_name = trim($product_full_image);
							...
							if(trim($rawdata['download_order'])=="" && $this->_Overwrite_Blank_Fields == 1)
							{

							# remove all current media for type images
						 	$query = "DELETE FROM `".$this->_table_prefix."media` WHERE `media_type` = 'images' and `section_id` = ".trim($product_id);
							$this->_db->setQuery($query);
							$this->_db->Query();
							# end
					...
					$category_id = trim($rawdata['category_id']);
					if(isset($rawdata['category_name']))
					$category_name = trim($rawdata['category_name']);
						...
						if($category)
							{
								...
								."WHERE `category_name` = '".trim($categoryArr[$i])."' ";
								$this->_db->setQuery($query);
								$category_id = $this->_db->loadResult();
								$category_id = trim($category_id);
							}else{
								 $category_id = trim($categoryArr[$i]);
							}
							...
							if ($count <= 0 )
							{
								...
								."VALUES ('".trim($category_id)."', '".trim($product_id)."')";
								$this->_db->setQuery($query);
								$this->_db->Query();
							}
						}
					...
					if(isset($rawdata['product_stock']))
					{
					...
					if(isset($rawdata['images']))
					{
						...
						if (is_array($section_images))
						{
							for($s=0;$s<count($section_images);$s++)
							{
								if (trim($section_images[$s])!="")
								{
									...
									."WHERE `media_name` LIKE '".trim($section_images[$s])."' "
						...
						if(trim($rawdata['images'])=="" && $this->_Overwrite_Blank_Fields == 1)
						{
							# remove all current media for type images
						 	 $query = "DELETE FROM `".$this->_table_prefix."media` WHERE `media_type` = 'images' and `section_id` = ".trim($product_id);
							$this->_db->setQuery($query);
					...
					if(isset($rawdata['video']))
					{
						...
						if (is_array($section_video))
						{
							for($s=0;$s<count($section_video);$s++)
							{
								if (trim($section_video[$s])!="")
								{
									...
									$query = "SELECT count(*) FROM `".$this->_table_prefix."media` "
									."WHERE `media_name` LIKE '".trim($section_video[$s])."' "
									."AND `media_section`='product' "
									."AND `section_id` = '".$product_id."' "
						...
						if(trim($rawdata['video'])=="" && $this->_Overwrite_Blank_Fields == 1)
						{
							# remove all current media for type video
						 	$query = "DELETE FROM `".$this->_table_prefix."media` WHERE `media_type` = 'video' and `section_id` = ".trim($product_id);
							$this->_db->setQuery($query);
							$this->_db->Query();
							# end
					...
					if(isset($rawdata['document']))
					{
						...
						if (is_array($section_document))
						{
							for($s=0;$s<count($section_document);$s++)
							{
								if (trim($section_document[$s])!="")
								{
									...
									$query = "SELECT count(*) FROM `".$this->_table_prefix."media` "
									."WHERE `media_name` LIKE '".trim($section_document[$s])."' "
									."AND `media_section`='product' "
									."AND `section_id` = '".$product_id."' "
						...
						if(trim($rawdata['document'])=="" && $this->_Overwrite_Blank_Fields == 1)
						{
							# remove all current media for type images
						 	$query = "DELETE FROM `".$this->_table_prefix."media` WHERE `media_type` = 'document' and `section_id` = ".trim($product_id);
							$this->_db->setQuery($query);
							$this->_db->Query();
							# end
					...
					if(isset($rawdata['download']))
					{
						...
						if (is_array($section_download))
						{
							for($s=0;$s<count($section_download);$s++)
							{
								if (trim($section_download[$s])!="")
								{
									...
									$query = "SELECT count(*) FROM `".$this->_table_prefix."media` "
									."WHERE `media_name` LIKE '".trim($section_download[$s])."' "
									."AND `media_section`='product' "
									."AND `section_id`='".$product_id."' "
						...
						if(trim($rawdata['download'])=="" && $this->_Overwrite_Blank_Fields == 1)
						{
							# remove all current media for type images
						 	$query = "DELETE FROM `".$this->_table_prefix."media` WHERE `media_type` = 'download' and `section_id` = ".trim($product_id);
							$this->_db->setQuery($query);
							$this->_db->Query();
							# end
					...
					/*$q = "SELECT id FROM #__redproductfinder_types;";
					 $this->_db->setQuery($q);
					 $totals['types'] = $db->_getList();
					 $q = "SELECT id FROM #__redproductfinder_tags;";
					 $this->_db->setQuery($q);
					 $totals['tags'] = $db->_getList();*/

					if(isset($rawdata['association_relation']) && $rawdata['association_relation']!=' ' && !empty($rawdata['association_relation']))
					{



						$sql="select id from #__redproductfinder_associations where product_id=".$rawdata['product_id'];
						$this->_db->setQuery($sql);
						$findassociation=$this->_db->loadResult();
						//print_r($findassociation);exit;
						if(count($findassociation) > 0 || $findassociation!=null)
						{
							$associationID=$findassociation;
							$truncate="DELETE FROM  `#__redproductfinder_association_tag` WHERE association_id=".$associationID;
							$this->_db->setQuery($truncate);$this->_db->loadResult();
						}
						else
						{
							$rows = $this->getTable('associations');
							$rows->published = 1;
							$rows->	checked_out = 0;
							$rows->checked_out_time = '0000-00-00 00:00:00';
							$rows->ordering 	 = 0;
							$rows->product_id 	 = $rawdata['product_id'];
							//$rows=$this->removeNullVal($rows);
							if(!$rows->store())
							{
								$this->setError($this->_db->getErrorMsg());
							...
							{
								$associationID=$this->_db->insertid();
							}
						}

						$typetags=explode("#",$rawdata['association_relation']);

						foreach($typetags as $eachtypetag)
						{
							$typetagsegments=explode(":",$eachtypetag);

							if(count($typetagsegments)==2)
							{
								$findquery="select id from #__redproductfinder_types where id = ".$typetagsegments[1];
								$this->_db->setQuery($findquery);
								$findtyp=$this->_db->loadResult();
								$findquery="select id from #__redproductfinder_tags where id = ".$typetagsegments[0];
								$this->_db->setQuery($findquery);
								$findtg=$this->_db->loadResult();

								if(count($findtyp) != 0 && count($findtg) != 0)
								{
									$sql1="select * from #__redproductfinder_association_tag where association_id=".$associationID." AND tag_id = ".$typetagsegments[0]." AND type_id = ".$typetagsegments[1];
									$this->_db->setQuery($sql1);
									$findtagtype=$this->_db->loadResult();


									$rows = $this->getTable('associationtag');
									$rows->association_id = $associationID;
									$rows->	tag_id = $typetagsegments[0];
									$rows->type_id = $typetagsegments[1];
									$rows->quality_score 	 = 0;
									//$rows=$this->removeNullVal($rows);
									if(!$rows->store())
									{
										$this->setError($this->_db->getErrorMsg());
					...
					unset($rawdata['association_relation']);
				}
						...
						if ($property_name != ""){
							...
							if ($proprow->store()){
								...
								if ($property_image != ""){
									...
									$dest = JPATH_COMPONENT_SITE.'/assets/images/product_attributes/'.$property_image;
								...
								if ($property_main_image != ""){
									...
									@fopen($property_image_path,"r");
									$dest = JPATH_COMPONENT_SITE.'/assets/images/property/'.$property_main_image;
								...
								if ($subattribute_color_name != ""){
									...
									if ($subproprow->store()){
										...
										if ($subattribute_color_image != "")
										{
											...
											$dest = JPATH_COMPONENT_SITE.'/assets/images/subcolor/'.$subattribute_color_image;
											# Copy If file is not already exist
											if(!file_exists($dest))
											{
				...
				if ($post['import'] == 'fields')
				{
					...
					if($data_id!='')
					{
						if(!$ch_data_id )
						{
							...
							."VALUES ('".$data_id."','".$new_field_id."','".$data_txt."','".$itemid."','".$section."')";
							$this->_db->setQuery($query);
							$this->_db->Query();
						}else{
	...
	function check_vm()
	{
		...
		$query_check = "SELECT id FROM #__components WHERE `option` = 'com_virtuemart' ";
			...
			foreach($data as $product_data)
			{
				...
				if($product_data->red_product_number == NULL)
				{
					...
					if($product_full_image != "")
					{
						...
						$dest = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS."product".DS.$product_full_image;
						if(is_file($src))
						{
							@copy($src,$dest);
						}
					...
					foreach($product_more_img as $more_img)
					{
						...
						$dest = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS."product".DS.$filename;
				...
				else
				{
					...
					if($product_full_image != $red_product_full_image)
					{
						...
						// Copy product images to redshop
						$src = JPATH_ROOT.DS."components".DS."com_virtuemart".DS."shop_image".DS."product".DS.$product_full_image;
						$redimagesrc = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS."product".DS.$red_product_full_image;
						$dest = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS."product".DS.$product_full_image;
	...
	function Category_sync($product_array)
	{
		...
		foreach($data as $cat_data)
		{
			...
			if($cat_data->category_full_image != "")
			{
				...
				$dest = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS."category".DS.$cat_data->category_full_image;
				if(is_file($src))
				{
					@copy($src,$dest);
				}
			...
			else
			{
				if($cat_data->category_thumb_image != "")
				{
					...
					$dest = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS."category".DS.$cat_data->category_thumb_image;
					if(is_file($src))
					{
						@copy($src,$dest);
					}
	...
	function removeNullVal($row)
	{
		...
		if($this->_Overwrite_Blank_Fields==0)
		{
			if(is_array($row))
			{
