...
class giftcard_detailModelgiftcard_detail extends JModel
{
  	...
  	function store($data)
	{
		...
		if($giftcardfile['name']!="")
		{
			...
			$dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/giftcard'.DS.$giftcardimg; //specific path of the file
		...
		if($giftcardbgfile['name']!="")
		{
			...
			$dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/giftcard'.DS.$giftcardbgimg; //specific path of the file
	...
	function copy($cid = array()){
		...
		foreach ($this->_copydata as $cdata){
			...
			$post['giftcard_id'] = 0;
			$post['giftcard_name'] = JText::_('COPY_OF').' '.$cdata->giftcard_name;
