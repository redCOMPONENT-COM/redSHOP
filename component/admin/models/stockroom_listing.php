...
//		}
		...
		$limit		= $mainframe->getUserStateFromRequest( $context2.'limit', 'limit', $mainframe->getCfg('list_limit'), 0);
		$limitstart = $mainframe->getUserStateFromRequest( $context2.'limitstart', 'limitstart', 0 );
		$stockroom_type     = $mainframe->getUserStateFromRequest( $context2.'stockroom_type','stockroom_type','product');
//		$atttype     = $mainframe->getUserStateFromRequest( $context2.'atttype','atttype',0);
...
//		$this->setState('atttype', $atttype);
	...
	function _buildQuery()
	{
		...
		$stockroom_type = $mainframe->getUserStateFromRequest( $context2.'stockroom_type',  'stockroom_type', '' );
//		$sec = $mainframe->getUserStateFromRequest( $context2.'atttype',  'atttype', '' );
	...
	function storeStockroomQuantity($stockroom_type,$sid,$pid,$quantity="", $preorder_stock=0, $ordered_preorder=0)
	{
		...
		else
		{
				if($preorder_stock < $ordered_preorder && $preorder_stock!="" && $ordered_preorder!="")
				{
						$msg =  JText::_('PREORDER_STOCK_NOT_ALLOWED'). "for Stockroom ";
						JError::raiseWarning ( '', $msg);
						return false;
	...
	function getcontainerproducts()
	{
		if(USE_CONTAINER)
		{
			...
			$query = "SELECT * FROM  ".$this->_table_prefix."stockroom as s , "
					...
					."WHERE ps.stockroom_id = s.stockroom_id "
					;
		}
		$this->_db->setQuery($query);
