...
class RedshopModelStatistic_Variant extends RedshopModelList
{
    ...
    public function getProductVariants()
    {
        ...
        $query  = $db->getQuery(true)
            ...
            ->leftjoin(
                $db->qn('#__redshop_order_item', 'oi') . ' ON ' . $db->qn('oai.order_item_id') . ' = ' . $db->qn(
                    'oi.order_item_id'
                )
            )
            ->leftjoin(
                $db->qn('#__redshop_product_attribute_property', 'pap') . ' ON ' . $db->qn(
                    'oai.section_id'
                ) . ' = ' . $db->qn('property_id')
            )
            ->leftjoin(
                $db->qn('#__redshop_product', 'p') . ' ON ' . $db->qn('oi.product_id') . ' = ' . $db->qn('p.product_id')
            )
        ...
        if (!empty($this->filterStartDate) && !empty($this->filterEndDate)) {
            $query->where(
                $db->qn('p.publish_date') . ' > ' . $db->q(date('Y-m-d H:i:s', strtotime($this->filterStartDate)))
            )
                ->where(
                    $db->qn('p.publish_date') . ' <= ' . $db->q(
                        date('Y-m-d H:i:s', strtotime($this->filterEndDate) + 86400)
                    )
                );
        }
