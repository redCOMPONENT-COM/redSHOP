...
class fields_detailModelfields_detail extends JModel
{
	...
	function field_save($id,$post)
	{
		...
		{
			//$extra_value = addslashes($post["extra_value"]);
		...
		for($j=0;$j<$total;$j++)
		{
			...
			if($post["field_type"]==11 || $post["field_type"]==13)
			{
				if($extra_value[$j]!="" && $extra_name['name'][$j]!="")
				{
					...
					$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/extrafield'.DS.$filename;
			...
			if($value_id[$j]=="")
			{
				...
				echo $query = "UPDATE ".$this->_table_prefix."fields_value "
