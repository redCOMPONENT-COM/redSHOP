...
class product_detailModelproduct_detail extends JModel
{
  	...
  	function store($data)
	{
		...
		if(isset($data['thumb_image_delete']))
		{
			...
			$unlink_path = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$data['old_thumb_image'];
		...
		if($thumbfile['name']!="")
		{
			...
			//$filename = JPath::clean(time().'_'.$thumbfile['name']); //Make the filename unique
			$row->product_thumb_image=$filename;
			// Image Upload
			$src=$thumbfile['tmp_name'];
			$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$filename; //specific path of the file
			JFile::upload($src,$dest);
			// End image upload
		}

		$file =& JRequest::getVar('product_full_image', '', 'files', 'array' ); //Get File name, tmp_name
		if(isset($data['image_delete']) || $file['name']!="" || $data['product_image'] !=null)
		{
			$unlink_path = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product/thumb'.DS.$data['old_image'];
			if(is_file($unlink_path))
				unlink($unlink_path);

			$unlink_path = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$data['old_image'];
			if(is_file($unlink_path))
				unlink($unlink_path);
		...
		else
		{
			if($data['product_image'] !=null)
			{
				...
				$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'product'.DS.$filename;
		...
		if(isset($data['back_thumb_image_delete']))
		{
			...
			$unlink_path = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$data['old_back_thumb_image'];
			if(is_file($unlink_path))
				unlink($unlink_path);
		}
		...
		if(isset($data['back_image_delete']))
		{
			...
			$unlink_path = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$data['old_back_image'];
			if(is_file($unlink_path))
				unlink($unlink_path);
		}
		...
		if(isset($data['preview_image_delete']))
		{
			...
			$unlink_path = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$data['old_product_preview_image'];
			if(is_file($unlink_path))
				unlink($unlink_path);
		}
		...
		{
			$row->product_preview_image="";
			$unlink_path = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$data['old_product_preview_back_image'];
			if(is_file($unlink_path))
				unlink($unlink_path);
		}
		...
		/**
		 *
		 * @var boolean checks if product is new
		 *
		 */
		...
		for($j = 0; $j < count ( $data ['product_category'] ); $j ++)
		{
			...
			/*
			 * check mass discount for category
			*/
		...
		*/
		...
		/*
		 * check mass discount for manufacturer
		 */
		$sel = "SELECT * FROM ".$this->_table_prefix."mass_discount WHERE FIND_IN_SET('".$row->manufacturer_id."',manufacturer_id) ORDER BY mass_discount_id desc limit 0,1";
		$this->_db->setQuery( $sel );
		$mass_discount = $this->_db->loadObject();
		...
		if($data['quantity'] || $data['preorder_stock'])
		{
		   	...
		   	for ($i=0;$i<count($data['quantity']);$i++)
		    {
				...
				{
					$msg =  JText::_('PREORDER_STOCK_NOT_ALLOWED');
				...
				$query = "DELETE FROM ".$this->_table_prefix."product_stockroom_xref "
				 ...
				 $this->_db->setQuery( $query );
			    	...
			    	{
			    		$this->insertProductStock($product_id,$data['stockroom_id'][$i],$data['quantity'][$i],$data['preorder_stock'][$i],$data['ordered_preorder'][$i]);
			    	}
				}
			}
		}
		// Save Preorder stock for Product
		...
		if(count($data['product_navigator'])>0 && is_array($data['product_navigator']))
		{
	 	...
	 	//------------ End Navigator Product insert --------------------

	 	//------------ Start Related Product insert --------------------
		...
		if (isset($data['area_start']) && count($data['area_start'])>0){
			...
			for ($c=0;$c<count($area_start);$c++){
				...
				if($data['discount_calc_method'] == 'volume'){
		        	$calcunit = pow($unit, 3);
		        }else if($data['discount_calc_method'] == 'area'){
					$calcunit = pow($unit, 2);
		        }else{
		        	$calcunit = $unit;
		        }
	...
	// if product_type = file and csv file uploaded than do this
		...
		if($productCSVfile['tmp_name']!="")
		{
			if($ext=='csv')
			{
				...
				{
					$this->setError(JText::_("CSV_FILE_NOT_UPLOADED_TRY_AGAIN"));
				}
			}
			else
			{
				$this->setError(JText::_("ONLY_CSV_FILE_ALLOWED"));
				return false;
			}
		}
		// reddesign
		...
		{
			//var_dump($_REQUEST['shoppergroup']); exit;
	...
	function updateproductdiscount($mass_discount,$row){
	...
	function delete($cid = array())
	{
		...
		if (count($cid))
		{
			...
			if (count($parentids)>0){
				...
				$errorMSG = sprintf(JText::_('PRODUCT_PARENT_ERROR_MSG'),$parentids);
			...
			$image_query = 'SELECT pa.attribute_id,pap.property_image, pap.property_id FROM '.$this->_table_prefix.'product_attribute as pa,'.$this->_table_prefix.'product_attribute_property as pap WHERE pa.product_id IN( '.$cids.') and pa.attribute_id = pap.attribute_id';
			$this->_db->setQuery( $image_query );
			$property_image = $this->_db->loadObjectlist();

			foreach($property_image as $imagename){

				$dest = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product_attributes/'.$imagename->property_image;

		 		$tsrc = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product_attributes/thumb/'.$imagename->property_image;
...
//			{
//				$wimg1 = JPATH_ROOT.DS.'components/'.$option.'/assets/images/wrapper/'.$wrapperimage[$i]->wrapper_image;
//		 		$wimg2 = JPATH_ROOT.DS.'components/'.$option.'/assets/images/wrapper/thumb/'.$wrapperimage[$i]->wrapper_image;
//		 		if(is_file($wimg1))
//		 			unlink($wimg1);
//		 		if(is_file($wimg2))
//		 			unlink($wimg2);
...
//			}
			...
			foreach($product_image as $imagename)
			{
				...
				$dest_full = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/'.$imagename->product_full_image;
				$tsrc_thumb = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/'.$imagename->product_thumb_image;
				$dest_back_full = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/'.$imagename->product_back_full_image;
				$tsrc_back_thumb = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/'.$imagename->product_back_thumb_image;
		 		$dest_preview = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/'.$imagename->product_preview_image;
				$tsrc_preview_back = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/'.$imagename->product_preview_back_image;
			...
			/*foreach($media_image as $mimage){
				...
				$dest = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/'.$mimage->media_id.'_'.$mimage->media_name;

		 		$tsrc = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product/thumb/'.$mimage->media_id.'_'.$mimage->media_name;
	...
	function copy($cid = array())
	{
		...
		foreach ($this->_copydata as $pdata)
		{
			...
			$post['product_name'] = JText::_('COPY_OF').' '.$pdata->product_name;
			...
			$post['product_number'] = trim(JText::_('COPY_OF').' '.$pdata->product_number);
			...
			{
				//Image Copy Start
				$old = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$pdata->product_full_image;
				$new = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.JPath::clean(time().$new_product_full_image);
				copy($old, $new);

				$old_thumb = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$pdata->product_thumb_image;
				$new_thumb = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.JPath::clean(time().$new_product_thumb_image);
				copy($old_thumb, $new_thumb);

               	$old_preview = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$pdata->product_preview_image;
				$new_preview = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.JPath::clean(time().$new_product_preview_image);
				copy($old_preview, $new_preview);

                $old_back_preview = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$pdata->product_preview_back_image;
				$new_back_preview = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.JPath::clean(time().$new_product_preview_back_image);
				copy($old_back_preview, $new_back_preview);

                $old_prod_back_full = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$pdata->product_back_full_image;
				$new_prod_back_full = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.JPath::clean(time().$new_product_back_full_image);
				copy($old_prod_back_full, $new_prod_back_full);

                $old_prod_back_thumb = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$pdata->product_back_thumb_image;
				$new_back_back_thumb = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.JPath::clean(time().$new_product_back_thumb_image);
				...
				for($j=0;$j<count($mediadata);$j++)
				{
					...
					$old_media = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.$mediadata[$j]->media_name;
					$new_media = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets/images/product'.DS.JPath::clean(time().$new_img);
	...
	/*function copyadditionalImage($mediadata){
			$rowmedia =& $this->getTable('media_detail');
			$data['media_id '] = 0;
			$data['media_name'] = $mediadata[$j]->media_name;
			$data['media_alternate_text'] = $mediadata[$j]->media_alternate_text;
			$data['media_section'] = $mediadata[$j]->media_section;
			$data['section_id'] = $row->product_id;
			$data['media_type'] = $mediadata[$j]->media_type;
	...
	function copyProductAttribute($cid,$product_id){
		...
		for($att=0;$att<count($attribute);$att++)
		{
			...
			for($prop=0;$prop<count($att_property);$prop++)
			{
				...
				{
						$property_image 						= 'components/com_redshop/assets/images/product_attributes/'.$att_property[$prop]->property_image;
						$property_save['property_image'] 		= $this->copy_image_from_path($property_image,'product_attributes',$property_id);
						$property_save['property_main_image']   = $this->copy_image_from_path($property_image,'product_attributes',$property_id);
						$property_save['property_id'] 			= $property_id;
						if(!empty($att_property[$prop]->property_main_image))
						{
							$property_main_image 					= 'components/com_redshop/assets/images/product_attributes/'.$att_property[$prop]->property_main_image;
							$property_save['property_main_image']   = $this->copy_image_from_path($property_main_image,'product_attributes',$property_id);
						}
						$property_array 						= $this->store_pro($property_save);
				}*/
				...
				if(!empty($att_property[$prop]->property_main_image))
				{
					$property_main_image 						= 'components/com_redshop/assets/images/property/'.$att_property[$prop]->property_main_image;
...
//					$subproperty_save['subattribute_color_image'] 			= $subatt_property[$subprop]->subattribute_color_price;
					...
					{
							$subattribute_color_image = 'components/com_redshop/assets/images/subcolor/'.$subatt_property[$subprop]->subattribute_color_image;
	...
	function getSubpropertyImages($subproperty_id){
	...
	function getPropertyMainImage($property_id){
	...
	function getSubAttributeColor($property_id){
		$query = "SELECT * FROM ".$this->_table_prefix."product_attribute_property AS p "
	...
	function getParentProduct($product_id){
	...
	function deleteprop($cid = array(),$image_name){
		...
		if (count($cid))
		{
			...
			foreach($image_name as $imagename){
				...
				$dest = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product_attributes/'.$imagename;

		 		$tsrc = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product_attributes/thumb/'.$imagename;
		 		if(file_exists($dest))
		 			unlink($dest);
		 		if(file_exists($tsrc))
		 			unlink($tsrc);
	...
	function deleteattr_current($cid = array()){
		...
		if (count($cid))
		{
			...
			foreach($prop as $property_image){
				...
				$dest = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product_attributes/'.$property_image->property_image;

		 		$tsrc = JPATH_ROOT.DS.'components/'.$option.'/assets/images/product_attributes/thumb/'.$property_image->property_image;
		 		if(file_exists($dest))
		 			unlink($dest);
		 		if(file_exists($tsrc))
		 			unlink($tsrc);
	...
	function property_image_list($cid){
		if (count($cid))
		{
...
//			print_r($this->_db->getErrorMsg());exit;
	...
	function accessory_product_data($product_id)
	{
	...
	function related_product_data($product_id)
	{
	...
	function property_more_img($post,$main_img,$sub_img){
		...
		if($main_img['name'] != ''){
			...
			if($filetype != 'png' && $filetype != 'gif' && $filetype != 'jpeg'  && $filetype != 'jpg' ){
				...
				if($post['fsec']=='subproperty'){
					$main_dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/subcolor'.DS.$main_name; //specific path of the file

					JFile::upload($main_src,$main_dest);

					$query = "UPDATE ".$this->_table_prefix."product_subattribute_color SET subattribute_color_image = '".$main_name."' WHERE subattribute_color_id ='".$post['section_id']."' ";
					$this->_db->setQuery( $query );
					if(!$this->_db->query())
					{
						$this->setError($this->_db->getErrorMsg());
					...
					$main_dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/property'.DS.$main_name; //specific path of the file
		...
		for($i=0;$i<$num;$i++){
			if($sub_img['name'][$i] != ""){
				...
				if($filetype != 'png' && $filetype != 'gif' && $filetype != 'jpeg'  && $filetype != 'jpg' ){
					...
					$sub__dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/property'.DS.$sub_name; //specific path of the file
	...
	function deletesubimage($mediaid){
		$query = 'SELECT * FROM '.$this->_table_prefix.'media  WHERE media_id = '.$mediaid;
		$this->_db->setQuery($query);
		$imgdata = $this->_db->loadObject();

		$dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/property/'.$imgdata->media_name;

 		$tsrc = JPATH_ROOT.DS.'components/com_redshop/assets/images/property/thumb/'.$imgdata->media_name;
	...
	function subattribute_color($post,$sub_img){
		...
		for($i=0;$i<$num;$i++){
			if($sub_img['name'][$i] != ""){
				...
				if($filetype != 'png' && $filetype != 'gif' && $filetype != 'jpeg'  && $filetype != 'jpg' ){
					...
					$sub__dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/subcolor'.DS.$sub_name; //specific path of the file

					JFile::upload($sub_src,$sub__dest);
					if($post['property_sub_img_tmp'][$i] != ""){

						$sub = JPATH_ROOT.DS.'components/com_redshop/assets/images/subcolor'.DS.$post['property_sub_img_tmp'][$i];
						$sub_thumb = JPATH_ROOT.DS.'components/com_redshop/assets/images/subcolor/thumb'.DS.$post['property_sub_img_tmp'][$i];
						if(file_exists($sub)){
							unlink($sub);
						}
	...
	function subattr_diff($subattr_id,$section_id){
	...
	function get_subattrprop($subattr_id,$section_id){
	...
	function delsubattr_diff($subattr_diff){

		foreach($subattr_diff as $diff){

			$sub_dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/subcolor'.DS.$diff->subattribute_color_image; //specific path of the file
	 ...
	 */
	function CheckRedProductFinder()
	{
		$query = "SELECT id FROM `#__components` WHERE `link` LIKE '%com_redproductfinder%'";
		$this->_db->setQuery($query);
		return $this->_db->loadResult();
	}
...
/*
 ...
 /**
  ...
  function StockRoomList(){
	...
	*/
  function StockRoomProductQuantity($pid,$sid){
  	$database =& JFactory::getDBO();
	...
	* Getting the  StockRoom Product Quantity
	*/
  function StockRoomAttProductQuantity($pid,$sid,$section){
	...
	function StockRoomAttProductPreorderstock($pid,$sid,$section){
 ...
 function StockRoomPreorderProductQuantity($pid,$sid,$section){
 ...
 function  attribute_empty()
 {
  ...
  function removepropertyImage($pid){
  		...
  		$imagethumbsrcphy = JPATH_COMPONENT_SITE."/assets/images/product_attributes/thumb/".$imagename;

  		if (is_file($imagethumbsrcphy))
  			@unlink($imagethumbsrcphy);

  		$imagesrc = JURI::root()."components/com_redshop/assets/images/product_attributes/".$imagename;
  		$imagesrcphy = JPATH_COMPONENT_SITE."/assets/images/product_attributes/".$imagename;
 ...
 // remove property image
  function removesubpropertyImage($pid){
  		...
  		$imagethumbsrcphy = JPATH_COMPONENT_SITE."/assets/images/subcolor/thumb/".$imagename;

  		if (is_file($imagethumbsrcphy))
  			@unlink($imagethumbsrcphy);


  		$imagesrcphy = JPATH_COMPONENT_SITE."/assets/images/subcolor/".$imagename;
 ...
 function SaveAttributeStockroom($post)
  {
				...
				if($preorder_stock < $ordered_preorder && $preorder_stock!="" && $ordered_preorder!="")
				{
						...
						if($preorder_stock!= "" ||  $quantity!="" )
						{
							...
							else
							{
	    						...
	    						$this->InsertStockroom($post['section_id'],$post['section'],$post['stockroom_id'][$i],$post['quantity'][$i],$preorder_stock,$ordered_preorder);
							}
						}
				}
		}
	 ...
	 */
	function CheckRedDesign()
	{
		$query = "SELECT id FROM `#__components` WHERE `link` LIKE '%com_reddesign%'";
		$this->_db->setQuery($query);
		return $this->_db->loadResult();
	}
	 ...
	 */
	function getProductDesignType($product_id)
	{
	...
	function orderup()
	{
		...
		$q .= "AND ".$this->_table_prefix."product_category_xref.product_id = ".$this->_table_prefix."product.product_id";echo '<br/>';
	 ...
	 */
	function getProdcutSerialNumbers($only_used='')
	{
		$usedCond = "";
		if($only_used == true)
	...
	function deleteProdcutSerialNumbers($serial_id)
	{
	...
	function copyadditionalImage($data)
	{
		...
		$path 				= 'components/com_redshop/assets/images/'.$section.'/'.$src_image;
	...
	function copy_image_additionalimage_from_path($imagePath,$section,$section_id)
	{
		$src = JPATH_ROOT.DS.$imagePath;
 		...
 		$imgname=$this->cleanFileName($imgname);
 		$dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/'.$section.'/'.$property_image;
	  ...
	  /*
	   *  copy attributes from attribute set into product attribute
	   */
	function copyAttributeSetAttribute($attribute_set_id,$product_id)
	{
		...
		for($att=0;$att<count($attribute);$att++)
		{
			...
			for($prop=0;$prop<count($att_property);$prop++)
			{
				...
				if($att_property[$prop]->property_image)
				{
					...
					$filename=$this->cleanFileName($image_split);
					//$filename = JPath::clean(time().'_'.$image_split); //Make the filename unique
					$att_property[$prop]->property_image = $filename;
					$src = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'product_attributes'.DS.$image_split;
					$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'product_attributes'.DS.$filename;
					copy($src,$dest);
				}
				if($att_property[$prop]->property_main_image)
				{
					...
					$src = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'property'.DS.$prop_main_img;
					$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'property'.DS.$filename;
					copy($src,$dest);
				}
			 	$proppost = array();
				...
				for($subprop=0;$subprop<count($subatt_property);$subprop++)
				{
					...
					if($subatt_property[$subprop]->subattribute_color_image)
					{
						...
						$src = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'subcolor'.DS.$image_split;
						$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'subcolor'.DS.$filename;
						copy($src,$dest);
					}
					if($subatt_property[$subprop]->subattribute_color_main_image)
					{
						...
						$src = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'subproperty'.DS.$sub_main_img;
						$dest = JPATH_ROOT.DS.'components'.DS.'com_redshop'.DS.'assets'.DS.'images'.DS.'subproperty'.DS.$filename;
						copy($src,$dest);
					}
	...
	function GetStockroomData($section_id,$name)
	{
	...
	function insertProductStock($product_id,$stockroom_id,$quantiy=0,$preorder_stock=0,$ordered_preorder=0)
	{
	...
	function InsertStockroom($section_id,$name,$stockroom_id,$quantiy,$preorder_stock,$ordered_preorder )
	{
	...
	function GetAttributepriceData($section_id,$name)
	{
	...
	function delete_prop($attribute_id,$property_id)
	{
		$producthelper 	= new producthelper();
		if($property_id)
		{
	...
	function delete_attibute($product_id,$attribute_id,$attribute_set_id)
	{
		$producthelper 	= new producthelper();
	...
	function delete_image($imagename,$section)
	{
/* 		$imagethumbsrcphy = JPATH_COMPONENT_SITE."/assets/images/".$section."/thumb/".$imagename;
  		if (is_file($imagethumbsrcphy))
  			@unlink($imagethumbsrcphy);*/
  		$imagesrcphy = JPATH_COMPONENT_SITE."/assets/images/".$section."/".$imagename;
  		if (is_file($imagesrcphy))
  			@unlink($imagesrcphy);
	}
	...
	function copy_image_from_path($imagePath,$section,$section_id)
	{
		$src = JPATH_ROOT.DS.$imagePath;
		//$imgname = basename($imagePath);
		$imgname=$this->cleanFileName($imagePath);
		//$property_image = $imgname;
	...
	function checkVirtualNumber($product_id=0,$vpnArray=array())
	{
		if(count($vpnArray)>0)
		{
			...
			/*$query = "SELECT sp.subattribute_color_number AS number FROM ".$this->_table_prefix."product_subattribute_color AS sp "
				...
				."WHERE a.product_id!='".$product_id."' ";
			$this->_db->setQuery($query);
			$subproperty = $this->_db->loadObjectList();
	...
	function removeaccesory($accessory_id, $category_id=0, $child_product_id=0)
	{
		$and = '';
		if($category_id!=0)
		{
	...
	function update_attr_property_image($property_id, $property_image, $property_main_image)
	{
	...
	function update_subattr_image($subproperty_id, $subattribute_color_image)
	{
	...
	// copy discount calculator data
	function copyDiscountCalcdata($old_product_id, $new_product_id , $discount_calc_method)
	{
	...
	function removenavigator($navigator_id)
	{
