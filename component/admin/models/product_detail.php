...
class RedshopModelProduct_Detail extends RedshopModel
{
    ...
    public function delete($cid = array())
    {
        ...
        $db         = $this->_db;
        $imageQuery->select(
            $db->qn('pa.attribute_id')
            . ', ' . $db->qn('pap.property_image')
        )
            ->from($db->qn('#__redshop_product_attribute', 'pa'))
            ->leftJoin(
                $db->qn('#__redshop_product_attribute_property', 'pap')
    ...
    public function store($data)
    {
        ...
        if ($backthumbfile['name'] != "") {
            $filename                      = RedshopHelperMedia::cleanFileName(
                $backthumbfile['name'],
                $row->product_id
            );
            ...
            for ($prop = 0, $countProperty = count($att_property); $prop < $countProperty; $prop++) {
                ...
                for ($ls = 0, $countStockroom = count($listStockroomData); $ls < $countStockroom; $ls++) {
                    $this->InsertStockroom(
                        $propertyId,
                        'property',
                        $listStockroomData[$ls]->stockroom_id,
                        $listStockroomData[$ls]->quantity,
                        0,
                        0
                    );
                }
                ...
                for ($subprop = 0; $subprop < $countSuboproperty; $subprop++) {
                    $listsubpropImages         = $this->getImageInfor(
                        $subatt_property[$subprop]->subattribute_color_id,
                        'subproperty'
                    );
                    $listSubStockroomData      = $this->GetStockroomData(
                        $subatt_property[$subprop]->subattribute_color_id,
                        'subproperty'
                    );
                    $listSubAttributepriceData = $this->GetAttributepriceData(
                        $subatt_property[$subprop]->subattribute_color_id,
                        'subproperty'
                    );
                    ...
                    $subpost['subattribute_color_title']      = htmlspecialchars(
     ...
     * @return boolean
     */
    public function InsertAttributeprice(
        $section_id,
        $name,
        $product_price,
        $product_currency,
    ...
    public function copyProductAttribute($cid, $productId)
    {
        ...
        for ($att = 0, $countAttribute = count($attribute); $att < $countAttribute; $att++) {
            ...
            for ($prop = 0, $countProperty = count($att_property); $prop < $countProperty; $prop++) {
                ...
                $property_save['property_alert_message'] = $att_property[$prop]->property_alert_message;
                ...
                $listImages                           = $this->getImageInfor(
                    $att_property[$prop]->property_id,
                    'property'
                );
                $propertyimage                        = '';
                $property_main_image                  = '';
                ...
                for ($subprop = 0; $subprop < $countSubProperty; $subprop++) {
                    ...
                    if (!empty($subatt_property[$subprop]->subattribute_color_image)) {
                        ...
                        $new_subattribute_color_image = $this->copy_image_from_path(
                    ...
                    if (!empty($subatt_property[$subprop]->subattribute_color_main_image)) {
                        ...
                        $new_subattribute_color_main_image = $this->copy_image_from_path(
                            $subattribute_color_main_image,
                            'subproperty'
                        );
                    }

                    $this->update_subattr_image(
                        $subPropertyId,
                        $new_subattribute_color_image,
                        $new_subattribute_color_main_image
                    );
