...
class wrapper_detailModelwrapper_detail extends JModel
{
	...
	function store($data)
	{
		...
		if($wrapperfile['name']!="")
		{
			...
			$dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/wrapper'.DS.$wrapperimg; //specific path of the file
		...
		if($row->wrapper_id)
		{
			...
			if(count($wrapper) > 0 && $wrapperimg!="") {
				$unlink_path = JPATH_ROOT.DS.'components/com_redshop/assets/images/wrapper/thumb'.DS.$wrapper[0]->wrapper_image;
				if(is_file($unlink_path))
					unlink($unlink_path);
						
				$unlink_path = JPATH_ROOT.DS.'components/com_redshop/assets/images/wrapper'.DS.$wrapper[0]->wrapper_image;
				if(is_file($unlink_path)) 	
					unlink($unlink_path);
			}
		}
