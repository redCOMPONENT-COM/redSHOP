...
class orderModelorder extends JModel
{
	...
	function _buildQuery()
	{
		...
		if($filter)
	    {
	    	$where[] = "(  uf.firstname like '%".$filter."%' OR uf.lastname like '%".$filter."%' OR o.order_id like '%".$filter."%' OR o.order_number like '%".$filter."%' OR uf.user_email like '%".$filter."%')";
	    }
		if($cid[0]!=0){
    	...
    	$query = 'SELECT distinct(o.cdate),o.*,uf.* FROM '.$this->_table_prefix.'orders AS o '
	...
	function gls_export($cid)
	{
		...
		echo "Order_number,Consignee_name,Consignee_address_1,Consignee_address_2,Consignee_postal_code,Consignee_city,Consignee_country,Date,Parcel_weight,Number_of_parcels,COD_amount,Parcel_value_amount,Parcel_type,Shipment_type,Attention,Comment,Customer_number,Alt_consignor_name,Consignee_mobile_phone_no,Alt_consignor_name,Alt_consignor_address_1,Alt_consignor_address_2,Alt_consignor_postal_code,Alt_consignor_city,Alt_consignor_country,Alt_consignor_phone_no";
				echo "\r\n";
		for($i=0;$i<count($gls_arr);$i++)
		{
			...
			if(($details[0]=='plgredshop_shippingdefault_shipping_GLS') && ($gls_arr[$i]->shop_id!=""))
			{
				...
				if($totalWeight>=$normal_parcel_weight_start && $totalWeight<=$normal_parcel_weight_end)
				{
					$parceltype='A';
				}
				else if($totalWeight>=$small_parcel_weight_start && $totalWeight<=$small_parcel_weight_end)
				{
					$parceltype='S';
				}
				else if($totalWeight>=$pallet_parcel_weight_start && $totalWeight<=$pallet_parcel_weight_end)
				{
					$parceltype='p';
				}
				$shopDetails = $shipping->decryptShipping(str_replace(" ","+",$gls_arr[$i]->shop_id));
				$shopDetails_arr=explode("|",$shopDetails);
				...
				$country=$order_helper->getCountry($shopDetails_arr[4]);
				echo "".$gls_arr[$i]->order_number.", ".$shopDetails_arr[1].",".$shopDetails_arr[2].",,".$shopDetails_arr[0].",".$shopDetails_arr[7].",".$country.",".date("D-M-Y",$gls_arr[$i]->cdate).",".$totalWeight.",".$qty.",,".$gls_arr[$i]->order_total.",".$parceltype.",".$shipmenttype.",,,,,,,,,,,,";
				echo "\r\n";
			}
		}
