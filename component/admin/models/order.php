...
class RedshopModelOrder extends RedshopModel
{
    ...
    public function _buildQuery()
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->innerJoin(
                $db->qn('#__redshop_order_status', 'os') . ' ON ' . $db->qn(
...
//			->where($db->qn('uf.address_type') . '=' . $db->q('BT'))
        ...
        if ($filter) {
            ...
            if ($filterBy == 'orderid') {
		        ...
		        $query->where($db->qn('uf.phone') . ' LIKE ' . $db->q('%' . $filter . '%'));
	        }// $filter_by == 'all'
            else {
                $query->where(
                    "(REPLACE(CONCAT(" . $db->qn('uf.firstname') . ", "
    ...
    public function export_data($cid = array())
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->from($db->qn('#__redshop_orders', 'o'))
            ->leftJoin(
                $db->qn('#__redshop_order_users_info', 'ouf') . ' ON ' . $db->qn('o.order_id') . ' = ' . $db->qn(
                    'ouf.order_id'
                )
            )
    ...
    public function business_gls_export($cid)
    {
        ...
        foreach ($ordersInfo as $order) {
            ...
            $extraFieldData = RedshopHelperExtrafields::getSectionFieldList(
    ...
    {
        $filterStatus        = $this->getUserStateFromRequest(
            $this->context . 'filter_status',
            'filter_status',
            '',
            'string'
        );
        $filterPaymentStatus = $this->getUserStateFromRequest(
            $this->context . 'filter_payment_status',
        ...
        $filterFromDate      = $this->getUserStateFromRequest(
            $this->context . 'filter_from_date',
            'filter_from_date',
            '',
            ''
        );
        $filterToDate        = $this->getUserStateFromRequest(
            $this->context . 'filter_to_date',
            'filter_to_date',
            '',
            ''
        );
