...
class RedshopModelRating_detail extends RedshopModel
{
	...
	public function _loadData()
	{
		if (empty($this->_data))
		{
			$query = 'SELECT p.*,u.name as username,pr.product_name FROM #__redshop_product as pr, '
				. '#__redshop_product_rating as p  left join #__users as u on u.id=p.userid WHERE p.rating_id = '
	...
	public function delete($cid = array())
	{
		if (count($cid))
		{
			...
			$query = 'DELETE FROM #__redshop_product_rating WHERE rating_id IN ( ' . $cids . ' )';
			$this->_db->setQuery($query);
	...
	public function publish($cid = array(), $publish = 1)
	{
		if (count($cid))
		{
			...
			$query = 'UPDATE #__redshop_product_rating'
