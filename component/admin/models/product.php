...
class productModelproduct extends JModel
{
	...
	function _buildQuery()
	{
		...
		if($where==''  &&  $search_field !='pa.property_number'){
			...
			$query = "SELECT p.product_id,p.product_id AS id,p.product_name,p.product_name AS name,p.product_price,p.product_parent_id,p.product_parent_id AS parent "
 					.",p.published,p.visited,p.manufacturer_id,p.product_number ,p.checked_out,p.checked_out_time,p.discount_price "
 					.",p.product_template FROM ".$this->_table_prefix."product AS p "
		 	...
		 	$query = "SELECT p.product_id AS id,p.product_id,p.product_name,p.product_name AS treename,p.product_name AS name,p.product_parent_id,p.product_parent_id AS parent,p.product_price "
	...
	function assignTemplate($data){
		...
		if (count( $cid ))
		{
			...
			$query = 'UPDATE '.$this->_table_prefix.'product'
				. ' SET `product_template` = "' . intval( $product_template ).'" '
	...
	function gbasefeed($data){
		...
		if(count( $cid ))
		{
			...
			{
			$query = "SELECT p.*,m.manufacturer_name FROM ".$this->_table_prefix."product AS p "
				." LEFT JOIN "
				.$this->_table_prefix."manufacturer AS m"
					...
					if(!$isStockExists)
					{
						...
						if(($product_preorder == "global" && ALLOW_PRE_ORDER) || ($product_preorder == "yes") || ($product_preorder == "" && ALLOW_PRE_ORDER))
						{
							 if (!$isPreorderStockExists)
							 {
							    ...
							    $product_status = JText::_ ( 'OUT_OF_STOCK' );
	
							} else {
	
								$product_status = JText::_ ( 'PREORDER' );
							}
						}
						else {
						...
						$product_status = JText::_ ( 'AVAILABLE_FOR_ORDER' );
					}
				} else {
					$product_status = JText::_ ( 'AVAILABLE_FOR_ORDER' );
				}
	...
	function getCategoryList()
	{
		...
		$q = "SELECT cx.category_child_id AS id, cx.category_parent_id AS parent, c.category_name AS name "
		...
		// second pass - get an indent list of the items
		$treelist = JHTML::_('menu.treerecurse', 0, '', array(), $children, max( 0, 9 ) );

		$this->_categorytreelist = $treelist;
		return $this->_categorytreelist;
	}
