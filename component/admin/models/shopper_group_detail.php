...
//			$detail->tax_exempt						= 0;
  	...
  	function store($data)
	{
		...
		{
			$logopath = JPATH_ROOT.DS.'components/com_redshop/assets/images/shopperlogo'.DS.$data['shopper_group_logo']; //specific path of the file
			if(is_file($logopath))
			{
				unlink($logopath);
			}
		...
		if($logo['name'] != "" )
		{
			...
			$src=$logo['tmp_name'];
			$dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/shopperlogo'.DS.$logoname; //specific path of the file
		...
		else
		{
			if($data['shopper_group_logo_tmp'] !=null)
			{
				...
				$dest = JPATH_COMPONENT_SITE.DS.'assets/images/shopperlogo'.DS.$logoname;
		...
		if(!$data['shopper_group_id'] && NEW_SHOPPER_GROUP_GET_VALUE_FROM)
		{
			...
			$logopath = JPATH_ROOT.DS.'components/com_redshop/assets/images/shopperlogo'.DS.$data['shopper_group_logo'];
			$copylogopath = JPATH_ROOT.DS.'components/com_redshop/assets/images/shopperlogo'.DS.$destname;
			if(is_file($logopath))
			{
...
//					return false;
	...
	function delete($cid = array())
	{
		if (count( $cid ))
		{
			...
			{
				$logopath = JPATH_ROOT.DS.'components/com_redshop/assets/images/shopperlogo'.DS.$list[$i]->shopper_group_logo;
				if(is_file($logopath))
				{
					unlink($logopath);
				}
