...
class RedshopModelMass_discount_detail extends RedshopModel
{
	...
	public function _initData()
	{
		if (empty($this->_data))
		{
			...
			$_post = JFactory::getApplication()->getUserState('com_redshop.edit.product.data', false);

			if ($_post)
			{
				$detail = json_decode($_post);
			}
			else
			{
				$detail = new stdClass;
			...
			if (!isset($detail->manufacturer_id))
			{
				$detail->manufacturer_id = null;
			}
