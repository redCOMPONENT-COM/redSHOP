...
class stockimage_detailModelstockimage_detail extends JModel
{
  	...
  	function store($data)
	{
		...
		if($_FILES['stock_amount_image']['name']!="")
		{
			...
			$src=$file['tmp_name'];
			$dest = JPATH_ROOT.DS.'components/com_redshop/assets/images/stockroom'.DS.$filename;
			JFile::upload($src,$dest);
			
			if(isset($data['stock_image'])!="" && is_file(JPATH_ROOT.DS.'components/com_redshop/assets/images/stockroom'.DS.$data['stock_image']))
			{
				unlink(JPATH_ROOT.DS.'components/com_redshop/assets/images/stockroom'.DS.$data['stock_image'] );
			}
		}
		if (!$row->bind($data)) {
	...
	function delete($cid = array())
	{
		if (count( $cid ))
		{
			...
			for($i=0;$i<count($cid);$i++)
			{
				...
				if($stock_amount_image!="" && is_file(JPATH_ROOT.DS.'components/com_redshop/assets/images/stockroom'.DS.$stock_amount_image))
				{
					unlink(JPATH_ROOT.DS.'components/com_redshop/assets/images/stockroom'.DS.$stock_amount_image);
				}
			}
	...
	function getStockAmountOption($select=0)
	{
		...
		$option[]   = JHTML::_('select.option', 1, JText::_('HIGHER_THAN'));
		$option[]   = JHTML::_('select.option', 2, JText::_('EQUAL'));
		$option[]   = JHTML::_('select.option', 3, JText::_('LOWER_THAN'));
		if($select!=0)
		{
