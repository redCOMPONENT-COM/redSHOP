...
class mediaModelmedia extends JModel
{
	...
	function getList()
	{
		...
		if($fdownload!=1)
		{
			...
			if (strlen($current) > 0) {
				$basePath = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS.$current;
			} else {
				$basePath = JPATH_COMPONENT_SITE.DS."assets".DS."images";
			}
			$mediaBase = str_replace(DS, '/', JPATH_COMPONENT_SITE.DS."assets".DS."images".'/');
		}
		else
		{
			if (strlen($current) > 0) {
