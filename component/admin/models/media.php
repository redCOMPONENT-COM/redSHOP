...
class RedshopModelMedia extends RedshopModel
{
    ...
    public function getList()
    {
        ...
        if ($fdownload != 1) {
            ...
            $mediaBase = str_replace(
                DIRECTORY_SEPARATOR,
                '/',
                Redshop::getConfig()->get('PRODUCT_DOWNLOAD_ROOT') . '/'
            );
        }
        ...
        if ($fileList !== false) {
            foreach ($fileList as $file) {
                if (
                    file_exists($basePath . '/' . $file) && substr($file, 0, 1) != '.' && strtolower(
                        $file
                    ) !== 'index.html'
                ) {
                    $tmp                = new JObject;
    ...
    {
        $filter_media_section = $this->getUserStateFromRequest(
            $this->context . '.filter_media_section',
            'filter_media_section',
            0
        );
