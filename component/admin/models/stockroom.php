...
class stockroomModelstockroom extends JModel
{
	...
	function _buildQuerycount()
	{
		...
		$where='';
		if($filter) 
	    	$where = " WHERE s.stockroom_name like '%".$filter."%' "; 

		if($where=='')
		{
		  	$query = "SELECT count(*) FROM ".$this->_table_prefix."stockroom s";
		}
		else
		{
			$query = "SELECT count(*) FROM ".$this->_table_prefix."stockroom s".$where;
		}
		return $query;
	}
	...
	function _buildQuery()
	{
		...
		if($where=='')
		{
		  	$query = "SELECT distinct(s.stockroom_id),s.* FROM ".$this->_table_prefix."stockroom s WHERE 1=1".$orderby.$limit;
		}
		else
		{
			$query = "SELECT distinct(s.stockroom_id),s.* FROM ".$this->_table_prefix."stockroom s".$where.$orderby.$limit;
		}
		return $query;
	}
