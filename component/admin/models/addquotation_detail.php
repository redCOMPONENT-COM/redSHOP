...
//				.'GROUP BY ui.user_id '
	...
	function replaceSubPropertyData($product_id=0,$accessory_id=0,$attribute_id=0,$property_id=0,$user_id,$uniqueid="")
	{
		...
		if (count($subproperty)>0)
		{
			...
			$tmp_array = array ();
			$tmp_array[0]->value = 0;
			$tmp_array[0]->text = JText::_('SELECT')."&nbsp;".urldecode($subproperty[0]->property_name);
...
//				//$attribute_table = str_replace ( "{subproperty_dropdown}", JText::_ ( 'PRODUCT_OUTOFSTOCK_MESSAGE' ), $attribute_table );
//			} else {
