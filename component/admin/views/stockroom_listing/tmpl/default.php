...
$print_link=JRoute::_ ( 'index3.php?option=' . $option . '&view=stockroom_listing&id=0&showbuttons=1' );
...
function submitform(pressbutton){
	...
	{
		window.open("<?php echo $print_link;?>","<?php echo JText::_ ( 'STOCKROOM_LISTING' );?>","scrollbars=1","location=1");
		return false;
	}
var form = document.adminForm;
   if (pressbutton)
...
<select name="search_field">
	<option value="product_name" <?php if($this->search_field=='product_name') echo "selected='selected'";?>><?php echo JTEXT::_("PRODUCT_NAME")?></option>
	<option value="product_number" <?php if($this->search_field=='product_number') echo "selected='selected'";?>><?php echo JTEXT::_("PRODUCT_NUMBER")?></option>
</select>
<input type="text" name="keyword" id="keyword" value="<?php echo $this->keyword;?>">
<input type="submit" value="<?php echo JTEXT::_("SEARCH")?>">
<input type="reset" value="<?php echo JTEXT::_("RESET")?>" onclick="clearForm();"></td></tr>
<tr><td><?php echo JTEXT::_("CATEGORY").": ".$this->lists['category'];?><?php echo $this->lists['stockroom_type']; ?></td></tr></table>

<div id="editcell1" >
<table class="adminlist"><thead>
	<tr><th width="5%"><?php echo JText::_ ( 'NUM' ); ?></th>
	<th width="10%"><?php echo JHTML::_ ( 'grid.sort', 'PRODUCT_SKU', 'p.product_number', $this->lists ['order_Dir'], $this->lists ['order'] );?></th>
	<th width="20%"><?php echo JHTML::_ ( 'grid.sort', 'PRODUCT_NAME', 'p.product_name', $this->lists ['order_Dir'], $this->lists ['order'] );?></th>
<?php
if($stockroom_type!='product')
{ ?>
	<th width="15%"><?php echo JText::_ ( 'PROPERTY_NUMBER' );	?></th>
	<th width="20%"><?php if($stockroom_type == 'property')
    {
    	echo JText::_ ('PROPERTY');
    }
    else if($stockroom_type == 'subproperty')
    {
    	echo JText::_ ('SUBPROPERTY');
    }?></th>
<?php
}
	for($j=0;$j<count($this->stockroom);$j++)
	 {	?>
	<th width="5%"><?php echo $this->stockroom[$j]->stockroom_name;//JText::_ ('QUANTITY'); ?>&nbsp;&nbsp;
	<img style="cursor:pointer" src="images/filesave.png" onclick="submitform('saveStock')"></th>
	<th width="5%"><?php echo $this->stockroom[$j]->stockroom_name;?>&nbsp;<?php  echo JText::_( 'PREORDER_STOCKROOM_QTY' ); ?>&nbsp;&nbsp;
	<img style="cursor:pointer" src="images/filesave.png" onclick="submitform('saveStock')"></th>
<?php }	?><tr>
</thead>
<?php
...
$preorder_stockalltotal= array(0);
	...
	<?php	if($stockroom_type!='product')
			{	?>
			<td><?php if($stockroom_type == 'property'){ echo $row->property_number; }
        	 else if($stockroom_type == 'subproperty'){ echo $row->subattribute_color_number; } ?></td>
			<td><?php if($stockroom_type == 'property'){ echo $row->property_name; }
        	 else if($stockroom_type == 'subproperty'){ echo $row->subattribute_color_name; } ?></td>
	<?php 	}	?>
	<?php 	for($j=0;$j<count($this->stockroom);$j++)
		 	{
				...
				( <?php echo $ordered_preorder?> )
				 ...
				 <input type="button" name="preorder_reset" value="Reset"  onclick="location.href = 'index.php?option=com_redshop&view=stockroom_listing&task=ResetPreorderStock&stockroom_type=<?php echo $stockroom_type?>&product_id=<?php echo $section_id?>&stockroom_id=<?php echo $this->stockroom[$j]->stockroom_id?>' ; ">
				 <?php }?>
				</td>
	...
	<tr><td colspan="<?php echo $colspan;?>"><?php echo JText::_ ('TOTAL'); ?></td>
<?php 	for($j=0;$j<count($this->stockroom);$j++)
		 	{	?>
