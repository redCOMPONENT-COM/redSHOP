        ...
        if (pressbutton == "print_data") {
            window.open(
                "<?php echo Redshop\IO\Route::_(
                    ...
                    false
                ); ?>", "<?php echo Text::_('COM_REDSHOP_STOCKROOM_LISTING'); ?>",
                "scrollbars=1",
                "location=1"
            );
            return false;
        ...
        <div class="filterItem">
            <div class="btn-wrapper input-append">
                <input type="text" name="keyword" id="keyword"
                    placeholder="<?php echo Text::_('COM_REDSHOP_SEARCH'); ?>"
                    value="<?php echo $this->state->get('keyword'); ?>">
                <input type="submit" class="btn" value="<?php echo Text::_("COM_REDSHOP_SEARCH") ?>">
                <input type="reset" class="btn reset" value="<?php echo Text::_("COM_REDSHOP_RESET") ?>"
                    onclick="clearForm();">
            </div>
        </div>
        <div class="filterItem">
            <?php
            $filterOptions[] = JHtml::_('select.option', 'product_name', Text::_('COM_REDSHOP_PRODUCT_NAME'));
            $filterOptions[] = JHtml::_('select.option', 'product_number', Text::_('COM_REDSHOP_PRODUCT_NUMBER'));
        ...
        <table class="adminlist table table-striped">
            <thead>
                <tr>
                    ...
                    <?php if ($this->stockroom_type != 'product'): ?>
                        ...
                        <th width="20%">
                            <?php if ($this->stockroom_type == 'property'): ?>
                                <?php echo Text::_('COM_REDSHOP_PROPERTY'); ?>
                            <?php elseif ($this->stockroom_type == 'subproperty'): ?>
                                <?php echo Text::_('COM_REDSHOP_SUBPROPERTY'); ?>
                            <?php endif; ?>
                        </th>
                    <?php endif; ?>
                    <?php for ($j = 0; $j < count($this->stockroom); $j++): ?>
                        ...
                        <th width="5%">
                            <?php echo Text::_('COM_REDSHOP_PREORDER_STOCKROOM_QTY'); ?><br />
                            <?php echo $this->stockroom[$j]->name; ?>
                            <a href="javascript:Joomla.submitbutton('saveStock')" class="saveorder pull-right"></a>
                        </th>
                    <?php endfor; ?>
            ...
            <?php for ($i = 0, $n = count($this->resultlisting); $i < $n; $i++): ?>
                ...
                <tr class="<?php echo "row$k"; ?>">
                    ...
                    <?php for ($j = 0, $countStockRoom = count($this->stockroom); $j < $countStockRoom; $j++): ?>
                        ...
                        ?>
                        <td align="center">
                            <input type="hidden" name="sid[]" value="<?php echo $this->stockroom[$j]->id; ?>">
                            <input type="hidden" name="pid[]" value="<?php echo $section_id; ?>">
                            <input type="text" value="<?php echo $quantity; ?>" name="quantity[]" class="input-small" size="4">
                        </td>
                        <td align="center">
                            <input type="text" value="<?php echo $preorder_stock; ?>" name="preorder_stock[]"
                                class="input-small" size="4">
            ...
            <tr>
                <td colspan="<?php echo $colspan; ?>">
                    <?php echo Text::_('COM_REDSHOP_TOTAL'); ?>
                </td>
                <?php for ($j = 0; $j < count($this->stockroom); $j++): ?>
                    <td align="center">
