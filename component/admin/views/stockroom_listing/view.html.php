...
//		$container_get = JRequest::getVar ( 'container_list','0' );
		...
		$document->setTitle ( JText::_ ( 'STOCKROOM_LISTING' ) );
		JToolBarHelper::title ( JText::_ ( 'STOCKROOM_LISTING_MANAGEMENT' ), 'redshop_stockroom48' );
...
//		$atttype = $mainframe->getUserStateFromRequest( $context2.'atttype','atttype','property' );
		...
		$optiontype[]   = JHTML::_('select.option', 'product', JText::_('PRODUCT'));
		$optiontype[]   = JHTML::_('select.option', 'property', JText::_('PROPERTY'));
		$optiontype[]   = JHTML::_('select.option', 'subproperty', JText::_('SUBPROPERTY'));
		//JHTML::_('select.option', 'product_attribute', JText::_('PRODUCT_ATTRIBUTE'));
...
//		$optionval = array();
//		$optionval[]   = JHTML::_('select.option', 'property', JText::_('PROPERTY'));
//		$optionval[]   = JHTML::_('select.option', 'subproperty', JText::_('SUBPROPERTY'));
//		$lists['atttype'] 		= JHTML::_('select.genericlist',$optionval,  'atttype', 'class="inputbox" size="1" onchange="document.adminForm.submit();" ', 'value', 'text',  $atttype);
		...
		$temps[0]->category_name=JText::_('SELECT');
		$categories=@array_merge($temps,$categories);
	    $lists['category'] 	= JHTML::_('select.genericlist',$categories,  'category_id', 'class="inputbox" onchange="document.adminForm.submit();" ', 'category_id', 'category_name', $category_id );
