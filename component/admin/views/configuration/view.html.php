...
class configurationViewconfiguration extends JView
{
	...
	function display($tpl = null)
	{
		...
		if(!is_writable($configpath)){
   		...
   		JToolBarHelper::title(   JText::_( 'CONFIG' ), 'redshop_icon-48-settings' );
   		if(is_writable($configpath)){
	 		JToolBarHelper::save();
		...
		$templatesel[0]->template_name = JText::_('SELECT');
		...
		if(count($shopper_groups)<=0) $shopper_groups = array();
		$show_price_shopper_group_list = explode(',',SHOW_PRICE_SHOPPER_GROUP_LIST);

		$lists['show_price_shopper_group_list'] 	=  JHTML::_('select.genericlist',   $shopper_groups, 'show_price_shopper_group_list[]', 'size="10" multiple', 'value', 'text', $show_price_shopper_group_list );
		$tmp = array();
		...
		$lists['show_price_user_group_list'] 	=  JHTML::_('select.genericlist',   $gtree, 'show_price_user_group_list[]', 'size="10" multiple', 'value', 'text', $show_price_user_group_list );
		...
		$lists['attribute_as_product_in_economic'] 	= JHTML::_('select.booleanlist',  'attribute_as_product_in_economic', 'class="inputbox" ', ATTRIBUTE_AS_PRODUCT_IN_ECONOMIC );
		...
		$lists['enable_sef_number_name'] 	= JHTML::_('select.booleanlist',  'enable_sef_number_name', 'class="inputbox"', ENABLE_SEF_NUMBER_NAME, 'Name','ID' );
		$lists['category_in_sef_url'] 	= JHTML::_('select.booleanlist',  'category_in_sef_url', 'class="inputbox"', CATEGORY_IN_SEF_URL, 'No','Yes' );
		...
		$discoupon_percent_or_total = array (JHTML::_('select.option',0 , JText::_('TOTAL')), JHTML::_('select.option',1, JText::_('PERCENTAGE')));
...
//		$lists['terms_article_id'] = JHTML::_('select.genericlist',$articles,'terms_article_id','class="inputbox" ','value','text', TERMS_ARTICLE_ID);
		...
		$product_comparison[]   = JHTML::_('select.option', '',JText::_('SELECT'));
		$product_comparison[]   = JHTML::_('select.option', 'category',JText::_('CATEGORY'));
		$product_comparison[]   = JHTML::_('select.option', 'global',JText::_('GLOBAL'));
		...
		$lists['quotation_mode'] 	= JHTML::_('select.booleanlist',   'default_quotation_mode', 'onclick="return quote_price(this.value);" class="inputbox" size="1"',DEFAULT_QUOTATION_MODE_PRE , $yes=JText::_('ON'),$no=JText::_('OFF') );
		...
		$lists['show_terms_and_conditions'] 	= JHTML::_('select.booleanlist',   'show_terms_and_conditions', 'class="inputbox" size="1"',SHOW_TERMS_AND_CONDITIONS , $yes=JText::_('SHOW_PER_USER'),$no=JText::_('SHOW_PER_ORDER') );
		...
		$menuitem = array();
		$menuitem[0]->value = 0;
		$menuitem[0]->text = JText::_('SELECT');
		$q = "SELECT m.id,m.name,mt.title FROM #__menu AS m "
		...
		$tmp = array();
		$tmp[] = JHTML::_('select.option', '', JText::_('SELECT'));
		$default_vat_country = @array_merge($tmp,$countries);

		$default_customer_register_type = array();
		$default_customer_register_type[]   = JHTML::_('select.option', '0',JText::_('SELECT'));
		$default_customer_register_type[]   = JHTML::_('select.option', '1',JText::_('PRIVATE'));
		$default_customer_register_type[]   = JHTML::_('select.option', '2',JText::_('COMPANY'));
		...
		$addtocart_behaviour[]   = JHTML::_('select.option', '0',JText::_('SELECT'));
		$addtocart_behaviour[]   = JHTML::_('select.option', '1',JText::_('DIRECT_TO_CART'));
		$addtocart_behaviour[]   = JHTML::_('select.option', '2',JText::_('STAY_ON_CURRENT_VIEW'));
		...
		$allow_customer_register_type[]   = JHTML::_('select.option', '0',JText::_('BOTH'));
		$allow_customer_register_type[]   = JHTML::_('select.option', '1',JText::_('PRIVATE'));
		$allow_customer_register_type[]   = JHTML::_('select.option', '2',JText::_('COMPANY'));
		...
		$vat_based_on = array();
		$vat_based_on[]   = JHTML::_('select.option', '0',JText::_('WEBSHOP_MODE'));
		$vat_based_on[]   = JHTML::_('select.option', '1', JText::_('CUSTOMER_MODE'));
		$vat_based_on[]   = JHTML::_('select.option', '2', JText::_('EU_MODE'));
		...
		$script .= "function changeStateList()
					{
				 	...
				 	writeDynaList( 'class=\"inputbox\" name=\"default_vat_state\" size=\"1\" id=\"default_vat_state\"', states, originalPos, originalPos, $selected_state_code );
					//-->
					//]]></script>";
		...
		$register_methods[]   = JHTML::_('select.option', '0',JText::_('REGISTER_WITH_ACCOUNT_CREATION'));
		$register_methods[]   = JHTML::_('select.option', '1', JText::_('REGISTER_WITHOUT_ACCOUNT_CREATION'));
		$register_methods[]   = JHTML::_('select.option', '2', JText::_('REGISTER_ACCOUNT_OPTIONAL'));
		$register_methods[]   = JHTML::_('select.option', '3', JText::_('REGISTER_ACCOUNT_SILENT'));
		...
		$lists['individual_add_to_cart_enable'] 	= JHTML::_('select.booleanlist',   'individual_add_to_cart_enable', 'class="inputbox" size="1"', INDIVIDUAL_ADD_TO_CART_ENABLE, JText::_('INDIVIDUAL_ADD_TO_CART_PER_PROPERTY'), JText::_('ADD_TO_CART_PER_PRODUCT') );
		...
		$bookinvoice = array();
		$bookinvoice[]   = JHTML::_('select.option', '0',JText::_('DIRECTLY_BOOK'));
		$bookinvoice[]   = JHTML::_('select.option', '1',JText::_('MANUALLY_BOOK'));
		$bookinvoice[]   = JHTML::_('select.option', '2',JText::_('BOOK_ON_ORDER_STATUS'));
		...
		$lists['economic_book_invoice_number'] 	= JHTML::_('select.booleanlist',   'economic_book_invoice_number', 'class="inputbox" size="1"', ECONOMIC_BOOK_INVOICE_NUMBER, JText::_('SEQUENTIALLY_IN_ECONOMIC_NO_MATCH_UP_WITH_ORDER_NUMBER'), JText::_('SAME_AS_ORDER_NUMBER') );

		//NEXT-PREVIOUS LINK
		$link_type = array();
		$link_type[]   = JHTML::_('select.option', '0',JText::_('DEFAULT_LINK'));
		$link_type[]   = JHTML::_('select.option', '1',JText::_('CUSTOM_LINK'));
		$link_type[]   = JHTML::_('select.option', '2',JText::_('IMAGE_LINK'));
		...
		$shipping_after 			= defined('SHIPPING_AFTER') ? SHIPPING_AFTER : 'total';
     	$lists['shipping_after']    = $extra_field->rs_booleanlist('shipping_after',  'class="inputbox"', $shipping_after ,$yes=JText::_('TOTAL'),$no=JText::_('SUBTOTAL_LBL'),'','total','subtotal');

     	$payment_calculation 			= defined('PAYMENT_CALCULATION_ON') ? PAYMENT_CALCULATION_ON : 'total';
     	$lists['payment_calculation_on']    = $extra_field->rs_booleanlist('payment_calculation_on',  'class="inputbox"', $payment_calculation ,$yes=JText::_('TOTAL'),$no=JText::_('SUBTOTAL_LBL'),'','total','subtotal');

		$calculate_vat_on 			= defined('CALCULATE_VAT_ON') ? CALCULATE_VAT_ON : 'BT';
     	$lists['calculate_vat_on']  = $extra_field->rs_booleanlist('calculate_vat_on',  'class="inputbox"', $calculate_vat_on ,$yes=JText::_('BILLING_ADDRESS_LBL'),$no=JText::_('SHIPPING_ADDRESS_LBL'),'','BT','ST');

     	$order_data = array();
        $order_data[0]->value="c.category_name ASC";
        $order_data[0]->text=JText::_( 'CATEGORY_NAME' );
        $order_data[1]->value="c.category_id DESC";
        $order_data[1]->text=JText::_( 'NEWEST' );
        $order_data[2]->value="c.ordering ASC";
        $order_data[2]->text=JText::_( 'ORDERING' );
		...
		$symbol_position[0]->text=JText::_('SELECT');
		$symbol_position[1]->value="front";
		$symbol_position[1]->text=JText::_('FRONT');
		$symbol_position[2]->value="behind";
		$symbol_position[2]->text=JText::_('BEHIND');
		$symbol_position[3]->value="none";
		$symbol_position[3]->text=JText::_('NONE');
		...
		//$lists['update_mail_enable'] = JHTML::_('select.booleanlist', 'update_mail_enable', 'class="inputbox"', UPDATE_MAIL_ENABLE );
		...
		$invoice_mail_send_option[0]->text=JText::_('SELECT');
		$invoice_mail_send_option[1]->value=1;
		$invoice_mail_send_option[1]->text=JText::_('ADMINISTRATOR');
		$invoice_mail_send_option[2]->value=2;
		$invoice_mail_send_option[2]->text=JText::_('CUSTOMER');
		$invoice_mail_send_option[3]->value=3;
		$invoice_mail_send_option[3]->text=JText::_('BOTH');
		...
		$discount_type[0]->text=JText::_('SELECT');
		$discount_type[1]->value=1;
		$discount_type[1]->text=JText::_('DISCOUNT_OR_VOUCHER_OR_COUPON');
		$discount_type[2]->value=2;
		$discount_type[2]->text=JText::_('DISCOUNT_VOUCHER_OR_COUPON');
		$discount_type[3]->value=3;
		$discount_type[3]->text=JText::_('DISCOUNT_VOUCHER_COUPON');
		$discount_type[4]->value=4;
		$discount_type[4]->text=JText::_('DISCOUNT_VOUCHER_COUPON_MULTIPLE');
		...
		$option= array();
		$option[0]->value=0;
		$option[0]->text=JText::_('SELECT');
		$option[1]->value='mm';
		$option[1]->text=JText::_('MILLIMETER');
		$option[2]->value='cm';
		$option[2]->text=JText::_('CENTIMETERS');
		$option[3]->value='inch';
		$option[3]->text=JText::_('INCHES');
		$option[4]->value='feet';
		$option[4]->text=JText::_('FEET');
		$option[5]->value='m';
		$option[5]->text=JText::_('METER');
		...
		$option[0]->text=JText::_('SELECT');
		$option[1]->value='gram';
		$option[1]->text=JText::_('GRAM');
		$option[2]->value='pounds';
		$option[2]->text=JText::_('POUNDS');
		$option[3]->value='kg';
		$option[3]->text=JText::_('KG');
		...
		$superadminlist = $redhelper->getSuperAdminList();
		$superadminid = explode(",", ADMINISTRATOR_EMAIL);
		$lists['administrator_email'] = JHTML::_('select.genericlist',$superadminlist,'administrator_email[]','class="inputbox" multiple="multiple" size="10" ','id','title',$superadminid);
		// end
