...
<table class="adminlist" width="100%">
		...
		<td><?php echo JText::_('COM_REDSHOP_NEW_STATUS' ); ?>: <?php echo $order_function->getstatuslist('order_status_all' ,'',"class=\"inputbox\" size=\"1\" "); ?> </td>
			...
			<button onclick="document.getElementById('filter').value='';document.getElementById('filter_status').value='0';document.getElementById('filter_payment_status').value='';this.form.submit();"><?php echo JText::_('COM_REDSHOP_RESET' ); ?></button>
	...
	<?php 	}?>
			...
			<td>
				<table cellpadding="0" cellspacing="0" border="0">
					...
					<tr>
						<td>
							<input type="checkbox" <?php echo $send_mail_to_customer;?>  value="" name="sendordermail<?php echo $row->order_id ; ?>" id="sendordermail<?php echo $row->order_id ; ?>" /><?php echo JText::_('COM_REDSHOP_SEND_ORDER_MAIL' ); ?>
						</td>
					</tr>
					<tr>
						<!-- <td></td>-->
						<td>
						<input class="button" onclick="location.href = '<?php echo $linkupdate; ?>&status='+document.adminForm.order_status<?php echo $row->order_id ;?>.value+'&customer_note='+encodeURIComponent(document.adminForm.customer_note<?php echo $row->order_id ;?>.value)+'&order_sendordermail='+document.adminForm.sendordermail<?php echo $row->order_id ;?>.checked+'&order_paymentstatus='+document.adminForm.order_paymentstatus<?php echo $row->order_id ;?>.value  ; " name="order_status" value="<?php echo JText::_('COM_REDSHOP_UPDATE_STATUS_BUTTON');?>" type="button">
						</td>
					</tr>
				</table>
			</td>
			...
			<td align="center"> <?php
			 if($stockroom_id != "")
			 {
				...
				echo $datediff." ".$max_delivery[0]->delivery_time;  } ?> </td>
				<?php  } ?>
			<td align="center">
