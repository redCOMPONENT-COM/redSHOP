...
?>		window.location = '<?php echo $link;?>';
	...
	if ((pressbutton=='allstatus'))
	{
		...
		{
		alert('<?php echo JText::_( 'SELECT_NEW_STATUS' ); ?>');
		return false;
		}
...
<table class="adminlist" width="100%">
		<tr>
		<td><?php echo JText::_( 'NEW_STATUS' ); ?>: <?php echo $order_function->getstatuslist('order_status_all' ,'',"class=\"inputbox\" size=\"1\" "); ?> </td>
			...
			<button onclick="document.getElementById('filter').value='';document.getElementById('filter_status').value='0';document.getElementById('filter_payment_status').value='';this.form.submit();"><?php echo JText::_( 'RESET' ); ?></button>
	...
	<thead>
		<tr>
			...
			<th width="10%">
				<?php echo  JHTML::_('grid.sort', 'BOOKINVOICE_NUMBER', 'bookinvoice_number', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<?php } ?>
			<th width="15%">
			...
			<th width="15%">
				<?php echo JText::_('STOCKROOM_NAME'); ?>
			</th>
			<th width="10%">
				<?php echo JText::_('STOCKROOM_DELIVERY_TIME'); ?>
			</th>
			<?php } ?>
			...
			<th width="7%">
				<?php echo JHTML::_('grid.sort', 'ORDER_TOTAL', 'order_total', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th>
			</th>
	...
	<?php 	}?>
			...
			<td>
				<table cellpadding="0" cellspacing="0" border="0">
					...
					<tr>
						<td><textarea name="customer_note<?php echo $row->order_id ;?>"><?php echo $row->customer_note;?></textarea></td>
						<!--<td>
							<input class="inputbox" name="ic<?php echo $row->order_id ;?>" id="include_comment<?php echo $row->order_id ;?>" value="N" type="checkbox" onclick="if(this.checked==true) {this.value = 'Y';} else {this.value = 'N';}"><?php echo JText::_( 'INCLUDE_COMMENT_MSG' ); ?><br />
							<input class="inputbox" name="nc<?php echo $row->order_id ;?>" id="notify_customer<?php echo $row->order_id ;?>" value="N" type="checkbox" onclick="if(this.checked==true) {this.value = 'Y';} else {this.value = 'N';}"><?php echo JText::_( 'NOTIFY_CUSTOMER_MSG' ); ?>
						</td>-->
					</tr>
					<tr>
						<td>
							<input type="checkbox"  value="" name="sendordermail<?php echo $row->order_id ; ?>" id="sendordermail<?php echo $row->order_id ; ?>" /><?php echo JText::_( 'SEND_ORDER_MAIL' ); ?>
						</td>
					</tr>
					<tr>
						<!-- <td></td>-->
						<td>
						<input class="button" onclick="location.href = '<?php echo $linkupdate; ?>&status='+document.adminForm.order_status<?php echo $row->order_id ;?>.value+'&customer_note='+encodeURIComponent(document.adminForm.customer_note<?php echo $row->order_id ;?>.value)+'&order_sendordermail='+document.adminForm.sendordermail<?php echo $row->order_id ;?>.checked+'&order_paymentstatus='+document.adminForm.order_paymentstatus<?php echo $row->order_id ;?>.value  ; " name="order_status" value="<?php echo JText::_('UPDATE_STATUS_BUTTON');?>" type="button">
						</td>
					</tr>
				</table>
			</td>
			...
			if($stockroom_id!="")
			{
				...
				for($s=0;$s<count($stockroom_list);$s++)
			 	...
			 	//echo $delivery_time= $stockroom_list[$st]->min_del_time."-".$stockroom_list[$st]->max_del_time." ".$stockroom_list[$st]->delivery_time;
			 	//echo "<br>";
			?>
			...
			<td><?php if($row->invoice_no !='')
			{
				if($row->is_booked==0)
				{
					if($row->is_company==1 && $row->ean_number!="")
					{
						echo JText::_('MANUALY_BOOK_INVOICE_FROM_ECONOMIC');
					}
					else
					{
						//bookInvoiceDate
						$confirm = 'if(confirm(\''.JText::_('CONFIRM_BOOK_INVOICE').'\')) { document.invoice.order_id.value=\''.$row->order_id.'\';document.invoice.bookInvoiceDate.value=document.getElementById(\'bookDate'.$i.'\').value;document.invoice.submit(); }';
					...
					<input type="button" class="button" value="<?php echo JTEXT::_("BOOK_INVOICE");?>" onclick="javascript:<?php echo $confirm;?>"><br/>
			<?php	}
				}
				elseif($row->is_booked==1 && $row->bookinvoice_date!=0)
				{
					echo JText::_('INVOICE_BOOKED_ON')."<br />".$config->convertDateFormat($row->bookinvoice_date);
				}
			}
			echo "</td>";
