...
$OrdersDetail = $order_functions->getOrderDetails($mysplit[$k]);

$billing = $order_functions->getOrderBillingUserInfo ($OrdersDetail->order_id);
$shipping = $order_functions->getOrderShippingUserInfo ($OrdersDetail->order_id);
...
."<img src=".$url."images/M_images/printButton.png  alt='".JText::_('PRINT')."' title='".JText::_('PRINT')."' /></a>";

$search[] = "{print}";
$replace[] = $print_tag;

$search[]  = "{order_id}";
$replace[]  = $OrdersDetail->order_id;

$search[]  = "{order_number}";
$replace[]  = $OrdersDetail->order_number;

$search[]  = "{order_date}";
$replace[]  = $config->convertDateFormat($OrdersDetail->cdate );
...
$shipping_method = '';
$shipping_rate_name = '';
if($OrdersDetail->ship_method_id!='')
{
	$ship_method = explode ( "|", $shippinghelper->decryptShipping( str_replace(" ","+",$OrdersDetail->ship_method_id) ));
	if(count($ship_method)<=1)
	{
		$ship_method = explode( "|", $OrdersDetail->ship_method_id);
	}
	$shipping_method 	= "";
	$shipping_rate_name	= "";
