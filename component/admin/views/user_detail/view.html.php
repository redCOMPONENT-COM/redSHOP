...
class user_detailVIEWuser_detail extends JView
{
	function display($tpl = null)
	{
		...
		if($shipping)
		{
			JToolBarHelper::title(   JText::_( 'USER_SHIPPING_DETAIL' ).': <small><small>[ ' . $text.' ]</small></small>', 'redshop_user48' );
		}
		else
		{
			JToolBarHelper::title(   JText::_( 'USER_MANAGEMENT_DETAIL' ).': <small><small>[ ' . $text.' ]</small></small>' , 'redshop_user48' );
		}
		...
		if ($isNew) {
			...
			JToolBarHelper::customX ( 'order', 'redshop_order32', '', JText::_('PLACE_ORDER'), false);
...
//		$lists['approved'] 			= JHTML::_('select.booleanlist',  'approved', 'class="inputbox"', $detail->approved );
		...
		$lists['is_company'] 		= JHTML::_('select.booleanlist',  'is_company', 'class="inputbox" onchange="showOfflineCompanyOrCustomer(this.value);" ', $detail->is_company ,JText::_('USER_COMPANY'),JText::_('USER_CUSTOMER') );
