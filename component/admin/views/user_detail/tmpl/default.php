...
function submitbutton(pressbutton)
{
	...
	{	?>
		if (trim(form.firstname.value) == "") {
			alert( "<?php echo JText::_ ( 'YOUR_MUST_PROVIDE_A_FIRSTNAME', true );?>" );
		} else if (form.lastname.value == "") {
			alert( "<?php echo JText::_ ( 'YOUR_MUST_PROVIDE_A_LASTTNAME', true );?>" );
		} else {
			submitform( pressbutton );
		}
<?php
	...
	{	?>
		if (trim(form.email.value) == "") {
			alert( "<?php echo JText::_ ( 'PROVIDE_EMAIL_ADDRESS', true );?>" );
			return false;
		}
		else if (email_valid ==0) {
			alert( "<?php echo JText::_ ( 'EMAIL_NOT_AVAILABLE', true );?>" );
			return false;
		}
<?php	if(!$this->silerntuser)
		{	?>
			if (form.username.value == "") {
				...
				alert( "<?php echo JText::_ ( 'USERNAME_NOT_AVAILABLE', true );?>" );
				return false;
			} else if (form.gid.value == "") {
				alert( "<?php echo JText::_ ( 'ASSAIGN_USER_GROUP', true );?>" );
				return false;
...
<?php 	}	?>
		...
		if (form.firstname.value== "") {
			alert( "<?php echo JText::_ ( 'PLEASE_ENTER_FIRST_NAME', true );?>" );
			return false;
		}
		if (form.lastname.value == "") {
			alert( "<?php echo JText::_ ( 'PLEASE_ENTER_LAST_NAME', true );?>" );
			return false;
		}
		if (document.getElementById('is_company1').checked)
		{
			...
			{
				alert( "<?php echo JText::_ ( 'PLEASE_ENTER_COMPANY_NAME', true );?>" );
				return false;
			}
			/*if (form.ean_number.value == "")
			{
				alert( "<?php echo JText::_ ( 'PLEASE_ENTER_EAN_NUMBER', true );?>" );
				return false;
			}*/
		}
		submitform( pressbutton );
...
{
	// Create 1st Tab
	echo $myTabs->startPanel ( JText::_ ( 'GENERAL_USER_INFO' ), 'tab1' );
	echo $this->loadTemplate ( 'user' );
	echo $myTabs->endPanel ();
	//Create 2nd Tab
}

$title = ($this->shipping == 1) ? JText::_ ( 'SHIPPING_INFORMATION' ) : JText::_ ( 'BILLING_INFORMATION' );
...
{
	echo $myTabs->startPanel ( JText::_ ( 'SHIPPING_INFORMATION' ), 'tab3' );
...
{
	echo $myTabs->startPanel ( JText::_ ( 'ORDER_INFORMATION' ), 'tab4' );
...
{
	echo $myTabs->startPanel ( JText::_ ( 'EXTRA_FIELD' ), 'tab5' );	?>
