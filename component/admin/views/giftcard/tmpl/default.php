...
--><table class="adminlist">
	<thead>
		<tr>
			...
			<th class="title">
				<?php
				echo JHTML::_ ( 'grid.sort', 'GIFTCARD_NAME', 'giftcard_name', $this->lists ['order_Dir'], $this->lists ['order'] );
				?>
				
			</th>
			<th>
				<?php
				echo JHTML::_ ( 'grid.sort', 'GIFTCARD_IMAGE', 'giftcard_image', $this->lists ['order_Dir'], $this->lists ['order'] );
				?>
		 	</th>
		 	<th>
				<?php
				echo JHTML::_ ( 'grid.sort', 'GIFTCARD_BGIMAGE', 'giftcard_bgimage', $this->lists ['order_Dir'], $this->lists ['order'] );
				?>
		 	</th>
			<th width="5%">
				<?php
				echo JHTML::_ ( 'grid.sort', 'GIFTCARD_PRICE', 'giftcard_price', $this->lists ['order_Dir'], $this->lists ['order'] );
				?>
		 	</th>
		 	<th width="5%">
				<?php
				echo JHTML::_ ( 'grid.sort', 'GIFTCARD_VALUE', 'giftcard_value', $this->lists ['order_Dir'], $this->lists ['order'] );
				?>
		 	</th>
		 	<th width="5%">
				<?php
				echo JHTML::_ ( 'grid.sort', 'GIFTCARD_VALIDITY', 'giftcard_validity', $this->lists ['order_Dir'], $this->lists ['order'] );
				?>
		 	</th>
	...
	for($i = 0, $n = count ( $this->giftcard ); $i < $n; $i ++) {
		...
		?>"
			title="<?php
		echo JText::_ ( 'EDIT_GIFTCARD' );
		?>"><?php
		echo $row->giftcard_name;
		?></a>
		</td>
		<td>
		<?php $giftcard_path = 'components/'.$option.'/assets/images/giftcard/'.$row->giftcard_image;
				if(is_file(JPATH_SITE.DS.$giftcard_path)) {	?>
				<a class="modal" href="<?php echo $url.$giftcard_path;?>"	title="<?php echo JText::_ ( 'VIEW_IMAGE' );?>"  rel="{handler: 'image', size: {}}">
			<?php echo $row->giftcard_image;?></a>
			<?php }	?>
		</td>
		<td>
		<?php $giftcard_bgpath = 'components/'.$option.'/assets/images/giftcard/'.$row->giftcard_bgimage;
				if(is_file(JPATH_SITE.DS.$giftcard_bgpath)) {	?>
				<a class="modal" href="<?php echo $url.$giftcard_bgpath;?>"	title="<?php echo JText::_ ( 'VIEW_IMAGE' );?>"  rel="{handler: 'image', size: {}}">
