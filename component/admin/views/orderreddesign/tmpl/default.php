...
<form action="<?php echo JRoute::_('index.php?option='.$option.'&view=orderreddesign'); ?>" method="post" name="adminForm" >
<div id="editcell">
<table class="adminlist">		 
		...
		<td><?php echo JText::_( 'NEW_STATUS' ); ?>: <?php echo $order_function->getstatuslist('order_status_all' ,'',"class=\"inputbox\" size=\"1\" "); ?> </td>
			...
			<button onclick="document.getElementById('filter').value='';document.getElementById('filter_status').value='0';this.form.submit();"><?php echo JText::_( 'RESET' ); ?></button>
	...
	for ($i=0, $n=count( $this->orders ); $i < $n; $i++)
	{
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<table cellpadding="0" cellspacing="0" border="0">
					...
					<tr>
						<td></td>
						<td>
						<input class="button" onclick="if(document.adminForm.order_status<?php echo $row->order_id ;?>.value=='<?php echo $row->order_status ;?>') { alert('Please change the Order Status first!'); return false;} else {  location.href = '<?php echo $linkupdate; ?>&status='+document.adminForm.order_status<?php echo $row->order_id ;?>.value+'&nc='+document.adminForm.nc<?php echo $row->order_id ;?>.value +'&ic='+document.adminForm.ic<?php echo $row->order_id ;?>.value+'&order_comment='+document.adminForm.order_comment<?php echo $row->order_id ;?>.value+'&order_paymentstatus='+document.adminForm.order_paymentstatus<?php echo $row->order_id ;?>.value  ; }" name="order_status" value="Update Status" type="button">
						</td>
					</tr>
				</table>
			</td>
			...
			?>
				<a href="../components/com_reddesign/assets/order/pdf/<?php echo $reddesigndetail[$design_i]->reddesignfile; ?>.pdf" target="_blank"  > <img src="<?php echo $live_site; ?>components/com_reddesign/assets/images/design.png" border="0"  alt="" /></a><br />
				<!--<a href="index3.php?option=com_redshop&view=orderreddesign&task=downloaddesign&filename=<?php echo $reddesigndetail[0]->reddesignfile; ?>&type=pdf"    > <img src="<?php echo $live_site; ?>components/com_reddesign/assets/images/design.png" border="0"  alt="" /></a>-->
			<?php } ?>
			</td>
			<td align="center">
