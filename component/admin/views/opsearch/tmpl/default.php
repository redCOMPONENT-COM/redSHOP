...
<form action="index.php?option=com_redshop" method="post" name="adminForm" id="adminForm">
        ...
        <?php if ($showbuttons != 1) {
            ...
            <div class="filterTool">
                <div class="filterItem">
                    ...
                    if (
                        $this->state->get('filter_product') && ($productData = \Redshop\Product\Product::getProductById(
                            $this->state->get('filter_product')
                        )
                        )
                    ) {
                    ...
                    ?>
                    <button class="btn"
                        onclick="document.getElementById('filter_product').value='0';document.getElementById('filter_user').value='0';document.getElementById('filter_status').value='0';this.form.submit();">
                        <?php echo Text::_(
                            'COM_REDSHOP_RESET'
                        ); ?>
                    </button>
        ...
        <table class="adminlist table table-striped">
            <thead>
                <tr>
                    ...
                    <th width="5%">
                        <?php echo Text::_('COM_REDSHOP_QUANTITY'); ?>
                    </th>
                    <th width="20%">
                        <?php echo JHTML::_(
                            'grid.sort',
                            'COM_REDSHOP_FULLNAME',
                            'fullname',
                            $this->lists['order_Dir'],
                            $this->lists['order']
                        ); ?>
            ...
            for ($i = 0, $n = count($this->products); $i < $n; $i++) {
                ...
                <tr class="<?php echo "row$k"; ?>">
                    ...
                    <td align="center">
                        <?php echo $row->order_item_sku; ?>
                    </td>
                    <td align="center"><a href="<?php echo $link_order; ?>" title="<?php echo Text::_(
                           'COM_REDSHOP_EDIT_ORDER'
                       ); ?>">
    ...
    <input type="hidden" name="view" value="opsearch" /> <input type="hidden" name="boxchecked" value="0" /> <input
        type="hidden" name="filter_order" value="<?php echo $this->lists['order']; ?>" /> <input type="hidden"
        name="filter_order_Dir" value="<?php echo $this->lists['order_Dir']; ?>" />
</form>