...
class addorder_detailVIEWaddorder_detail extends JView
{
	function display($tpl = null)
	{
		...
		$shippingop[0]->text = JText::_('SELECT');
		...
		JToolBarHelper::title(JText::_( 'ORDER' ).': <small><small>[ '.JText::_( 'NEW' ).' ]</small></small>', 'redshop_order48' );
		if($err=='' && array_key_exists("users_info_id",$billing) && $billing->users_info_id) 
		{
			...
			JToolBarHelper::custom('save_without_sendmail','save.png','save_f2.png',JText::_( 'SAVE_WITHOUT_SEND_ORDERMAIL_LBL' ),false);
			JToolBarHelper::save();
		}
		...
		$countryarray = $Redconfiguration->getCountryList($shipping,"country_code_ST","ST");
		...
		$statearray = $Redconfiguration->getStateList($shipping,"state_code_ST","country_code_ST","ST",1);
		$lists['state_code_ST'] = $statearray['state_dropdown'];
		
		$lists['is_company'] = JHTML::_('select.booleanlist',  'is_company', 'class="inputbox" onchange="showOfflineCompanyOrCustomer(this.value);" ', $billing->is_company ,JText::_('USER_COMPANY'),JText::_('USER_CUSTOMER') );
