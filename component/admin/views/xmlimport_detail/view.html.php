...
class RedshopViewXmlimport_detail extends RedshopViewAdmin
{
    ...
    public function display($tpl = null)
    {
        ...
        JHtml::script('com_redshop/redshop.xmlfunc.min.js', false, true);
        ...
        $lists['auto_sync']          = JHTML::_(
            'select.booleanlist',
            'auto_sync',
            'class="inputbox" size="1"',
            $detail->auto_sync
        );
        $lists['sync_on_request']    = JHTML::_(
            'select.booleanlist',
            'sync_on_request',
            'class="inputbox" size="1"',
            $detail->sync_on_request
        );
        $lists['auto_sync_interval'] = JHTML::_(
            'select.genericlist',
            $auto_sync_interval,
            'auto_sync_interval',
            'class="inputbox" size="1" ',
            'value',
            'text',
            $detail->auto_sync_interval
        );
        $lists['override_existing']  = JHTML::_(
            'select.booleanlist',
            'override_existing',
            'class="inputbox" size="1"',
            $detail->override_existing
        );
        $lists['xmlpublished']       = JHTML::_(
            'select.booleanlist',
            'xmlpublished',
            'class="inputbox"',
            $detail->published
        ...
        $lists['section_type'] = JHTML::_(
            'select.genericlist',
            $section_type,
            'section_type',
            'class="inputbox" size="1" onchange="setExportSectionType();" ',
            'value',
            'text',
            $detail->section_type
        );
        ...
        if ($detail->section_type != "") {
            ...
            if (count($xmlbillingtag) > 0) {
                ...
                for ($i = 0, $in = count($xmlbillingtag); $i < $in; $i++) {
                    $colvalue                            = $xmlhelper->getXMLFileTag(
                        $xmlbillingtag[$i],
                        $detail->xmlimport_billingtag
                    );
                    $updatebillingtag[$i]                = $colvalue[1];
                    $lists["bill_" . $xmlbillingtag[$i]] = JHTML::_(
                        'select.genericlist',
                        $columns,
                        "bill_" . $xmlbillingtag[$i],
                        'class="inputbox" size="1" ',
                        'value',
                        'text',
                        $colvalue[0]
            ...
            if (count($xmlshippingtag) > 0) {
                ...
                for ($i = 0, $in = count($xmlshippingtag); $i < $in; $i++) {
                    $colvalue                              = $xmlhelper->getXMLFileTag(
                        $xmlshippingtag[$i],
                        $detail->xmlimport_shippingtag
                    );
                    $updateshippingtag[$i]                 = $colvalue[1];
                    $lists["shipp_" . $xmlshippingtag[$i]] = JHTML::_(
                        'select.genericlist',
                        $columns,
                        "shipp_" . $xmlshippingtag[$i],
                        'class="inputbox" size="1" ',
                        'value',
                        'text',
                        $colvalue[0]
            ...
            if (count($xmlitemtag) > 0) {
                ...
                for ($i = 0, $in = count($xmlitemtag); $i < $in; $i++) {
                    $colvalue                         = $xmlhelper->getXMLFileTag(
                        $xmlitemtag[$i],
                        $detail->xmlimport_orderitemtag
                    );
                    $updateitemtag[$i]                = $colvalue[1];
                    $lists["item_" . $xmlitemtag[$i]] = JHTML::_(
                        'select.genericlist',
                        $columns,
                        "item_" . $xmlitemtag[$i],
                        'class="inputbox" size="1" ',
            ...
            if (count($xmlstocktag) > 0) {
                ...
                for ($i = 0, $in = count($xmlstocktag); $i < $in; $i++) {
                    $colvalue                           = $xmlhelper->getXMLFileTag(
                        $xmlstocktag[$i],
                        $detail->xmlimport_stocktag
                    );
                    $updatestocktag[$i]                 = $colvalue[1];
                    $lists["stock_" . $xmlstocktag[$i]] = JHTML::_(
                        'select.genericlist',
                        $columns,
                        "stock_" . $xmlstocktag[$i],
                        'class="inputbox" size="1" ',
                        'value',
                        'text',
                        $colvalue[0]
            ...
            if (count($xmlprdextrafieldtag) > 0) {
                ...
                for ($i = 0, $in = count($xmlprdextrafieldtag); $i < $in; $i++) {
                    $colvalue                                    = $xmlhelper->getXMLFileTag(
                        $xmlprdextrafieldtag[$i],
                        $detail->xmlimport_prdextrafieldtag
                    );
                    $updateprdexttag[$i]                         = $colvalue[1];
                    $lists["prdext_" . $xmlprdextrafieldtag[$i]] = JHTML::_(
                        'select.genericlist',
                        $columns,
                        "prdext_" . $xmlprdextrafieldtag[$i],
                        'class="inputbox" size="1" ',
                        'value',
                        'text',
                        $colvalue[0]
