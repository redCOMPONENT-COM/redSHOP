        ...
        <table class="adminlist table table-striped">
            ...
            <tr>
                ...
                <th class="title" width="5%">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_QUOTATION_NUMBER', 'quotation_number', $this->lists['order_Dir'], $this->lists['order']); ?>
                </th>
                <th width="20%">
					<?php echo JText::_('COM_REDSHOP_FULLNAME'); ?>
                </th>
                <th width="20%">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_QUOTATION_STATUS', 'quotation_status', $this->lists['order_Dir'], $this->lists['order']); ?>
                </th>
                <th width="10%">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_TOTAL', 'quotation_total', $this->lists['order_Dir'], $this->lists['order']); ?>
                </th>
                <th width="10%" nowrap="nowrap">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_QUOTATION_DATE', 'quotation_cdate', $this->lists['order_Dir'], $this->lists['order']); ?>
                </th>
                <th class="title" width="5%">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_QUOTATION_ID', 'quotation_id', $this->lists['order_Dir'], $this->lists['order']); ?>
                </th>
            </tr>
            </thead>
			<?php $k = 0; ?>
			<?php foreach ($this->quotation as $index => $quotation): ?>
				<?php
				$display = $quotation->user_email;
				if ($quotation->user_id)
				{
					$userInformation = $producthelper->getUserInformation($quotation->user_id);

					if (!empty($userInformation))
					{
						$display = $userInformation->firstname . ' ' . $userInformation->lastname;
						$display .= ($userInformation->is_company && $userInformation->company_name != "") ? "<br>" . $userInformation->company_name : "";
					}
				}

				$link   = JRoute::_('index.php?option=com_redshop&view=quotation_detail&task=edit&cid[]=' . $quotation->quotation_id);
				$status = RedshopHelperQuotation::getQuotationStatusName($quotation->quotation_status);
				if ($quotation->quotation_status == 5)
				{
					$status .= " (" . JText::_('COM_REDSHOP_ORDER_ID') . "-" . $quotation->order_id . " )";
				}
				?>
