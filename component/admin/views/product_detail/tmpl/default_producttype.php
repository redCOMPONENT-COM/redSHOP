...
<table>
      <tr>
        <td><?php echo JText::_('REDDESIGN_NAME'); ?> </td>
        <td><?php echo $this->lists['designlist']; ?><?php echo JHTML::tooltip(JText::_('REDDESIGN_TIP'), JText::_('REDDESIGN_NAME'), 'tooltip.png', '', '', false); ?> </td>
      </tr>
      <tr>
        <td><?php echo JText::_('SHOPPERGROUP_NAME'); ?> </td>
        <td><table><tr><td><?php
					...
					<?php echo JHTML::tooltip(JText::_('SHOPPERGROUP_TIP'), JText::_('SHOPPERGROUP_NAME'), 'tooltip.png', '', '', false); ?>
					</td>
					</tr></table>
        </td>
      </tr>
      <?php // Edited 210110 ?>
      <tr>
        <td><?php echo JText::_('REDDESIGN_ENABLE_NAME'); ?> </td>
        <td><?php echo $this->lists['reddesign_enable']; ?><?php echo JHTML::tooltip(JText::_('REDDESIGN_ENABLE_TIP'), JText::_('REDDESIGN_ENABLE_NAME'), 'tooltip.png', '', '', false); ?>  </td>
      </tr>
    </table>
  <?php
}
...
<div id="div_file" style="display:<?php echo $div_file; ?>;">
	<table class="admintable">
		<tr>
            <td valign="top" align="right" class="key"><?php echo JText::_( 'PRODUCT_DOWNLOAD' ); ?>: </td>
        ...
        <tr id="download_limit" <?php echo $td_style;?> >
            <td valign="top" align="right" class="key"><?php echo JText::_( 'PRODUCT_DOWNLOAD_LIMIT_LBL' ); ?>: </td>
        ...
        <tr id="download_days" <?php echo $td_style;?> >
            <td valign="top" align="right" class="key"><?php echo JText::_( 'PRODUCT_DOWNLOAD_DAYS_LBL' ); ?>: </td>
        ...
        <tr id="download_clock" <?php echo $td_style;?> >
            <td valign="top" align="right" class="key"><?php echo JText::_( 'PRODUCT_DOWNLOAD_CLOCK_LBL' ); ?>: </td>
            <td>
		...
		<tr>
			<td class="key"><?php echo JText::_('UPLOAD_CSV_FILE')?>:</td>
	...
	<table class="adminlist">
		<tr>
			...
			<th><?php echo JText::_('SERIAL_NUMBERS'); ?></th>
			<th><?php echo JText::_('IS_USED'); ?></th>
		</tr>
	<?php
	$serial = 1;
	for($si=0;$si<$total_serial;$si++) {
		...
		<tr>
			<td><a href='index.php?option=com_redshop&view=product_detail&task=deleteProdcutSerialNumbers&serial_id=<?php echo $serial_row->serial_id;?>&product_id=<?php echo $this->detail->product_id;?>'><img class="delete_item" src="<?php echo JURI::root(); ?>/components/com_redshop/assets/images/cross.jpg" title="<?php echo JText::_('DELETE'); ?>" alt="<?php echo JText::_('DELETE');?>" ></a></td>
			<td><?php echo $serial; ?></td>
			<td><?php echo $serial_row->serial_number; ?></td>
			<td><?php echo $serial_row->is_used ? JText::_('YES') : JText::_('NO'); ?></td>
		</tr>
	<?php
	$serial++;
	} ?>
...
<div id="div_subscription" style="display:<?php echo $div_subscription; ?>;">
	<table id="tbl_scbscription" class="adminlist" >
		<tr>
			<td colspan="4"><input type="button" value="<?php echo JText::_('ADD'); ?>" onClick="add_subscription_row();" /></td>
		</tr>
		<tr>
			<th><?php echo JText::_('NUM');?></th>
			<th><?php echo JText::_('SUBSCRIPTION_PERIOD')." ";
			echo JHTML::tooltip(JText::_('SUBSCRIPTION_PERIOD_TIP'), JText::_('SUBSCRIPTION_PERIOD'), 'tooltip.png', '', '', false);
			?></th>
			<th><?php echo JText::_('SUBSCRIPTION_PRICE');?>
			<?php echo JHTML::tooltip(JText::_('SUBSCRIPTION_PRICE_TIP'), JText::_('SUBSCRIPTION_PRICE'), 'tooltip.png', '', '', false); ?>
			</th>
			<th><?php echo JText::_('DELETE');?></th>
		</tr>
		<?php
			$option = array();
			$option[]   	= JHTML::_('select.option', 'days', JText::_('DAYS'));
			$option[]   	= JHTML::_('select.option', 'month', JText::_('MONTH'));
			$option[]   	= JHTML::_('select.option', 'year', JText::_('YEAR'));
		...
		<tr id="tr_subsc<?php echo $sub; ?>">
			...
			<td><input type="button" value="X" title="<?php echo JText::_('DELETE'); ?>" name="btndelete" onClick="delete_subscription_row(<?php echo $sub;?>);" /></td>
		</tr>
		<?php } ?>
	</table>
	<br />
	<table>
	<tr>
	<td class="key"><?php echo JText::_('SUBSCRIPTION_RENEWAL_MAIL'); ?>:
	</td>
	<td><input type="text" name="before_no_days" value="<?php echo isset($renewal_detail[0]->before_no_days) ? $renewal_detail[0]->before_no_days : 1; ?>" maxlength="2" size="3" />
		...
		<?php echo JText::_('DAYS_BEFORE_SUBSCRIPTION_END'); ?>
	</td>
	</tr>
	</table>
</div>
...
function add_subscription_row()
{
	...
	newTD1.innerHTML = '<input type="text" name="subscription_period[]" class="input" size="10" /><?php echo " ".JHTML::_('select.genericlist',$option,  'period_type[]', 'class="inputbox" size="1" ' , 'value', 'text'); ?>';
	newTD2.innerHTML = '<input type="text" name="subscription_price[]" class="input" size="10" />';
	newTD3.innerHTML = '<input type="button" value="X" name="btndelete" title="<?php echo JText::_('DELETE'); ?>" onClick="delete_subscription_row('+rowCount+');" />';
