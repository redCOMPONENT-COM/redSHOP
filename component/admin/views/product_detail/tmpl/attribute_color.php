...
<table class="admintable" border="0" width="100%" id="admintable">
	<tr>		
		...
		<?php echo JText::_ ( 'SUBATTRIBUTE_NAME' ); ?>
		</td>
		<td align="left" colspan="1" class="key">
		<?php echo JText::_ ( 'PROPERTY_SUB_IMAGE' ); ?>
		</td>
		<td align="left" colspan="1" class="key">
		<input type="button" name="addvalue" id="addvalue" class="button" Value="<?php echo JText::_( 'ADD' ); ?>" onclick="addNewRowOfsub('admintable');" />
		</td>	
	</tr>
	<?php
	if(count($images) >0){ 
	...
	<tr>		
		...
		<input type="hidden" name="property_sub_img_tmp[]" value="<?php echo $image->subattribute_color_image;?>" />
		<input type="hidden" name="subattribute_color_id[]" value="<?php echo $image->subattribute_color_id;?>" />		
		</td>
		<td>
		<input value="<?php echo JText::_( 'DELETE' ); ?>" onclick="deleteRowOfsub(this)" class="button" type="button" />		
		</td>		
	</tr>
	<?php 
	}
