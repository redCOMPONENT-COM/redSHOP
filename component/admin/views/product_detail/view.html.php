	...
	function display($tpl = null)
	{
		...
		if (0) {
			...
			foreach ($types as $key => $type) {
				/* Add the type */
				$html .= '<option value="">'.JText::_('TYPE_LIST').' '.$type['type_name'].'</option>';
				/* Add the tags */
				if (count($type['tags']) > 0) {
					foreach ($type['tags'] as $tagid => $tag) {
						...
						$html .= '<option '.$selected.' value="'.$tagid.'" >--- '.JText::_('TAG_LIST').' '.$tag['tag_name'].'</option>';
					}
				}
			}
			$html .= '</select>';
		...
		if (count($types) > 0)
		{
			foreach ($types as $typeid => $type) {
				...
				$rand = rand();
				/* Add the type */
				$html .= '<div class="select_box_parent" onClick="showBox('.$rand.')">'.JText::_('TYPE_LIST').' '.$type['type_name'].'</div>';
				...
				if ($counttags > 0) {
					foreach ($type['tags'] as $tagid => $tag) {
						...
						$html .= '<input type="checkbox" class="select_box" '.$selected.' name="tag_id[]" value="'.$typeid.'.'.$tagid.'" />'.JText::_('TAG_LIST').' '.$tag['tag_name'];
						$html .= '<br />';
						if (@array_key_exists($typeid.'.'.$tagid, $qs)) $qs_value = $qs[$typeid.'.'.$tagid]['quality_score'];
						else $qs_value = '';

						$html .= '<span class="quality_score">'.JTEXT::_('QUALITY_SCORE').'</span> <input type="text" class="quality_score_input"  name="qs_id['.$typeid.'.'.$tagid.']" value="'.$qs_value.'" />';
						$html .= '<br />';
						...
						$html .= '</select>&nbsp;<a href="#" onClick="javascript:add_dependency('.$typeid.','.$tagid.','.$detail->product_id.');" >'.JText::_('ADD_DEPENDENCY').'</a><br />';
					}
				}
				$html .= '</div>';
			}
		...
		$temps[0]->template_name=JText::_('SELECT');
		...
		JToolBarHelper::title(JText::_( 'PRODUCT_MANAGEMENT_DETAIL' ),'redshop_products48');

		$document = &JFactory::getDocument();

		$document->addScriptDeclaration ("

		var WANT_TO_DELETE = '".JText::_('DO_WANT_TO_DELETE')."';
		...
		$text = $isNew ? JText::_( 'NEW' ) : $detail->product_name." - ".JText::_( 'EDIT' );

		JToolBarHelper::title(   JText::_( 'PRODUCT' ).': <small><small>[ ' . $text.' ]</small></small>' , 'redshop_products48' );

		if($detail->product_id > 0){
//			JToolBarHelper::addNewX('wrapper',JText::_( 'ADD_WRAPPER' ));
			JToolBarHelper::addNewX('prices',JText::_( 'ADD_PRICE_LBL' ));
		}
...
//------------------------- End ----------------------------------
		...
		$option = array();
		$option[]   	= JHTML::_('select.option', '1', JText::_('RANGE'));
		$option[]   	= JHTML::_('select.option', '0', JText::_('PRICE_PER_PIECE'));
		...
		$option[]   	= JHTML::_('select.option', 'volume', JText::_('VOLUME'));
		$option[]   	= JHTML::_('select.option', 'area', JText::_('AREA'));
		$option[]   	= JHTML::_('select.option', 'circumference', JText::_('CIRCUMFERENCE'));
		...
		// calculation UNIT
		$option[]   	= JHTML::_('select.option', 'mm', JText::_('MILLIMETER'));
		$option[]   	= JHTML::_('select.option', 'cm', JText::_('CENTIMETER'));
		$option[]   	= JHTML::_('select.option', 'm', JText::_('METER'));
		...
		/*$prod_types = array();
		$prod_types[] = JHTML::_('select.option',JText::_( 'REAL_PRODUCT'),JText::_( 'REAL_PRODUCT') );
		$prod_types[] = JHTML::_('select.option',JText::_( 'CONTAINER_PRODUCT'),JText::_( 'CONTAINER_PRODUCT'));
		...
		$prop_oprand = array();
		$prop_oprand[] = JHTML::_('select.option','select',JText::_( 'SELECT') );
		$prop_oprand[] = JHTML::_('select.option','+',JText::_( 'PLUS'));
		$prop_oprand[] = JHTML::_('select.option','=',JText::_( 'EQUAL'));
		$prop_oprand[] = JHTML::_('select.option','-',JText::_( 'MINUS'));
	...
	//	$append_to_global_seo[] = JHTML::_('select.option','select',JText::_( 'SELECT') );
		$append_to_global_seo[] = JHTML::_('select.option','append',JText::_( 'APPEND_TO_GLOBAL_SEO'));
		$append_to_global_seo[] = JHTML::_('select.option','prepend',JText::_( 'PREPEND_TO_GLOBAL_SEO'));
		$append_to_global_seo[] = JHTML::_('select.option','replace',JText::_( 'REPLACE_TO_GLOBAL_SEO'));
		...
		if($CheckRedDesign)
		{
			...
			$optiondtype = array();
			$optiondtype[] = JHTML::_('select.option', '0',JText::_('SELECT'));
		...
		// reddesign End
		 ...
		 $product_type_opt[] = JHTML::_('select.option','product',JText::_( 'PRODUCT') );

		 if($CheckRedDesign)
		 	$product_type_opt[] = JHTML::_('select.option','design',JText::_( 'DESIGN'));

		 $product_type_opt[] = JHTML::_('select.option','file',JText::_( 'FILE'));
		 $product_type_opt[] = JHTML::_('select.option','subscription',JText::_( 'SUBSCRIPTION'));
		...
		$op = array();
		$op[] = JHTML::_('select.option', '0',JText::_('SELECT'));
