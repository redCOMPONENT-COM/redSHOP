					...
					if (count($stockrooms) > 0)
					{
						foreach ($stockrooms as $s)
						{
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="quantity[]" size="5" class="text_area input-small"
                                           value="<?php echo $quantity; ?>"/>
                                    <input type="hidden" name="stockroom_id[]" value="<?php echo $s->stockroom_id; ?>"/>
                                </td>
                                <td>
                                    <input type="text" name="preorder_stock[]" size="5" class="text_area input-small"
                                           value="<?php echo $preorder_stock; ?>"/>
                                    <input type="button" class="btn btn-small"
                                           name="preorder_reset"
                                           value="<?php echo JText::_('COM_REDSHOP_RESET'); ?>"
                                           onclick="location.href = 'index.php?option=com_redshop&view=product_detail&task=ResetPreorderStockBank&stockroom_type=<?php echo $section ?>&section_id=<?php echo $section_id ?>&cid=<?php echo $cid ?>&product_id=<?php echo $this->item->product_id ?>&stockroom_id=<?php echo $s->stockroom_id ?>' ; "
                                    />
                                </td>
                                <td>
