            ...
            <table id="accessory_table" class="adminlist table table-striped" border="0">
				...
				if (count($stockrooms) > 0)
				{
					foreach ($stockrooms as $s)
					{
						$ordered_preorder    = "";
						$preorder_stock      = "";
						$quantity            = $model->StockRoomProductQuantity($this->item->product_id, $s->stockroom_id);
						$preorder_stock_data = $model->StockRoomPreorderProductQuantity($this->item->product_id, $s->stockroom_id);
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="button"
                                       ...
                                       onclick="location.href = 'index.php?option=com_redshop&view=product_detail&task=ResetPreorderStock&stockroom_type=product&product_id=<?php echo $this->item->product_id ?>&stockroom_id=<?php echo $s->stockroom_id ?>' ; ">
				...
				?>
                <input type="hidden" name="product_id" value="<?php echo $this->item->product_id; ?>"/>
