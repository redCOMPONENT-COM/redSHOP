...
$ordering = ($this->order == 'x.ordering');
...
<form action="index.php?option=com_redshop&view=product" method="post" name="adminForm" id="adminForm">
    <div id="editcell">
        ...
        <div class="filterTool">
            ...
            <div class="filterItem">
				<?php echo $this->filterCategoriesHtml; ?>
            </div>
            <div class="filterItem">
				<?php echo $this->filterProductsSortHtml; ?>
            </div>
        </div>
        ...
        <table class="adminlist table table-striped">
            ...
            <tr>
                ...
                <th class="title">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_PRODUCT_NAME', 'p.product_name', $this->orderDir, $this->order); ?>
                </th>
                <th class="title">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_PRODUCT_NUMBER', 'p.product_number', $this->orderDir, $this->order); ?>
                </th>
                <th class="title">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_PRODUCT_PRICE', 'p.product_price', $this->orderDir, $this->order); ?>
                </th>
                ...
                <th>
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_NUMBER_OF_VIEWS', 'p.visited', $this->orderDir, $this->order); ?>
                </th>
                <th>
                ...
                <th width="5%" nowrap="nowrap">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_PUBLISHED', 'p.published', $this->orderDir, $this->order); ?>
                </th>
                <th width="5%" nowrap="nowrap">
					<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_ID', 'p.product_id', $this->orderDir, $this->order); ?>
                </th>
				<?php if ($category_id): ?>
                    <th width="15%" nowrap="nowrap">
						<?php echo JHTML::_('grid.sort', 'COM_REDSHOP_ORDERING', 'x.ordering', $this->orderDir, $this->order); ?>
						<?php if ($ordering): ?>
			...
			<?php foreach ($this->products as $index => $product): ?>
				<?php $link = JRoute::_('index.php?option=com_redshop&view=product_detail&task=edit&cid[]=' . $product->id); ?>
                <tr class="<?php echo "row$key"; ?>">
                    ...
                    <td>
						<?php $canCheckin = $user->authorise('core.manage', 'com_checkin') || $product->checked_out == $userId || $product->checked_out == 0; ?>
					...
					<?php foreach ($this->list_in_products as $lIndex => $listInProduct) : ?>
						<?php
						$field_arr   = RedshopHelperExtrafields::getSectionFieldDataList($listInProduct->id, 1, $product->id);
						$field_value = '';
						?>
						<?php if (!empty($field_arr)): ?>
							<?php $field_value = $field_arr->data_txt; ?>
						<?php endif; ?>
                        <td><?php echo $field_value; ?></td>
					<?php endforeach; ?>

                    <td align="center">
						<?php $mediadetail = $product->mediaDetail; ?>
                        <a class="joom-box"
                           href="index.php?option=com_redshop&view=media&section_id=<?php echo $product->id; ?>&showbuttons=1&media_section=product&section_name=<?php echo $product->product_name; ?>&tmpl=component"
                    ...
                    <td align="center">
						<?php $wrapper = $producthelper->getWrapper($product->id, 0, 1); ?>
                        <a class="joom-box"
                           href="index.php?option=com_redshop&showall=1&view=wrapper&product_id=<?php echo $product->id; ?>&tmpl=component"
                    ...
                    <td>
                        <a href="index.php?option=com_redshop&view=rating_detail&task=edit&cid[]=0&pid=<?php echo $product->id ?>"><?php echo JText::_('COM_REDSHOP_ADD_REVIEW'); ?></a>
                    </td>
                    <td align="center" width="8%">
						<?php echo JHtml::_('jgrid.published', $product->published, $index, '', 1); ?>
                    </td>
					...
					<?php if ($category_id): ?>
                        <td class="order">
							<?php if ($ordering) : ?>
								<?php $orderDir = strtoupper($this->orderDir); ?>
                                <div class="input-prepend">
									<?php if ($orderDir == 'ASC' || empty($orderDir)) : ?>
                                    ...
                                    <input
                                            ...
                                            value="<?php echo $product->ordering; ?>"
