...
function AssignTemplate(){
	...
	if(form.boxchecked.value==0){
		...
		alert('<?php echo JText::_('PLEASE_SELECT_PRODUCT');?>');
...
<tr>
	<td>
		...
		<input type="text"  name="keyword" value="<?php echo $this->keyword;?>"> <input type="submit" value="<?php echo JTEXT::_("SEARCH")?>">
		<select name="search_field" onchange="javascript:document.adminForm2.submit();">
			<option value="p.product_name" <?php if($this->search_field=='p.product_name') echo "selected='selected'";?>><?php echo JTEXT::_("PRODUCT_NAME")?></option>
			<option value="c.category_name" <?php if($this->search_field=='c.category_name') echo "selected='selected'";?>><?php echo JTEXT::_("CATEGORY")?></option>
			<option value="p.product_number" <?php if($this->search_field=='p.product_number') echo "selected='selected'";?>><?php echo JTEXT::_("PRODUCT_NUMBER")?></option>
			<option value="p.name_number" <?php if($this->search_field=='p.name_number') echo "selected='selected'";?>><?php echo JTEXT::_("PRODUCT") . ' ' . JTEXT::_("NAME AND NUMBER"); ?></option>
			<option value="pa.property_number" <?php if($this->search_field=='pa.property_number') echo "selected='selected'";?>><?php echo JText::_("ATTRIBUTE_SKU")?></option>
		</select>
...
<input type="hidden" name="unpublished_data" value="">
	...
	<thead>
		<tr>
			...
			<th class="title">
				<?php echo JHTML::_('grid.sort', 'PRODUCT_NAME', 'p.product_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th class="title">
				<?php echo JHTML::_('grid.sort', 'PRODUCT_NUMBER', 'p.product_number', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th class="title">
				<?php echo JHTML::_('grid.sort', 'PRODUCT_PRICE', 'p.product_price', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<?php
			...
			<th>
		 	...
		 	<th>
				<?php echo JTEXT::_('WRAPPER'); ?>
		 	</th>
		 	<th>
				<?php echo JHTML::_('grid.sort',  'NUMBER_OF_VIEWS', 'p.visited', $this->lists['order_Dir'], $this->lists['order'] ); ?>
		 	</th>
		 	...
		 	<th>
				<?php echo JTEXT::_('CUSTOMER_REVIEWS'); ?>
		 	</th>
			<th width="5%" nowrap="nowrap">
				<?php echo JHTML::_('grid.sort', 'PUBLISHED', 'p.published', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th width="5%" nowrap="nowrap">
				<?php echo JHTML::_('grid.sort', 'ID', 'p.product_id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<?php if($category_id > 0)
			{?>
			<th width="15%" nowrap="nowrap">
				<?php echo JHTML::_('grid.sort',  'ORDERING', 'x.ordering', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				<?php
				if($ordering)
	...
	for ($i=0, $n=count( $this->products ); $i < $n; $i++)
	{
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			if (  JTable::isCheckedOut($this->user->get('id'), $row->checked_out ) ) {
				...
				{
				?>
				<a href="<?php echo $link; ?>" title="<?php echo JText::_( 'EDIT_PRODUCT' ); ?>"><?php echo $row->treename; ?></a>
				<?php
				} else {
				...
				<?php
						...
						?>
				<a href="<?php echo $link; ?>" title="<?php echo JText::_( 'EDIT_PRODUCT' ); ?>"><?php echo $row->treename; ?> </a>[child: <?php echo $pro_array->product_name;?>]
				<?php
					}
				}
			...
			<a class="modal" href="index3.php?option=<?php echo $option;?>&amp;view=media&amp;section_id=<?php echo $row->product_id;?>&amp;showbuttons=1&amp;media_section=product&amp;section_name=<?php echo $row->product_name;?>" rel="{handler: 'iframe', size: {x: 1050, y: 450}}" title=""><img src="components/<?php echo $option;?>/assets/images/media16.png" align="absmiddle" alt="media" >(<?php  echo count($mediadetail);?>)</a>
			</td>
			<td align="center">
			<?php $wrapper = $producthelper->getWrapper($row->product_id,0,1);?>
			<a class="modal" href="index3.php?option=<?php echo $option;?>&showall=1&view=wrapper&product_id=<?php echo $row->product_id;?>" rel="{handler: 'iframe', size: {x: 700, y: 450}}">
			<img src="components/<?php echo $option;?>/assets/images/wrapper16.png" align="absmiddle" alt="<?php echo JText::_('WRAPPER');?>" ><?php echo "(".count($wrapper).")";?></a>
			</td>
			<td align="center">
			...
			<td width="90">
				<a href="index.php?option=com_redshop&view=rating_detail&task=edit&cid[]=0&pid=<?php echo $row->product_id ?>"><?php echo JTEXT::_('ADD_REVIEW'); ?></a>
			</td>
			...
			<td class="order">
				...
				<span><?php ///echo $row->orderdown;
				 ...
				 <input type="text" name="order[]" size="5" <?php echo $disabled; ?> value="<?php echo $row->ordering;?>"   class="text_area" style="text-align: center" />
