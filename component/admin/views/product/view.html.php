...
class productViewproduct extends JView
{
	...
	function display($tpl = null)
	{
		...
		$document->setTitle( JText::_('PRODUCT') );
   	    $layout =  JRequest::getVar ( 'layout' );
   		JToolBarHelper::title(   JText::_( 'PRODUCT_MANAGEMENT' ), 'redshop_products48' );

   		if($layout!='importproduct' && $layout!='importattribute' && $layout!='listing' &&  $layout!='ins_product') {

			JToolBarHelper::customX( 'gbasefeed', 'gbase.png', 'gbase.png', 'Googlebase', true );
			JToolBarHelper::custom( 'assignCategory', 'save.png', 'save_f2.png',JText :: _('ASSIGN_CATEGORY'), true );
			JToolBarHelper::custom( 'removeCategory', 'delete.png', 'delete_f2.png',JText :: _('REMOVE_CATEGORY'), true );
...
//		$product_category = new product_category();
		...
		$temps = array();
		$temps[0]->id="0";
		$temps[0]->treename=JText::_('SELECT');
		$categories=@array_merge($temps,$categories);
	    $lists['category'] 	= JHTML::_('select.genericlist',$categories,  'category_id', 'class="inputbox" onchange="document.adminForm2.submit();" ', 'id', 'treename', $category_id );
...
//		$total = & $this->get( 'Total');
		...
		$temps[0]->template_name = JText::_('ASSIGN_TEMPLATE');
