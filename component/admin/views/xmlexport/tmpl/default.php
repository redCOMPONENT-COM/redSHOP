            ...
            <thead>
                <tr>
                    ...
                    <th class="title" width="15%">
                        <?php echo JHTML::_(
                            'grid.sort',
                            'COM_REDSHOP_XMLEXPORT_DISPLAY_FILENAME',
                            'display_filename',
                    ...
                    <th class="title" width="30%">
                        <?php echo JHTML::_(
                            'grid.sort',
                            'COM_REDSHOP_XMLEXPORT_FILENAME',
                            'filename',
                            $this->lists['order_Dir'],
                            $this->lists['order']
                        ); ?>
            ...
            for ($i = 0, $n = count($this->data); $i < $n; $i++) {
                ...
                <tr class="<?php echo "row$k"; ?>">
                    ...
                    <td><a href="<?php echo $link; ?>" title="<?php echo Text::_(
                           'COM_REDSHOP_EDIT_XMLEXPORT'
                       ); ?>">
                            <?php echo $row->display_filename; ?>
                        </a>
                    </td>
                    <td>
                        <?php if ($row->filename != "") {
                            echo '<a href="' . JURI::root(
                            ) . 'index.php?option=com_redshop&view=category&tmpl=component&task=download&file=' . $row->filename . '" title="' . Text::_(
                                        'COM_REDSHOP_DOWNLOAD'
                                    ) . '" target="_black">' . $row->filename . '</a>';
                        } ?>
                    </td>
                    <td align="center">
                        <?php echo $row->section_type; ?>
                    </td>
                    <td align="center">
                        <?php $row->published = $row->auto_sync;
                        echo $auto_sync = JHTML::_(
                            'grid.published',
                            $row,
                            $i,
                            'tick.png',
                    ...
                    <td align="center">
                        <?php $row->published = $row->use_to_all_users;
                        echo $usetoall = JHTML::_(
                            'grid.published',
                            $row,
                            $i,
                            'tick.png',
