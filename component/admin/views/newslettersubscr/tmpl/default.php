...
<div id="editcell">
	<table width="100%">
			...
			<td valign="top" align="right" class="key">
				<?php echo JText::_( 'NEWSLETTER_FILTER' ); ?>:
	...
	<thead>
		<tr>
			...
			<th>
				<input type="checkbox" name="toggle" value="" onclick="checkAll(<?php echo count( $this->newslettersubscrs ); ?>);" />
			</th>
			<th>
				<?php echo JHTML::_('grid.sort', 'NEWSLETTER_USERNAME', 'user_id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th>
				<?php echo JHTML::_('grid.sort', 'NEWSLETTER_SUBSCR_DATE', 'date', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th>
				<?php echo JHTML::_('grid.sort', 'NEWSLETTER', 'n.newsletter_id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th width="5%" nowrap="nowrap">
				<?php echo JHTML::_('grid.sort', 'PUBLISHED', 'published', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
			<th width="5%" nowrap="nowrap">
				<?php echo JHTML::_('grid.sort', 'ID', 'subscription_id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
	...
	for ($i=0, $n=count( $this->newslettersubscrs ); $i < $n; $i++)
	{
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
			<a href="<?php echo $link; ?>" title="<?php echo JText::_( 'EDIT_NEWSLETTER_SUBSCR' ); ?>"><?php $row->user_id == 0 ?  $name = $row->name : $name = $order_function->getUserFullname($row->user_id);  echo $name;?></a>
			</td>
