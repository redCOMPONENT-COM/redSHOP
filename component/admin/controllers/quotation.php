 ...
 */
class RedshopControllerQuotation extends RedshopControllerForm
{
	...
	public function save($send = 0, $apply = 0)
	{
		...
		if ($row)
		{
			...
			$quotationStatus = $post['status'] > 0 ? $post['status'] : 2;
			RedshopHelperQuotation::updateQuotationStatus($row->id, $quotationStatus);

			if ($send == 1)
			{
				if ($model->sendQuotationMail($row->id) && JFactory::getConfig()->get('mailonline') == 1)
				{
	...
	public function newQuotationItem()
	{
		...
		$this->setRedirect('index.php?option=com_redshop&view=quotation&id=' . $cid[0], $msg);
	}
