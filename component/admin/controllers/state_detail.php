...
class state_detailController extends JController
 {
	...
	function save($apply=0) {
		...
		if ($row)
		{
			...
			$msg = JText::_ ( 'STATE_DETAIL_SAVED' );

		} else
		{

			$msg = JText::_ ( 'ERROR_SAVING_IN_STATE_DETAIL' );
	...
	function cancel() {
		...
		$model = $this->getModel('state_detail');
		$model->checkin();
		$msg = JText::_ ( 'state_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=state',$msg );
	}
	...
	function remove() {
		...
		$msg = JText::_ ( 'state_DETAIL_DELETED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=state',$msg );
	}
