...
class stockroom_detailController extends JController 
{
	...
	function save($apply=0) 
	{
		...
		if ($row=$model->store ( $post )) {
			...
			$msg = JText::_ ( 'STOCKROOM_DETAIL_SAVED' );

		} else {

			$msg = JText::_ ( 'ERROR_SAVING_STOCKROOM_DETAIL' );
		}
	...
	function remove() 
	{
		...
		$msg = JText::_ ( 'STOCK_ROOM_DETAIL_DELETED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=stockroom',$msg );
	}
	...
	function publish() 
	{
		...
		$msg = JText::_ ( 'STOCK_ROOM_DETAIL_PUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=stockroom',$msg );
	}
	...
	function unpublish() 
	{
		...
		$msg = JText::_ ( 'STOCK_ROOM_DETAIL_UNPUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=stockroom',$msg );
	}
	...
	{
		$option = JRequest::getVar('option');
		$msg = JText::_ ( 'STOCK_ROOM_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=stockroom',$msg );
	}
	...
	function copy()
	{
		...
		if ($model->copy($cid)) {
			$msg = JText::_ ( 'STOCK_ROOM_DETAIL_COPIED' );
		} else {
			$msg = JText::_ ( 'ERROR_COPING_STOCKROOM_DETAIL' );
		}
		$this->setRedirect ( 'index.php?option=' . $option . '&view=stockroom', $msg );
	}
		...
		if(ECONOMIC_INTEGRATION == 1)
		{
			...
			for($i=0;$i<count($prd);$i++)
			{
				...
				if(count($ecoProductNumber)>0 && isset($ecoProductNumber[0]))
				{
					...
					$db->setQuery($query);
				 	$db->Query();
					$responcemsg .= "<span style='color: #00ff00'>".JText::_( 'IMPORT_STOCK_FROM_ECONOMIC_SUCCESS' )."</span>";
				}
				else
				{
					$errmsg = JText::_( 'ERROR_IN_IMPORT_STOCK_FROM_ECONOMIC' );
			...
			{
				$msg = JText::_("IMPORT_STOCK_FROM_ECONOMIC_IS_COMPLETED");
			}
		}
