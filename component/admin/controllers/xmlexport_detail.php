...
class xmlexport_detailController extends JController 
{
	...
	function save($export=0) 
	{
		...
		if ($row) 
		{
			if($export==1)
			{
				$msg = JText::_ ( 'XMLEXPORT_FILE_SUCCESSFULLY_SYNCHRONIZED' );
			} else {
				$msg = JText::_ ( 'XMLEXPORT_DETAIL_SAVED' );
			}
		} else {
			if($export==1)
			{
				$msg = JText::_ ( 'ERROR_XMLEXPORT_FILE_SYNCHRONIZED' );
			} else {
				$msg = JText::_ ( 'ERROR_SAVING_XMLEXPORT_DETAIL' );
			}
		}
...
<?php	 
	...
	function remove() 
	{
		...
		$msg = JText::_ ( 'XMLEXPORT_DETAIL_DELETED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlexport',$msg );
	}
	...
	function cancel() 
	{
		...
		$session =& JFactory::getSession();
		$session->set ( 'childelement', NULL );
		$msg = JText::_ ( 'XMLEXPORT_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlexport',$msg );
	}
	/**
	...
	function auto_syncpublish() 
	{
		...
		if (! is_array ( $cid ) || count ( $cid ) < 1) {
			JError::raiseError ( 500, JText::_ ( 'SELECT_AN_ITEM_TO_AUTO_SYNCHRONIZE' ) );
		}
		$model = $this->getModel ( 'xmlexport_detail' );
		if (! $model->auto_syncpublish ( $cid, 1 )) {
			echo "<script> alert('" . $model->getError ( true ) . "'); window.history.go(-1); </script>\n";
		}
		$msg = JText::_ ( 'AUTO_SYNCHRONIZE_ENABLE_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlexport',$msg );
	}
	...
	function auto_syncunpublish() 
	{
		...
		$msg = JText::_ ( 'AUTO_SYNCHRONIZE_DISABLE_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlexport',$msg );
	}
	...
	function usetoallpublish() 
	{
		...
		if (! is_array ( $cid ) || count ( $cid ) < 1) {
			JError::raiseError ( 500, JText::_ ( 'SELECT_AN_ITEM_TO_USE_EXPORTFILE_TO_ALL' ) );
		}
		$model = $this->getModel ( 'xmlexport_detail' );
		if (! $model->usetoallpublish ( $cid, 1 )) {
			echo "<script> alert('" . $model->getError ( true ) . "'); window.history.go(-1); </script>\n";
		}
		$msg = JText::_ ( 'EXPORTFILE_USE_TO_ALL_ENABLE_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlexport',$msg );
	}
	...
	function usetoallunpublish() 
	{
		...
		$msg = JText::_ ( 'EXPORTFILE_USE_TO_ALL_DISABLE_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlexport',$msg );
	}
	...
	function publish()
	{
		...
		$msg = JText::_ ( 'XMLEXPORT_PUBLISHED_SUCCESSFULLY' );
	...
	function unpublish()
	{
		...
		$msg = JText::_ ( 'XMLEXPORT_UNPUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index'.$page.'.php?option='.$option.'&view=xmlexport',$msg );
	}
}?>