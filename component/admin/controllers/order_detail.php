...
class order_detailController extends JController {
	...
	function save() {
		...
		if ($model->store ( $post )) {
			...
			$msg = JText::_ ( 'ORDER_DETAIL_SAVED' );

		} else {

			$msg = JText::_ ( 'ERROR_SAVING_ORDER_DETAIL' );
		}
	...
	function remove() {
		...
		$msg = JText::_ ( 'ORDER_DETAIL_DELETED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=order',$msg );
	}
	function cancel()
	{
		$option = JRequest::getVar('option','','request','string');
		$msg = JText::_ ( 'ORDER_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=order',$msg );
	}
	...
	function neworderitem()
	{
		...
		if(USE_STOCKROOM==1)
		{
			...
			{
				if ($order_item_id == 0)
					$msg = JText::_('ORDER_ITEM_ADDED');
				else
					$msg = JText::_('QUANTITY_UPDATED');
			}else {
				if ($order_item_id == 0)
					$msg = JText::_('ERROR_ADDING_ORDER_ITEM');
				else
					$msg = JText::_('ERROR_UPDATING_QUANTITY');
			}
   	 	}else{
	...
	function delete_item()
	{
		...
		{
			$model->delete($cid);
			$msg = JText::_ ( 'ORDER_DELETED_SUCCESSFULLY' );
		...
		if ($model->delete_item($post)){
			$msg = JText::_('ORDER_ITEM_DELETED');
		}else {
			$msg = JText::_('ERROR_DELETING_ORDER_ITEM');
		}
	...
	function updateItem()
	{
		...
		if ($model->updateItem($post)){
			$msg = JText::_('ORDER_ITEM_PRICE_UPDATED');
		}else {
			$msg = JText::_('ERROR_UPDATING_PRICE');
		}
		 if($tmpl)
		 {
           $this->setRedirect('index.php?option='.$option.'&view=order_detail&cid[]='.$cid[0].'&tmpl='.$tmpl,$msg);
	...
	function update_discount() {
		...
		if ($model->update_discount($post)){
			$msg = JText::_('DISCOUNT_UPDATED');
		}else {
			$msg = JText::_('ERROR_UPDATING_DISCOUNT');
		}
	...
	function special_discount(){
		...
		if ($model->special_discount($post)){
			$msg = JText::_('SPECIAL_DISCOUNT_APPLIED');
		}else {
			$msg = JText::_('ERROR_IN_SPECIAL_DISCOUNT');
		}
	...
	function update_shippingrates(){
		...
		if ($model->update_shippingrates($post)){
			$msg = JText::_('SHIPPING_RATE_UPDATED');
		}else {
			$msg = JText::_('ERROR_UPDATING_SHIPPING_RATE');
		}
	...
	function updateShippingAdd(){
		...
		if ($model->updateShippingAdd($post)){
			...
			$msg = JText::_('SHIPPING_INFORMATION_UPDATED');
		}else{
			$msg = JText::_('ERROR_UPDATING_SHIPPING_INFORMATION');
		}
...
// update billing address information
	function updateBillingAdd(){
		...
		if ($model->updateBillingAdd($post)){
			...
			$msg = JText::_('BILLING_INFORMATION_UPDATED');
		}else{
			$msg = JText::_('ERROR_UPDATING_BILLING_INFORMATION');
		}
	...
	function send_downloadmail()
	{
		...
		if ($model->send_downloadmail($cid[0])){
			$msg = JText::_('DOWNLOAD_MAIL_HAS_BEEN_SENT');
		}else{
			$msg = JText::_('ERROR_DOWNLOAD_MAIL_FAIL');
		}
		if($tmpl)
		{
			 $this->setRedirect('index.php?option='.$option.'&view=order_detail&cid[]='.$cid[0].'&tmpl='.$tmpl,$msg);
	...
	function send_invoicemail()
	{
		...
		if ($redshopMail->sendInvoiceMail($cid[0])){
			$msg = JText::_('INVOICE_MAIL_HAS_BEEN_SENT');
		}else{
			$msg = JText::_('ERROR_INVOICE_MAIL_FAIL');
		}
		if($tmpl)
		{
			 $this->setRedirect('index.php?option='.$option.'&view=order_detail&cid[]='.$cid[0].'&tmpl='.$tmpl,$msg);
	...
	/**
	 * order detail task to trigger plugin event
	 *
	 */
	function archive(){
		...
		$results = $dispatcher->trigger('downloadArchive', array ());
	}
}	?>