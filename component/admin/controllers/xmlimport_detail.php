...
class xmlimport_detailController extends JController 
{
	...
	function save($import=0) 
	{
		...
		if ($row) 
		{
			if($import==1)
			{
				$msg = JText::_ ( 'XMLIMPORT_FILE_SUCCESSFULLY_SYNCHRONIZED' );
			} else {
				$msg = JText::_ ( 'XMLIMPORT_DETAIL_SAVED' );
			}
		} else {
			if($import==1)
			{
				$msg = JText::_ ( 'ERROR_XMLIMPORT_FILE_SYNCHRONIZED' );
			} else {
				$msg = JText::_ ( 'ERROR_SAVING_XMLIMPORT_DETAIL' );
			}
		}
	...
	function remove() 
	{
		...
		$msg = JText::_ ( 'XMLIMPORT_DETAIL_DELETED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlimport',$msg );
	}
	...
	{
		$option = JRequest::getVar('option','','request','string');
		$msg = JText::_ ( 'XMLIMPORT_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=xmlimport',$msg );
	}
	...
	function publish()
	{
		...
		$msg = JText::_ ( 'XMLIMPORT_PUBLISHED_SUCCESSFULLY' );
	...
	function unpublish()
	{
		...
		$msg = JText::_ ( 'XMLIMPORT_UNPUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index'.$page.'.php?option='.$option.'&view=xmlimport',$msg );
	}
}?>