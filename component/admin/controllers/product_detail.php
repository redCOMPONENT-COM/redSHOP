...
class product_detailController extends JController {
	...
	function save($apply=0)
	{
		...
		/*if(isset($post['copy_attribute']))
		{
			...
			/// Extra Field Data Saved ////////////////////////
			$msg = JText::_ ( 'PRODUCT_DETAIL_SAVED' );
			...
			$msg = JText::_ ( 'PRODUCT_NUMBER_ALREADY_EXISTS' );
			$msg = $model->getError();
	...
	function remove() {
		...
		$msg = JText::_ ( 'PRODUCT_DETAIL_DELETED_SUCCESSFULLY' );
	...
	function publish() {
		...
		$msg = JText::_ ( 'PRODUCT_DETAIL_PUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=product',$msg );
	}
	function unpublish() {
		...
		$msg = JText::_ ( 'PRODUCT_DETAIL_UNPUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=product',$msg );
	}
	function cancel() {
		...
		$model = $this->getModel('product_detail');
		$model->checkin();
		$msg = JText::_ ( 'PRODUCT_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=product',$msg );
	}
	function copy(){
		...
		if ($model->copy($cid)) {
			...
			$msg = JText::_ ( 'PRODUCT_COPIED' );

		} else {

			$msg = JText::_ ( 'ERROR_PRODUCT_COPIED' );
		}
...
//			print_r($tmpproptyimagename);
	...
	function media_bank(){
		...
		if(!$folder_path){
			$path = JPATH_COMPONENT_SITE.DS."assets".DS."images";
		...
		$tbl ='';//"<table cellspacing='0' cellpdding='0' width='100%'  border='0'><tr><td align='right' colspan='4'><button type='button' onclick=\"javascript:window.parent.document.getElementById('sbox-window').close();\">".JText::_( 'CLOSE' )."</button></td></tr></table>";
		...
		if($folder_path){
			...
			if($t[$n] != 'assets'){
				if($t[$n] == 'images'){
					$path_bk = JPATH_COMPONENT_SITE.DS."assets".DS."images";
					$dir_path = "components".DS."com_redshop".DS."assets".DS."images".DS.$t[$na];
				}else{
					$path_bk = JPATH_COMPONENT_SITE.DS."assets".DS."images".DS.$t[$n];
...
//				$imgthumbsize = $this->getImageThumbSize($folder_img_bk);
				...
				$link_bk="index3.php?option=com_redshop&view=product_detail&task=media_bank&path=".$path_bk."&dirpath=".$dir_path;
		...
		if ($handle = opendir($path))
		{
			...
			for($f=0;$f<count($files);$f++){
				$link = "index3.php?option=com_redshop&view=product_detail&task=media_bank&folder=1&path=".$files[$f]['fullname']."&dirpath=".$files[$f]['relname'];
			...
			while (false !== ($filename = readdir($handle)))
			{
				...
				if (($info[0] > 50) || ($info[1] > 50)) {
					...
					$tbl.="<td width='25%'><table width='120' height='70' style='background-color:#C0C0C0;' cellspacing='1' cellpdding='1'><tr><td align='center' style='background-color:#FFFFFF;'><a href=\"javascript:window.parent.jimage_insert('".$dir_path.DS.$filename."');window.parent.document.getElementById('sbox-window').close();\"><img width='".$width_60."' height='".$height_60."' alt='".$filename."' src='".$live_path."'></a></td></tr><tr height='15'><td style='background-color:#F7F7F7;' align='center'><label>".substr($filename,0,10)."</label></td></tr></table></td>";
					if($i%4==0)
						$tbl.="</tr><tr>";
					$i++;
				}
	...
	function property_more_img(){
		...
		if($filetype != 'png' && $filetype != 'gif' && $filetype != 'jpeg'  && $filetype != 'jpg' && $main_img['name'] != ''  &&  $filetype_sub != 'png' && $filetype_sub != 'gif' && $filetype_sub != 'jpeg'  && $filetype_sub != 'jpg' && $sub_img['name'][0] != ''){
			$msg = JText::_("FILE_EXTENTION_WRONG_PROPERTY");
			$link = $url."administrator/index3.php?option=com_redshop&view=product_detail&section_id=".$post['section_id']."&cid=".$post['cid']."&layout=property_images&showbuttons=1";
	...
	function deleteimage(){
		...
		if($model->deletesubimage($mediaid)){
			$msg = JText::_("PROPERTY_SUB_IMAGE_IS_DELETE");
			$link = $url."administrator/index3.php?option=com_redshop&view=product_detail&section_id=".$section_id."&cid=".$cid."&layout=property_images&showbuttons=1";
			$this->setRedirect ( $link, $msg );
		}
	}
	...
	function saveAttributeStock(){
		...
		if ($model->SaveAttributeStockroom($post)){
			...
			$msg = JText::_('STOCKROOM_ATTRIBUTE_XREF_SAVE');
		}else{

			$msg = JText::_('ERROR_SAVING_STOCKROOM_ATTRIBUTE_XREF');
		}
		$link = "index3.php?option=com_redshop&view=product_detail&section_id=".$post['section_id']."&cid=".$post['cid']."&layout=productstockroom&property=".$post['section'];
		$this->setRedirect($link,$msg);
	}
	...
	function deleteProdcutSerialNumbers(){
		...
		$msg = JText::_( 'PRODUCT_SERIALNUMBER_DELETED' );
	...
	function ResetPreorderStockBank()
	{
		...
		$link = "index3.php?option=com_redshop&view=product_detail&section_id=".$section_id."&cid=".$cid."&layout=productstockroom&property=".$stockroom_type;
	...
	function DeleteMergeImages($rmdir='mergeImages')
	{ 	
		$dirname = JPATH_COMPONENT_SITE.DS."assets/images/".$rmdir;	
			...
			if ($dir_handle)
			{
				...
				{	 
					...
					{	
						...
						{	
							$thumbDir = 'mergeImages/'.$file;	
							$this->DeleteMergeImages($thumbDir);
							
						}else 	
						{
							if(file_exists($dirname.DS.$file))
							{
								if(!is_writeable($dirname.DS.$file))
								{
									chmod($dirname.DS.$file,0777); 
								}
