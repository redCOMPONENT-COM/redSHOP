        ...
        foreach ($attributes as $a => $attribute) {
            ...
            foreach ($properties as $pIndex => $property) {
                ...
                $property_image                        = $this->input->files->get(
                    'attribute_' . $tmpordering . '_property_' . $tmpproptyimagename[$pIndex] . '_image',
                    array(),
                    'array'
                );
                ...
                if (empty($property['mainImage'])) {
                    if (!empty($property_image['name'])) {
                        $propertiesSave['property_image'] = $model->copy_image(
                            $property_image,
                            'product_attributes',
                            $propertyId
                        );
                ...
                if (!empty($property['mainImage'])) {
                    $propertiesSave['property_image'] = $model->copy_image_from_path(
                        $property['mainImage'],
                        'product_attributes',
                        $propertyId
                    );
                ...
                for ($sp = 0; $sp < count($subproperty) - 1; $sp++) {
                    ...
                    $subproperty_image                              = $this->input->files->get(
                        'attribute_' . $tmpordering . '_property_' . $pIndex . '_subproperty_' . $tmpimagename[$sp] . '_image',
                        array(),
                        'array'
                    );
                    ...
                    if (empty($subproperty[$sp]['mainImage'])) {
                        if (!empty($subproperty_image['name'])) {
                            $subPropertiesSave['subattribute_color_image'] = $model->copy_image(
                                $subproperty_image,
                                'subcolor',
                                $subPropertyId
                            );
    ...
    public function ajaxDisplayAttributeSet()
    {
        ...
        if ($post['attribute_set']) {
            ...
            foreach ($attributes as $attribute) {
                $attribute->propeties = \RedshopHelperProduct_Attribute::getAttributeProperties(
                    0,
                ...
                foreach ($attribute->propeties as $propety) {
                    $propety->subProperties = \RedshopHelperProduct_Attribute::getAttributeSubProperties(
                        0,
                        $propety->property_id
                    );
                }
