...
class newsletter_detailController extends JController {
	...
	function save($apply=0) {
		...
		if ($row=$model->store ( $post )) {
			...
			$msg = JText::_ ( 'NEWSLETTER_DETAIL_SAVED' );

		} else {

			$msg = JText::_ ( 'ERROR_SAVING_NEWSLETTER_DETAIL' );
		}
	...
	function remove() {
		...
		if($val==1)
		$msg=JText::_ ( 'DEFAULT_NEWSLETTER_CAN_NOT_BE_DELETED' );

		else
		$msg = JText::_ ( 'NEWSLETTER_DETAIL_DELETED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=newsletter',$msg);
	}
	function publish() {
		...
		$msg = JText::_ ( 'NEWSLETTER_DETAIL_PUBLISHED_SUCCESFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=newsletter',$msg );
	}
	function unpublish() {
		...
		$msg = JText::_ ( 'NEWSLETTER_DETAIL_UNPUBLISHED_SUCCESFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=newsletter',$msg );
	}
	function cancel() {

		$option = JRequest::getVar ('option');
		$msg = JText::_ ( 'NEWSLETTER_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=newsletter',$msg );
	}
	function copy()
	{
		...
		$model = $this->getModel ( 'newsletter_detail' );
		if ($model->copy($cid)) {
			$msg = JText::_ ( 'NEWSLETTER_COPIED_WITH_SUBSCRIBER' );
		} else {
			$msg = JText::_ ( 'ERROR_COPYING_NEWSLETTER' );
		}
		$this->setRedirect ( 'index.php?option='.$option.'&view=newsletter', $msg );
	}
}?>
