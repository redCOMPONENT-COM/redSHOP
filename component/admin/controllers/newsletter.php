...
class newsletterController extends JController
{
	...
	function sendRecursiveNewsletter()
	{
		...
		for($i=0;$i<count($cid);$i++)
		{
			...
			if($retuser[$i])
			{
				$responcemsg .= "<span style='color: #00ff00'>".JText::_( 'NEWSLETTER_SENT_SUCCESSFULLY' )."</span>";
			} else {
				$responcemsg .= "<span style='color: #ff0000'>".JText::_( 'NEWSLETTER_MAIL_NOT_SENT' )."</span>";
			}
			$responcemsg .= "</div>";
			$incNo++;
		}
