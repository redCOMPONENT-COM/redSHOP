...
class textlibrary_detailController extends JController {
	...
	function save($apply=0) { 
		...
		if ($row=$model->store ( $post )) {
			...
			$msg = JText::_ ( 'TEXTLIBRARY_DETAIL_SAVED' );
		
		} else {
			
			$msg = JText::_ ( 'ERROR_SAVING_TEXTLIBRARY_DETAIL' );
		}
	...
	function remove() {
		...
		$msg = JText::_ ( 'TEXT_LIBRARY_DETAIL_DELETED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=textlibrary',$msg );
	}
	function publish() {
		...
		$msg = JText::_ ( 'TEXT_LIBRARY_DETAIL_PUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=textlibrary',$msg );
	}
	function unpublish() {
		...
		$msg = JText::_ ( 'TEXT_LIBRARY_DETAIL_UNPUBLISHED_SUCCESSFULLY' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=textlibrary',$msg );
	}
	function cancel() {
		
		$option = JRequest::getVar('option','','request','string');
		$msg = JText::_ ( 'TEXT_LIBRARY_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=textlibrary',$msg );
	}
	function copy(){
		...
		if ($model->copy($cid)) {
			...
			$msg = JText::_ ( 'TEXT_LIBRARY_DETAIL_COPIED' );
		
		} else {
			
			$msg = JText::_ ( 'ERROR_COPYING_TEXTLIBRARY_DETAIL' );
		}
