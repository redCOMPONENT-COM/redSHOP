...
class RedshopControllerOrder extends RedshopController
{
    ...
    public function multiprint_order()
    {
        ...
        if (empty($invoicePdf)) {
            $this->setMessage(Text::_('COM_REDSHOP_ERROR_GENERATE_PDF'), 'error');
            $this->setRedirect(Redshop\IO\Route::_('index.php?option=com_redshop&view=order', false));
        }

        $invoiceLink = REDSHOP_FRONT_DOCUMENT_ABSPATH . 'invoice/' . $invoicePdf . '.pdf';
        $this->setMessage(
            JText::sprintf('COM_REDSHOP_ORDER_DOWNLOAD_INVOICE_LINK', $invoiceLink, $invoicePdf . '.pdf')
        );
        ...
        foreach ($orderIds as $orderId) {
            if (file_exists(JPATH_COMPONENT_SITE . "/assets/labels/label_" . $orderId . ".pdf")) {
                ...
                $this->setMessage(
                    JText::sprintf('COM_REDSHOP_ORDER_DOWNLOAD_LABEL', $labelLink, 'label_' . $orderId . '.pdf')
                );
            }
        }
    ...
    public function updateOrderStatus()
    {
        ...
        $response = array(
            'message' => '<li class="success text-success">' . JText::sprintf(
                'COM_REDSHOP_AJAX_ORDER_UPDATE_SUCCESS',
                $orderId
            ) . '</li>'
        );
    ...
    public function bookInvoice()
    {
        ...
        $ecomsg          = Text::_('COM_REDSHOP_INVOICE_NOT_BOOKED_IN_ECONOMIC');
        $msgType         = 'warning';
    ...
    public function createInvoice()
    {
        if (
            ...
            if ($paymentInfo) {
                ...
                $economicdata['economic_payment_terms_id'] = $paymentInfo->plugin->params->get(
                    'economic_payment_terms_id'
                );
                $economicdata['economic_design_layout']    = $paymentInfo->plugin->params->get(
                    'economic_design_layout'
                );
    ...
    public function export_fullorder_data()
    {
        ...
        for ($i = 0, $in = count($data); $i < $in; $i++) {
            ...
            for ($it = 0, $countItem = count($no_items); $it < $countItem; $it++) {
                ...
                $product_attribute = RedshopHelperProduct::makeAttributeOrder(
                    $no_items[$it]->order_item_id,
                    0,
                    $no_items[$it]->product_id,
                    0,
                    1
                );
    ...
    public function export_data()
    {
        ...
        for ($i = 1; $i <= $no_products; $i++) {
            echo Text::_('PRODUCT_NAME') . $i . ',';
            echo Text::_('PRODUCT') . ' ' . Text::_('PRODUCT_PRICE') . $i . ',';
            echo Text::_('PRODUCT_ATTRIBUTE') . $i . ',';
        }
    ...
    public function generateParcel()
    {
        ...
        if ($generate_label == "success") {
            $sussces_message = Text::_('COM_REDSHOP_XML_GENERATED_SUCCESSFULLY');
    ...
    public function download_token()
    {
        ...
        for ($i = 0, $in = count($download_id_arr); $i < $in; $i++) {
            ...
            $endtime = mktime(
                ...
                $clock_min,
                0,
                date("m", $product_download_days_time),
                date("d", $product_download_days_time),
