...
class RedshopControllerAttribute_Set_Detail extends RedshopController
{
    ...
    public function save($apply = 0)
    {
        ...
        if ($apply == 1) {
            $this->setRedirect(
                'index.php?option=com_redshop&view=attribute_set_detail&task=edit&cid[]=' . $row->attribute_set_id,
                $msg
            );
        } else {
        ...
        for ($a = 0, $countAttribute = count($attribute); $a < $countAttribute; $a++) {
            ...
            for ($p = 0, $countProperty = count($property); $p < $countProperty; $p++) {
                ...
                for ($sp = 0; $sp < count($subProperties) - 1; $sp++) {
                    ...
                    $subPropertiesSave['subattribute_show_fe']       = ($subProperties[$sp]['subattribute_show_fe'] == 'on'
                        || $subProperties[$sp]['subattribute_show_fe'] == '1') ? '1' : '0';
                    ...
                    if (empty($subProperties[$sp]['mainImage'])) {
                        if (!empty($subproperty_image['name'])) {
                            $subPropertiesSave['subattribute_color_image'] = $model->copy_image(
                                $subproperty_image,
                                'subcolor',
                                $subproperty_array->subattribute_color_id
                            );
