...
class productController extends JController
{
	...
	function importeconomic()
	{
		...
		if(ECONOMIC_INTEGRATION == 1)
		{
			...
			for($i=0;$i<count($prd);$i++)
			{
				...
				$responcemsg .= "<div>".$incNo.": ".JText::_('PRODUCT_NUMBER')." ".$prd[$i]->product_number." -> ";
				if(count($ecoProductNumber)>0 && is_object($ecoProductNumber[0]) && isset($ecoProductNumber[0]->Number))
				{
					$responcemsg .= "<span style='color: #00ff00'>".JText::_( 'IMPORT_PRODUCTS_TO_ECONOMIC_SUCCESS' )."</span>";
				}
				else
				{
					$errmsg = JText::_( 'ERROR_IN_IMPORT_PRODUCT_TO_ECONOMIC' );
			...
			{
				$msg = JText::_("IMPORT_PRODUCT_TO_ECONOMIC_IS_COMPLETED");
			}
		}
	...
	function importatteco()
	{
		...
		if(ECONOMIC_INTEGRATION == 1 && ATTRIBUTE_AS_PRODUCT_IN_ECONOMIC == 1)
		{
			...
			for($i=0;$i<count($list);$i++)
			{
				...
				$responcemsg .= "<div>".$incNo.": ".JText::_('PROPERTY_NUMBER')." ".$list[$i]->property_number." -> ";
				if(count($ecoProductNumber)>0 && is_object($ecoProductNumber[0]) && isset($ecoProductNumber[0]->Number))
				{
					$responcemsg .= "<span style='color: #00ff00'>".JText::_( 'IMPORT_ATTRIBUTES_TO_ECONOMIC_SUCCESS' )."</span>";
				}
				else
				{
					$errmsg = JText::_( 'ERROR_IN_IMPORT_ATTRIBUTES_TO_ECONOMIC' );
					if(JError::isError(JError::getError()))
					{
						$error = JError::getError();
						$errmsg = $error->message;
			...
			for($i=0;$i<count($list);$i++)
			{
				...
				$responcemsg .= "<div>".$incNo.": ".JText::_('SUBPROPERTY_NUMBER')." ".$list[$i]->subattribute_color_number." -> ";
				if(count($ecoProductNumber)>0 && is_object($ecoProductNumber[0]) && isset($ecoProductNumber[0]->Number))
				{
					$responcemsg .= "<span style='color: #00ff00'>".JText::_( 'IMPORT_ATTRIBUTES_TO_ECONOMIC_SUCCESS' )."</span>";
				}
			...
			{
				$msg = JText::_("IMPORT_ATTRIBUTES_TO_ECONOMIC_IS_COMPLETED");
			}
		}
		echo "<div id='sentresponse'>".$totalprd."`_`".$msg."</div>";
		die();
	...
	function assignTemplate(){
		...
		if($model->assignTemplate($post)){
			$msg = JText::_('TEMPLATE_ASSIGN_SUCESS');
		}else {
			$msg = JText::_('ERROR_ASSIGNING_TEMPLATE');
		}
	...
	function gbasefeed(){
		...
		if($model->gbasefeed($post)){
			$msg = JText::_('GBASE_XML_IS_GENERATED_SUCCESSFULLY');
		}else {
			$msg = JText::_('ERROR_IN_GENERATING_GBASE_XML');
		}
		$this->setRedirect( 'index.php?option=com_redshop&view=product',$msg );
	}
}
