	...
	function save() 
	{
		...
		if($bulkfile['name'] == null && $file['name'][0] == null && $post['oldmedia'] != "")
		{
			if($post['media_bank_image'] == "")
			{
				...
				{
					$msg = JText::_ ( 'MEDIA_DETAIL_SAVED' );
...
//					$model->selectMainProductImage($save->section_id);
					...
					if(isset($post['set']) && $post['media_section'] != 'manufacturer'){
						$this->setRedirect ( 'index3.php?option=' . $option . '&view=media&section_id='.$post['section_id'].'&showbuttons=1&section_name='.$post['section_name'].'&media_section='.$post['media_section'], $msg );
					...
					$msg = JText::_ ( 'ERROR_SAVING_MEDIA_DETAIL' );

	        		if(isset($post['set'])){
						$this->setRedirect ( 'index3.php?option=' .$option . '&view=media_detail&section_id='.$post['section_id'].'&showbuttons=1&section_name='.$post['section_name'].'&media_section='.$post['media_section'], $msg );
	        		}else{
		...
		else
		{
			...
			if($post['hdn_download_file'] != "")
			{
				...
				if($save->media_section == 'product')	{
					$this->setRedirect ( 'index3.php?option=' . $option . '&view=media_detail', $msg );
						 }
				}else{
					$msg = JText::_ ( 'ERROR_SAVING_MEDIA_DETAIL' );
...
// 		       						$model->selectMainProductImage($row->section_id);
			...
			$msg = JText::_ ( 'FILE_EXTENSION_WRONG' );
			if(isset($post['set'])){
				$this->setRedirect ( 'index3.php?option=' . $option . '&view=media_detail&section_id='.$post['section_id'].'&showbuttons=1&section_name='.$post['section_name'].'&media_section='.$post['media_section'], $msg );
			}else{
				$this->setRedirect ( 'index.php?option=' . $option . '&view=media_detail', $msg );
	...
	if($file['name'][0] != ''){
		...
		for($i=0;$i<$num;$i++){
			...
			if($filetype != 'png' && $filetype != 'gif' && $filetype != 'jpeg'  && $filetype != 'jpg' && $filetype != 'zip' && $filetype != 'mpeg' && $filetype != 'mp4' && $filetype != 'avi' && $filetype != '3gp' && $filetype != 'swf' && $filetype != 'pdf' && $post['media_type'] != 'download' && $post['media_type'] != 'document'){
				...
				$msg = JText::_ ( 'SELECT_MEDIA_SECTION_FIRST' );
				if(isset($post['set'])){
					$this->setRedirect ( 'index3.php?option=' . $option . '&view=media_detail&section_id='.$post['section_id'].'&showbuttons=1&section_name='.$post['section_name'].'&media_section='.$post['media_section'] , $msg );
				}else{
					$this->setRedirect ( 'index.php?option=' . $option . '&view=media_detail', $msg );
				...
				$msg = JText::_ ( 'PLEASE_SELECT_BULK_OPTION' );
				if(isset($post['set'])){
					$this->setRedirect ( 'index3.php?option=' . $option . '&view=media_detail&section_id='.$post['section_id'].'&showbuttons=1&section_name='.$post['section_name'].'&media_section='.$post['media_section'] , $msg );
				}else{
					$this->setRedirect ( 'index.php?option=' . $option . '&view=media_detail', $msg );
				...
				$msg = JText::_ ( 'YOU_HAVE_SELECTED_NO_OPTION' );
				if(isset($post['set'])){
					$this->setRedirect ( 'index3.php?option=' . $option . '&view=media_detail&section_id='.$post['section_id'].'&showbuttons=1&section_name='.$post['section_name'].'&media_section='.$post['media_section'] , $msg );
				}else{
					$this->setRedirect ( 'index.php?option=' . $option . '&view=media_detail', $msg );
...
//						}
	...
	function remove() 
	{
		...
		$msg = JText::_ ( 'MEDIA_DETAIL_DELETED_SUCCESSFULLY' );
		if(isset($section_id))
		{
			$this->setRedirect ( 'index3.php?option='.$option.'&view=media&section_id='.$section_id.'&showbuttons=1&media_section='.$media_section,$msg );
		}
		else if(isset($post['set']) && $post['media_section'] == 'manufacturer')
		{
			$link 	= 'index.php?option=' . $option . '&view=manufacturer';	?>
	...
	function publish() 
	{
		...
		$msg = JText::_ ( 'MEDIA_DETAIL_PUBLISHED_SUCCESSFULLY' );
		if(isset($section_id))
		{
			$this->setRedirect ( 'index3.php?option='.$option.'&view=media&section_id='.$section_id.'&showbuttons=1&media_section='.$media_section,$msg );
		}
	...
	function defaultmedia() 
	{
		...
		if (! is_array ( $cid ) || count ( $cid ) < 1) 
		{
			JError::raiseError ( 500, JText::_ ( 'SELECT_AN_ITEM_TO_MAKE_PRIMARY_MEDIA' ) );
		}
	...
	function unpublish() 
	{
		...
		$msg = JText::_ ( 'MEDIA_DETAIL_UNPUBLISHED_SUCCESSFULLY' );
		if(isset($section_id))
		{
			$this->setRedirect ( 'index3.php?option='.$option.'&view=media&section_id='.$section_id.'&showbuttons=1&media_section='.$media_section,$msg );
		}
	...
	{
		$option = JRequest::getVar ('option');
		$msg = JText::_ ( 'MEDIA_DETAIL_EDITING_CANCELLED' );
		$this->setRedirect ( 'index.php?option='.$option.'&view=media',$msg );
	}
	...
	function saveorder()
	{
		...
		if (! is_array ( $cid ) || count ( $cid ) < 1)
		{
			JError::raiseError ( 500, JText::_ ( 'SELECT_ORDERING' ) );
		}
		$model = $this->getModel('media_detail');
