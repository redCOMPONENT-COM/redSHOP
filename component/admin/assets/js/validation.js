...
function chk_preorder()
{
	if(document.getElementById("porder1").checked)
	{
				...
				xmlHttp=xml_object();
				 ...
				 xmlHttp.open("GET","index3.php?option=com_redshop&view=product_container&showbuttons=1&print_display=1&preorder=1",true);
				 xmlHttp.send(null);
...
function removePropertyImage(image,task){
	...
	if(con == true){	
		...
		request.open("GET", "index3.php?option=com_redshop&view=product_detail&task=remove"+task+"Image&pid="+id, true);
		request.send(null);
	}
}
