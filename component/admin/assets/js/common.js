...
//		alert("Product ID is missing");
	...
	var url="index.php?option=com_redshop&view=order_detail&tmpl=component&task=displayProductItemInfo";
	url=url+pval;
...
//			}
			...
			if(document.getElementById("tdShipping"))
			{
				...
				var newurl="index.php?option=com_redshop&view=addorder_detail&layout=productorderinfo&tmpl=component&ordertotal="+ordertotal+"&ordersubtotal="+ordersubtotal+"&productarr="+prdArr+"&qntarr="+qntArr + "&shipp_users_info_id="+shipp_users_info_id+"&order_user_id="+order_user_id;
...
function changeOfflinePropertyDropdown(product_id,accessory_id,attribute_id,unique_id)
{
	...
	var url = "index.php?option=com_redshop&view=addquotation_detail&tmpl=component&task=displayOfflineSubProperty";
	url = url + suburl;
//	alert(url);
	xmlhttp = GetXmlHttpObject();
	xmlhttp.onreadystatechange=function() {
...
function calculateOfflineShipping()
{
	if(document.getElementById("shipping_rate_id"))
	{
		...
		var url = "index.php?option=com_redshop&view=addorder_detail&tmpl=component&task=getShippingRate&shipping_rate_id=" + order_shipping;
		xmlhttp = GetXmlHttpObject();
		xmlhttp.onreadystatechange=function() 
		{
			if(xmlhttp.readyState == 4)
			{
				...
				var displaytotal = parseFloat(subtotal) + parseFloat(order_shipping) + parseFloat(order_shipping_vat);
				if(document.getElementById("divFinalTotal"))
				{
					document.getElementById("divFinalTotal").innerHTML = number_format(displaytotal,PRICE_DECIMAL,PRICE_SEPERATOR,THOUSAND_SEPERATOR);
				}
...
function getQuotationDetail(unqid)
{
	...
	var url = "index.php?option=com_redshop&view=quotation_detail&tmpl=component&task=getQuotationPriceTax";
	url = url + suburl;
	xmlhttp = GetXmlHttpObject();
	xmlhttp.onreadystatechange=function() 
	{
...
function getStateList(){
	...
	if (xmlhttp==null)
	{
		...
		return;
	       ...
	       var url="index.php?option=com_redshop&view=shipping_rate_detail&tmpl=component&task=GetStateDropdown&shipping_rate_id="+shipping_rate_id;
