...
class redshopMail {
	...
	function sendOrderMail($order_id, $reddesign = 0)
	{
		...
		$message = str_replace ( "{order_mail_intro_text_title}", JText::_('ORDER_MAIL_INTRO_TEXT_TITLE'), $message );
		$message = str_replace ( "{order_mail_intro_text}", JText::_('ORDER_MAIL_INTRO_TEXT'), $message );
		...
		{
			$issplitdisplay = "<br/>" . JText::_ ( 'RECEIPT_PARTIALLY_PAID_AMOUNT' ) . ": " . $producthelper->getProductFormattedPrice ( $split_amount );
			$issplitdisplay2 = "<br/>" . JText::_ ( 'REMAINING_PARTIALLY_AMOUNT' ) . ": " . $producthelper->getProductFormattedPrice ( $split_amount );
		}
		...
		$search [] = "{debitor_number_lbl}";
		$replace [] = JText::_ ( "DEBITOR_NUMBER" );
		$search [] = "{debitor_number}";
...
//			$message = $loopmessage_start . implode ( "<br>", $reddesign_msg ) . $loopmessage_end;
		...
		if ($email != "")
		{
			...
			if($thirdpartyemail!='')
			{
				...
				{
					$this->setError ( JText::_('ERROR_SENDING_CONFIRMATION_MAIL' ));
				}
			}
	...
	function sendOrderSpecialDiscountMail($order_id)
	{
		...
		$replace [] = ""; //$paymentmethod->payment_method_name;
		...
		$replace [] = JText::_('SPECIAL_DISCOUNT');
	...
	function createMultiprintInvoicePdf($oid)
	{
		...
		for($o=0;$o<count($oid);$o++)
		{
			...
			$print_tag = "<a onclick='window.print();' title='".JText::_('PRINT')."'>"
			."<img src=".JSYSTEM_IMAGES_PATH."printButton.png  alt='".JText::_('PRINT')."' title='".JText::_('PRINT')."' /></a>";
			...
			$search[]  = "{order_number}";
			$replace[]  = $OrdersDetail->order_number;

			$search[]  = "{order_date}";
			$replace[]  = $redconfig->convertDateFormat($OrdersDetail->cdate );
			...
			// set order paymethod name
			$search[] = "{payment_lbl}";
			$replace[] = JText::_('ORDER_PAYMENT_METHOD');

			$search[] = "{payment_method}";
			$replace[] = $paymentmethod->order_payment_name;

			$statustext = $this->_order_functions->getOrderStatusTitle( $OrdersDetail->order_status );
			...
			$ordersprint_template=$this->_carthelper->replaceShippingMethod($OrdersDetail,$ordersprint_template);
			$ordersprint_template=$this->_carthelper->replaceBillingAddress($ordersprint_template,$billing);
			$ordersprint_template=$this->_carthelper->replaceShippingAddress($ordersprint_template,$shipping);
			...
			for($i = 0; $i < count ( $OrderProducts ); $i ++) {
				...
				if($OrderProducts[$i]->wrapper_id)
				{
					...
					if(count($wrapper)>0)
					{
						$wrapper_name = JText::_('WRAPPER').":<br/>".$wrapper[0]->wrapper_name."(".$producthelper->getProductFormattedPrice($OrderProducts[$i]->wrapper_price).")";
					}
				}
			...
			$replace[]  = $producthelper->getProductFormattedPrice($OrdersDetail->order_subtotal);

			if( $OrdersDetail->order_tax <=0){
				...
				$replace[] 	= '';
				$search[]  = "{order_tax}";
				$replace[]  = $producthelper->getProductFormattedPrice($OrdersDetail->order_tax);
				$search[]  = "{tax}";
				$replace[]  = $producthelper->getProductFormattedPrice($OrdersDetail->order_tax);
				$search[] = "{vat_lbl}";
			...
			if( $OrdersDetail->payment_discount <=0)
			{
				...
				$OrdersDetail->order_discount = $OrdersDetail->order_discount - $OrdersDetail->payment_discount;
				$search[] 	= "{if payment_discount}";
				$replace[] 	= '';
				$search[]  = "{payment_order_discount}";
				$replace[]  = $producthelper->getProductFormattedPrice($OrdersDetail->payment_discount);
				$search[] = "{payment_discount_lbl}";
				$replace[] = JText::_('PAYMENT_DISCOUNT_LBL');
			...
			if($OrdersDetail->order_discount<=0)
			{
				...
				$replace[]  = $producthelper->getProductFormattedPrice($OrdersDetail->order_discount);
				$search[] = "{discount_lbl}";
				$replace[] = JText::_('ORDER_DISCOUNT');
			...
			$search[] = "{order_id_lbl}";
			$replace[] = JText::_('ORDER_ID');

			$search[] = "{order_number_lbl}";
			$replace[] = JText::_('ORDER_NUMBER');

			$search[] = "{order_date_lbl}";
			$replace[] = JText::_('ORDER_DATE');

			$search[] = "{order_status_lbl}";
			$replace[] = JText::_('ORDER_STATUS');

			$search[] = "{shipping_lbl}";
			$replace[] = JText::_('ORDER_SHIPPING');

			$search[] = "{order_information_lbl}";
			$replace[] = JText::_('ORDER_INFORMATION');

			$search[] = "{billing_address_information_lbl}";
			$replace[] = JText::_('BILLING_ADDRESS_INFORMATION');

			$search[] = "{shipping_address_info_lbl}";
			$replace[] = JText::_('SHIPPING_ADDRESS_INFORMATION');

			$search[] = "{order_detail_lbl}";
			$replace[] = JText::_('ORDER_DETAILS');

			$search[] = "{product_name_lbl}";
			$replace[] = JText::_('PRODUCT_NAME');

			$search[] = "{note_lbl}";
			$replace[] = JText::_('NOTE');

			$search[] = "{price_lbl}";
			$replace[] = JText::_('PRICE');

			$search[] = "{quantity_lbl}";
			$replace[] = JText::_('QUANTITY');

			$search[] = "{total_price_lbl}";
			$replace[] = JText::_('TOTAL_PRICE');

			$search[] = "{order_subtotal_lbl}";
			$replace[] = JText::_('ORDER_SUBTOTAL');

			$search[] = "{product_number_lbl}";
			$replace[] = JText::_('PRODUCT_NUMBER');

			$search[] = "{total_lbl}";
			$replace[] = JText::_('ORDER_TOTAL');

			$search[]  = "{order_shipping}";
			$replace[]  = $producthelper->getProductFormattedPrice($OrdersDetail->order_shipping);

			$search[]  = "{shipping}";
			$replace[]  = $producthelper->getProductFormattedPrice($OrdersDetail->order_shipping);

			$search[]  = "{order_total}";
			$replace[] = $producthelper->getProductFormattedPrice($OrdersDetail->order_total);
	...
	function createShippedInvoicePdf($order_id)
	{
		...
		if (count ( $mailinfo ) > 0)
		{
			$message_desc = $mailinfo [0]->template_desc;
			//$subject = $mailinfo [0]->mail_subject;
		}
		else
		{
		...
		for($o=0;$o<count($oid);$o++)
		{
			...
			{
				$img_url = JPATH_SITE."/components/com_redshop/assets/images/barcode/".$barcode_code.".png";
			// for pdf
	...
	function sendInvoiceMail($order_id)
	{
		...
		$body = $message;
		$body1 = $message;
        $img_url1 = JUri::root()."components/com_redshop/assets/images/barcode/".$barcode_code.".png";
		$img_url = JPATH_SITE."/components/com_redshop/assets/images/barcode/".$barcode_code.".png";
...
// for mail
		...
		$pdfObj = new TCPDF ( PDF_PAGE_ORIENTATION, PDF_UNIT, 'A5', true, 'UTF-8', false );
		$pdfObj->SetTitle ( JTEXT::_ ( 'INVOICE' ) . $row->order_id );
		$pdfObj->SetAuthor ( 'redSHOP' );
		$pdfObj->SetCreator ( 'redSHOP' );
	...
	function sendRegistrationMail(&$data)
	{
		...
		if(USE_TAX_EXEMPT && $data['tax_exempt']==1)
		{
			$this->sendTaxExemptMail("tax_exempt_waiting_approval_mail", $data, $admin_mail);
		}
		//tax exempt waiting approval mail
		return true;
	}
	...
	function sendQuotationMail($quotation_id, $status = 0)
	{
		...
		for($i=0;$i<count($rowitem);$i++)
		{
			...
			if($rowitem[$i]->product_wrapperid)
			{
				...
				$wrapper_name = JText::_('WRAPPER').": ".$wrapper_name;
			}
...
//		$cartArr = array();
//		$cartArr = $this->_carthelper->repalceOrderItems($template_middle,$rowitem);
//		$cart= $cartArr[0];
		...
		else
		{
			if ($row->quotation_email != "") {
				...
				$billadd .= JText::_ ( "EMAIL" ).' : '.$row->quotation_email.'<br />';

			}
			$message=str_replace("{billing_address_information_lbl}",JText::_('BILLING_ADDRESS_INFORMATION_LBL'),$message);
...
//		echo $row->quotation_status;exit;
		...
		{
			$total_lbl 		= JText::_('TOTAL_LBL');
			$subtotal_lbl 	= JText::_ ( 'QUOTATION_SUBTOTAL' );
			$vat_lbl 		= JText::_ ( 'QUOTATION_VAT' );
		}
		...
		$replace [] = "<a href='" . $quotationdetailurl . "'>" . JText::_ ( "QUOTATION_DETAILS" ) . "</a>";
	...
	function sendNewsletterConfirmationMail($subscription_id)
	{
		if(NEWSLETTER_CONFIRMATION)
		{
			...
			$link = '<a href="' . $url . 'index.php?option=' . $option . '&view=newsletter&sid=' . $subscription_id . '">' . JText::_ ( 'CLICK_HERE' ) . '</a>';
