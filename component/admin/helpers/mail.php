...
class redshopMail
{
	...
	public function sendQuotationMail($quotation_id, $status = 0)
	{
		...
		for ($i = 0; $i < count($rowitem); $i++)
		{
			...
			$cart_mdata = str_replace("{product_attribute}", $producthelper->makeAttributeQuotation(
					$rowitem[$i]->quotation_item_id,
					0,
					$rowitem[$i]->product_id, $row->quotation_status), $cart_mdata
			);

			$cart_mdata = str_replace("{product_accessory}", $producthelper->makeAccessoryQuotation(
					$rowitem[$i]->quotation_item_id,
