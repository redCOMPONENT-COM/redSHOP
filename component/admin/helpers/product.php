...
class adminproducthelper {
	...
	function replaceAccessoryData($product_id=0,$accessory=array(),$user_id=0,$uniqueid="")
	{
		...
		if($totalAccessory>0)
		{
			$accessorylist .= "<tr><th>".JText::_('ACCESSORY_PRODUCT')."</th></tr>";
	...
	function replaceAttributeData($product_id=0,$accessory_id=0,$attributes=array(),$user_id,$uniqueid="")
	{
		...
		$attributelist .= '<span id="att_lebl" style="display:none;">'.JText::_('ATTRIBUTE_IS_REQUIRED').'</span>';
	...
	function replaceWrapperData($product_id=0,$user_id,$uniqueid="")
	{
		...
		if(count($wrapper)>0)
		{
			...
			$warray [0]->wrapper_name = JText::_ ( 'SELECT' );
			...
			$wrapperlist .= "<tr><td>".JText::_('WRAPPER')." : ".$lists ['wrapper_id']."</td></tr>";
		}
		return $wrapperlist;
	}
	...
	function replaceShippingMethod($d=array(),$shipp_users_info_id=0,$shipping_rate_id=0,$shipping_box_post_id=0)
	{
		...
		if($shipp_users_info_id > 0 )
		{
			...
			if(count($ratearr)>0)
			{
				...
				$displayrespoce = JText::_( 'NO_SHIPPING_METHODS_TO_DISPLAY' );
			}
		}
		else
		{
			$displayrespoce = '<div class="shipnotice">'.JText::_( 'FILL_SHIPPING_ADDRESS' ).'</div>';
		}
		return $displayrespoce;
	}
	...
	function replaceUserfield($product_id=0,$template_id=0,$unique_id="")
	{
		...
		{
			 $result_arr = $extra_field->list_all_user_fields($returnArr[1][$ui],12,"",$commonid);
			 $hidden_arr = $extra_field->list_all_user_fields($returnArr[1][$ui],12,"hidden",$commonid);
			 if($result_arr[0]!="")
	...
	function getProductrBySortedList()
	{
		...
		$product_data[0]->text=JText::_('SELECT' );
		$product_data[1]->value="p.published";
		$product_data[1]->text=JText::_('PRODUCT_PUBLISHED' );
		$product_data[2]->value="p.unpublished";
		$product_data[2]->text=JText::_('PRODUCT_UNPUBLISHED' );
		$product_data[3]->value="p.product_on_sale";
		$product_data[3]->text=JText::_('PRODUCT_ON_SALE' );
		$product_data[4]->value="p.product_not_on_sale";
		$product_data[4]->text=JText::_('PRODUCT_NOT_ON_SALE' );
		$product_data[5]->value="p.product_special";
		$product_data[5]->text=JText::_('PRODUCT_SPECIAL' );
		$product_data[6]->value="p.expired";
		$product_data[6]->text=JText::_('PRODUCT_EXPIRED' );
		$product_data[7]->value="p.not_for_sale";
		$product_data[7]->text=JText::_('PRODUCT_NOT_FOR_SALE' );
		$product_data[8]->value="p.sold_out";
		$product_data[8]->text=JText::_('PRODUCT_SOLD_OUT' );
		return $product_data;
	}
}?>