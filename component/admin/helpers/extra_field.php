 ...
 */
	...
	function list_all_field($field_section="",$section_id=0,$field_name="",$table="",$template_desc="")
	{
		...
		for($i=0;$i<count($row_data);$i++)
		{
			...
			$ex_field .= '<tr>';
			$extra_field_value = "";
			$extra_field_lable = $row_data[$i]->field_title;
			...
			{
				$required = ' required="1" ';
				$reqlbl = ' reqlbl="'.$extra_field_lable.'" ';
				$errormsg = ' errormsg="'.JText::_('THIS_FIELD_IS_REQUIRED').'" ';
			}
			...
			switch ($type)
			{
				case 1: // 1 :- Text Field
						...
						$extra_field_value = '<input class="'.$row_data[$i]->field_class.'" '.$required.$reqlbl.$errormsg.' type="text" maxlength="'.$row_data[$i]->field_maxlength.'"  name="'.$row_data[$i]->field_name.'"  id="'.$row_data[$i]->field_name.'" value="'.htmlspecialchars($text_value).'" size="'.$size.'" />';
						$ex_field .='<td valign="top" width="100" align="right" class="key">'.$extra_field_lable.'</td>';
						$ex_field .='<td>'.$extra_field_value;
				        break;
				case 2: // 2 :- Text Area
						$textarea_value=($data_value && $data_value->data_txt) ? $data_value->data_txt : '';
						$extra_field_value = '<textarea class="'.$row_data[$i]->field_class.'" '.$required.$reqlbl.$errormsg.' name="'.$row_data[$i]->field_name.'"  id="'.$row_data[$i]->field_name.'" cols="'.$row_data[$i]->field_cols.'" rows="'.$row_data[$i]->field_rows.'" >'.htmlspecialchars($textarea_value).'</textarea>';
						$ex_field .='<td valign="top" width="100" align="right" class="key">'.$extra_field_lable.'</td>';
						$ex_field .='<td>'.$extra_field_value;
				        break;
				case 3: // 3 :- Check Box
						...
						for($c=0;$c<count($field_chk);$c++)
						{
							...
							$extra_field_value .='<input  class="'.$row_data[$i]->field_class.'" '.$required.$reqlbl.$errormsg.' type="checkbox"  '.$checked.' name="'.$row_data[$i]->field_name.'[]"  id="'.$row_data[$i]->field_name."_".$field_chk[$c]->value_id.'" value="'.$field_chk[$c]->field_value.'" />'.$field_chk[$c]->field_value.'<br />';
						}
						$ex_field .='<td>'.$extra_field_value;
				        break;
				case 4: // 4 :- Radio Button
						...
						{
							$checked=(@in_array($field_chk[$c]->field_value,$chk_data)) ? ' checked="checked" ' : '';
							$extra_field_value .='<input class="'.$row_data[$i]->field_class.'" '.$required.$reqlbl.$errormsg.' type="radio" '.$checked.'  name="'.$row_data[$i]->field_name.'"  id="'.$row_data[$i]->field_name."_".$field_chk[$c]->value_id.'" value="'.$field_chk[$c]->field_value.'" />'.$field_chk[$c]->field_value.'<br />';
						}
						$ex_field .='<td>'.$extra_field_value;
				        break;
				case 5: // 5 :-Select Box (Single select)
						...
						$extra_field_value .='<option value="">'.JText::_('SELECT').'</option>';
						for($c=0;$c<count($field_chk);$c++)
						{
							$selected=($field_chk[$c]->field_value==$data_value->data_txt)?' selected="selected" ' : '';
							$extra_field_value .='<option value="'.$field_chk[$c]->field_value.'" '.$required.$reqlbl.$errormsg.' '.$selected.'>'.$field_chk[$c]->field_value.'</option>';
						}
				...
				case 6: // 6 :- Select Box (Multiple select)
						...
						for($c=0;$c<count($field_chk);$c++)
						{
			        		...
			        		$extra_field_value .='<option value="'.urlencode($field_chk[$c]->field_value).'" '.$required.$reqlbl.$errormsg.' '.$selected.' >'.$field_chk[$c]->field_value.'</option>';
						}
						$extra_field_value .='</select>';
						$ex_field .='<td>'.$extra_field_value;
				        break;
				case 7: // 7 :-Select Country box
						...
						for($c=0;$c<count($field_chk);$c++)
						{
							...
							$extra_field_value .='<option value="'.$field_chk[$c]->country_id.'" '.$required.$reqlbl.$errormsg.' '.$selected.' >'.$field_chk[$c]->country_name.'</option>';
						}
						$extra_field_value .='</select>';
				        $ex_field .='<td>'.$extra_field_value;
				        break;
				...
				case 9: // 9 :- Media
				      	$ex_field .='<td valign="top" width="100" align="right" class="key">'.$extra_field_lable.'</td>';
						$extra_field_value = '<input class="'.$row_data[$i]->field_class.'"  name="'.$row_data[$i]->field_name.'" '.$required.$reqlbl.$errormsg.' id="'.$row_data[$i]->field_name.'" type="file" />';

						$textarea_value='';
						if($data_value && $data_value->data_txt)
						{
							$dest_prefix = $url.'components/'.$option.'/assets/images/media/';
							$dest_prefix_phy = JPATH_SITE.'/components/'.$option.'/assets/images/media/';
							...
							if (is_file($media_image))
							{
								...
								if ($media_type=='jpg' || $media_type=='jpeg' || $media_type=='png' || $media_type=='gif'){
									$extra_field_value .='<div id ="mediadiv'.$i.'"><img width="100"  src="'.$media_image.'" border="0" />';
									$extra_field_value .='&nbsp;<a href="#123"   onclick="delimg(\''.$data_value->data_txt.'\', \'mediadiv'.$i.'\',\''.$dest_prefix_del.'\' , \''.$data_value->data_id.'\');"> Remove Media</a>';
								} else {
									$extra_field_value .='<div id ="mediadiv'.$i.'"><a href="'.$media_image.'">'.$data_value->data_txt.'</a></div>';
									$extra_field_value .='&nbsp;<a href="#123"   onclick="delimg(\''.$data_value->data_txt.'\', \'mediadiv'.$i.'\',\''.$dest_prefix_del.'\' , \''.$data_value->data_id.'\');"> Remove Media</a>';
								}
							} else {
								$extra_field_value .=JTExt::_('FILE_NOT_EXIST');
							}
							$extra_field_value .='<input class="'.$row_data[$i]->field_class.'" name="'.$row_data[$i]->field_name.'"  id="'.$row_data[$i]->field_name.'" value="'.$data_value->data_txt.'" type="hidden" />';
						}
						$ex_field .='<td>'.$extra_field_value;
				        break;
				case 10: // 10 :- Documents
				        $textarea_value='';
						$textarea_value_text = "";
						...
						if($data_value && $data_value->data_txt)
						{
						    $data_value->data_txt;
							$text_value = explode("\n",$data_value->data_txt);
							$textarea_value_text = $text_value[0];
							$textarea_value = $text_value[1];
						}
						$ex_field .='<td valign="top" width="100" align="right" class="key">'.$extra_field_lable.'</td>';
						$extra_field_value ='<input type="text" name="text_'.$row_data[$i]->field_name.'" '.$required.$reqlbl.$errormsg.'  id="text_'.$row_data[$i]->field_name.'" value="'.$textarea_value_text.'" />&nbsp;';
						$extra_field_value .='<input class="'.$row_data[$i]->field_class.'"  name="'.$row_data[$i]->field_name.'"  id="'.$row_data[$i]->field_name.'" type="file"  />';
						...
						if($data_value && $data_value->data_txt)
						{
							...
							$dest_prefix = $url.'components/'.$option.'/assets/document/extrafields/';
							$dest_prefix_phy = JPATH_SITE.'/components/'.$option.'/assets/document/extrafields/';
							$dest_prefix_del = '/components/'.$option.'/assets/document/extrafields/';
							 $media_image = $dest_prefix_phy.$textarea_value;
							if (is_file($media_image))
							{
							 	$media_image = $dest_prefix.$textarea_value;
							 	$media_type = strtolower(JFile::getExt($textarea_value));
								if ($media_type == 'jpg' || $media_type == 'jpeg' || $media_type == 'png' || $media_type == 'gif'){
									$extra_field_value .='<div id="docdiv'.$i.'"><img width="100"  src="'.$media_image.'" border="0" /><div>';
									$extra_field_value .='&nbsp;<a href="#123"   onclick="delimg(\''.$textarea_value.'\', \'docdiv'.$i.'\',\''.$dest_prefix_del.'\', \''.$data_value->data_id.'\');"> Remove Media</a>';
								}else{
									$extra_field_value .='<div id="docdiv'.$i.'"><a href="'.$media_image.'">'.$textarea_value.'</a></div>';
								    $extra_field_value .='&nbsp;<a href="#123"   onclick="delimg(\''.$textarea_value.'\', \'docdiv'.$i.'\',\''.$dest_prefix_del.'\', \''.$data_value->data_id.'\');"> Remove Media</a>';
								}
							}else{
								$extra_field_value .=JTExt::_('FILE_NOT_EXIST');
							}
							$extra_field_value .='<input class="'.$row_data[$i]->field_class.'"  name="'.$row_data[$i]->field_name.'"  id="'.$row_data[$i]->field_name.'" value="'.$textarea_value.'" type="hidden"  />';
						}
						$ex_field .='<td>'.$extra_field_value;
						break;
				 case 11: // 11 :- Image select
						...
						for($c=0;$c<count($field_chk);$c++)
						{
							...
							$extra_field_value .='<td><div class="userfield_input"><img id="'.$field_chk[$c]->value_id.'" name="imgField[]" '.$class.' src="'.$url.'/components/com_redshop/assets/images/extrafield/'.$field_chk[$c]->field_name.'" title="'.$field_chk[$c]->field_value.'" alt="'.$field_chk[$c]->field_value.'" onclick="javascript:setProductUserFieldImage(\''.$field_chk[$c]->value_id.'\',\''.$section_id.'\',\''.$field_chk[$c]->field_id.'\',this);" /></div></td>';
						}
				...
				case 12: // 12 :- Date Picker
				        ...
				        $ex_field .='<td>'.$extra_field_value;
				        break;
				case 13: // 13 :- Image selection with Link And hover
				  		$field_chk=$this->getFieldValue($row_data[$i]->field_id);
				  		$data_value = $this->getSectionFieldDataList($row_data[$i]->field_id,$field_section,$section_id);
						...
						for($c=0;$c<count($field_chk);$c++)
						{
							...
							$extra_field_value .='<table><tr><td valign="top" width="100" align="right" class="key">'.JText::_('IMAGE_HOVER').'</td><td><input type="text" name="image_hover'.$field_chk[$c]->value_id.'"  value="'.$alt_text.'"/></td></tr>';
							$extra_field_value .='<tr><td valign="top" width="100" align="right" class="key">'.JText::_('IMAGE_LINK').'</td><td><input type="text" name="image_link'.$field_chk[$c]->value_id.'" value="'.$str_image_link.'"/></td></tr>';
						...
						$ex_field .='<td>'.$extra_field_value;
				        break;
				  case 15: // 15 :- Product Date Range
				    	$date = date("d-m-Y",time());
						if($data_value)
						{
							if($data_value->data_txt)
							{
								$mainsplit_date_total=explode(" ",$data_value->data_txt);
								$mainsplit_date=explode(":",$mainsplit_date_total[0]);
								$mainsplit_date_extra=explode(":",$mainsplit_date_total[1]);
						...
						$extra_field_value .="Enter Available Dates: <input type='button' name='addvalue' id='addvalue' class='button'  Value='".JText::_( 'ADD_VALUE' )."' onclick='addNewRowcustom(".$row_data[$i]->field_name.");'/>";
						$extra_field_value .="<fieldset class='adminform'>";
						$extra_field_value .="<legend>'".JText::_( 'VALUE' )."'</legend>";
						$extra_field_value .="<table cellpadding='0' cellspacing='5' border='0' id='extra_table' width='95%'>";
						$extra_field_value .="<tr><th colspan='3'>'".JText::_( 'OPTION_VALUE' )."'&nbsp;&nbsp;&nbsp;'".JText::_( 'EXPIRY_TIME' )."'</th></tr>";
						if(count($mainsplit_date_extra) >0)
						{
							for($k=0;$k<count($mainsplit_date_extra);$k++)
							{
								...
								{
									$extra_field_value .="<tr><td width='50%'><div id='divfieldText'><input type='text' id='".$row_data[$i]->field_name."_extra_name[]'  value='".date("d-m-Y",$mainsplit_date_extra[$k])."' name='".$row_data[$i]->field_name."_extra_name[]' size='10'>&nbsp;&nbsp;<input type='text' id='".$row_data[$i]->field_name."_extra_name_time[]'  value='".date("H:i",$mainsplit_date_extra[$k])."' name='".$row_data[$i]->field_name."_extra_name_time[]' size='5'></div></td><td><input value='Delete' onclick='deleteRow(this)' class='button' type='button' /></td>";
									$extra_field_value .="</tr>";
								}
							}
						}
						...
						{
							$k=1;
							$extra_field_value .="<tr><td width='50%'><div id='divfieldText'><input type='text' id='".$row_data[$i]->field_name."_extra_name[]' value='".date('d-m-Y')."' name='".$row_data[$i]->field_name."_extra_name[]' size='10'>&nbsp;&nbsp;<input type='text' id='".$row_data[$i]->field_name."_extra_name_time[]'  value='24:00' name='".$row_data[$i]->field_name."_extra_name_time[]' size='5'></div>
						</td>
						</tr>";
						}
						$extra_field_value .="</table>
				</fieldset>
				</div><input type='hidden' value='".$k."' name='total_extra' id='total_extra'>";
						$ex_field .='<td>'.$extra_field_value;
			        	break;
			}
			if(trim($template_desc)!='')
			{
				if(strstr($template_desc,"{".$row_data[$i]->field_name."}"))
				{
					...
					$template_desc = str_replace("{".$row_data[$i]->field_name."_lbl}", $extra_field_lable, $template_desc);
				}
	...
	function extra_field_save($data,$field_section,$section_id="",$user_email="")
	{
		...
		for($i=0;$i<count($row_data);$i++)
		{
			...
			{
				if ($row_data[$i]->field_type==10 && isset($data['text_'.$row_data[$i]->field_name]))
				{
					$data_txt = $data['text_'.$row_data[$i]->field_name]."\n".$data[$row_data[$i]->field_name];
				} else if ($row_data[$i]->field_type==8 || $row_data[$i]->field_type==1 || $row_data[$i]->field_type==2) {
					$data_txt = JRequest::getVar( $row_data[$i]->field_name, '', ' ', 'string', JREQUEST_ALLOWRAW );
					//$data_txt = addslashes($data_txt);
				} else {
					$data_txt= $data[$row_data[$i]->field_name];
				}
			}
			if($row_data[$i]->field_type==9 || $row_data[$i]->field_type==10)
			{
				if ($row_data[$i]->field_type==9)
					$dest_prefix = JPATH_ROOT.DS.'components/com_redshop/assets/images/media/';
				elseif ($row_data[$i]->field_type==10)
					 $dest_prefix = JPATH_ROOT.DS.'components/com_redshop/assets/document/extrafields/';
			...
			if($row_data[$i]->field_type==15)
			{
				if($data[$row_data[$i]->field_name]!="" && $data[$row_data[$i]->field_name."_expiry"]!="")
				{
					...
					if(count($data[$row_data[$i]->field_name."_extra_name"])>0)
					{
						...
						{
							$data_txt.=strtotime($data[$row_data[$i]->field_name."_extra_name"][$r]." ".$data[$row_data[$i]->field_name."_extra_name_time"][$r]).":";
						}
					}
				}
			}
...
//					 	echo $this->_db->getQuery();exit;
			...
			/*if($row_data[$i]->field_type==13)
			{
					...
					for($fi=0;$fi<count($field_value_array);$fi++)
					{
						$sql="UPDATE ".$this->_table_prefix."fields_value "
							."SET image_link='".$data['image_link'.$field_value_array[$fi]]."' , alt_text='".$data['image_hover'.$field_value_array[$fi]]."'"
							." WHERE "
							." value_id='".$field_value_array[$fi]."' ";
	...
	function list_all_field_display($field_section="",$section_id=0,$flag=0,$user_email="",$template_desc="")
	{
		...
		for($i=0;$i<count($row_data);$i++)
		{
			...
			if($flag==1){
				if( $i > 0 )
					$ex_field .= "<br />";
				$ex_field .= JText::_($extra_field_lable).' : ';
			}else{
				$ex_field .= '<tr><td valign="top" align="left">'.JText::_($extra_field_lable).' : </td><td>';
			}
	...
	function list_all_user_fields($field_section="",$section_id=12,$field_type='',$unique_id)
	{
		...
		for($i=0;$i<count($row_data);$i++)
		{
			...
			if($field_type == 'hidden')
			{
				...
				{
					case 1: // 1 :- Text Field
						$ex_field .='<div class="userfield_input"><input class="'.$row_data[$i]->field_class.'" type="text" maxlength="'.$row_data[$i]->field_maxlength.'" onkeyup="var f_value = this.value;" name="extrafieldname'.$unique_id.'[]"  id="'.$row_data[$i]->field_name.'" '.$req.' userfieldlbl="'.$row_data[$i]->field_title.'" value="'.$text_value.'" size="'.$row_data[$i]->field_size.'" /></div>';
				        break;
					case 2: // 2 :- Text Area
						$ex_field .='<div class="userfield_input"><textarea class="'.$row_data[$i]->field_class.'"  name="extrafieldname'.$unique_id.'[]"  id="'.$row_data[$i]->field_name.'" '.$req.' userfieldlbl="'.$row_data[$i]->field_title.'" cols="'.$row_data[$i]->field_cols.'" onkeyup=" var f_value = this.value;" rows="'.$row_data[$i]->field_rows.'" >'.$text_value.'</textarea></div>';
				        break;
					case 3: // 3 :- Check Box
						$field_chk=$this->getFieldValue($row_data[$i]->field_id);
				 	...
				 	case 4: // 4 :- Radio Button
				  	...
				  	case 5: // 5 :-Select Box (Single select)
						...
						$ex_field .='<option value="">'.JText::_('SELECT').'</option>';
						for($c=0;$c<count($field_chk);$c++)
						{
					...
					case 10 : /// File Upload
						...
						$ajax = "";
						$ex_field .='<div class="userfield_input"><input class="'.$row_data[$i]->field_class.'" type="button" value="'.JText::_('UPLOAD').'" name="file'.$row_data[$i]->field_name.'_'.$unique_id.'"  id="file'.$row_data[$i]->field_name.'_'.$unique_id.'" '.$req.' userfieldlbl="'.$row_data[$i]->field_title.'" size="'.$row_data[$i]->field_size.'" /><p>'.JText::_('UPLOADED_FILE').':<ol id="ol_'.$row_data[$i]->field_name.'"></ol></p></div>';
			        ...
			        case 11: // 11 :- Image select
						...
						for($c=0;$c<count($field_chk);$c++)
						{
							$ex_field .='<td><div class="userfield_input"><img id="'.$row_data[$i]->field_name."_".$field_chk[$c]->value_id.'" class="pointer imgClass_'.$unique_id.'" src="'.$url.'/components/com_redshop/assets/images/extrafield/'.$field_chk[$c]->field_name.'" title="'.$field_chk[$c]->field_value.'" alt="'.$field_chk[$c]->field_value.'" onclick="javascript:setProductUserFieldImage(\''.$row_data[$i]->field_name.'\',\''.$unique_id.'\',\''.$field_chk[$c]->field_value.'\',this);"/></div></td>';
						}
						$ex_field .='</tr></table>';
						$ajax = '';
