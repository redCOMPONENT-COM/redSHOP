...
class order_functions
{
	...
	{
		$query = 'SELECT order_status_name FROM '.$this->_table_prefix.'order_status '
				.'WHERE order_status_code ="' . $order_status_code.'"';
		$this->_db->setQuery ( $query );
	...
	function updateOrderStatus($order_id,$newstatus)
	{
	 	$query = 'UPDATE '.$this->_table_prefix.'orders '
				.'SET order_status="'.$newstatus.'", mdate='.time()
				.' WHERE order_id IN('.$order_id.')';
		$this->_db->setQuery( $query );
		$this->_db->query();

		$query = "SELECT p.element,op.order_transfee, op.order_payment_trans_id,op.order_payment_amount, op.authorize_status FROM #__plugins AS p "
					."LEFT JOIN ".$this->_table_prefix."order_payment AS op ON op.payment_method_class=p.element "
					."WHERE op.order_id='".$order_id."' "
					."AND p.folder='redshop_payment' ";
		$this->_db->setQuery( $query );
	...
	function generateParcel($order_id,$specifiedSendDate) 
	{
		...
		$billingInfo->country_code = $Redconfiguration->getCountryCode2(SHOP_COUNTRY);
		...
		$shippingInfo->country_code = $Redconfiguration->getCountryCode2($shippingInfo->country_code);
	...
	function changeorderstatus($data)
	{
		...
		// set redshop order status log table path
		...
		if($checkupdateordersts == 0 && $data->order_status_code!="" && $data->order_payment_status_code!="")
		{
		    ...
		    $orderstatuslog 				= &  JTable::getInstance( 'order_status_log' ,'Table');
			$orderstatuslog->order_id 		= $order_id;
			$orderstatuslog->order_status 	= $data->order_status_code;
			$orderstatuslog->order_payment_status=$data->order_payment_status_code;
			$orderstatuslog->date_changed 	= time();
			$orderstatuslog->customer_note 	= $data->log;
			$orderstatuslog->store();
			...
			if($data->order_payment_status_code == "Paid")
			{
				...
				$invociepdfname = $this->createShippedInvoicePdf($order_shipped_id,$data->order_status_code);
	...
	{
		$query = 'UPDATE '.$this->_table_prefix.'orders '
				.'SET order_payment_status="'.$newstatus.'", mdate='.time()
				.' WHERE order_id IN('.$order_id.')';
		$this->_db->setQuery( $query );
		$this->_db->query();
	}
	...
	function updateOrderItemStatus($order_id=0,$product_id=0,$newstatus="",$comment="",$order_item_id=0)
	{
		...
		$query = "UPDATE ".$this->_table_prefix."order_item "
	...
	function getOrderStatus()
	{
		$query = "SELECT order_status_code AS value, order_status_name AS text "
				."FROM ".$this->_table_prefix."order_status "
				."where published='1' ";
		$this->_db->setQuery($query);
	...
	function getstatuslist($name='statuslist', $selected='', $attributes=' class="inputbox" size="1" ')
	{
		...
		$types[] = JHTML::_('select.option',  '0', '- '. JText::_( 'SELECT_STATUS_LBL' ) .' -' );
	...
	function getCustomOrderStatus($field)
	{
		...
		$query = "SELECT ".$fieldname." AS value "
				."FROM ".$this->_table_prefix_crm."configuration ";
		$this->_db->setQuery($query);
	...
	{
 		$types[] = JHTML::_('select.option','', JText::_('SELECT_PAYMENT_STATUS'));
 		$types[] = JHTML::_('select.option','Paid', JText::_('PAYMENT_STA_PAID'));
 		$types[] = JHTML::_('select.option','Unpaid', JText::_('PAYMENT_STA_UNPAID'));
 		$types[] = JHTML::_('select.option','Partial Paid', JText::_('PAYMENT_STA_PARTIAL_PAID'));
	...
	function update_status()
	{
		...
		if(!$isproduct)
		{
			...
			{
				$order_shipped_id[] = $order_id;
				$invociepdfname = $this->createShippedInvoicePdf($order_shipped_id,$newstatus);
		...
		$msg = JText::_ ( 'ORDER_STATUS_SUCCESSFULLY_SAVED_FOR_ORDER_ID' )." ".$order_id;
		if ($return == 'order')
		{
	...
	function update_status_all()
	{
		...
		// For shipped pdf generaton
		$invociepdfname = $this->createShippedInvoicePdf($cid,$newstatus);
		...
		if($invociepdfname!="")
		{
			if(file_exists(JPATH_COMPONENT_SITE.DS."assets/document/invoice/".$invociepdfname.".pdf"))
			{
			?>
				window.open("<?php echo JURI::root()?>/components/com_redshop/assets/document/invoice/<?php echo $invociepdfname?>.pdf");
	...
	function getOrderPaymentDetail($order_id,$payment_order_id=0)
	{
		...
		$query = 'SELECT * FROM '.$this->_table_prefix.'order_payment '
				.'WHERE order_id="'.$order_id.'" '
				.$and;
		$this->_db->setQuery($query);
		$list =  $this->_db->loadObjectlist();
	...
	{
		$query = 'SELECT order_payment_amount FROM '.$this->_table_prefix.'order_payment '
				.'WHERE order_id="'.$order_id.'" ';
		$this->_db->setQuery( $query );
		$list = $this->_db->loadObjectlist();
	...
	function getBillingAddress($user_id=0)
	{
		...
		if ($helper->isredCRM()){
			...
			$query = 'SELECT ui.*,CONCAT(firstname," ",lastname) AS text,d.*,ui.users_info_id FROM '.$this->_table_prefix.'users_info as ui '
					 .'LEFT JOIN #__redcrm_debitors as d ON d.users_info_id = ui.users_info_id '
					 .'WHERE address_type like "BT" '
				     .'AND user_id="'.$user_id.'" ';
			$this->_db->setQuery($query);
			$list =  $this->_db->loadObject();
	...
	function getOrderBillingUserInfo($order_id)
	{
		...
		$query = 'SELECT * FROM '.$this->_table_prefix.'order_users_info '
				.'WHERE address_type LIKE "BT" '
		...
		if ($helper->isredCRM())
		{
			$query = 'SELECT oui.*,cd.customer_number,IFNULL(cp.person_name ,CONCAT(oui.firstname," ",oui.lastname)) AS text FROM '.$this->_table_prefix.'order_users_info as oui '//,cd.ean_number
				...
				.'LEFT JOIN #__redcrm_debitors as cd ON cd.users_info_id = co.debitor_id '
				.'WHERE oui.address_type LIKE "BT" '
		...
		if ($helper->isredCRM()){
			...
			if (count($crmusers) >0 ){
				...
				$query = 'SELECT ui.*,IFNULL(destination_name,CONCAT(firstname," ",lastname)) AS text FROM '.$this->_table_prefix.'users_info as ui'
					...
					.' AND ui.users_info_id IN ('.$crmusersinfo .') ';
	...
	function getOrderShippingUserInfo($order_id)
	{
		...
		if ($helper->isredCRM())
		{
			 ...
			 */
			$crmusers = $crmDebitorHelper->getOrderShippingInfo($order->user_info_id,$order_id);
	...
	function getUserFullname($user_id)
	{
		...
		$query = "SELECT firstname, lastname FROM ".$this->_table_prefix."users_info "
				."WHERE address_type like 'BT' "
				."AND user_id='".$user_id."' ";
		$this->_db->setQuery($query);
		$list = $this->_db->loadObject();
	...
	{
		$query = "SELECT fd.*,f.field_title,f.field_type,f.field_name FROM ".$this->_table_prefix."fields_data AS fd "
				."LEFT JOIN ".$this->_table_prefix."fields AS f ON f.field_id=fd.fieldid "
				."WHERE fd.itemid='".$order_item_id."' "
	...
	function generateOrderNumber($p_length = '30')
	{
		...
		else if(INVOICE_NUMBER_TEMPLATE)
		{
	...
	function getCountryName($cnt3="")
	{
		...
		$query = 'SELECT country_3_code AS value,country_name AS text,country_jtext FROM '.$this->_table_prefix.'country '
				.'WHERE 1=1 '
				.$and
				;
		$this->_db->setQuery( $query );
	...
	function getCountry($cnt3="")
	{
		$and = '';
		$cntname = '';
		if($cnt3!="")
		{
			$and .= ' AND country_2_code="'.$cnt3.'" ';
		} else {
			return $cntname;
		}
		$query = 'SELECT country_name FROM '.$this->_table_prefix.'country '
				.'WHERE 1=1 '
				.$and
				;
		$this->_db->setQuery( $query );
		$cntname = $this->_db->loadResult();
		return $cntname;
	}
   ...
   function barcode_randon_number($lenth =12, $barcodekey=0)
   {
	...
	function checkupdateordersts($data)
	{
		$res = 1;
		$query = "SELECT * FROM ".$this->_table_prefix."orders "
				."WHERE order_status='".$data->order_status_code."' "
				."AND order_payment_status='".$data->order_payment_status_code."' "
	...
	function changeOrderStatusMail($order_id,$newstatus,$order_comment='')
	{
		...
		if(count($mailtemplate)>0)
		{
			...
			{
				if($barcode_code !="" && file_exists(JPATH_SITE."/components/com_redshop/assets/images/barcode/".$barcode_code.".png"))
				{
				...
				$img_url = JUri::root()."components/com_redshop/assets/images/barcode/".$barcode_code.".png";
			...
			$replace[] = "<a href='" . $orderdetailurl . "'>" . JText::_("ORDER_DETAIL_LINK_LBL" ) . "</a>";

			$search[] = "{order_track_no}";
			$replace[] = $orderdetail->track_no;
	...
	function placeGoogleOrder($request){
		...
		JTable::addIncludePath(JPATH_SITE.DS.'administrator'.DS.'components'.DS.'com_redshop'.DS.'tables');
   		...
   		$bname = preg_split(" ",$billinginfo['name'],2);

   		$stateQuery = "SELECT * FROM  `".$this->_table_prefix."state` WHERE  `state_name` LIKE  '%".strtolower(trim($billinginfo['state']))."%' LIMIT 0,1";
	    $this->_db->setQuery($stateQuery);
	    $stateInfo = $this->_db->loadObject();

	    $stateQuery = "SELECT * FROM  `".$this->_table_prefix."country` WHERE  `country_2_code` LIKE  '".trim($billinginfo['country'])."' LIMIT 0,1";
	    $this->_db->setQuery($stateQuery);
	    $countryInfo = $this->_db->loadObject();

	    $shipping = $request->shipping['address'];
	    //print_r($shipping);

	    $stateQuery = "SELECT * FROM  `".$this->_table_prefix."state` WHERE  `state_name` LIKE  '%".strtolower(trim($shipping['state']))."%' LIMIT 0,1";
	    $this->_db->setQuery($stateQuery);
	    $stateInfoShip = $this->_db->loadObject();

	    $stateQuery = "SELECT * FROM  `".$this->_table_prefix."country` WHERE  `country_2_code` LIKE  '".trim($shipping['country'])."' LIMIT 0,1";
	    $this->_db->setQuery($stateQuery);
	    $countryInfoShip = $this->_db->loadObject();

   		# user management
   		$email = $request->billinginfo['email'];
   		$query = "SELECT id,uf.users_info_id FROM `#__users` as u LEFT JOIN ".$this->_table_prefix."users_info as uf ON uf.user_id = u.id AND address_type = 'BT'
WHERE u.email = '".$email."' OR uf.user_email = '".$email."'";
   		$this->_db->setQuery($query);
   		$userInfo = $this->_db->loadObject();

		$isNewJoomla = $isNewShop = true;
   		if(isset($userInfo->id)){
			$userId = $userInfo->id;
			$isNewJoomla = ($userId)? false : true;
			$userInfoId = $userInfo->users_info_id;
			$isNewShop = ($userInfoId)? false : true;
   		}

   		if($isNewJoomla){

   		 	$better_token = substr(uniqid(md5(rand()), true),0,10);
   			$userpre['password'] = $better_token;
			$userpre['password2'] = $better_token;
			$userpre['email'] = $billinginfo['email'];
			$userpre['name'] = $billinginfo['name'];
			$userpre['username'] = @$bname[0];
			$userpre['sendEmail'] = 1;
			$userpre['guest'] = 0;
			...
			$this->_sendUserMail($user, $password);
   		...
   		if($isNewShop){
   			...
   			$userpre['users_info_id'] = 0;
   			$userpre['email1'] = $billinginfo['email'];
   			$userpre['firstname'] = @$bname[0];
   			$userpre['lastname'] = @$bname[1];
   			$userpre['company_name'] = $billinginfo['company_name'];
   			$userpre['is_company'] = (trim($billinginfo['company_name']) == '') ? 0 : 1;
   			$userpre['address'] = $billinginfo['address']."\n".$billinginfo['address2'];
   			$userpre['city'] = $billinginfo['city'];
    		$userpre['country_code'] = $countryInfo->country_3_code;
    		$userpre['state_code'] = ($stateInfo->state_2_code) ? $stateInfo->state_2_code : $billinginfo['state'];
    		$userpre['zipcode'] = $billinginfo['zipcode'];
    		$userpre['phone'] = $billinginfo['phone'];

			$reduser = $rsUserhelper->storeRedshopUser($userpre,$userId,0);

    		$userpre['user_email'] = $shipping['email'];
   			$userpre['firstname_ST'] = @$bname[0];
   			$userpre['lastname_ST'] = @$bname[1];
   			$userpre['company_name'] = $shipping['company_name'];
   			$userpre['is_company'] = (trim($shipping['company_name']) == '') ? 0 : 1;
   			$userpre['address_ST'] = $shipping['address']."\n".$shipping['address2'];
   			$userpre['city_ST'] = $shipping['city'];
    		$userpre['country_code_ST'] = $countryInfoShip->country_3_code;
    		$userpre['state_code_ST'] = ($stateInfoShip->state_2_code) ? $stateInfoShip->state_2_code : $shipping['state'];
    		$userpre['zipcode_ST'] = $shipping['zipcode'];
    		$userpre['phone_ST'] = $shipping['phone'];
    		$userpre['user_id'] = $userId;

			$redShipuser = $rsUserhelper->storeRedshopUserShipping($userpre);
			$userInfoId = $reduser->users_info_id;
   		}

   		# End

   		# Order management
		$order =& JTable::getInstance('order_detail','Table');

		$order->user_id = $userId;
		$order->order_number = $this->generateOrderNumber();
		$order->user_info_id = $userInfoId;

		$order_total = $request->order_total['value'];
		$adjustment_total = $request->adjustment_total['rate'];
		$order_subtotal = $order_total - $adjustment_total;

		$order->order_total = $order_total;
	    $order->order_subtotal = $order_subtotal;

	    $order_tax = $request->tax['rate'];
	    $order->order_tax = $order_tax;

	    $order_shipping = $request->shipping['shipping_rate'];
	    $order->order_shipping = $order_shipping;

	    $default_shipping 			= JText::_('DEFAULT_SHOPPER_GROUP_SHIPPING');
	    $order->ship_method_id = $shippinghelper->encryptShipping( "plgredshop_shippingdefault_shipping|default_shipping|".$default_shipping."|".number_format( $order_shipping, 2, '.', '' )."|".$default_shipping."|single|0");

	    $order->order_status = "P";
	    $order->order_payment_status = "Unpaid";
	    $timestamp = $request->timestamp;
	    $order->cdate = strtotime($timestamp);
	    $order->mdate = strtotime($timestamp);

	    # for barcode generation
 		$order->barcode = $this->barcode_randon_number(12,0);
		# end
		...
		$order->store();
 		...
 		if($row->order_status == CLICKATELL_ORDER_STATUS)
				$helper->clickatellSMS( $order->order_id );
		# End

		$rowOrderStatus 				= &  JTable::getInstance( 'order_status_log' ,'Table');
		$rowOrderStatus->order_id 		= $order->order_id;
		$rowOrderStatus->order_status 	=  $order->order_status;
		$rowOrderStatus->date_changed 	= time();
		$rowOrderStatus->customer_note 	= JText::_('GOOGLE_CHECOUT_ORDER_PLACED');
	    ...
	    for($i=0;$i<count($cartitems);$i++){
			...
			$productInfo = JTable::getInstance('product_detail','Table');
			$productInfo->load($product_id);
			//print_r($productInfo);
			...
			/*if($helper->isredCRM()){
				...
				$orderItem->product_purchase_price	= $crmproduct->product_purchase_price > 0 ? $crmproduct->product_purchase_price : $crmproduct->product_price ;
		    ...
		    $orderItem->user_info_id = $order->user_info_id;
		    $orderItem->supplier_id = $productInfo->supplier_id;
		    $orderItem->product_id = $product_id;
		    $orderItem->order_item_sku = $productInfo->product_number;
		    $orderItem->order_item_name = $item->product_name;
		    $orderItem->product_quantity = $item->quantity;
		    $orderItem->product_item_price = $item->product_price;
		    $orderItem->product_item_old_price = $productInfo->product_price;
		    $orderItem->product_item_price_excl_vat = $productInfo->product_price * $item->quantity;
		    $orderItem->product_final_price = $item->product_price;
		    $orderItem->order_item_currency = $item->product_currency;
		    $orderItem->order_status = "P";
		    $orderItem->cdate = $order->cdate;
		    $orderItem->mdate = $order->mdate;
		    ...
		    if(isset($item->attribute)){
			    ...
			    for($a=0;$a<count($attributes);$a++){
			    	...
			    	$pattribute = JTable::getInstance('product_attribute','Table');
					$pattribute->load($attribute_id);

					$generateAttributeCart[$a]['attribute_id'] = $attribute_id;
					$generateAttributeCart[$a]['attribute_name'] = $pattribute->attribute_name;

			    	$property_id = $attribute->property_id;
			    	$attribute_property = JTable::getInstance('attribute_property','Table');
					$attribute_property->load($property_id);

			    	$generateAttributeCart[$a]['attribute_childs'][0]['property_id'] = $property_id;
					$generateAttributeCart[$a]['attribute_childs'][0]['property_name'] = $attribute_property->property_name;
					$generateAttributeCart[$a]['attribute_childs'][0]['property_oprand'] = $attribute_property->oprand;
					$generateAttributeCart[$a]['attribute_childs'][0]['property_price'] = $attribute_property->property_price;

					if($attribute->subproperty_id > 0){

						$subproperty_id = $attribute->subproperty_id;
						$subproperty = JTable::getInstance('subattribute_property','Table');
						$subproperty->load($subproperty_id);

				    	$generateAttributeCart[$a]['attribute_childs'][0]['property_childs'][0]['subproperty_id'] = $subproperty_id;
						$generateAttributeCart[$a]['attribute_childs'][0]['property_childs'][0]['subproperty_name'] = $subproperty->subattribute_color_name;
						$generateAttributeCart[$a]['attribute_childs'][0]['property_childs'][0]['subproperty_oprand'] = $subproperty->oprand;
						$generateAttributeCart[$a]['attribute_childs'][0]['property_childs'][0]['subproperty_price'] = $subproperty->subattribute_color_price;
					}
			    }
		    ...
		    # accessory section
		    $product_accessory = "";
		    if(isset($item->accessory)){

		    	$accessorys = $item->accessory;
				$generateAccessoryCart = array();
		    	for($a=0;$a<count($accessorys);$a++){
					$accessory = $accessorys[$a];

					$accInfo = $producthelper->getProductAccessory($accessory,$product_id);

					$generateAccessoryCart[$a]['accessory_id'] = $accessory;
					$generateAccessoryCart[$a]['accessory_name'] = $accInfo[0]->product_name;
					$generateAccessoryCart[$a]['accessory_oprand'] = $accInfo[0]->oprand;
					$generateAccessoryCart[$a]['accessory_price'] = $accInfo[0]->accessory_price;
		    	}
		    ...
		    $orderItem->product_attribute = $product_attribute;
		    $orderItem->product_accessory = $product_accessory;

		    $orderItem->store();

			# order item end

			# order_attribute_item
			if(isset($item->attribute)){
			    ...
			    for($a=0;$a<count($attributes);$a++){
			    	...
			    	$pattribute = JTable::getInstance('product_attribute','Table');
					$pattribute->load($attribute_id);
					$attribute_name = $pattribute->attribute_name;

					$order_attribute_item = JTable::getInstance('order_attribute_item','Table');
					$order_attribute_item->order_att_item_id = 0;
				    $order_attribute_item->order_item_id = $orderItem->order_item_id;
				    $order_attribute_item->section_id = $attribute_id;
				    $order_attribute_item->section = "attribute";
				    $order_attribute_item->parent_section_id = $product_id;
				    $order_attribute_item->section_name = $attribute_name;
				    $order_attribute_item->section_vat = 0;
				    $order_attribute_item->section_price = 0;
				    $order_attribute_item->section_oprand = "";
				    $order_attribute_item->is_accessory_att = 0;
					$order_attribute_item->store();
				    //print_r($order_attribute_item);

				    $property_id = $attribute->property_id;
			    	$attribute_property = JTable::getInstance('attribute_property','Table');
					$attribute_property->load($property_id);

				    $order_property_item = JTable::getInstance('order_attribute_item','Table');
					$order_property_item->order_att_item_id = 0;
				    $order_property_item->order_item_id = $orderItem->order_item_id;
				    $order_property_item->section_id = $property_id;
				    $order_property_item->section = "property";
				    $order_property_item->parent_section_id = $attribute_id;
				    $order_property_item->section_name = $attribute_property->property_name;
				    $order_property_item->section_vat = 0;
				    $order_property_item->section_price = $attribute_property->property_price;
				    $order_property_item->section_oprand = $attribute_property->oprand;
				    $order_property_item->is_accessory_att = 0;
					$order_property_item->store();
				    //print_r($order_property_item);
			    	...
			    	if($attribute->subproperty_id > 0){
					    ...
					    $order_subproperty_item = JTable::getInstance('order_attribute_item','Table');
						$order_subproperty_item->order_att_item_id = 0;
					    $order_subproperty_item->order_item_id = $orderItem->order_item_id;
					    $order_subproperty_item->section_id = $subproperty_id;
					    $order_subproperty_item->section = "subproperty";
					    $order_subproperty_item->parent_section_id = $property_id;
					    $order_subproperty_item->section_name = $subproperty->subattribute_color_name;
					    $order_subproperty_item->section_vat = 0;
					    $order_subproperty_item->section_price = $subproperty->subattribute_color_price;
					    $order_subproperty_item->section_oprand = $subproperty->oprand;
					    $order_subproperty_item->is_accessory_att = 0;
						$order_subproperty_item->store();
					    //print_r($order_subproperty_item);
			    	}
			    }
			}
			...
			if(isset($item->accessory)){
		    	...
		    	for($a=0;$a<count($accessorys);$a++){
					...
					$accProduct = JTable::getInstance('product_detail','Table');
					$accProduct->load($accessory);

					$accInfo = $producthelper->getProductAccessory($accessory,$product_id);

					/*$accessory;
					$accInfo[0]->product_name;
					$accInfo[0]->oprand;
					$accInfo[0]->accessory_price;*/

					$order_acc_item = JTable::getInstance('order_acc_item','Table');
					$order_acc_item->order_item_acc_id = 0;
				    $order_acc_item->order_item_id = $orderItem->order_item_id;
				    $order_acc_item->product_id = $accessory;
				    $order_acc_item->order_acc_item_sku= $accProduct->product_number;
				    $order_acc_item->order_acc_item_name= $accInfo[0]->product_name;
				    $order_acc_item->order_acc_price= $accInfo[0]->accessory_price;
				    $order_acc_item->order_acc_vat= 0;
				    $order_acc_item->product_quantity= $item->quantity;
				    $order_acc_item->product_acc_item_price= $accInfo[0]->accessory_price;
				    $order_acc_item->product_acc_final_price= $accInfo[0]->accessory_price * $item->quantity;
				    $order_acc_item->product_attribute= '';
					$order_acc_item->store();

					//print_r($order_acc_item);
		    	}
			}
			# End
	    }
	    ...
	    # order_payment
		$order_payment = JTable::getInstance('order_payment','Table');

		$order_payment->order_id 				= $order->order_id;
		$order_payment->payment_method_id 		= $request->plugin->name;
		$order_payment->order_payment_amount 	= $order->order_total;
		$order_payment->order_payment_name 		= $request->plugin->name;
		$order_payment->payment_method_class 	= $request->plugin->name;
		$order_payment->order_payment_trans_id  = $request->transaction_id;
	    ...
	    $order_user_detail = JTable::getInstance('order_user_detail','Table');

	  //  print_r($billinginfo);

	    # store billing info
	    $order_user_detail->order_info_id = 0;
	    $order_user_detail->users_info_id = $order->user_info_id;
	    $order_user_detail->order_id = $order->order_id;
	    $order_user_detail->user_id = $order->user_id;
	    $order_user_detail->firstname = @$bname[0];
	    $order_user_detail->address_type = 'BT';
	    $order_user_detail->lastname = @$bname[1];
	    $order_user_detail->vat_number = 0;
	    $order_user_detail->tax_exempt = 0;
	    $order_user_detail->requesting_tax_exempt = 0;
	    $order_user_detail->shopper_group_id = 0;
	    $order_user_detail->published = 1;
	    $order_user_detail->is_company = (trim($billinginfo['company_name']) == '') ? 0 : 1;
	    $order_user_detail->country_code = $countryInfo->country_3_code;
	    $order_user_detail->state_code = ($stateInfo->state_2_code) ? $stateInfo->state_2_code : $billinginfo['state'];
	    $order_user_detail->zipcode = $billinginfo['zipcode'];
	    $order_user_detail->phone = 0;
	    $order_user_detail->city = $billinginfo['city'];
	    $order_user_detail->address = $billinginfo['address']."\n".$billinginfo['address2'];
	    $order_user_detail->tax_exempt_approved = 0;
	    $order_user_detail->approved = 0;
	    $order_user_detail->user_email = $billinginfo['email'];
	    $order_user_detail->company_name = $billinginfo['company_name'];
	    $order_user_detail->thirdparty_email = 0;

	    $order_user_detail->store();

	    # End store billing info
		//print_r($order_user_detail);

		# Store Shipping info

		$bname = preg_split(" ",$shipping['name'],2);
		$order_user_detail->order_info_id = 0;
	    $order_user_detail->users_info_id = $order->user_info_id;
	    $order_user_detail->order_id = $order->order_id;
	    $order_user_detail->user_id = $order->user_id;
	    $order_user_detail->firstname = @$bname[0];
	    $order_user_detail->address_type = 'ST';
	    $order_user_detail->lastname = @$bname[1];
	    $order_user_detail->vat_number = 0;
	    $order_user_detail->tax_exempt = 0;
	    $order_user_detail->requesting_tax_exempt = 0;
	    $order_user_detail->shopper_group_id = 0;
	    $order_user_detail->published = 1;
	    $order_user_detail->is_company = (trim($shipping['company_name']) == '') ? 0 : 1;
	    $order_user_detail->country_code = $countryInfoShip->country_3_code;
	    $order_user_detail->state_code = ($stateInfoShip->state_2_code) ? $stateInfoShip->state_2_code : $shipping['state'];
	    $order_user_detail->zipcode = $shipping['zipcode'];
	    $order_user_detail->phone = 0;
	    $order_user_detail->city = $shipping['city'];
	    $order_user_detail->address = $shipping['address']."\n".$shipping['address2'];
	    $order_user_detail->tax_exempt_approved = 0;
	    $order_user_detail->approved = 0;
	    $order_user_detail->user_email = $shipping['email'];
	    $order_user_detail->company_name = $shipping['company_name'];
	    $order_user_detail->thirdparty_email = 0;

	    $order_user_detail->store();

	    //print_r($order_user_detail);
	    # End store Shipping info

		# End order user information

	    $redshopMail->sendOrderMail($order->order_id,0);

 		# mines one coupon
   		if(isset($request->merchant_codes)){
			$codes = "'".implode("','",$request->merchant_codes)."'";
			$query = "UPDATE `".$this->_table_prefix."coupons` SET `coupon_left` = coupon_left - 1  WHERE `coupon_code` IN (".$codes.")";
			$this->_db->setQuery($query);
			$this->_db->Query();
   		}
		...
		if ($helper->isredCRM()){
			...
			if(ENABLE_ITEM_TRACKING_SYSTEM){
				...
				$getStatus = array();
				$getStatus['orderstatus'] = $order->order_status;
				$getStatus['paymentstatus'] = $order->order_payment_status;

				$crmSupplierOrderHelper->redSHOPOrderUpdate($order->order_id,$getStatus);
				unset($getStatus);
			}
		}
		# End
	...
	function _sendUserMail(&$user, $password)
	{
		global $mainframe;
		...
		$subject2 = sprintf ( "Account Details for %s at %s" , $name, $sitename);
		...
		foreach ( $rows as $row )
		{
			if ($row->sendEmail)
			{
				$message2 = sprintf ( "Hello %s,\n\nA new user has registered at %s.\nThis e-mail contains their details:\n\nName: %s\nE-mail: %s\nUsername: %s\n\nPlease do not respond to this message. It is automatically generated and is for information purposes only.", $row->name, $sitename, $name, $email, $username);
	...
	/**
	 * Tax calculation for google checkout merchant calculation api
	 */
	function calculateGoogleTax($merchantTaxdata){

		if(isset($merchantTaxdata['product'])){

			$products = $merchantTaxdata['product'];
			...
			for($i=0;$i<count($products);$i++){
				...
				$pquery = "SELECT  `product_tax_group_id` FROM  `".$this->_table_prefix."product` WHERE product_id IN (".$product_id.")";
				$query = "SELECT tax_rate
								FROM  `".$this->_table_prefix."tax_rate`
								WHERE (
										`tax_group_id`
										IN (".$pquery.")
										OR tax_group_id =1
									   )
								AND  `tax_country` =  '".$merchantTaxdata['country_code']."'
								AND  `tax_state` =  '".$merchantTaxdata['state_code']."'";
				$this->_db->setQuery($query);
				$ratevalue = $this->_db->loadResult();

				if($ratevalue >0)
					$rate[] = $price*$ratevalue;
			}

			$rates = array_sum($rate);
	...
	/**
	 * Coupon calculation for google checkout merchant calculation api
	 */
	function calculateGoogleCoupon($merchantCouponData){

		$rates = new stdClass();

		if(!isset($merchantCouponData['coupon']['code'])){

			$rates->isvalid = "false";
			$rates->message = "Not Valid";
			$rates->rate = 0;
			$rates->free_shipping = 0;
			return $rates;
		}

		$query = "SELECT percent_or_total,coupon_value,free_shipping
						FROM  `".$this->_table_prefix."coupons`
						WHERE  `coupon_code` LIKE  '".$merchantCouponData['coupon']['code']."'
						AND start_date <".time()."
						AND end_date > ".time()."
						AND published=1
						AND coupon_left >0";
		...
		if(isset($coupon->coupon_value)){

			$rates->isvalid = "true";
			$rates->message = ($rates->free_shipping == 1) ? "Shipping Is Free" : "Success";

			if($coupon->percent_or_total == 1){
				$rates->rate = $merchantCouponData['subtotal']*$coupon->coupon_value;
			}else{
				$rates->rate = $coupon->coupon_value;
			}

			$rates->free_shipping = $coupon->free_shipping;

			return $rates;
		}else{

			$rates->isvalid = "false";
			$rates->message = "Wrong Code";
			$rates->rate = 0;
			$rates->free_shipping = 0;
			return $rates;
		}
	...
	function createShippedInvoicePdf($order_id,$order_status)
	{
			$invoice="";
			if($order_status == "S")
			{
				$redshopMail = new redshopMail();
				$invoice = $redshopMail->createShippedInvoicePdf($order_id);
