...
class economic
{
	...
	{
		$eco['product_desc'] = substr($row->product_desc,0,499);
		$eco['product_s_desc'] = substr($row->product_s_desc,0,499);
		...
		$eco['product_number'] = $row->product_number;
	...
	function createInvoiceInEconomic($order_id, $data=array())
	{
		...
		if($orderdetail->is_booked==0 && !$orderdetail->invoice_no)
		{
			...
			if(count($ecodebtorNumber)>0 && is_object($ecodebtorNumber[0]))
			{
		    	...
		    	$eco['setAttname'] = ($user_billinginfo->is_company==1) ? 1 : 0;
...
//				}
				$eco['vatzone'] = $this->getEconomicTaxZone($user_billinginfo->country_code,$user_billinginfo);
				...
				if(count($invoiceHandle)>0 && $invoiceHandle[0]->Id)
				{
					...
					if($this->_redhelper->isredCRM())
					{
						...
						$crmorder = $crmOrderHelper->getcrmOrderinfo($order_id);
						if(count($crmorder)>0)
						{
							$query = "SELECT * FROM #__redcrm_shipping AS rcs "
									." WHERE rcs.shipping_id ='".$crmorder->shipping_id."' ";
							$this->_db->setQuery($query);
							$rs = $this->_db->loadObject();
							$eco['name_ST'] = $rs->destination_name;

//							if(isset($crmorder->person_id) && $crmorder->person_id != 0)
//							{
//								$contact_persons = $crmDebitorHelper->getContactPersons($crmorder->person_id);
//								if(count($contact_persons)>0 && $contact_persons[0]->person_name!="")
//								{
//									$contact_person = $contact_persons[0]->person_name.''.chr(13);
//								}
//							}
						}
					}
					$eco['address_ST'] = $contact_person.$user_shippinginfo->address;
	...
	function createInvoiceLineInEconomic($orderitem=array(), $invoice_no="", $user_id=0)
	{
		for($i=0;$i<count($orderitem);$i++)
		{
			...
			if ($orderitem[$i]->wrapper_id)
			{
				...
				$displaywrapper = "\n".JText::_ ( 'WRAPPER' ).": ".$wrapper_name."(".$orderitem[$i]->wrapper_price.")";
			}
...
//			}
					...
					if($user_billinginfo->is_company==0 || (!$user_billinginfo->ean_number && $user_billinginfo->is_company==1))
					{
						...
						if(count($currectinvoiceData)>0 && trim($currectinvoiceData[0]->OtherReference)==$orderdetail->order_number)
						{
							...
							if(count($paymentInfo)>0)
							{
								...
								$eco['economic_payment_method']= $payment_name;
								$paymentmethod 						= $this->_order_functions->getPaymentMethodInfo($paymentInfo[0]->payment_method_class);
								if(count($paymentmethod)>0)
								{
									$paymentparams 						= new JParameter( $paymentmethod[0]->params );
									...
									$eco['economic_is_creditcard'] 		= $paymentparams->get('is_creditcard');
								}
							}
	...
	function getEconomicTaxZone($country_code="",$row=array())
	{
		if(isset($row->override_vat_zone) && $row->override_vat_zone==1)
		{
			$taxzone = $row->vat_zone;
		}
		else
		{
