...
function manageStockAmount($section_id=0,$quantity=0,$stockroom_id=0,$section="product")
	...
	function getStockAmountImage($section_id=0,$section="product",$stock_amount=0)
	{
		...
		if(USE_STOCKROOM==1)
		{
			...
			$query = "SELECT * FROM ".$this->_table_prefix."stockroom_amount_image ";

			$query1 = $query."WHERE stock_option=2 AND stock_quantity='".$stock_amount."'";
			$db->setQuery($query1);
			$list = $db->loadObjectList();
			if(count($list)<=0)
			{
				$query1 = $query."WHERE stock_option=1 AND stock_quantity < '".$stock_amount."' ORDER BY stock_quantity DESC ";
				$db->setQuery($query1);
				$list = $db->loadObjectList();
				if(count($list)<=0)
				{
					$query1 = $query."WHERE stock_option=3 AND stock_quantity > '".$stock_amount."' ORDER BY stock_quantity ASC ";
					$db->setQuery($query1);
					$list = $db->loadObjectList();
				}
			}
