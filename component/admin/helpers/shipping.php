...
class shipping
{
	...
	function getDefaultShipping($d)
	{
		...
		$shoppergroup = $this->_userhelper->getShoppergroupData($user_id);
			...
			if ($idx) {
				...
				$newpwhere = str_replace("AND (","OR (",$pwhere);
				 $sql = "SELECT * FROM ".$this->_table_prefix."shipping_rate as sr "
						."LEFT JOIN #__plugins AS s ON sr.shipping_class = s.element
 	     				 WHERE s.folder='redshop_shipping' and s.published =1  and  $wherecountry
						 $iswhere
			...
			if (! $shippingrate) {
				for($i = 0; $i < $idx; $i ++) {
					...
					if ($categorydata) {
						...
						$sql = "SELECT * FROM ".$this->_table_prefix."shipping_rate as sr
									 LEFT JOIN #__plugins AS s
									 ON
									 sr.shipping_class = s.element
			 	     				 WHERE  s.folder='redshop_shipping' and s.published =1  and $wherecountry $whereshopper
									 $iswhere
									 AND ((shipping_rate_volume_start <= '$volume' AND  shipping_rate_volume_end >= '$volume') OR (shipping_rate_volume_end = 0) )
									 AND ((shipping_rate_ordertotal_start <= '$order_subtotal' AND  shipping_rate_ordertotal_end >= '$order_subtotal')  OR (shipping_rate_ordertotal_end = 0))
									 AND ((shipping_rate_weight_start <= '$weighttotal' AND  shipping_rate_weight_end >= '$weighttotal')  OR (shipping_rate_weight_end = 0))
    ...
    function listshippingrates($shipping_class, $users_info_id, &$d)
    {
	    ...
	    $shoppergroup = $this->_userhelper->getShoppergroupData($userInfo->user_id);
		if(count($shoppergroup)>0)
		{
			$shopper_group_id = $shoppergroup->shopper_group_id;
			$whereshopper = ' AND (FIND_IN_SET( "'.$shopper_group_id.'", shipping_rate_on_shopper_group ) OR shipping_rate_on_shopper_group="") ';
	...
	function getShippingVatRates($shipping_tax_group_id, $user_id=0)
	{
		...
		else if($shipping_tax_group_id > 0)
		{
...
//				}
	...
	function getShippingRateError(&$d)
	{
		...
		if($bool)
		{
			...
			if($bool)
			{
				...
				{
					return JText::_("PRODUCT_DETAIL_NOT_MATCH");
				}
			}
			else
			{
				return JText::_("USER_INFORMATION_NOT_MATCH");
			}
		}
		else
		{
			return JText::_("CART_DIMENTION_NOT_MATCH");
		}
	}
    ...
    {
		$cart	 	= $this->_session->get('cart',$cart);
		$idx = ( int ) ($cart ['idx']);
		...
		if(!empty($idx))
			$text 		  = JText::_ ( 'NO_SHIPPING_RATE_AVAILABLE' );
		else
			return JText::_ ( 'NO_SHIPPING_RATE_AVAILABLE_WHEN_NOPRODUCT_IN_CART' );
		...
		if($state)
			...
			$sql = "SELECT * FROM ".$this->_table_prefix."shipping_rate  as sr
								...
								AND $wherecountry $wherestate $whereshopper $zipCond $where
								ORDER BY s.ordering, sr.shipping_rate_priority  LIMIT 0,1";
			...
			if($shippingrate)
			{
				if($shippingrate->shipping_rate_ordertotal_start > $order_subtotal )
				{
					...
					$text 		  = sprintf ( JText::_('SHIPPING_TEXT_LBL'), $this->producthelper->getProductFormattedPrice($diff)  );
				}else if($shippingrate->shipping_rate_ordertotal_start < $order_subtotal && $shippingrate->shipping_rate_ordertotal_end > $order_subtotal){
					$text 		  = JText::_ ( 'FREE_SHIPPING_RATE_IS_IN_USED' );
				}
				else
				{
					$text 		  = JText::_ ( 'NO_SHIPPING_RATE_AVAILABLE' );
