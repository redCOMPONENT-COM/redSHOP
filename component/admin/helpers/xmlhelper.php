...
class xmlHelper
{
	...
	{
	  	$section	= array();
		$section[]	= JHTML::_('select.option', '',			JText::_('SELECT'));
		$section[]  = JHTML::_('select.option', 'product',	JText::_('PRODUCT'));
		$section[]  = JHTML::_('select.option', 'order',	JText::_('ORDER'));
		return $section;
	}
	...
	{
	  	$section	= array();
		$section[]	= JHTML::_('select.option', 0,	JText::_('SELECT'));
		$section[]  = JHTML::_('select.option', 24,	JText::_('24_HOURS'));
		$section[]  = JHTML::_('select.option', 12,	JText::_('12_HOURS'));
		$section[]  = JHTML::_('select.option', 6,	JText::_('6_HOURS'));
		return $section;
	}
	...
	function getSynchIntervalName($value=0)
	{
		...
		{
			case 24:
				$name=JText::_('24_HOURS');
				break;
			case 12:
				$name=JText::_('12_HOURS');
				break;
			case 6:
				$name=JText::_('6_HOURS');
				break;
		}
		return $name;
	}
        ...
        for($i=0;$i<count($datalist);$i++)
		{
	        ...
	        else
	        {
				...
				while (list($prop, $val) = each($datalist[$i]))
		        {
		        	...
		        	{
		        		$val = $url."components/com_redshop/assets/images/product/".$val;
		        	}
		        	if($prop==$prdthmbimage && $val!="")
		        	{
		        		$val = $url."components/com_redshop/assets/images/product/thumb/".$val;
		        	}
			...
			case "product":
				for($i=0;$i<count($datalist);$i++)
				{
					...
					if(array_key_exists('product_full_image',$datalist[$i]) && $datalist[$i]['product_full_image']!="")
					{
						...
						$dest = JPATH_SITE.DS."components".DS."com_redshop".DS."assets/images/product".DS.$filename;
					...
					if(array_key_exists('product_thumb_image',$datalist[$i]) && $datalist[$i]['product_thumb_image']!="")
					{
						...
						$filename = basename($src);
						$dest = JPATH_SITE.DS."components".DS."com_redshop".DS."assets/images/product/thumb".DS.$filename;
...
//				        	echo "<br>";print_r($value);
						...
						if(!empty($datalist[$i]['product_number']))
						{
//							if($catname!="")
//							{
...
//					$field[] = "GROUP_CONCAT(c.category_id) AS category_id";
				...
				else if($key=="product_price")
				{
					...
					//$field[] = "if(p.product_on_sale='1', p.discount_price, p.".$key.") AS ".$value;
				}
				else
				{
