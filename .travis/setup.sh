...
if [ "${ACCEPTANCE}" = "false" ]; then
	...
	sed -i 's|^upload_max_filesize = 2M|upload_max_filesize = 64M|' ~/.phpenv/versions/$(phpenv version-name)/etc/php.ini
    sed -i 's|^post_max_size = 8M|post_max_size = 64M|' ~/.phpenv/versions/$(phpenv version-name)/etc/php.ini
