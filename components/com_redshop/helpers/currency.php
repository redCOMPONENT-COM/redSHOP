    ...
    public function init()
    {
        ...
        if (!is_array(@$GLOBALS['converter_array']) && @$GLOBALS['converter_array'] !== -1)
        {
            ...
            if (file_exists($archivefile_name) && filesize($archivefile_name) > 0)
            {
                ...
                if (date('w') > 0 && date('w') < 6 // compare filedate and actual date
                    && $file_datestamp != $date_now_local // if localtime is greater then ecb-update-time go on to update and write files
