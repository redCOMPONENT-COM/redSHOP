...
class producthelper
{
    ...
    {
        $query = 'SELECT * FROM #__redshop_wishlist_userfielddata  WHERE wishlist_id = "' . $wishlistid . '"  and product_id="' . $productid . '" order by fieldid ASC';
        $this->_db->setQuery($query);
    ...
    {
        $eu_countries = array('AUT', 'BGR', 'BEL', 'CYP', 'CZE', 'DEU', 'DNK', 'ESP', 'EST', 'FIN', 'FRA', 'FXX', 'GBR', 'GRC', 'HUN', 'IRL', 'ITA', 'LVA', 'LTU', 'LUX', 'MLT', 'NLD', 'POL', 'PRT', 'ROM', 'SVK', 'SVN', 'SWE');
    ...
    function getProductPrices($product_id, $userid, $quantity = 1)
    {
        ...
        $query->where($and);
        $query->where(' (( p.price_quantity_start <= "' . $quantity . '" AND p.price_quantity_end >= "' . $quantity . '" ) OR (p.price_quantity_start = "0" AND p.price_quantity_end = "0"))');
    ...
    function getProductSpecialId($userid)
    {
        if ($userid)
        {
            $sql = "SELECT ps.discount_product_id FROM " . $this->_table_prefix . "users_info AS ui " . " LEFT JOIN " . $this->_table_prefix . "discount_product_shoppers AS ps ON ui.shopper_group_id = ps.shopper_group_id " . " WHERE user_id = '" . $userid . "' AND address_type='BT'";
        ...
        else
        {
            ...
            {
                $query = "SELECT * FROM " . $this->_table_prefix . "discount_product_shoppers AS ps " . "WHERE ps.shopper_group_id ='" . $shopperGroupId . "'";
                $this->_db->setQuery($query);
    ...
    function getVatUserinfo($user_id = 0)
    {
        ...
        else
        {
            ...
            {
                $query = "SELECT country_code,state_code FROM " . $this->_table_prefix . "users_info AS u " . "LEFT JOIN " . $this->_table_prefix . "shopper_group AS sh ON sh.shopper_group_id=u.shopper_group_id " . "WHERE u.users_info_id='" . $users_info_id . "' " . "order by u.users_info_id ASC LIMIT 0,1";
                $this->_db->setQuery($query);
    ...
    function getVatRates($product_id = 0, $user_id = 0, $vat_rate_id = 0)
    {
        ...
        $query = 'SELECT tr.* FROM ' . $this->_table_prefix . 'tax_rate as tr ' . $q2 . 'WHERE tr.tax_country="' . $userdata->country_code . '" ' . 'AND (tr.tax_state = "' . $userdata->state_code . '" OR tr.tax_state = "") ' . $and . ' ORDER BY `tax_rate` DESC LIMIT 0,1';
        $this->_db->setQuery($query);
    ...
    function getGoogleVatRates($product_id = 0, $product_price = 0, $tax_exempt = 0)
    {
        ...
        $where = $q2 . 'WHERE tr.tax_country="' . $country_code . '" ' . 'AND (tr.tax_state = "' . $state_code . '" OR tr.tax_state = "") ' . $and;

        $query = 'SELECT tr.* FROM ' . $this->_table_prefix . 'tax_rate as tr ' . $where . ' ORDER BY `tax_rate` DESC';
        $this->_db->setQuery($query);
    ...
    function getProductMinDeliveryTime($product_id = 0, $section_id = 0, $section = '', $loadDiv = 1)
    {
        ...
        {
            $query = "SELECT  min_del_time as deltime, s.max_del_time, s.delivery_time " . " FROM " . $this->_table_prefix . "product_stockroom_xref AS ps , " . $this->_table_prefix . "stockroom as s " . " WHERE " . " ps.product_id = '" . $product_id . "' AND ps.stockroom_id = s.stockroom_id and ps.quantity >0 ORDER BY min_del_time ASC LIMIT 0,1";
        }
        else
        {
            $query = "SELECT  min_del_time as deltime, s.max_del_time, s.delivery_time " . " FROM " . $this->_table_prefix . "product_attribute_stockroom_xref AS pas , " . $this->_table_prefix . "stockroom as s " . " WHERE " . " pas.section_id = '" . $section_id . "' AND pas.section = '" . $section . "' AND pas.stockroom_id = s.stockroom_id and pas.quantity >0 ORDER BY min_del_time ASC LIMIT 0,1";
        }
        $this->_db->setQuery($query);
        $result = $this->_db->loadObject();
        ...
        if ($result)
        {
            if (!$section_id && !$section)
            {
                $sql = "SELECT  min_del_time as deltime, s.max_del_time,s.delivery_time " . " FROM " . $this->_table_prefix . "stockroom as s, " . $this->_table_prefix . "product_stockroom_xref AS ps  " . " WHERE " . " ps.product_id = '" . $product_id . "' AND ps.stockroom_id = s.stockroom_id AND s.min_del_time = " . $result->deltime . " and ps.quantity >=0 ORDER BY max_del_time ASC LIMIT 0,1";
            }
            else
            {
                $sql = "SELECT  min_del_time as deltime, s.max_del_time,s.delivery_time " . " FROM " . $this->_table_prefix . "stockroom as s, " . $this->_table_prefix . "product_attribute_stockroom_xref AS pas  " . " WHERE " . " pas.section_id = '" . $section_id . "' AND pas.section = '" . $section . "' AND pas.stockroom_id = s.stockroom_id AND s.min_del_time = " . $result->deltime . " and pas.quantity >=0 ORDER BY max_del_time ASC LIMIT 0,1";
            }
            $this->_db->setQuery($sql);
    ...
    function getProductQuantityPrice($product_id, $userid)
    {
        ...
        if ($userid)
        {
            $query = "SELECT p.* FROM " . $this->_table_prefix . "users_info AS u " . "LEFT JOIN " . $this->_table_prefix . "product_price AS p ON u.shopper_group_id = p.shopper_group_id " . "WHERE p.product_id = '" . $product_id . "' " . "AND u.user_id='" . $userid . "' AND u.address_type='BT' " . "ORDER BY price_quantity_start ASC ";
        }
        else
        {
            $query = "SELECT p.* FROM " . $this->_table_prefix . "product_price AS p " . "WHERE p.product_id = '" . $product_id . "' " . "AND p.shopper_group_id = '" . $shopperGroupId . "' " . "ORDER BY price_quantity_start ASC ";
        }
        $this->_db->setQuery($query);
        $result        = $this->_db->loadObjectList();
        $quantitytable = '';
        ...
        {
            $quantitytable = "<table>";
            $quantitytable .= "<tr><th>" . JText::_('COM_REDSHOP_QUANTITY') . "</th><th>" . JText::_('COM_REDSHOP_PRICE') . "</th></tr>";
            foreach ($result as $r)
            {
    ...
    function getDiscountId($subtotal = 0, $user_id = 0)
    {
        ...
        $sql = "SELECT ds.discount_id FROM " . $this->_table_prefix . "discount_shoppers AS ds " . " WHERE ds.shopper_group_id = '" . $shopperGroupId . "' ";
        ...
        if ($discountid)
        {
            $query   = "SELECT * FROM " . $this->_table_prefix . "discount " . "WHERE published =1 " . "AND discount_id IN (" . $discountid . ") " . "AND `start_date`<='" . time() . "' " . "AND `end_date` >='" . time() . "' ";
            $orderby = " ORDER BY `amount` DESC LIMIT 0,1";
    ...
    function getAdditionMediaImage($section_id = 0, $section = "", $mediaType = "images")
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "media AS m " . $left . "WHERE m.media_section='" . $section . "' " . "AND m.media_type='" . $mediaType . "' " . "AND m.section_id='" . $section_id . "' " . "AND m.published=1 " . "ORDER BY m.ordering,m.media_id ASC";
        $this->_db->setQuery($query);
    ...
    function getAltText($media_section, $section_id, $media_name = '', $media_id = 0, $mediaType = "images")
    {
        ...
        $query = 'SELECT * FROM ' . $this->_table_prefix . 'media ' . 'WHERE media_section = "' . $media_section . '" ' . 'AND section_id ="' . $section_id . '" ' . 'AND media_type="' . $mediaType . '" ' . $and;
        $this->_db->setQuery($query);
    ...
    function getUserInformation($userid = 0, $address_type = 'BT', $rs_user_info_id = 0)
    {
        ...
        if ($userid)
        {
            $query = "SELECT sh.*,u.* FROM " . $this->_table_prefix . "users_info AS u " . "LEFT JOIN " . $this->_table_prefix . "shopper_group AS sh ON sh.shopper_group_id=u.shopper_group_id " . "WHERE u.user_id='" . $userid . "' " . $and . "order by u.users_info_id ASC LIMIT 0,1";
            $this->_db->setQuery($query);
    ...
    function checkDiscountDate($productid = 0)
    {
        $discountprice = 0;
        $today         = time();
        $query         = "SELECT * FROM " . $this->_table_prefix . "product " . "WHERE product_id = $productid " . "AND ( (discount_enddate='' AND discount_stratdate='') " . "OR ( discount_enddate>='" . $today . "' AND discount_stratdate<='" . $today . "') )";
    ...
    function getPropertyPrice($section_id = '', $quantity = '', $section = '', $user_id = 0)
    {
        ...
        $query = "SELECT p.price_id,p.product_price,p.product_currency,p.discount_price, p.discount_start_date, p.discount_end_date  " . "FROM " . $this->_table_prefix . "product_attribute_price AS p " . $leftjoin . " WHERE p.section_id = '" . $section_id . "' AND section = '" . $section . "' " . $and . " AND ( (p.price_quantity_start <= '" . $quantity . "' and p.price_quantity_end >= '" . $quantity . "') OR (p.price_quantity_start = '0' AND p.price_quantity_end = '0')) " . "ORDER BY price_quantity_start ASC LIMIT 0,1 ";
        $this->_db->setQuery($query);
        $result = $this->_db->loadObject();
    ...
    function getProperty($section_id, $section)
    {
        if ($section == 'property')
        {
            $query = "SELECT p.*,property_price as product_price  " . " FROM " . $this->_table_prefix . "product_attribute_property AS p " . " WHERE " . " p.property_id = '" . $section_id . "' ";
        }
        if ($section == 'subproperty')
        {
            $query = "SELECT p.*,subattribute_color_price as product_price  " . " FROM " . $this->_table_prefix . "product_subattribute_color AS p " . " WHERE " . " p.subattribute_color_id = '" . $section_id . "' ";
        }
        $this->_db->setQuery($query);
        $result = $this->_db->loadObject();
        return $result;
    ...
    function getWrapper($product_id, $wrapper_id = 0, $default = 1)
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "product_category_xref " . "WHERE product_id = '" . $product_id . "' ";
        $this->_db->setQuery($query);
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "wrapper " . "WHERE published = 1 " . "AND (FIND_IN_SET(" . $product_id . ",product_id) " . $usetoall . " )" . $and;
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectList();
    ...
    function getSection($section = "", $id = 0)
    {
        ...
        $query = " SELECT * FROM " . $this->_table_prefix . $section . $and;
        $this->_db->setQuery($query);
    ...
    function getMenuDetail($link = "")
    {
        ...
        $query = "SELECT * FROM #__menu " . "WHERE published=1 " . $and;
        $this->_db->setQuery($query);
        $res = $this->_db->loadObject();
        return $res;
    }
    ...
    function getMenuInformation($Itemid = 0, $sectionid = 0, $sectioname = "", $menuview = "", $isRedshop = true)
    {
        ...
        $query = "SELECT * FROM #__menu " . "WHERE published=1 " . $and . " ORDER BY id ";
        $this->_db->setQuery($query);
        $res = $this->_db->loadObject();
        return $res;
    }
    ...
    function getProductCategory($id = 0)
    {
        ...
        $query = "SELECT p.product_id FROM " . $this->_table_prefix . "product_category_xref pc" . " LEFT JOIN " . $this->_table_prefix . "product AS p ON pc.product_id=p.product_id " . " WHERE category_id='" . $id . "' " . $and;
        $this->_db->setQuery($query);
        $res = $this->_db->loadObjectlist();
        return $res;
    }
    ...
    {
        $query = 'SELECT product_download,product_download_days,product_download_limit,product_download_clock,product_download_clock_min,product_download_infinite FROM ' . $this->_table_prefix . 'product ' . 'WHERE product_id ="' . $pid . '" ';
    ...
    {
        $query = 'SELECT media_name FROM ' . $this->_table_prefix . 'media ' . 'WHERE media_section = "product" ' . 'AND media_type="download" ' . 'AND published=1 AND section_id="' . $product_id . '" ';
        $this->_db->setQuery($query);
        $res = $this->_db->loadObjectList();
        return $res;
    }
    ...
    {
        $query = "SELECT * FROM " . $this->_table_prefix . "giftcard " . "WHERE giftcard_id='" . $gid . "' ";
        $this->_db->setQuery($query);
        $res = $this->_db->loadObject();
        return $res;
    }
    ...
    function getValidityDate($period, $data)
    {
        ...
        return $data;
        //return JText::_('COM_REDSHOP_TO').' '.$fromdate.' '.JText::_('COM_REDSHOP_FROM')." ". $todate;
        //$date = strtotime(date());
    ...
    function insertProdcutUserfield($id = 'NULL', $cart = array(), $order_item_id = 0, $section_id = 12)
    {
        ...
        for ($i = 0; $i < count($row_data); $i++)
        {
            if (array_key_exists($row_data[$i]->field_name, $cart[$id]) && $cart[$id][$row_data[$i]->field_name])
            {
                ...
                if (trim($user_fields) != '')
                {
                    $sql = "INSERT INTO " . $this->_table_prefix . "fields_data " . "(fieldid,data_txt,itemid,section) " . "value ('" . $row_data[$i]->field_id . "','" . addslashes($user_fields) . "','" . $order_item_id . "','" . $section_id . "')";
                    $this->_db->setQuery($sql);
                    $this->_db->query();
                }
            }
    ...
    function getProductAttribute($product_id = 0, $attribute_set_id = 0, $attribute_id = 0, $published = 0, $attribute_required = 0, $notAttributeId = 0)
    {
        ...
        $query = "SELECT a.attribute_id AS value,a.attribute_name AS text,a.*,ast.attribute_set_name " . "FROM " . $this->_table_prefix . "product_attribute AS a " . "LEFT JOIN " . $this->_table_prefix . "attribute_set AS ast ON ast.attribute_set_id=a.attribute_set_id " . $astpublished . "WHERE a.attribute_name!='' " . $and . " and attribute_published=1 ORDER BY a.ordering ASC ";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    function getAttibuteProperty($property_id = 0, $attribute_id = 0, $product_id = 0, $attribute_set_id = 0, $required = 0, $notPropertyId = 0)
    {
        ...
        $query = "SELECT ap.property_id AS value,ap.property_name AS text,ap.*, a.attribute_name " . "FROM " . $this->_table_prefix . "product_attribute_property AS ap " . "LEFT JOIN " . $this->_table_prefix . "product_attribute AS a ON a.attribute_id = ap.attribute_id " . "WHERE 1=1 " . $and . "ORDER BY ap.ordering ASC ";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    function getAttibuteSubProperty($subproperty_id = 0, $property_id = 0)
    {
        ...
        $query = "SELECT sp.subattribute_color_id AS value, sp.subattribute_color_name AS text" . ",sp.*,p.property_name,p.setrequire_selected,p.setmulti_selected FROM " . $this->_table_prefix . "product_subattribute_color AS sp " . "LEFT JOIN " . $this->_table_prefix . "product_attribute_property AS p ON p.property_id=sp.subattribute_id " . "WHERE 1=1 " . $and . " ORDER BY sp.ordering ASC";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    function getProductAccessory($accessory_id = 0, $product_id = 0, $child_product_id = 0, $cid = 0)
    {
        ...
        $query = "SELECT a.*,p.product_number, p.product_name, " . $mainpricequery . ", p.product_s_desc, p.product_full_image, p.product_on_sale " . $switchquery . "FROM " . $this->_table_prefix . "product_accessory AS a " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = a.child_product_id " . "WHERE p.published = 1 " . $and . $groupby . $orderby;
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    function getProductNavigator($accessory_id = 0, $product_id = 0, $child_product_id = 0, $cid = 0)
    {
        ...
        $query = "SELECT a.*, p.product_name, p.product_number " . "FROM " . $this->_table_prefix . "product_navigator AS a " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = a.child_product_id " . "WHERE p.published = 1 " . $and . $orderby;
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    function getRelatedProduct($product_id = 0, $related_id = 0)
    {
        ...
        if ($product_id != 0)
        {
            ...
            if (TWOWAY_RELATED_PRODUCT)
            {
                ...
                $query = "SELECT * FROM " . $this->_table_prefix . "product_related AS r " . "WHERE r.product_id IN (" . $product_id . ") OR r.related_id IN (" . $product_id . ")" . $orderby_related . "";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObjectlist();
                ...
                $query = "SELECT " . $product_id . " AS mainproduct_id,p.* " . "FROM " . $this->_table_prefix . "product AS p " . "WHERE p.published = 1 " . "AND p.product_id IN (" . $InProduct . ") " . $orderby;
                $this->_db->setQuery($query);
                $list = $this->_db->loadObjectlist();
                return $list;
            }
        ...
        $query = "SELECT r.product_id AS mainproduct_id,p.* " . $add_e . " " . "FROM " . $this->_table_prefix . "product_related AS r " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = r.related_id ";
        if (count($finaltypetype_result) > 0 && $finaltypetype_result->extrafield != '' && (DEFAULT_RELATED_ORDERING_METHOD == 'e.data_txt ASC' || DEFAULT_RELATED_ORDERING_METHOD == 'e.data_txt DESC'))
        {
        ...
        if ($totalAccessory > 0)
        {
            ...
            if (strstr($acctemplate_data, "{if accessory_main}") && strstr($acctemplate_data, "{accessory_main end if}"))
            {
                ...
                if (is_file(REDSHOP_FRONT_IMAGES_RELPATH . "product/" . $accessory_main_image))
                {
                    ...
                    {
                        $accessorymainimage = "<a id='a_main_image' href='" . REDSHOP_FRONT_IMAGES_ABSPATH . "product/" . $accessory_main_image . "' title='' class=\"modal\" rel=\"{handler: 'image', size: {}}\"><img id='main_image' class='redAttributeImage' src='" . $url . "components/com_redshop/helpers/thumb.php?filename=product/" . $accessory_main_image . "&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
                    }
                    else
                    {
            ...
            if (strstr($acctemplate_data, "{accessory_product_start}") && strstr($acctemplate_data, "{accessory_product_end}"))
            {
                ...
                for ($a = 0; $a < count($accessory); $a++)
                {
                    ...
                    if (ACCESSORY_PRODUCT_IN_LIGHTBOX == 1)
                    {
                        ...
                        {
                            $accessoryimage = "<a id='a_main_image" . $accessory [$a]->accessory_id . "' href='" . REDSHOP_FRONT_IMAGES_ABSPATH . "product/" . $accessory_image . "' title='' class=\"modal\" rel=\"{handler: 'image', size: {}}\"><img id='main_image" . $accessory [$a]->accessory_id . "' class='redAttributeImage' src='" . $url . "/components/com_redshop/helpers/thumb.php?filename=product/" . $accessory_image . "&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
                        }
                        else
                        {
                            $accessoryimage = "<a id='a_main_image" . $accessory [$a]->accessory_id . "' href='" . REDSHOP_FRONT_IMAGES_ABSPATH . "noimage.jpg' title='' class=\"modal\" rel=\"{handler: 'image', size: {}}\"><img id='main_image" . $accessory [$a]->accessory_id . "' class='redAttributeImage' src='" . $url . "/components/com_redshop/helpers/thumb.php?filename=noimage.jpg&newxsize=" . $aw_thumb . "&newysize=" . $ah_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "' /></a>";
                        }
                    }
                    else
                    {
                    ...
                    $accessory_productdetail = "<a href='" . $readmorelink . "' title='" . $accessory [$a]->product_name . "'>" . JText::_('COM_REDSHOP_READ_MORE') . "</a>";
                    //				$accessory_productdetail = "<a href='#' onclick='window.open(\"".$url. "index.php?option=com_redshop&view=product&pid=".$ac_id."&tmpl=component\",\"accessoryproduct\",\"scrollbars=1\",\"location=1\")' title='".JText::_('COM_REDSHOP_READ_MORE')."'>".JText::_('COM_REDSHOP_READ_MORE')."</a>";
                    ...
                    $accessory_div      = str_replace("{accessory_add_chkbox_lbl}", JText::_('COM_REDSHOP_ACCESSORY_ADD_CHKBOX_LBL') . '&nbsp;' . $accessory [$a]->product_name, $accessory_div);
            ...
            for ($a = 0; $a < count($attributes); $a++)
            {
                ...
                if ($attributes[$a]->text != "" && count($property) > 0 && strstr($attribute_table, "{property_start}") && strstr($attribute_table, "{property_end}"))
                {
                    ...
                    for ($i = 0; $i < count($property); $i++)
                    {
                        ...
                        $property_image = "";
                        if ($property[$i]->property_image && is_file(REDSHOP_FRONT_IMAGES_RELPATH . "product_attributes/" . $property[$i]->property_image))
                        {
                    ...
                    for ($i = 0; $i < count($property); $i++)
                    {
                        ...
                        if ($property[$i]->property_image)
                        {
                            if (is_file(REDSHOP_FRONT_IMAGES_RELPATH . "product_attributes/" . $property[$i]->property_image))
                            {
                                ...
                                //$property_scrollerdiv .= "<td id='".$propertyid."_propimg_".$property[$i]->value."'><a onclick='setPropImage(\"".$product_id."\",\"".$propertyid."\",\"".$property[$i]->value."\");changePropertyDropdown(\"" .$product_id. "\",\"" .$accessory_id. "\",\"" .$relproduct_id. "\",\"" . $attributes [$a]->value . "\",\"".$property[$i]->value."\");'><img class='redAttributeImage' width='50' height='50' src='".REDSHOP_FRONT_IMAGES_ABSPATH."product_attributes/thumb/".$property[$i]->property_image."'></a></td>";
                        ...
                        $attribute_table .= '<input type="hidden" id="' . $propertyid . '_proprice' . $property [$i]->value . '" value="' . $attributes_property_vat_show . '" />';
                ...
                for ($i = 0; $i < count($subproperty); $i++)
                {
                    ...
                    if ($subproperty[$i]->subattribute_color_image)
                    {
                        if (is_file(REDSHOP_FRONT_IMAGES_RELPATH . "subcolor/" . $subproperty[$i]->subattribute_color_image))
                        {
                            ...
                            $subproperty_woscrollerdiv .= "<div id='" . $subpropertyid . "_subpropimg_" . $subproperty[$i]->value . "' class='subproperty_image_inner'><a onclick='setSubpropImage(\"" . $product_id . "\",\"" . $subpropertyid . "\",\"" . $subproperty[$i]->value . "\");calculateTotalPrice(\"" . $product_id . "\",\"" . $relatedprd_id . "\");displayAdditionalImage(\"" . $product_id . "\",\"" . $accessory_id . "\",\"" . $relatedprd_id . "\",\"" . $property_id . "\",\"" . $subproperty[$i]->value . "\");'><img class='redAttributeImage'  src='" . $url . "/components/com_redshop/helpers/thumb.php?filename=subcolor/" . $subproperty[$i]->subattribute_color_image . "&newxsize=" . $mpw_thumb . "&newysize=" . $mph_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING . "'></a></div>";
    ...
    function replacePropertyAddtoCart($product_id = 0, $property_id = 0, $category_id = 0, $commonid = "", $property_stock = 0, $property_data = "", $cart_template = array(), $data_add = "")
    {
        ...
        if (strstr($cartform, "{addtocart_image_aslink}"))
        {
            ...
            $cartIcon = '<span ' . $class . ' ' . $title . ' id="pdaddtocart' . $stockId . '"><img ' . $onclick . $cartTitle . ' alt="' . $ADD_OR_LBL . '" style="cursor: pointer;" src="' . REDSHOP_FRONT_IMAGES_ABSPATH . $ADD_CART_IMAGE . '" /></span>';
        }
        if (strstr($cartform, "{addtocart_image}"))
        {
            $cartTag  = "{addtocart_image}";
            $cartIcon = '<span id="pdaddtocart' . $stockId . '" ' . $class . ' ' . $title . '><div ' . $onclick . $cartTitle . ' align="center" style="cursor:pointer;background:url(' . REDSHOP_FRONT_IMAGES_ABSPATH . $ADD_CART_BACKGROUND . ');background-position:bottom;background-repeat:no-repeat;">' . $ADD_OR_LBL . '</div></span>';
        }
    ...
    function replaceCartTemplate($product_id = 0, $category_id = 0, $accessory_id = 0, $relproduct_id = 0, $data_add = "", $isChilds = false, $userfieldArr = array(), $totalatt = 0, $totalAccessory = 0, $count_no_user_field = 0, $module_id = 0, $giftcard_id = 0)
    {
        ...
        if (!$isStockExists)
        {
            ...
            else
            {
                ...
                $stockdisplay  = true;
                $add_cart_flag = true;
                //				$data_add = str_replace ( "{form_addtocart:$cart_template->template_name}", JText::_('COM_REDSHOP_PRODUCT_OUTOFSTOCK_MESSAGE' ), $data_add );
            }
        }
        else
        {
    ...
    function replaceWishlistButton($product_id = 0, $data_add = "")
    {
        ...
        if (MY_WISHLIST != 0)
        {
            ...
            { //allow user to send wishlist while user is not logged in
                $mywishlist_link = JURI::root() . 'index.php?tmpl=component&option=com_redshop&view=wishlist&task=addtowishlist&tmpl=component&product_id=' . $product_id;
            ...
            else
            {
                ...
                if ($wishtag == '{wishlist_button}')
                {
                    $wishlist = "<input type='submit' name='btnwishlist' id='btnwishlist' value='" . JText::_("COM_REDSHOP_ADD_TO_WISHLIST") . "'>";
                }
                if ($wishtag == '{wishlist_link}')
                {
                    ...
                    {
                        $wishlist = "<a href='javascript:document.form_wishlist_" . $product_id . ".submit();' class='wishlistlink'>" . JText::_("COM_REDSHOP_ADD_TO_WISHLIST") . "</a>";
                    }
                }
                if (WISHLIST_LOGIN_REQUIRED != 0)
                {
    ...
    function replaceCompareProductsButton($product_id = 0, $category_id = 0, $data_add = "", $is_relatedproduct = 0)
    {
        ...
        if (PRODUCT_COMPARISON_TYPE != "")
        {
            if (strstr($data_add, '{' . $prefix . 'compare_product_div}'))
            {
                ...
                $compare_product_div .= "<a href='javascript:compare();' >" . JText::_('COM_REDSHOP_COMPARE') . "</a><br />";
            ...
            if (strstr($data_add, '{' . $prefix . 'compare_products_button}'))
            {
                ...
                $compare_product = "<input type='checkbox' id='chk" . $category_id . $product_id . "' " . $chked . " onClick='add_to_compare(" . $product_id . "," . $category_id . ")'>" . JText::_("COM_REDSHOP_ADD_TO_COMPARE");
    ...
    function makeAttributeCart($attArr = array(), $product_id = 0, $user_id = 0, $new_product_price = 0, $quantity = 1, $data = '')
    {
        ...
        for ($i = 0; $i < count($attArr); $i++)
        {
            ...
            for ($k = 0; $k < count($propArr); $k++)
            {
                ...
                for ($l = 0; $l < count($subpropArr); $l++)
                {
                    ...
                    //$displayattribute .= "<div class='checkout_subattribute_price'>".urldecode($subpropArr[$l]['subattribute_color_title'])." : ".urldecode($subpropArr[$l]['subproperty_name']).$displayPrice.$virtualNumber."</div>";
        ...
        if ($displayattribute != "")
        {
            $displayattribute = "<div class='checkout_attribute_static'>" . JText::_("COM_REDSHOP_ATTRIBUTE") . "</div>" . $displayattribute;
        }
    ...
    {
        $query = "SELECT * FROM " . $this->_table_prefix . "product_subscribe_detail AS p " . "LEFT JOIN " . $this->_table_prefix . "product_subscription AS ps ON ps.subscription_id=p.subscription_id " . "WHERE order_item_id='" . $order_item_id . "' ";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObject();
        return $list;
    }
    ...
    function insertProductDownload($product_id, $user_id, $order_id, $media_name, $serial_number)
    {
        ...
        $sql = "INSERT INTO " . $this->_table_prefix . "product_download " . "(product_id,user_id,order_id, end_date, download_max, download_id, file_name,product_serial_number) " . "VALUES('" . $product_id . "', '" . $user_id . "', '" . $order_id . "', " . "'" . $endtime . "', '" . $product_download_limit . "', " . "'" . $token . "', '" . $media_name . "','" . $serial_number . "')";
        $this->_db->setQuery($sql);
        $this->_db->query();
        return true;
    }
    ...
    {
        $query = "SELECT * FROM " . $this->_table_prefix . "product_serial_number " . "WHERE product_id = '" . $product_id . "' " . " AND is_used='" . $is_used . "' " . " LIMIT 0,1";
        $this->_db->setQuery($query);
    ...
    {
        $update_query = "UPDATE " . $this->_table_prefix . "product_serial_number " . " SET is_used='1' WHERE serial_id='" . $serial_id . "'";
    ...
    {
        $query = "SELECT * FROM " . $this->_table_prefix . "product_subscription " . "WHERE product_id='" . $product_id . "' " . "ORDER BY subscription_id ";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    function getQuestionAnswer($questionid = 0, $productid = 0, $faq = 0, $front = 0)
    {
        ...
        $query = "SELECT q.* FROM " . $this->_table_prefix . "customer_question AS q " . "WHERE 1=1 " . $and . "ORDER BY q.ordering ";
        $this->_db->setQuery($query);
    ...
    function getProductRating($product_id)
    {
        $url        = JURI::base();
        $avgratings = 0;
        $query      = "SELECT pr.* FROM " . $this->_table_prefix . "product_rating AS pr " . "WHERE pr.product_id='" . $product_id . "' AND pr.published=1";
        $this->_db->setQuery($query);
        $allreviews   = $this->_db->loadObjectList();
        $totalreviews = count($allreviews);

        $query = "SELECT SUM(user_rating) AS rating FROM " . $this->_table_prefix . "product_rating AS pr " . "WHERE pr.product_id='" . $product_id . "' AND pr.published=1";
        $this->_db->setQuery($query);
    ...
    {
        $query = "SELECT ui.firstname,ui.lastname,pr.* FROM " . $this->_table_prefix . "product_rating AS pr " . "LEFT JOIN " . $this->_table_prefix . "users_info AS ui ON ui.user_id=pr.userid " . "WHERE pr.product_id='" . $product_id . "' " . "AND pr.published = 1 AND ui.address_type LIKE 'BT' " . "ORDER BY pr.favoured DESC";
        $this->_db->setQuery($query);
    ...
    function makeCompareProductDiv()
    {
        ...
        for ($i = 0; $i < $idx; $i++)
        {
            ...
            $div .= "<li>" . $product->product_name . " <a onClick='javascript:add_to_compare(" . $product_id . "," . $category_id . ",\"remove\")' href='javascript:void(0)'>" . JText::_('COM_REDSHOP_DELETE') . "</a></li>";
            $moddiv .= '<tr valign="top"><td width="95%"><span><a href="' . $product_link . '">' . $product->product_name . '</a></span></td>';
            $moddiv .= '<td width="5%"><span><a href="javascript:void(0);" onClick="javascript:remove_compare(' . $product_id . ',' . $category_id . ')">' . JText::_('COM_REDSHOP_DELETE') . '</a></span></td></tr>';
        }
        $moddiv .= "</table>";
    ...
    {
        $query = "SELECT product_parent_id,product_id,product_name,product_number FROM " . $this->_table_prefix . "product " . "WHERE product_parent_id='" . $product_id . "' and published = 1 order by product_id";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    {
        $query = "SELECT product_parent_id FROM " . $this->_table_prefix . "product " . "WHERE published=1 " . "AND product_id='" . $parent_id . "' ";
        $this->_db->setQuery($query);
    ...
    function getCategoryCompareTemplate($cid)
    {
        ...
        $query = "SELECT t.template_id  FROM " . $this->_table_prefix . "template  AS t " . "LEFT JOIN " . $this->_table_prefix . "category  AS c ON c.compare_template_id=t.template_id " . "WHERE c.category_id='" . $cid . "' " . "AND t.published=1";
        $this->_db->setQuery($query);
    ...
    function getProductCaterories($product_id, $displaylink = 0)
    {
        ...
        $query               = "SELECT  ct.category_name, ct.category_id FROM " . $this->_table_prefix . "category AS ct " . "LEFT JOIN " . $this->_table_prefix . "product_category_xref AS pct ON ct.category_id=pct.category_id " . "WHERE pct.product_id='" . $product_id . "' " . "AND ct.published=1 ";
        $this->_db->setQuery($query);
        ...
        for ($i = 0; $i < count($rows); $i++)
        {
            ...
            $query = "SELECT cx.category_parent_id,c.category_name FROM " . $this->_table_prefix . "category_xref AS cx " . "LEFT JOIN " . $this->_table_prefix . "category AS c ON cx.category_parent_id=c.category_id " . "WHERE cx.category_child_id='" . $row->category_id . "' ";
            $this->_db->setQuery($query);
            $parentCat = $this->_db->loadObject();
            if (count($parentCat) > 0 && $parentCat->category_parent_id)
            {
                $pCat  = $parentCat->category_name;
                $query = "SELECT cx.category_parent_id,c.category_name FROM " . $this->_table_prefix . "category_xref AS cx " . "LEFT JOIN " . $this->_table_prefix . "category AS c ON cx.category_parent_id=c.category_id " . "WHERE cx.category_child_id='" . $parentCat->category_parent_id . "' ";
                $this->_db->setQuery($query);
    ...
    function getdisplaymainImage($product_id = 0, $property_id = 0, $subproperty_id = 0, $pw_thumb = 0, $ph_thumb = 0, $redview = "")
    {
        ...
        if ($property_id > 0)
        {
            ...
            if (count($property) > 0 && is_file(REDSHOP_FRONT_IMAGES_RELPATH . "property/" . $property[0]->property_main_image))
            {
                $type                = 'property';
        ...
        if ($subproperty_id > 0)
        {
            ...
            if (count($subproperty) > 0 && is_file(REDSHOP_FRONT_IMAGES_RELPATH . "subproperty/" . $subproperty[0]->subattribute_color_main_image))
            {
                $type                = 'subproperty';
		...
		{
			$productmainimg= $redhelper->watermark($type,$imagename,$pw_thumb,$ph_thumb,WATERMARK_PRODUCT_IMAGE);
			...
			if((!PRODUCT_ADDIMG_IS_LIGHTBOX || !PRODUCT_DETAIL_IS_LIGHTBOX) && $redview!="category")
				    $mainImageResponse = $url."components/com_redshop/helpers/thumb.php?filename=$type/".$imagename."&newxsize=".$pw_thumb."&newysize=".$ph_thumb."&swap=".USE_IMAGE_SIZE_SWAPPING;
		}*/
    ...
    function displayAdditionalImage($product_id = 0, $accessory_id = 0, $relatedprd_id = 0, $property_id = 0, $subproperty_id = 0, $main_imgwidth = 0, $main_imgheight = 0, $redview = "", $redlayout = "")
    {
                ...
                if ($thumb && ($thumb != $media_image [$m]->subattribute_color_main_image) && is_file(REDSHOP_FRONT_IMAGES_RELPATH . $filedir . "/" . $thumb))
                {
                    ...
                    else
                    {
                        ...
                        if (is_file($subproperty_filename_org))
                        {
                            ...
                            $subproperty_img_path_org   = $url . "components/" . $option . "/helpers/thumb.php?filename=subproperty/" . $media_image [$m]->subattribute_color_main_image . "&newxsize=" . $pw_thumb . "&newysize=" . $ph_thumb . "&swap=" . USE_IMAGE_SIZE_SWAPPING;
                        }
                        else
                        {
        ...
        if (USE_STOCKROOM == 1 && $accessory_id == 0)
        {
            ...
            if (count($stockamountList) > 0)
            {
                ...
                $stockamountSrc     = REDSHOP_FRONT_IMAGES_ABSPATH . 'stockroom' . DS . $stockamountList[0]->stock_amount_image;
            }
        }
    ...
    {
        $query = "SELECT c.category_name FROM " . $this->_table_prefix . "product_category_xref AS pcx " . "LEFT JOIN " . $this->_table_prefix . "category AS c ON c.category_id=pcx.category_id " . "WHERE pcx.product_id=" . $pid . " AND c.category_name IS NOT NULL ORDER BY c.category_id ASC LIMIT 0,1";
