...
class cron {
		...
		if($data != 1){ // Default $data != 1
			...
			if(USE_CONTAINER){
				...
				foreach ($data as $cont_data){
					...
					$remain_day = ($cont_data->max_del_time*7) - floor($diff_day); // calculation of Days For moving

					if($remain_day>=0 && $remain_day<=7)
					{
						$container_id = $cont_data->container_id;
						$stockroom_id = $cont_data->stockroom_id;
						$move =  cron::move($container_id,$stockroom_id); // Move Container into stockroom

						if($move)
						{
							cron::order_status($container_id); // Call Order Status Change
						}
	 ...
	 * @return boolean
	 */
	function move($container_id,$stockroom_id){  // Move Container To Stockroom
	 ...
	 *  update status to Payment Recieve
	 * @access public
	 * @return boolean
	 */
	function order_status($container_id){  // Change Order Status
		...
		foreach ($data as $newdata){
			...
			if ($newdata->order_status == 'PR'){ // Payment Is recieved then Status will change
		...
		foreach($data as $catalog_detail){
			...
			if($catalog_detail->reminder_2 == 0){
				...
				if( $fdate == $send_date ){ // ==
					...
					if($sent == 1){
						$q_update = "UPDATE #__".TABLE_PREFIX."_catalog_request SET reminder_2 = 1 WHERE catalog_user_id = ".$catalog_detail->catalog_user_id ;
						$db->setQuery($q_update);
						$db->query();
					}
				}
			}else{
				if($catalog_detail->reminder_3 == 0){ // coupon reminder
					...
					if( $fdate == $send_date ){
						...
						$body =str_replace("{name}",$catalog_detail->name,$bodytmp);
						//$body =str_replace("{days}",CATALOG_DAYS,$body);
						$body =str_replace("{discount}",DISCOUNT_PERCENTAGE,$body);
...
function after_purchased_order_mail()
	{
		...
		{	
			...
			$sql = "SELECT CONCAT(firstname,' ',lastname) as name,user_email as email FROM  `#__redshop_order_users_info` WHERE `order_id` =  '".$mail_detail->order_id."' AND `address_type` = 'BT' limit 0,1";
			$db->setQuery($sql);
			$orderuserarr = $db->loadObject();
			
			$sql = "SELECT coupon_left as total,coupon_code,end_date FROM  `#__redshop_coupons` WHERE `order_id` =  '".$order_id."' AND coupon_left != 0 limit 0,1";
			$db->setQuery($sql);
