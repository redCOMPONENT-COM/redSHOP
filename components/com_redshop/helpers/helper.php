...
class redhelper
{
    ...
    */
    ...
    public function getItemid($product_id = '', $cat_id = 0)
    {
        ...
        if ($cat_id)
        {
            $sql = "SELECT id FROM #__menu " . "WHERE published=1 " . "AND `link` LIKE '%com_redshop%' " . "AND `link` LIKE '%view=category%' " . "AND ( link LIKE '%cid=" . $cat_id . "' OR link LIKE '%cid=" . $cat_id . "&%' ) " . "ORDER BY 'ordering'";
        ...
        {
            $cat = $cats[$i];
            $sql = "SELECT id FROM #__menu " . "WHERE published=1 " . "AND `link` LIKE '%com_redshop%' " . "AND `link` LIKE '%view=category%' " . "AND ( link LIKE '%cid=" . $cat->category_id . "' OR link LIKE '%cid=" . $cat->category_id . "&%' ) " . "ORDER BY 'ordering'";
            $this->_db->setQuery($sql);
            if ($Itemid = $this->_db->loadResult())
            {
                return $Itemid;
    ...
    public function getCategoryItemid($category_id = 0)
    {
        ...
        $query = "SELECT id FROM #__menu " . "WHERE 1=1  and published='1' " . $and . "ORDER BY 'ordering'";
        $this->_db->setQuery($query);
    ...
    public function getShopperGroupPortal()
    {
        ...
        $query = "SELECT * FROM `" . $this->_table_prefix . "shopper_group` " . "WHERE 1=1 " . $where;
        $this->_db->setQuery($query);
    ...
    public function getShopperGroupCategory($cid = 0)
    {
        ...
        $query = "SELECT *, count(`shopper_group_id`) as total FROM `" . $this->_table_prefix . "shopper_group` " . "WHERE 1=1 " . $where . "AND FIND_IN_SET(" . $cid . ",shopper_group_categories) " . "GROUP BY shopper_group_id ";
        $this->_db->setQuery($query);
    ...
    public function getShopperGroupProductCategory($pid = 0)
    {
        ...
        $query = "SELECT p.product_id,cx.category_id FROM `" . $this->_table_prefix . "product` AS p " . "LEFT JOIN " . $this->_table_prefix . "product_category_xref AS cx ON p.product_id=cx.product_id " . "WHERE p.product_id='" . $pid . "' ";
        $this->_db->setQuery($query);
    ...
    public function clickatellSMS($order_id)
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "order_users_info AS oui " . "LEFT JOIN " . $this->_table_prefix . "orders AS o ON o.order_id = oui.order_id " . "WHERE oui.order_id = '" . $order_id . "' " . "AND address_type='ST' ";
        $this->_db->setQuery($query);
        $orderData = $this->_db->loadobject();

        $query = "SELECT payment_method_name, oy.payment_method_id FROM " . $this->_table_prefix . "order_payment AS oy " . "LEFT JOIN " . $this->_table_prefix . "orders AS o ON o.order_id = oy.order_id " . "LEFT JOIN " . $this->_table_prefix . "payment_method AS p ON p.payment_method_id = oy.payment_method_id " . "WHERE oy.order_id = '" . $order_id . "' ";
        $this->_db->setQuery($query);
        ...
        $query   = "SELECT * FROM " . $this->_table_prefix . "template AS t " . "WHERE t.template_section = 'clicktell_sms_message' " . "AND (FIND_IN_SET( '" . $orderData->order_status . "', order_status )) ";
        $to      = $orderData->phone;
    ...
    */
    ...
    public function getEconomicAccountGroup($accountgroup_id = 0, $front = 0)
    {
        ...
        $query = 'SELECT ea.*, ea.accountgroup_id AS value, ea.accountgroup_name AS text FROM ' . $this->_table_prefix . 'economic_accountgroup AS ea ' . 'WHERE 1=1 ' . $and;
        $this->_db->setQuery($query);
