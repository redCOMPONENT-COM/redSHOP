...
class rsUserhelper
{
    ...
    // get User groups
    function getUserGroupList($user_id = 0)
    {
        $query = 'SELECT group_id FROM ' . $this->_table_prefix . 'users_info AS uf ' . 'LEFT JOIN #__user_usergroup_map as u on u.user_id = uf.user_id ' . 'WHERE users_info_id="' . $user_id . '" ';
    ...
    function updateUserTermsCondition($users_info_id = 0, $isSet = 0)
    {
        ...
        $query = "UPDATE " . $this->_table_prefix . "users_info " . "SET accept_terms_conditions='" . $isSet . "' " . "WHERE 1=1 " . $and;
        $this->_db->setQuery($query);
        $this->_db->Query();
    }
    ...
    function getShoppergroupData($user_id = 0)
    {
        ...
        if ($user_id != 0)
        {
            ...
            {
                $query = "SELECT sg.* FROM " . $this->_table_prefix . "shopper_group AS sg " . "LEFT JOIN " . $this->_table_prefix . "users_info AS ui ON ui.shopper_group_id=sg.shopper_group_id " . "WHERE ui.user_id = '" . $user_id . "' and ui.address_type='BT' ORDER BY shopper_group_id DESC  LIMIT 0,1";
                $this->_db->setQuery($query);
    ...
    function getShopperGroupList($shopper_group_id = 0)
    {
        ...
        $query = 'SELECT sh.*, shopper_group_id AS value, shopper_group_name AS text FROM ' . $this->_table_prefix . 'shopper_group AS sh ' . 'WHERE published=1 ' . $and;
        $this->_db->setQuery($query);
    ...
    {
        $query = "SELECT username FROM #__users " . "WHERE username='" . $username . "' " . "AND id!='" . $id . "' ";
        $this->_db->setQuery($query);
        ...
        if (ECONOMIC_INTEGRATION)
        {
            ...
            if ($isNew)
            {
                ...
                if (count($maxDebtor) > 0)
                {
                    ...
                    {
                        $nextId = $maxDebtor + 1;
                        $sql    = "UPDATE " . $this->_table_prefix . "users_info " . "SET users_info_id = '" . $nextId . "' " . "WHERE users_info_id='" . $row->users_info_id . "' ";
            ...
            if ($row->is_company && trim($row->ean_number) != '' & JError::isError(JError::getError()))
            {
                if ($isNew)
                {
                    $sql = "DELETE FROM " . $this->_table_prefix . "users_info " . "WHERE users_info_id='" . $original_info_id . "' ";
                    $this->_db->setQuery($sql);
                    $this->_db->Query();
    ...
    public function newsletterUnsubscribe($email = "")
    {
        ...
        if ($and != "")
        {
            $query = "DELETE FROM " . $this->_table_prefix . "newsletter_subscription " . "WHERE email='" . $email . "' " . $and;
            $this->_db->setQuery($query);
            $this->_db->query();
            $redshopMail = new redshopMail();
