...
*/
    ...
    public function replaceTermsConditions($template_desc = "", $Itemid = 1)
    {
        if (strstr($template_desc, "{terms_and_conditions"))
        {
            ...
            if ($user->id)
            {
                $query = "SELECT u.* FROM " . $this->_table_prefix . "users_info AS u " . "WHERE u.user_id='" . $user->id . "' " . "AND address_type='BT' ";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObject();
            }
            else if (isset($auth['users_info_id']) && $auth['users_info_id'] > 0)
            {
                $query = "SELECT u.* FROM " . $this->_table_prefix . "users_info AS u " . "WHERE u.users_info_id='" . $auth['users_info_id'] . "' " . "AND address_type='BT' ";
                $this->_db->setQuery($query);
                $list = $this->_db->loadObject();
            }
            $terms_left_final = "";
        ...
        if ($coupon_code != "")
        {
            ...
            if (count($coupon) > 0)
            {
                ...
                if ($coupon_type == 1)
                {
                    if ($user->id)
                    {
                        $sel = "SELECT SUM(coupon_value) AS usertotal FROM " . $this->_table_prefix . "coupons_transaction " . "WHERE userid='" . $user->id . "' " . "GROUP BY userid ";
                        $this->_db->setQuery($sel);
    ...
    public function getVoucherData($voucher_code, $product_id = 0)
    {
        ...
        if ($this->_globalvoucher != 1)
        {
            if ($user->id)
            {
                $query = "SELECT vt.transaction_voucher_id,vt.amount as total,vt.product_id from " . $this->_table_prefix . "product_voucher_xref as pv " . " left join " . $this->_table_prefix . "product_voucher as v on v.voucher_id = pv.voucher_id where voucher_code='" . $voucher_code . "' ) as nproduct , v.* FROM " . $this->_table_prefix . "product_voucher as v " . " left join " . $this->_table_prefix . "product_voucher_transaction as vt on vt.voucher_id = v.voucher_id " . "\nWHERE vt.amount > 0 AND v.voucher_type = 'Total' AND v.published = 1 and vt.voucher_code='" . $voucher_code . "' AND ((start_date<='" . $current_time . "' and end_date>='" . $current_time . "') OR ( start_date =0 AND end_date = 0) ) AND vt.user_id='" . $user->id . "' ORDER BY transaction_voucher_id DESC limit 0,1";
                $this->_db->setQuery($query);
            ...
            if ((count($voucher)) <= 0)
            {
                $query = "SELECT (select GROUP_CONCAT(DISTINCT CAST(product_id AS CHAR)  SEPARATOR ', ') as product_id from " . $this->_table_prefix . "product_voucher_xref as pv " . " left join " . $this->_table_prefix . "product_voucher as v on v.voucher_id = pv.voucher_id where voucher_code='" . $voucher_code . "') as nproduct," . " amount as total ,voucher_type,free_shipping,voucher_id,voucher_code,voucher_left  FROM " . $this->_table_prefix . "product_voucher as v  " . "\nWHERE published = 1 and voucher_code='" . $voucher_code . "' and ((start_date<='" . $current_time . "' and end_date>='" . $current_time . "') OR ( start_date =0 AND end_date = 0) ) and voucher_left>0 limit 0,1";
                $this->_db->setQuery($query);
                $voucher = $this->_db->loadObject();
            }
        }
    ...
    public function globalvoucher($voucher_code)
    {
        ...
        $query        = "SELECT product_id,v.* from " . $this->_table_prefix . "product_voucher_xref as pv  " //. $this->_table_prefix."product_voucher as v "
            . "left join " . $this->_table_prefix . "product_voucher as v on v.voucher_id = pv.voucher_id " . " \nWHERE v.published = 1" . " and v.voucher_code='" . $voucher_code . "' " . " and ((v.start_date<='" . $current_time . "' and v.end_date>='" . $current_time . "') OR ( v.start_date =0 AND v.end_date = 0) ) and v.voucher_left>0 limit 0,1";
        $this->_db->setQuery($query);
        $voucher = $this->_db->loadObject();
        if (count($voucher) <= 0)
        {
            $this->_globalvoucher = 1;
            $query                = "SELECT v.*,v.amount as total from " . $this->_table_prefix . "product_voucher as v " . "WHERE v.published = 1 " . "AND v.voucher_code='" . $voucher_code . "' " . "AND ((v.start_date<='" . $current_time . "' AND v.end_date>='" . $current_time . "') OR ( v.start_date =0 AND v.end_date = 0) ) " . "AND v.voucher_left>0 LIMIT 0,1 ";
            $this->_db->setQuery($query);
            $voucher = $this->_db->loadObject();
        }
        return $voucher;
    ...
    public function getcouponData($coupon_code)
    {
        ...
        if ($user->id)
        {
            $query = "SELECT ct.coupon_value as coupon_value,c.free_shipping, c.coupon_id,c.coupon_code,c.percent_or_total,ct.userid,ct.transaction_coupon_id FROM " . $this->_table_prefix . "coupons as c " . "left join " . $this->_table_prefix . "coupons_transaction as ct on ct.coupon_id = c.coupon_id " . "WHERE ct.coupon_value > 0 AND c.published = 1 and ct.coupon_code='" . $coupon_code . "' AND (c.start_date<='" . $current_time . "' AND c.end_date>='" . $current_time . "' ) AND ct.userid='" . $user->id . "' ORDER BY transaction_coupon_id DESC limit 0,1";
            $this->_db->setQuery($query);
        ...
        if (count($coupon) <= 0)
        {
            $query = "SELECT * FROM " . $this->_table_prefix . "coupons   " . "WHERE published = 1 and coupon_code='" . $coupon_code . "' and (start_date<='" . $current_time . "' and end_date>='" . $current_time . "' ) AND coupon_left > 0 AND ( '" . $subtotal . "' >= subtotal OR subtotal = 0 OR subtotal = '' ) limit 0,1";
            $this->_db->setQuery($query);
            $coupon = $this->_db->loadObject();
        }
        return $coupon;
    ...
    public function getCartItemAccessoryDetail($cart_item_id = 0)
    {
        ...
        $query = "SELECT * FROM  " . $this->_table_prefix . "usercart_accessory_item " . "WHERE 1=1 " . $and;
        $this->_db->setQuery($query);
    ...
    public function getCartItemAttributeDetail($cart_item_id = 0, $is_accessory = 0, $section = "attribute", $parent_section_id = 0)
    {
        ...
        $query = "SELECT * FROM  " . $this->_table_prefix . "usercart_attribute_item " . "WHERE is_accessory_att='" . $is_accessory . "' " . "AND section='" . $section . "' " . $and;
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    public function getProductAccAttribute($product_id = 0, $attribute_set_id = 0, $attribute_id = 0, $published = 0, $attribute_required = 0, $notAttributeId = 0)
    {
        ...
        $query = "SELECT a.attribute_id AS value,a.attribute_name AS text,a.*,ast.attribute_set_name " . "FROM " . $this->_table_prefix . "product_attribute AS a " . "LEFT JOIN " . $this->_table_prefix . "attribute_set AS ast ON ast.attribute_set_id=a.attribute_set_id " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.attribute_set_id=a.attribute_set_id " . $astpublished . "WHERE a.attribute_name!='' " . $and . " and attribute_published=1 ORDER BY a.ordering ASC ";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    public function getDiscountCalcData($area = 0, $pid, $areabetween = 0)
    {
        ...
        $query = "SELECT * FROM `" . $this->_table_prefix . "product_discount_calc` " . "WHERE `product_id`='" . $pid . "' " . $and . "ORDER BY id ASC ";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
    ...
    public function getDiscountCalcDataExtra($pdcextraids = "", $product_id = 0)
    {
        ...
        $query = "SELECT * FROM `" . $this->_table_prefix . "product_discount_calc_extra` " . "WHERE 1=1 " . $and . "ORDER BY option_name ";
        $this->_db->setQuery($query);
        $list = $this->_db->loadObjectlist();
        return $list;
    }
