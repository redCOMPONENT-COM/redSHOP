...
class categoryController extends JControllerLegacy
{
    ...
    function download()
    {
        ...
        $query = "SELECT x.* FROM " . $this->_table_prefix . "xml_export AS x " . "WHERE x.published=1 " . "AND x.filename='" . $filename . "' ";
        $db->setQuery($query);
        $data = $db->loadObject();
        if (count($data) > 0)
        {
            ...
            {
                $query = "SELECT x.*,xl.*,xi.* FROM " . $this->_table_prefix . "xml_export AS x " . "LEFT JOIN " . $this->_table_prefix . "xml_export_log AS xl ON x.xmlexport_id=xl.xmlexport_id " . "LEFT JOIN " . $this->_table_prefix . "xml_export_ipaddress AS xi ON x.xmlexport_id=xi.xmlexport_id " . "WHERE x.published=1 " . "AND (x.filename='" . $filename . "' " . "OR xl.xmlexport_filename='" . $filename . "') " . "AND xi.access_ipaddress='" . $_SERVER['REMOTE_ADDR'] . "' " . "ORDER BY xl.xmlexport_date DESC ";
                $db->setQuery($query);
                $data = $db->loadObject();
                if (count($data) <= 0)
                {
