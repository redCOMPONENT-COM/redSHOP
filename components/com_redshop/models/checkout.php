...
class checkoutModelcheckout extends RedshopCoreModel
{
    ...
    public function orderplace()
    {
        ...
        for ($i = 0; $i < $idx; $i++)
        {
            ...
            {
                $reddesign = true;
                $query     = "INSERT INTO `#__reddesign_order` " . "(order_id, product_id, reddesignfile, designhdnargs, image_id, order_item_id) " . "VALUES " . "('" . $order_id . "', '" . $cart[$i]['product_id'] . "', '" . $cart[$i]['reddesignfile'] . "', '" . addslashes($cart[$i]['designhdnargs']) . "'," . $cart[$i]['reddesign_image_id'] . "," . $rowitem->order_item_id . ")";
                $db->setQuery($query);
                $db->query();
            }
            // reddesign end
    ...
    public function sendGiftCard($order_id)
    {
        ...
        $query = 'SELECT * FROM ' . $this->_table_prefix . 'order_item ' . 'WHERE order_id = ' . $order_id . ' AND is_giftcard=1';
    ...
    public function voucher()
    {
        ...
        if (isset($cart['voucher']))
        {
            ...
            for ($i = 0; $i < count($cart['voucher']); $i++)
            {
                ...
                $sql                    = "UPDATE " . $this->_table_prefix . "product_voucher SET voucher_left=voucher_left-'" . $voucher_volume . "' " . "WHERE voucher_id  = '" . $voucher_id . "'";
    ...
    public function coupon()
    {
        ...
        if (isset($cart['coupon']))
        {
            ...
            for ($i = 0; $i < count($cart['coupon']); $i++)
            {
                ...
                $sql             = "UPDATE " . $this->_table_prefix . "coupons SET coupon_left=coupon_left-'" . $coupon_volume . "' " . "WHERE coupon_id  = '" . $coupon_id . "'";
                $this->_db->setQuery($sql);
                $this->_db->Query();
