...
class accountModelaccount extends RedshopCoreModel
{
    ...
    public function _buildQuery()
    {
        ...
        switch ($layout)
        {
            ...
            case 'mywishlist':
                ...
                {
                    $query = "SELECT DISTINCT w.* ,p.* FROM " . $this->_table_prefix . "wishlist AS w " . "LEFT JOIN " . $this->_table_prefix . "wishlist_product AS pw ON w.wishlist_id=pw.wishlist_id " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = pw.product_id " . "WHERE w.user_id='" . $user->id . "' " . "AND w.wishlist_id='" . $wishlist_id . "' and pw.wishlist_id='" . $wishlist_id . "'";
                }
                else
                {
                    ...
                    $query = "SELECT DISTINCT p.* FROM " . $this->_table_prefix . "product AS p " . "WHERE p.product_id IN ('" . substr_replace($prod_id, "", -1) . "') ";
                }
                break;
            default:
                $query = "";
    ...
    public function countMyTags()
    {
        $user   = JFactory::getUser();
        $userid = $user->id;
        $query  = "SELECT COUNT(pt.tags_id) FROM " . $this->_table_prefix . "product_tags AS pt " . "LEFT JOIN " . $this->_table_prefix . "product_tags_xref AS ptx ON pt.tags_id = ptx.tags_id " . "WHERE ptx.users_id='" . $userid . "' AND pt.published = 1";
        $this->_db->setQuery($query);
    ...
    public function removeWishlistProduct()
    {
        ...
        $query = "SELECT wishlist_id FROM " . $this->_table_prefix . "wishlist " . "WHERE user_id='" . $user->id . "' AND wishlist_id='" . $wishlist_id . "' ";
        echo "<pre>";
        print_r($query);
        $this->_db->setQuery($query);
        ...
        if (count($list) > 0)
        {
            $query = "DELETE FROM " . $this->_table_prefix . "wishlist_product " . "WHERE product_id = '" . $pid . "' AND wishlist_id='" . $wishlist_id . "' ";
    ...
    public function sendWishlist($post)
    {
        ...
        {
            $query = "SELECT DISTINCT w.* ,p.* FROM " . $this->_table_prefix . "wishlist AS w " . "LEFT JOIN " . $this->_table_prefix . "wishlist_product AS pw ON w.wishlist_id=pw.wishlist_id " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = pw.product_id " . "WHERE w.user_id='" . $user->id . "' " . "AND w.wishlist_id='" . $wishlist_id . "' ";
        }
        else
        {
            // add this code to send wishlist while user is not loged in ...
    ...
    public function getReserveDiscount()
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "product_voucher_transaction " . "WHERE user_id='" . $user->id . "' AND amount > 0 limit 0,1 ";
        $this->_db->setQuery($query);
        $Data = $this->_getList($query);
        if ($Data)
        {
