...
class categoryModelcategory extends RedshopCoreModel
{
    ...
    public function _buildQuery()
    {
        ...
        if ($manufacturer_id)
        {
            $left    = "LEFT JOIN " . $this->_table_prefix . "product_category_xref AS pcx ON pcx.category_id = c.category_id " . "LEFT JOIN " . $this->_table_prefix . "product AS p ON p.product_id = pcx.product_id " . "LEFT JOIN " . $this->_table_prefix . "manufacturer AS m ON m.manufacturer_id = p.manufacturer_id ";
            $and     = "AND m.manufacturer_id='" . $manufacturer_id . "' ";
            $groupby = "GROUP BY c.category_id ";
        }
        $query = "SELECT c.* FROM " . $this->_table_prefix . "category AS c " . "LEFT JOIN " . $this->_table_prefix . "category_xref AS cx ON cx.category_child_id=c.category_id " . $left . "WHERE c.published = 1 " . "AND cx.category_parent_id='" . $this->_id . "' " . $and . $groupby . $orderby;
        return $query;
    }
    ...
    public function getCategoryProduct($minmax = 0, $isSlider = false)
    {
        ...
        $query = "SELECT * FROM " . $this->_table_prefix . "product AS p " . "LEFT JOIN " . $this->_table_prefix . "product_category_xref AS pc ON pc.product_id=p.product_id " . "LEFT JOIN " . $this->_table_prefix . "category AS c ON c.category_id=pc.category_id " . "LEFT JOIN " . $this->_table_prefix . "manufacturer AS m ON m.manufacturer_id=p.manufacturer_id " . "WHERE p.published = 1 AND p.expired = 0 " . "AND pc.category_id='" . $this->_id . "' " . "AND p.product_parent_id = 0 " . $and . $finder_condition . $order_by;
    ...
    public function getManufacturer($mid = 0)
    {
        ...
        $query = "SELECT DISTINCT(m.manufacturer_id ),m.* FROM " . $this->_table_prefix . "manufacturer AS m " . "LEFT JOIN #__redshop_product AS p ON m.manufacturer_id  = p.manufacturer_id ";
        if ($cid != 0)
        {
