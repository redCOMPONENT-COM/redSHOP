...
class RedshopEntityUser_Cart_Item_Attribute extends RedshopEntity
{
	...
	{
		return JTable::getInstance('Usercart_Attribute_Item', 'Table');
	}
	 ...
	 * @return  self
	 */
	public function loadItem($key = 'cart_att_item_id', $keyValue = null)
	{
		if ($key == 'cart_att_item_id' && !$this->hasId())
		{
			return $this;
		}

		if (($table = $this->getTable()) && $table->load(array($key => ($key == 'cart_att_item_id' ? $this->id : $keyValue))))
		{
