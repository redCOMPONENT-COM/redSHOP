...
class RedshopEntityOrder_Status_Log extends RedshopEntity
{
	 ...
	 * @return  self
	 */
	public function loadItem($key = 'order_status_log_id', $keyValue = null)
	{
		if ($key == 'order_status_log_id' && !$this->hasId())
		{
			return $this;
		}

		if (($table = $this->getTable()) && $table->load(array($key => ($key == 'order_status_log_id' ? $this->id : $keyValue))))
		{
