    ...
    public static function replaceCartItem($data, $cart = array(), $isReplaceButton = false, $quotationMode = 0)
    {
        ...
        for ($i = 0; $i < $idx; $i++) {
            ...
            if (isset($cart[$i]['giftcard_id']) && $cart[$i]['giftcard_id']) {
                ...
                $cartHtml = str_replace("{product_vat}",
                    ($cart[$i]['product_vat'] ?? 0) * ($cart[$i]['quantity'] ?? 0), $cartHtml);
                $productUserFields = RedshopHelperProduct::getProductUserField($i);
                $cartHtml = str_replace("{product_userfields}", $productUserFields, $cartHtml);
                $productFields = RedshopHelperProduct::getProductField($i);
