    ...
    public static function replaceCartItem($data, $cart = array(), $isReplaceButton = false, $quotationMode = 0)
    {
        ...
        for ($i = 0; $i < $idx; $i++) {
            ...
            if (isset($cart[$i]['giftcard_id']) && $cart[$i]['giftcard_id']) {
                ...
                $cartHtml = productHelper::getInstance()->getProductFinderDatepickerValue(
                    $cartHtml,
                    $giftCardId,
                    $fieldArray,
                    1
...
<<<<<<< HEAD
                ...
                $retAccArr     = productHelper::getInstance()->makeAccessoryCart(
                ...
                if (Redshop::getConfig()->get(
                        'WANT_TO_SHOW_ATTRIBUTE_IMAGE_INCART'
                ...
                if ($productImage && JFile::exists(REDSHOP_FRONT_IMAGES_RELPATH . $productImage)) {
                    ...
                    $productImg = $val[1];
                    $type       = $val[0];
                } elseif ($product->product_full_image && JFile::exists(
                ...
                if (!$quotationMode || ($quotationMode && Redshop::getConfig()->get('SHOW_QUOTATION_PRICE'))) {
                    if (!$isApplyVAT) {
                        $productTotalPrice .= RedshopHelperProductPrice::formattedPrice(
                            $cart[$i]['product_price_excl_vat'] * $quantity
                        );
                    } else {
                        $productTotalPrice .= RedshopHelperProductPrice::formattedPrice(
                            $cart[$i]['product_price'] * $quantity
                        );
                    }
                }
                ...
                $cartHtml           = str_replace(
                    "{product_vat}",
                    $cart[$i]['product_vat'] * $cart[$i]['quantity'],
                    $cartHtml
                );
                ...
                if ($isReplaceButton) {
                    ...
                    if ($view == 'cart') {
                        $attrChange      = JURI::root(
                            ) . 'index.php?option=com_redshop&view=cart&layout=change_attribute&tmpl=component&pid=' . $productId . '&cart_index=' . $i;
                        $updateAttribute = '<a class="modal" rel="{handler: \'iframe\', size: {x: 550, y: 400}}" href="' . $attrChange . '">' . JText::_(
                                'COM_REDSHOP_CHANGE_ATTRIBUTE'
                            ) . '</a>';
                    }
            ...
            if ($isReplaceButton) {
                ...
                if ($view == 'checkout') {
                    ...
                    $updateCart = '<form style="padding:0px;margin:0px;" name="update_cart' . $i . '" method="POST" '
                        ...
                        . ' maxlength="' . Redshop::getConfig()->get(
                            'DEFAULT_QUANTITY'
...
>>>>>>> upstream/develop
						...
						<input type="hidden" name="cart_index" value="' . $i . '">
						<input type="hidden" name="Itemid" value="' . $itemId . '">
						<input type="hidden" name="task" value=""><img class="update_cart" src="' . REDSHOP_FRONT_IMAGES_ABSPATH . $updateImage . '" title="' . JText::_(
                        'COM_REDSHOP_UPDATE_PRODUCT_FROM_CART_LBL'
                    ) . '" alt="' . JText::_(
                        'COM_REDSHOP_UPDATE_PRODUCT_FROM_CART_LBL'
                    ) . '" onclick="document.update_cart' . $i . '.task.value=\'update\';document.update_cart' . $i . '.submit();">
						</form>';
                ...
                if ($view == 'checkout') {
                    ...
                    $removeProduct = '<form name="delete_cart' . $i . '" method="POST" >
							...
							<input type="hidden" name="task" value="">
							<input type="hidden" name="Itemid" value="' . $itemId . '">
							<img class="delete_cart" src="' . REDSHOP_FRONT_IMAGES_ABSPATH . $deleteImg . '" title="' . JText::_(
                            'COM_REDSHOP_DELETE_PRODUCT_FROM_CART_LBL'
                        ) . '" alt="' . JText::_(
                            'COM_REDSHOP_DELETE_PRODUCT_FROM_CART_LBL'
                        ) . '" onclick="document.delete_cart' . $i . '.task.value=\'delete\';document.delete_cart' . $i . '.submit();"></form>';
                }
     ...
     */
    public static function replaceTemplate($cart, $cartEData, $checkout = 1)
    {
        JPluginHelper::importPlugin('redshop_checkout');
        JPluginHelper::importPlugin('redshop_shipping');

        $dispatcher = RedshopHelperUtility::getDispatcher();
        $dispatcher->trigger('onBeforeReplaceTemplateCart', array(&$cart, &$cartEData, $checkout));

        if (strpos($cartEData, "{product_loop_start}") !== false && strpos(
                $cartEData,
                "{product_loop_end}"
            ) !== false) {
            $templateStartData = explode('{product_loop_start}', $cartEData);
            $templateStart     = $templateStartData[0];
            $templateEndData   = explode('{product_loop_end}', $templateStartData[1]);
            $templateEnd       = $templateEndData[1];
            $templateMiddle    = $templateEndData[0];
            $templateMiddle    = RedshopHelperCartTag::replaceCartItem(
                $templateMiddle,
                $cart,
                1,
                Redshop::getConfig()->get('DEFAULT_QUOTATION_MODE')
            );
            $cartEData         = $templateStart . $templateMiddle . $templateEnd;
        }

        $cartEData                 = Redshop\Cart\Render\Label::replace($cartEData);
        $total                     = $cart ['total'];
        $subtotalExcludeVat        = $cart ['subtotal_excl_vat'];
        $productSubtotal           = $cart ['product_subtotal'];
        $productSubtotalExcludeVat = $cart ['product_subtotal_excl_vat'];
        $subtotal                  = $cart ['subtotal'];
        $discountExcludeVat        = $cart ['discount_ex_vat'];
        ...
        if (!Redshop::getConfig()->get('DEFAULT_QUOTATION_MODE')
            ...
            if (strpos($cartEData, '{product_subtotal_lbl}') !== false) {
                $cartEData = str_replace(
                    "{product_subtotal_lbl}",
                    JText::_('COM_REDSHOP_PRODUCT_SUBTOTAL_LBL'),
                    $cartEData
                );
            }

            if (strpos($cartEData, '{product_subtotal_excl_vat_lbl}') !== false) {
                $cartEData = str_replace(
                    "{product_subtotal_excl_vat_lbl}",
                    JText::_('COM_REDSHOP_PRODUCT_SUBTOTAL_EXCL_LBL'),
                    $cartEData
                );
            }

            if (strpos($cartEData, '{shipping_with_vat_lbl}') !== false) {
                $cartEData = str_replace(
                    "{shipping_with_vat_lbl}",
                    JText::_('COM_REDSHOP_SHIPPING_WITH_VAT_LBL'),
                    $cartEData
                );
            }

            if (strpos($cartEData, '{shipping_excl_vat_lbl}') !== false) {
                $cartEData = str_replace(
                    "{shipping_excl_vat_lbl}",
                    JText::_('COM_REDSHOP_SHIPPING_EXCL_VAT_LBL'),
                    $cartEData
                );
            }

            if (strpos($cartEData, '{product_price_excl_lbl}') !== false) {
                $cartEData = str_replace(
                    "{product_price_excl_lbl}",
                    JText::_('COM_REDSHOP_PRODUCT_PRICE_EXCL_LBL'),
                    $cartEData
                );
            }
            ...
            $cartEData = str_replace(
                "{total}",
                "<span id='spnTotal'>" . RedshopHelperProductPrice::formattedPrice(
                    $total,
                    true
                ) . "</span>",
                $cartEData
            ...
            $chktag = \Redshop\Template\Helper::isApplyVat($cartEData);

            if (!empty($chktag)) {
                $cartEData = str_replace(
                    "{subtotal}",
                    RedshopHelperProductPrice::formattedPrice($subtotal),
                    $cartEData
                );
                $cartEData = str_replace(
                    "{product_subtotal}",
                    RedshopHelperProductPrice::formattedPrice($productSubtotal),
                    $cartEData
                );
            } else {
                $cartEData = str_replace(
                    "{subtotal}",
                    RedshopHelperProductPrice::formattedPrice($subtotalExcludeVat),
                    $cartEData
                );
                $cartEData = str_replace(
                    "{product_subtotal}",
                    RedshopHelperProductPrice::formattedPrice($productSubtotalExcludeVat),
                    $cartEData
                );
            }

            if ((strpos($cartEData, "{discount_denotation}") !== false || strpos(
                        $cartEData,
                        "{shipping_denotation}"
                    ) !== false) && ($discountTotal != 0 || $shipping != 0)) {
                $cartEData = str_replace("{denotation_label}", JText::_('COM_REDSHOP_DENOTATION_TXT'), $cartEData);
            } else {
                $cartEData = str_replace("{denotation_label}", "", $cartEData);
            }

            if (strpos($cartEData, "{discount_excl_vat}") !== false) {
                $cartEData = str_replace("{discount_denotation}", "*", $cartEData);
            } else {
                $cartEData = str_replace("{discount_denotation}", "", $cartEData);
            }
            ...
            $cartEData = str_replace(
                "{product_subtotal_excl_vat}",
                RedshopHelperProductPrice::formattedPrice($productSubtotalExcludeVat),
                $cartEData
            );
            $cartEData = str_replace(
                "{sub_total_vat}",
                RedshopHelperProductPrice::formattedPrice($subtotalVat),
                $cartEData
            );
            $cartEData = str_replace(
                "{discount_excl_vat}",
                RedshopHelperProductPrice::formattedPrice($discountExcludeVat),
                $cartEData
            );
            ...
            if (!empty($rep)) {
                if (strpos($cartEData, "{shipping_excl_vat}") !== false) {
                    $cartEData = str_replace("{shipping_denotation}", "*", $cartEData);
                } else {
                    $cartEData = str_replace("{shipping_denotation}", "", $cartEData);
                }
                ...
                $cartEData = str_replace(
                    "{order_shipping}",
                    RedshopHelperProductPrice::formattedPrice($shipping, true),
                    $cartEData
                );
                $cartEData = str_replace(
                    "{shipping_excl_vat}",
                    "<span id='spnShippingrate'>" . RedshopHelperProductPrice::formattedPrice(
                        $shipping - $cart['shipping_tax'],
                        true
                    ) . "</span>",
                    $cartEData
                );
                $cartEData = str_replace("{shipping_lbl}", JText::_('COM_REDSHOP_CHECKOUT_SHIPPING_LBL'), $cartEData);
                $cartEData = str_replace(
                    "{shipping}",
                    "<span id='spnShippingrate'>" . RedshopHelperProductPrice::formattedPrice(
                        $shipping,
                ...
                $cartEData = str_replace(
                    "{tax_with_shipping_lbl}",
                    JText::_('COM_REDSHOP_CHECKOUT_SHIPPING_LBL'),
                    $cartEData
                );
                $cartEData = str_replace(
                    "{vat_shipping}",
                    RedshopHelperProductPrice::formattedPrice($shippingVat),
                    $cartEData
                );
            } else {
                $cartEData = str_replace("{order_shipping}", '', $cartEData);
                $cartEData = str_replace("{shipping_excl_vat}", '', $cartEData);
                $cartEData = str_replace("{shipping_lbl}", '', $cartEData);
                $cartEData = str_replace("{shipping}", '', $cartEData);
                $cartEData = str_replace("{tax_with_shipping_lbl}", '', $cartEData);
                $cartEData = str_replace("{vat_shipping}", '', $cartEData);
                $cartEData = str_replace("{shipping_denotation}", "", $cartEData);
            }
        } else {
            $cartEData = str_replace("{total}", "<span id='spnTotal'></span>", $cartEData);
            $cartEData = str_replace("{shipping_excl_vat}", "<span id='spnShippingrate'></span>", $cartEData);
            $cartEData = str_replace("{order_shipping}", "", $cartEData);
            $cartEData = str_replace("{shipping_lbl}", '', $cartEData);
            $cartEData = str_replace("{shipping}", "<span id='spnShippingrate'></span>", $cartEData);
            $cartEData = str_replace("{subtotal}", "", $cartEData);
            $cartEData = str_replace("{tax_with_shipping_lbl}", '', $cartEData);
            $cartEData = str_replace("{vat_shipping}", '', $cartEData);
            $cartEData = str_replace("{subtotal_excl_vat}", "", $cartEData);
            $cartEData = str_replace("{shipping_excl_vat}", "", $cartEData);
            $cartEData = str_replace("{subtotal_excl_vat}", "", $cartEData);
            $cartEData = str_replace("{product_subtotal_excl_vat}", "", $cartEData);
            $cartEData = str_replace("{product_subtotal}", "", $cartEData);
            $cartEData = str_replace("{sub_total_vat}", "", $cartEData);
            $cartEData = str_replace("{discount_excl_vat}", "", $cartEData);
            $cartEData = str_replace("{discount_denotation}", "", $cartEData);
            $cartEData = str_replace("{shipping_denotation}", "", $cartEData);
            $cartEData = str_replace("{denotation_label}", "", $cartEData);
            $cartEData = str_replace("{total_excl_vat}", "", $cartEData);
        }

        if (!Redshop::getConfig()->get('APPLY_VAT_ON_DISCOUNT')) {
            $total_for_discount = $subtotalExcludeVat;
        } else {
            $total_for_discount = $subtotal;
        }

        $cartEData = RedshopHelperCartTag::replaceDiscount(
            $cartEData,
        ...
        if ($checkout) {
            $cartEData = RedshopHelperPayment::replaceConditionTag(
                $cartEData,
                $cart['payment_amount'],
                0,
                $cart['payment_oprand']
            ...
            $cartEData     = RedshopHelperPayment::replaceConditionTag($cartEData, 0, 1, $paymentOprand);
        }

        $cartEData = RedshopHelperCartTag::replaceTax(
            $cartEData,
            $tax + $shippingVat,
            $discountAmount + $tmpDiscount,
            0,
        ...
        $dispatcher->trigger('onAfterReplaceTemplateCart', array(&$cartEData, $checkout));
