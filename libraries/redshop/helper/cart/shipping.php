...
class RedshopHelperCartShipping
{
    ...
    public static function getDefault($data)
    {
        ...
        $whereCountry = '(FIND_IN_SET(' . (string)$db->quote($country) . ', ' . $db->qn(
                'shipping_rate_country'
            ) . ') OR '
            . $db->qn('shipping_rate_country') . ' = ' . $db->quote(0) . ' OR ' . $db->qn(
                'shipping_rate_country'
            ) . ' = "")';
        ...
        $isWhere     = ' AND (' . $db->qn('company_only') . ' = ' . $companyOnly . ' OR ' . $db->qn(
                'company_only'
            ) . ' = 0) ';
        ...
        if (null === $shippingRate) {
            $shippingRate = self::getShippingRateSecond(
                $volume,
                $weightTotal,
                $orderSubtotal,
                $whereCountry,
        ...
        if (null === $shippingRate) {
            $shippingRate = self::getShippingRateThird(
                $volume,
                $weightTotal,
                $orderSubtotal,
                $whereCountry,
     ...
     * @since  2.1.0
     */
    public static function getShippingRateFirst(
        $volume = 0,
        $weightTotal = 0.0,
        $orderSubtotal = 0,
        $whereCountry = '',
     ...
     * @since  2.1.0
     */
    public static function getShippingRateSecond(
        $volume = 0,
        $weightTotal = 0.0,
        $orderSubtotal = 0,
        $whereCountry = '',
     ...
     * @since  2.1.0
     */
    public static function getShippingRateThird(
        $volume = 0,
        $weightTotal = 0.0,
        $orderSubtotal = 0,
        $whereCountry = '',
        ...
        $sql = "SELECT * FROM " . $db->qn('#__redshop_shipping_rate') . " AS sr
							 ...
							 ON
							 " . $db->qn('sr.shipping_class') . " = " . $db->qn('s.element') . "
					WHERE " . $db->qn('s.folder') . " = " . $db->quote('redshop_shipping') . " AND " . $db->qn(
                's.enabled'
            ) . " = 1 AND "
