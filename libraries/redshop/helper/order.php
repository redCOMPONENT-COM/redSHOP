...
class RedshopHelperOrder
{
    ...
    public static function changeOrderStatus($data)
    {
        ...
        if ($checkUpdateOrders == 0 && $data->order_status_code != "" && $data->order_payment_status_code != "") {
            ...
            if (
                !Redshop::getConfig()->get('ORDER_MAIL_AFTER')
            ...
            if ($data->order_payment_status_code == "Paid") {
                JModelList::addIncludePath(JPATH_SITE . '/components/com_redshop/models');
                $checkoutModelCheckout = JModelList::getInstance('Checkout', 'RedshopModel');
    ...
    public static function sendDownload($orderId = 0)
    {
        ...
        $mailData = str_replace(
            "{order_date}",
            RedshopHelperDatetime::convertDateFormat($orderDetail->cdate),
            $mailData
        );
        ...
        foreach ($rows as $row) {
            ...
            $mailToken = "<a href='" . JUri::root(
            ) . "index.php?option=com_redshop&view=product&layout=downloadproduct&tid="
        ...
        if ($mailBody && $userEmail != "") {
            if (
                ...
                $app->enqueueMessage(Text::_('COM_REDSHOP_ERROR_DOWNLOAD_MAIL_FAIL'), 'error');
            }
        }
    ...
    public static function getDownloadProduct($orderId)
    {
        if (!array_key_exists($orderId, self::$orderProductsDownload)) {
            ...
            $query = $db->getQuery(true)
                ...
                ->leftJoin(
                    $db->qn('#__redshop_product', 'p') . ' ON ' . $db->qn('pd.product_id') . ' = ' . $db->qn(
                        'p.product_id'
                    )
                )
                ->where($db->qn('order_id') . ' = ' . (int) $orderId);
    ...
    public static function generateInvoiceNumber($orderId)
    {
        ...
        if (
            ($number <= 0 && 'C' == $orderInfo->order_status && 'Paid' == $orderInfo->order_payment_status)
    ...
    public static function changeOrderStatusMail($orderId, $newStatus, $orderComment = '')
    {
        ...
        $mailTemplate = Redshop\Mail\Helper::getTemplate(
            0,
            '',
            '`mail_section` LIKE "' . $mailSection . '" AND `mail_order_status` LIKE "' . $newStatus . '"'
        );
        ...
        if (count($mailTemplate) > 0) {
            ...
            if (count($fieldArray) > 0) {
                for ($i = 0, $in = count($fieldArray); $i < $in; $i++) {
                    ...
                    if ($fieldValueArray->data_txt != "") {
                        $mailData = str_replace(
                            '{' . $fieldArray[$i]->name . '}',
                            $fieldValueArray->data_txt,
                            $mailData
                        );
                        $mailData = str_replace(
                            '{' . $fieldArray[$i]->name . '_lbl}',
                            $fieldArray[$i]->title,
                            $mailData
                        );
                    } else {
            ...
            $mailData    = str_replace(
                ...
                Text::_('COM_REDSHOP_ORDER_MAIL_INTRO_TEXT'),
                $mailData
            );

            $mailData = Template::replaceTemplate($orderDetail, $mailData, true);
            ...
            for ($d = 0, $dn = count($arrDiscount); $d < $dn; $d++) {
                if ($arrDiscount[$d]) {
                    ...
                    if ($arrDiscountType[0] == 'c') {
                        $discountType .= Text::_('COM_REDSHOP_COUPON_CODE') . ' : ' . $arrDiscountType[1] . '<br>';
                    }

                    if ($arrDiscountType[0] == 'v') {
                        $discountType .= Text::_('COM_REDSHOP_VOUCHER_CODE') . ' : ' . $arrDiscountType[1] . '<br>';
                    }
                }
            }
            ...
            $replace[] = Text::_('COM_REDSHOP_COMMENT');

            $search[]  = "{customer_note}";
            $replace[] = $orderComment;

            $search[]  = "{order_detail_link_lbl}";
            $replace[] = Text::_('COM_REDSHOP_ORDER_DETAIL_LBL');

            $orderDetailurl = JUri::root(
            ) . 'index.php?option=com_redshop&view=order_detail&oid=' . $orderId . '&encr=' . $orderDetail->encr_key;
            $search[]       = "{order_detail_link}";
            $replace[]      = "<a href='" . $orderDetailurl . "'>" . Text::_(
                "COM_REDSHOP_ORDER_DETAIL_LINK_LBL"
            ) . "</a>";
    ...
    public static function createWebPackLabel($orderId, $orderStatus, $paymentStatus)
    {
        ...
        if ($orderStatus == Redshop::getConfig()->get('GENERATE_LABEL_ON_STATUS') && $paymentStatus == "Paid") {
            ...
            $allowPacsoftLabel = ($details[0] === 'plgredshop_shippingdefault_shipping' || (boolean) $shippingParams->get(
                'allowPacsoftLabel'
            )
            );
    ...
    public static function generateParcel($orderId)
    {
        ...
        $unitRatio = \Redshop\Helper\Utility::getUnitConversation(
            'kg',
            Redshop::getConfig()->get('DEFAULT_WEIGHT_UNIT')
        );
        ...
        try {
            ...
            if (empty($xmlResponse) || !empty($error)) {
                return Text::_('LIB_REDSHOP_PACSOFT_ERROR_NO_RESPONSE');
            }
    ...
    public static function createBookInvoice($orderId, $orderStatus)
    {
        ...
        if (
            ...
            if (
                Redshop::getConfig()->get('ECONOMIC_INVOICE_DRAFT') == 2 && $orderStatus == Redshop::getConfig()->get(
                ...
                if (!empty($paymentInfo)) {
                    ...
                    $paymentMethod                           = self::getPaymentMethodInfo(
            ...
            $bookInvoicePdf = RedshopEconomic::bookInvoiceInEconomic(
                $orderId,
    ...
    public static function getStatusList(
        ...
        $types[]   = JHtml::_('select.option', '', '- ' . Text::_('COM_REDSHOP_SELECT_STATUS_LBL') . ' -');
    ...
    public static function getFilterByList(
        ...
        $filterByList = array(
            'orderid'     => Text::_('COM_REDSHOP_ORDERID'),
            'ordernumber' => Text::_('COM_REDSHOP_ORDERNUMBER'),
            'fullname'    => Text::_('COM_REDSHOP_FULLNAME'),
            'useremail'   => Text::_('COM_REDSHOP_USEREMAIL')
        );
    ...
    public static function getPaymentStatusList(
        ...
        $types[] = JHtml::_('select.option', '', Text::_('COM_REDSHOP_SELECT_PAYMENT_STATUS'));
        $types[] = JHtml::_('select.option', 'Paid', Text::_('COM_REDSHOP_PAYMENT_STA_PAID'));
        $types[] = JHtml::_('select.option', 'Unpaid', Text::_('COM_REDSHOP_PAYMENT_STA_UNPAID'));
        $types[] = JHtml::_('select.option', 'Partial Paid', Text::_('COM_REDSHOP_PAYMENT_STA_PARTIAL_PAID'));
    ...
    public static function updateStatus()
    {
        ...
        if (!$isProduct) {
            ...
            if ($paymentStatus == "Paid") {
                JModelList::addIncludePath(JPATH_SITE . '/components/com_redshop/models');
                $checkoutModel = JModelList::getInstance('Checkout', 'RedshopModel');
        ...
        if (
            $app->input->getCmd('order_sendordermail') == 'true' && JFactory::getApplication()->isClient(
                'administrator'
            )
        ) {
        ...
        $msg = Text::_('COM_REDSHOP_ORDER_STATUS_SUCCESSFULLY_SAVED_FOR_ORDER_ID') . " " . $orderId;
    ...
    public static function updateOrderStatus($orderId, $newStatus)
    {
        ...
        $query  = $db->getQuery(true)
            ...
            ->leftJoin(
                $db->qn('#__redshop_order_payment', 'op') . ' ON ' . $db->qn(
                    'op.payment_method_class'
                ) . ' = ' . $db->qn('e.element')
            )
        ...
        if (
            ...
            $data    = RedshopHelperUtility::getDispatcher()->trigger(
                'onCapture_Payment' . $result->element,
                array($result->element, $values)
            );
            $results = $data[0];
        ...
        if (
            ...
            $data    = RedshopHelperUtility::getDispatcher()->trigger(
                'onStatus_Payment' . $result->element,
                array($result->element, $values)
            );
            $results = $data[0];
     ...
     * @since   2.0.3
     */
    public static function updateOrderItemStatus(
        $orderId = 0,
        $productId = 0,
        $newStatus = '',
        $comment = '',
     ...
     * @since   2.0.3
     */
    public static function getOrderItemAttributeDetail(
        $orderItemId = 0,
        $isAccessory = 0,
        $section = "attribute",
        $parentSectionId = 0
    ...
    public static function getDownloadProductLog($orderId, $did = '')
    {
        ...
        if (!array_key_exists($key, self::$orderProductsDownloadLog)) {
            ...
            $query = $db->getQuery(true)
                ...
                ->leftJoin(
                    $db->qn('#__redshop_product_download', 'pd') . ' ON ' . $db->qn('pd.download_id') . ' = ' . $db->qn(
                        'pdl.download_id'
                    )
                )
    ...
    public static function generateInvoicePdf($orderId, $code = 'F', $isEmail = false)
    {
        ...
        $print_tag = "<a onclick='window.print();' title='" . Text::_('COM_REDSHOP_PRINT') . "'>"
            . "<img src=" . JSYSTEM_IMAGES_PATH . "printButton.png  alt='" . Text::_('COM_REDSHOP_PRINT') . "' title='"
        ...
        RedshopHelperUtility::getDispatcher()->trigger(
            'onRedshopOrderCreateInvoicePdf',
            array($orderId, $message, $code, $isEmail)
        );
    }
    ...
    public static function writeOrderLog($orderId, $byUserId, $orderStatusCode, $orderPaymentStatus, $customerNote)
    {
        ...
        RedshopDbTool::safeExecute($db, $query);
    }
}