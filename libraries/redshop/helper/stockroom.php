...
class RedshopHelperStockroom
{
    ...
    public static function isStockExists($sectionId = 0, $section = "product", $stockroomId = 0)
    {
        ...
        RedshopHelperUtility::getDispatcher()->trigger(
            'onIsStockExists',
            array($sectionId, $section, $stockroomId, &$stock)
        );
    ...
    public static function updateStockroomQuantity($sectionId = 0, $quantity = 0, $section = "product", $productId = 0)
    {
        ...
        if (Redshop::getConfig()->get('USE_STOCKROOM') == 1) {
            ...
            if ($quantity > 0) {
                ...
                if ($productData->preorder == "yes" || ($productData->preorder == "global" && Redshop::getConfig()->get(
                            'ALLOW_PRE_ORDER'
                        ))
                    ...
                    foreach ($preOrderStockrooms as $preOrderStockroom) {
                        ...
                        self::updatePreorderStockAmount(
                            $sectionId,
                            $remainingQuantity,
                            $preOrderStockroom->stockroom_id,
                            $section
                        );
                    }
                }
     ...
     * @since  2.0.0.3
     */
    public static function updatePreorderStockAmount(
        $sectionId = 0,
        $quantity = 0,
        $stockroomId = 0,
        $section = "product"
        ...
        if (Redshop::getConfig()->get('IS_PRODUCT_RESERVE') && Redshop::getConfig()->get('USE_STOCKROOM')) {
            ...
            if ($qty !== null) {
                ...
                $query = $db->getQuery(true)
                    ...
                    ->values(
                        $db->q($sessionId) . ',' . $db->q((int)$sectionId) . ',' . $db->q(
                            (int)$quantity
                        ) . ',' . $db->q($time) . ',' . $db->q($section)
                    );
            }
    ...
    public static function getFinalPreorderStockofProduct($productId, $totalAtt)
    {
        ...
        if ($totalAtt > 0 && !$isStockExists) {
            ...
            foreach ($properties as $property) {
                ...
                foreach ($subProperties as $subProperty) {
                    $isSubpropertyStock = self::isPreorderStockExists(
                        $subProperty->subattribute_color_id,
                        'subproperty'
                    );
        ...
        if ($section == 'product' && $stockroomId == 0 && $sectionIds) {
            ...
            if (Redshop::getConfig()->get('USE_STOCKROOM') == 1) {
                ...
                if (!$isInfinite) {
                    ...
                    $query->select(
                        'IF(SUM(' . $db->qn('x.quantity') . ') IS NULL, 1000000000, SUM(' . $db->qn(
                            'x.quantity'
                        ) . ')) AS ' . $db->qn('quantity')
                    );
                }
