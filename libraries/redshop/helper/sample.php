...
class RedshopHelperSample
{
    ...
    public static function sendSecondReminder($sampleRequest = null, $currentDate = '', $from = '', $fromName = '')
    {
        ...
        $sendDate    = date(
            "Y-m-d",
            $sampleRequest->registerdate + (Redshop::getConfig()->get(
                    'COLOUR_SAMPLE_REMAINDER_2'
                ) * (60 * 60 * 24))
        );
    ...
    {
        if (empty($sampleRequest) || !is_object(
                $sampleRequest
            ) || ($sampleRequest->reminder_3 != 0 && $sampleRequest->reminder_coupon != 0)) {
            return;
        }
        ...
        if ($sampleRequest->reminder_3 == 0) {
            ...
            if (!empty($uid)) {
                $query->clear()
                    ...
                    ->values(
                        $db->quote($token) . ',' . $db->quote(1) . ',' . $db->quote(
                            Redshop::getConfig()->get('DISCOUNT_PERCENTAGE')
                        ) . ',' .
                        $db->quote($startDate) . ',' . $db->quote($endDate) . ',' . $db->quote(1) . ',' . $db->quote(
                            $uid
                        ) . ',' . $db->quote(1)
                    );
