...
class RedshopHelperCategory
{
    ...
    public static function getCategoryListArray($categoryId = null, $cid = null)
    {
        ...
        $categoryMainFilter = $app->getUserStateFromRequest(
            $context . 'category_main_filter',
        ...
        if ($view == 'category') {
            $filter_order     = urldecode(
                $app->getUserStateFromRequest($context . 'filter_order', 'filter_order', 'ordering')
            );
            $filter_order_Dir = urldecode(
    ...
    public static function buildQueryFilterProduct($categoryId, $allCategories = array(), $filters)
    {
        ...
        foreach ($attribute as $key => $value) {
            $query->leftJoin(
                $db->qn('#__redshop_product_attribute', 'a' . $key)
            ...
            $query->leftJoin(
                $db->qn('#__redshop_product_attribute_property', 'ap' . $key)
