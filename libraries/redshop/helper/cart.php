...
abstract class RedshopHelperCart
{
        ...
        try {
            ...
            for ($i = 0; $i < $idx; $i++) {
                ...
                foreach ($cartAccessories as $cartAccessory) {
                    ...
                    $userCartItemAccessory               = RedshopTable::getInstance(
                        'Usercart_Accessory_Item',
                        'RedshopTable'
                    );
                    ...
                    self::addCartAttributeToDatabase(
                        $cartAccessory['accessory_childs'],
                        $cartItemId,
                        $userCartItemAccessory->accessory_id,
                        true
                    );
                }
            }
     ...
     * @since   2.0.3
     */
    public static function addCartAttributeToDatabase(
        $attributes = array(),
        $cartItemId = 0,
        $productId = 0,
        $isAccessory = false
            ...
            $query->select(
                $db->qn(
                    array(
                        ...
                        'ci.product_id',
                        'ci.product_quantity',
                        'ci.product_wrapper_id',
                        'ci.product_subscription_id',
                        'ci.giftcard_id',
                        'ci.attribs'
                    )
                )
        ...
        foreach ($cartItems as $cartItem) {
            ...
            if ($cartItem->giftcard_id) {
                ...
                if ($productData->product_type === 'subscription') {
                    ...
                    if (!empty($productSubscription->subscription_id)) {
                        ...
                        $setCartItem = false;
                        $msg         = Text::_('COM_REDSHOP_SELECT_PRODUCT_SUBSCRIPTION');
                    }
                }

                // Accessory price
                $generateAccessoryCart = \Redshop\Accessory\Helper::generateAccessoryFromCart(
                    $cartItemId,
                    $productId,
                    $quantity
                );
                ...
                if (count($requiredAttributeData) > 0) {
                    ...
                    $msg         = $requiredAttributeName . " " . Text::_('COM_REDSHOP_IS_REQUIRED');
                    $setCartItem = false;
                }
                ...
                if ($cartItem->product_wrapper_id) {
                    $wrapperArr   = \Redshop\Wrapper\Helper::getWrapperPrice(
                        array('product_id' => $productId, 'wrapper_id' => $cartItem->product_wrapper_id)
                    );
            ...
            if ($setCartItem) {
                ...
                if ($giftCardId) {
                    ...
                    $cart[$idx]['quantity']                   = \Redshop\Stock\Helper::checkQuantityInStock(
                        $cart[$idx],
                        $quantity
                    );
                }
     ...
     * @since  2.0.3
     */
    public static function generateAttributeFromCart(
        $cartItemId = 0,
        $isAccessory = 0,
        $parentSectionId = 0,
        $quantity = 1
     ...
     */
    public static function modifyCalculation()
    {
    ...
    public static function calculateTaxAfterDiscount($tax = 0, $discount = 0)
    {
        ...
        if (
            \Redshop::getConfig()->get('APPLY_VAT_ON_DISCOUNT')
