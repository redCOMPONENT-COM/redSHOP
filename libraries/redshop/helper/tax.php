    ...
    public static function getVatRates($productId = 0, $userId = 0)
    {
        ...
        if (!array_key_exists($key, self::$vatRate)) {
            ...
            $query = $db->getQuery(true)
                ...
                ->leftJoin(
                    $db->qn('#__redshop_tax_group', 'tg') . ' ON ' . $db->qn('tg.id') . ' = ' . $db->qn(
                        'tr.tax_group_id'
                    )
                )
                ->leftJoin(
                    $db->qn('#__redshop_country', 'c') . ' ON ' . $db->qn('tr.tax_country') . ' = ' . $db->qn(
                        'c.country_3_code'
                    )
                    . ' AND ' . $db->qn('c.country_3_code') . ' = ' . $db->quote($userData->country_code)
                )
                ->leftJoin(
                    $db->qn('#__redshop_state', 's') . ' ON ' . $db->qn('tr.tax_state') . ' = ' . $db->qn(
                        's.state_3_code'
                    )
                )
	            ->leftJoin(
