...
abstract class RedshopHelperProduct_Attribute
{
    ...
    public static function getPropertyPrice($sectionId = 0, $quantity = '', $section = '', $userId = 0)
    {
        ...
        if (!array_key_exists($key, static::$propertyPrice)) {
            ...
            if ($userId) {
                $query->leftJoin(
                    $db->qn('#__redshop_users_info', 'u') . ' ON ' . $db->qn('u.shopper_group_id') . ' = ' . $db->qn(
                        'p.shopper_group_id'
                    )
                )
            ...
            RedshopHelperUtility::getDispatcher()->trigger(
                'onGetPropertyPrice',
                array(&static::$propertyPrice[$key], $sectionId, $section, $userId)
            );
        }
     ...
     * @since   2.1.0
     */
    public static function defaultAttributePrice(
        $productId = 0,
        $showPrice = 0.0,
        $templateHtml = '',
        $userId = 0,
        ...
        foreach ($attributes as $attribute) {
            $properties = empty($attribute->properties) ? self::getAttributeProperties(
                0,
            ...
            foreach ($properties as $property) {
                ...
                if ($property->property_price > 0) {
                    ...
                    if ($applyTax) {
                        $attributePropertyVAT = RedshopHelperProduct::getProductTax(
                            $productId,
            ...
            foreach ($selectedProperties as $selectedProperty) {
                ...
                // Add default selected Property price to product price
                $defaultPrices = RedshopHelperProduct::makeTotalPriceByOprand(
                    $showPrice,
                    $subPropertiesOprand,
                    $subPropertiesPrice
                );
    ...
    public static function getAttributeProperties(
        ...
        $key = md5(
            $propertyId . '_' . $attributeId . '_' . $productId . '_' . $attributeSetId . '_' . $required . '_' . $notPropertyId
        );
        ...
        if (!array_key_exists($key, static::$attributeProperties)) {
            if ($productId) {
                ...
                foreach ($productId as $item) {
                    if ($productData = \Redshop\Product\Product::getProductById($item)) {
                ...
                $query->select('pa.product_id')
                    ->leftJoin(
                        $db->qn('#__redshop_product_attribute', 'pa') . ' ON ' . $db->qn(
                            'ap.attribute_id'
            ...
            JPluginHelper::importPlugin('redshop_product');
            RedshopHelperUtility::getDispatcher()->trigger(
                'onGetAttributeProperties',
                array(&static::$attributeProperties[$key])
            );
        }
    ...
    public static function getAttributeSubProperties($subPropertyId = 0, $propertyId = 0, $includeUnpublished = false)
    {
        ...
        if (!array_key_exists($key, static::$subProperties)) {
            ...
            JPluginHelper::importPlugin('redshop_product');
            RedshopHelperUtility::getDispatcher()->trigger(
                'onGetAttributeSubProperties',
                array(&static::$subProperties[$key])
            );
        }
