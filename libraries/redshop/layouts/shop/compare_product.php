...
?>
    <ul id="compare_ul">
        <?php for ($i = 0; $i < $idx; $i++): ?>
            <?php $product = \Redshop\Product\Product::getProductById($compareProducts[$i]["product_id"]); ?>
            <li>
                ...
                <td width="5%">
                    <span>
                        <a href="javascript:void(0);"
                           onClick="javascript:remove_compare(<?php echo $compareProducts[$i]["product_id"] ?>,<?php echo $compareProducts[$i]["category_id"] ?>)">
                            <?php echo JText::_('COM_REDSHOP_DELETE') ?>
                        </a>
                    </span>
                </td>
