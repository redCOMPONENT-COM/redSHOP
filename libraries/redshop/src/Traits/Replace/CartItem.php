		...
		if (isset($cart[$i]['giftcard_id']) && $cart[$i]['giftcard_id'])
		{
			...
			$receiverInfor = \RedshopLayoutHelper::render(
				...
				[
					'tag' => 'div',
					'class' => 'reciverInfo',
					'text' => \JText::_('LIB_REDSHOP_GIFTCARD_RECIVER_NAME_LBL') . ': ' . $cart[$i]['reciver_name'] . '<br />' . \JText::_('LIB_REDSHOP_GIFTCARD_RECIVER_EMAIL_LBL') . ': ' . $cart[$i]['reciver_email']
				],
				'',
				$optionLayout
			);
			...
			if ($quotationMode && !\Redshop::getConfig()->getBool('SHOW_QUOTATION_PRICE')) {
				...
				$replaceData['{product_price}'] = \RedshopHelperProductPrice::formattedPrice($cart[$i]['product_price']);
				$replaceData['{product_total_price}'] = \RedshopHelperProductPrice::formattedPrice($cart[$i]['product_price'] * $cart[$i]['quantity'], true);
			}
			...
			$replaceData['{product_price_excl_vat}'] = \RedshopHelperProductPrice::formattedPrice($cart[$i]['product_price']);
			$replaceData['{product_total_price_excl_vat}'] = \RedshopHelperProductPrice::formattedPrice($cart[$i]['product_price'] * $cart[$i]['quantity']);
			...
			if (strpos($cartHtml, "{product_attribute_loop_start}") !== false && strpos($cartHtml, "{product_attribute_loop_end}") !== false) {
				$templateAttr = $this->getTemplateBetweenLoop('{product_attribute_loop_start}', '{product_attribute_loop_end}', $cartHtml);
			...
			$attributeCart = \RedshopHelperProduct::makeAttributeCart($cart[$i]['cart_attribute'] ?? [], $productId,
				0, 0, $quantity, $cartHtml
			);
			$cartAttribute = $attributeCart[0];
			$retAccArr = \RedshopHelperProduct::makeAccessoryCart($cart[$i]['cart_accessory'] ?? [],
				$productId, $cartHtml);
			...
			$link = Route::_('index.php?option=com_redshop&view=product&cid=' . $catId . '&pid=' . $productId . '&Itemid=' . $itemId);
			...
			if (!$quotationMode || ($quotationMode && \Redshop::getConfig()->get('SHOW_QUOTATION_PRICE'))) {
				if (!$isApplyVAT) {
					...
					$productTotalPrice = \RedshopHelperProductPrice::formattedPrice(($cart[$i]['product_price'] ?? 0) * $quantity);
				}
			}
			...
			if (!$quotationMode || ($quotationMode && \Redshop::getConfig()->get('SHOW_QUOTATION_PRICE'))) {
				if (!$isApplyVAT) {
					$productPrice = \RedshopHelperProductPrice::formattedPrice($cart[$i]['product_price_excl_vat'] ?? 0, true);
				} else {
			...
			if (strpos($cartHtml, "{product_attribute_loop_start}") !== false && strpos($cartHtml, "{product_attribute_loop_end}") !== false) {
				$templateAttrData = $this->getTemplateBetweenLoop('{product_attribute_loop_start}', '{product_attribute_loop_end}', $cartHtml);
			...
			if (!$quotationMode || ($quotationMode && \Redshop::getConfig()->get('SHOW_QUOTATION_PRICE'))) {
				$replaceData['{product_price_excl_vat}'] = \RedshopHelperProductPrice::formattedPrice($productPriceNoVAT);
				$replaceData['{product_total_price_excl_vat}'] = \RedshopHelperProductPrice::formattedPrice($productPriceNoVAT * $quantity);
			} else {
				...
				if ($view == 'cart') {
					$updateAttribute = \RedshopLayoutHelper::render(
						...
						[
							...
							'x' => '550',
							'y' => '400',
							'link' => \JURI::root() . 'index.php?option=com_redshop&view=cart&layout=change_attribute&tmpl=component&pid=' . $productId . '&cart_index=' . $i,
