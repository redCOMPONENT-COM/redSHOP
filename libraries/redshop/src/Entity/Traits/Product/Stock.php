    ...
    public function getStockstatus($totalAttribute = 0, $selectedPropertyId = 0, $selectedsubpropertyId = 0)
    {
        ...
        if ($selectedPropertyId) {
            if ($selectedsubpropertyId) {
                ...
                if (!$stockStatus && (($productPreOrder == "global" && \Redshop::getConfig()->get(
                                'ALLOW_PRE_ORDER'
                            )) || ($productPreOrder == "yes"))) {
                    $prestocksts            = \RedshopHelperStockroom::isPreorderStockExists(
            ...
            if (!$stockStatus && (($productPreOrder == "global" && \Redshop::getConfig()->get(
                ...
                $prestocksts            = \RedshopHelperStockroom::getFinalPreorderStockofProduct(
                    $this->getId(),
                    $totalAttribute
                );
                $data['preorder']       = 1;
