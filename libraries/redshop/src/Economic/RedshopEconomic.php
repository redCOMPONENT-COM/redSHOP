...
class RedshopEconomic
{
    ...
    public static function createInvoiceInEconomic($orderId, $data = array())
    {
        ...
        $economicData['name_ST']       = ($userShippingInfo->get('is_company') == 1 && $userShippingInfo->get(
                'company_name'
            ) != '')
            ? $userShippingInfo->get('company_name') : $userShippingInfo->get(
                'firstname'
            ) . ' ' . $userShippingInfo->get('lastname');
        ...
        for ($i = 0, $in = count($orderItem); $i < $in; $i++) {
            ...
            if ($orderItem[$i]->wrapper_id) {
                ...
                if (count($wrapper) > 0) {
                    ...
                    $displayWrapper = "\n" . \JText::_(
                            'COM_REDSHOP_WRAPPER'
                        ) . ": " . $wrapperName . "(" . $orderItem[$i]->wrapper_price . ")";
                }
            }
            ...
            if (count($orderItemAttData) > 0) {
                ...
                if (count($orderPropData) > 0) {
                    ...
                    $orderProperty = \RedshopHelperProduct_Attribute::getAttributeProperties(
                        $propertyId,
                        $attributeId,
                        $productId
                    );
    ...
    {
        if (\Redshop::getConfig()->get('ATTRIBUTE_AS_PRODUCT_IN_ECONOMIC') == 2 && self::getTotalProperty(
                $row->product_id
            ) > 0) {
            return array();
        }
    ...
    public static function createProductGroupInEconomic($row = null, $isShipping = 0, $isDiscount = 0, $isVat = 0)
    {
        ...
        if (!empty($row) && $row->accountgroup_id != 0) {
            ...
            $accountGroup = RedshopHelperUtility::getEconomicAccountGroup(
                \Redshop::getConfig()->get('DEFAULT_ECONOMIC_ACCOUNT_GROUP')
            );
        }
        ...
        if (count($accountGroup) > 0) {
            ...
            $groupHandle              = \RedshopHelperUtility::getDispatcher()->trigger(
    ...
    public static function makeAccessoryOrder($invoiceNo, $orderItem, $userId = 0)
    {
        ...
        if (count($orderItemData) > 0) {
            ...
            for ($i = 0, $in = count($orderItemData); $i < $in; $i++) {
                ...
                $eco['delivery_date']    = date("Y-m-d") . "T" . date("h:i:s");
                $invoiceLineNo           = \RedshopHelperUtility::getDispatcher()->trigger(
                    'createInvoiceLine',
                    array($eco)
                );
                    ...
                    if (count($orderSubPropertyData) > 0) {
                        foreach ($orderSubPropertyData as $aData) {
                            ...
                            if (!$hideAttributePrice) {
                                ...
                                $disPrice = " (" . $aData->section_oprand . \RedshopHelperProductPrice::formattedPrice(
                                        $subpropertyPrice
                                    ) . ")";
                            }
    ...
    public static function createSubpropertyInEconomic($productRow = null, $row = null)
    {
        ...
        $eco['product_stock'] = \RedshopHelperStockroom::getStockroomTotalAmount(
            $row->subattribute_color_id,
            "subproperty"
        );
    ...
    public static function createInvoiceShippingLineInEconomic($shipMethodId = "", $invoiceNo = "")
    {
        ...
        if ($shipMethodId != "") {
            ...
            if (count($orderShipping) > 5) {
                ...
                $ecoShippingrateNumber = self::createShippingRateInEconomic(
                    $shippingNumber,
                    $shippingName,
                    $shippingRate,
                    $isVat
     ...
     * @since   2.0.3
     */
    public static function createInvoiceDiscountLineInEconomic(
        $orderDetail = array(),
        $invoiceNo = "",
        $data = array(),
        $isPaymentDiscount = 0,
        ...
        if (\Redshop::getConfig()->get('DEFAULT_ECONOMIC_ACCOUNT_GROUP')) {
            ...
            if (count($accountGroup) > 0) {
                ...
                if ($isPaymentDiscount) {
                    ...
                    $productName   = ($orderDetail->payment_oprand == '+') ? \JText::_(
                        'PAYMENT_CHARGES_LBL'
                    ) : \JText::_('PAYMENT_DISCOUNT_LBL');
                ...
                $debtorHandle          = \RedshopHelperUtility::getDispatcher()->trigger(
    ...
    public static function bookInvoiceInEconomic($orderId, $checkOrderStatus = 1, $bookInvoiceDate = 0)
    {
        ...
        if ((\Redshop::getConfig()->getInt('ECONOMIC_INVOICE_DRAFT') == 2
                && $orderEntity->order_status == \Redshop::getConfig()->getString(
            ...
            if ($userBillingInfo->is_company == 0 || (!$userBillingInfo->ean_number && $userBillingInfo->is_company == 1)) {
                ...
                if (count($currentInvoiceData) > 0 && trim(
                        $currentInvoiceData[0]->OtherReference
                    ) == $order->order_number) {
                    ...
                    if (\Redshop::getConfig()->get('ECONOMIC_BOOK_INVOICE_NUMBER') == 1) {
                        $bookHandle = \RedshopHelperUtility::getDispatcher()->trigger(
                            'CurrentInvoice_Book',
                            array($eco)
                        );
                    } else {
