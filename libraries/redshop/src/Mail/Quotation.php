    ...
    public static function sendMail($quotationId, $status = 0)
    {
        ...
        foreach ($quotationProducts as $quotationProduct) {
            ...
            $productTotalPrice        = "<div class='product_price'>" .
                \RedshopHelperProductPrice::formattedPrice(
                    ($quotationProduct->product_price * $quotationProduct->product_quantity)
                ) . "</div>";
            ...
            $productTotalPriceExclVat = "<div class='product_price'>" .
                \RedshopHelperProductPrice::formattedPrice(
                    ($quotationProduct->product_excl_price * $quotationProduct->product_quantity)
                ) . "</div>";
        ...
        if ($quotation->user_id != 0) {
            ...
            if (strstr($message, "{quotation_custom_field_list}")) {
                $billAdd .= \RedshopHelperExtrafields::listAllFieldDisplay(
                    16,
                    $quotation->user_info_id,
                    1,
                    $quotation->quotation_email
                );
                $message = str_replace("{quotation_custom_field_list}", "", $message);
            } else {
                $message = \RedshopHelperExtrafields::listAllFieldDisplay(
                    16,
                    $quotation->user_info_id,
                    1,
                    $quotation->quotation_email,
        ...
        if ($quotation->quotation_status == 1 && !\Redshop::getConfig()->getBool('SHOW_QUOTATION_PRICE')) {
            ...
            $quotationSubtotalExclVat       = \RedshopHelperProductPrice::formattedPrice(
                $quotation->quotation_subtotal - $quotation->quotation_tax
            );
            ...
            $quotationSubtotal              = \RedshopHelperProductPrice::formattedPrice(
                $quotation->quotation_subtotal
            );
            $quotationTotal                 = \RedshopHelperProductPrice::formattedPrice($quotation->quotation_total);
            $quotationDiscount              = \RedshopHelperProductPrice::formattedPrice(
