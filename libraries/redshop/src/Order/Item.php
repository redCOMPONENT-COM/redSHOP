...
class Item
{
    ...
    public static function replaceItems($content, $items = array(), $sendMail = false)
    {
        ...
        for ($i = 0; $i < $orderCount; $i++) {
            ...
            $itemId      = !empty($itemData) ? $itemData->id : \RedshopHelperRouter::getItemId(
                $productId,
            ...
            $path           = \JPath::clean(
                JPATH_COMPONENT_SITE . '/assets/images/orderMergeImages/' . $items[$i]->attribute_image
            );
            $attributeImage = '';
            ...
            if (!\Redshop\Template\Helper::isApplyVat($content)) {
                $productTotalPrice .= \RedshopHelperProductPrice::formattedPrice(
                    $items[$i]->product_item_price_excl_vat * $quantity
                );
            } else {
                $productTotalPrice .= \RedshopHelperProductPrice::formattedPrice(
                    $items[$i]->product_item_price * $quantity
                );
            }
            ...
            $attributeData = \RedshopHelperProduct::makeAttributeOrder(
            ...
            $cartHtmlContent = str_replace(
                $attributeData->attribute_middle_template_core,
                $attributeData->attribute_middle_template,
                $cartHtmlContent
            );
            ...
            $cartHtmlContent   = str_replace(
                "{product_accessory}",
                \Redshop\Order\Helper::generateAccessories($items[$i]->order_item_id),
                $cartHtmlContent
            );
            ...
            if ($items[$i]->is_giftcard) {
                ...
                if ($product->product_type == 'subscription') {
                    $userSubscribeDetail  = \RedshopHelperProduct::getUserProductSubscriptionDetail(
                        $items[$i]->order_item_id
                    );
                    $subscription         = \RedshopHelperProduct::getProductSubscriptionDetail(
                        $product->product_id,
                        $userSubscribeDetail->subscription_id
                    );
            ...
            $cartHtmlContent = \RedshopHelperProduct::getProductFinderDatepickerValue(
                $cartHtmlContent,
                $productId,
                $fieldArray
            );
