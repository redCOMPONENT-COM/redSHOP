...
class Template
{
    ...
    public static function replaceTemplate($order, $template, $sendMail = false)
    {
        ...
        $totalForDiscount = !\Redshop::getConfig()->get(
            'APPLY_VAT_ON_DISCOUNT'
        ) ? $subTotalExcludeVAT : $order->order_subtotal;
        ...
        \RedshopHelperUtility::getDispatcher()->trigger(
            ...
            array($order->order_id, &$orderTrackURL)
        );
        ...
        $message = \RedshopHelperPayment::replaceConditionTag(
            $message,
            $order->payment_discount,
            0,
            $order->payment_oprand
        );
        ...
        $message = \RedshopHelperCartTag::replaceTax(
            $message,
            $order->order_tax + $order->order_shipping_tax,
            $order->tax_after_discount,
            1
        );
