...
class RedshopTagsSectionsCategory extends RedshopTagsAbstract
{
	...
	public function replace()
	{
		...
		$this->template = $this->replaceExtantCategory($this->data['category'], $this->template);
	...
	{
		$replacements = [];
		$manufacturerId = (!empty($this->data['manufacturer_id'])) ? $this->data['manufacturer_id'] : '';
			...
			if (count($subCategories) > 0)
			{
				foreach ($subCategories as $row)
				{
					...
					if (strstr($dataAdd, '{subcategory_link}'))
					{
						...
						$link .= isset($this->data['manufacturer_id']) ? '&manufacturer_id=' . $this->data['manufacturer_id'] : '';
	 ...
	 * @since   __DEPLOY_VERSION__
	 */
	private function replaceExtantCategory($category, $template)
	{
		$replacements = [];
		...
		if (!$slide)
		{
			...
			{
				if ($category->pageheading != "")
				{
					$heading = JFactory::getDbo()->escape($category->pageheading);
				}
				else
				{
			...
			else
			{
				...
				$printUrl .= '&limit=' . $endLimit . '&texpricemin=' . $texPriceMin . '&texpricemax=' . $texPriceMax . '&order_by=' . $this->order_by_select;
				$printUrl .= '&manufacturer_id=' . $this->manufacturer_id . '&category_template=' . $this->category_template_id;
			...
			)
			{
				...
				if ($parentId != 0)
				{
					...
					$returnToCategory = RedshopLayoutHelper::render(
						...
						array(
							'link' => $returnCatLink,
							'content' => Redshop::getConfig()->get('DAFULT_RETURN_TO_CATEGORY_PREFIX') . '&nbsp;' . $categoryList->name
						),
						'',
						RedshopLayoutHelper::$layoutOption
					);
			...
			if (strpos($template, "{category_loop_start}") !== false && strpos($template, "{category_loop_end}") !== false)
			{
				$templateSubCat = $this->getTemplateBetweenLoop('{category_loop_start}', '{category_loop_end}', $template);
				...
				for ($i = 0;  $i < $nc; $i++)
				{
					...
					{
						$dataAdd[$i] = Redshop\Helper\ExtraFields::displayExtraFields(2, $row->id, $extraFieldsForCurrentTemplate, $dataAdd[$i]);
					}
			...
			if (strpos($template, "{if subcats}") !== false && strpos($template, "{subcats end if}") !== false)
			{
				$templateSubCats = $this->getTemplateBetweenLoop('{if subcats}', '{subcats end if}', $template);
		...
		if (strpos($template, "{product_loop_start}") !== false && strpos($template, "{product_loop_end}") !== false)
		{
			...
			list($templateUserField, $userFieldArr) = \Redshop\Product\Product::getProductUserfieldFromTemplate($templateProduct);
			...
			foreach ($this->product as $product)
			{
				...
				$dataAddStr = RedshopHelperProduct::redunitDecimal($product->product_volume) . "&nbsp;" . $productVolumeUnit;
				...
				$productReplacements['{product_length}'] = RedshopHelperProduct::redunitDecimal($product->product_length) . "&nbsp;" . $productUnit;
				$productReplacements['{product_width}']  = RedshopHelperProduct::redunitDecimal($product->product_width) . "&nbsp;" . $productUnit;
				$productReplacements['{product_height}'] = RedshopHelperProduct::redunitDecimal($product->product_height) . "&nbsp;" . $productUnit;
				...
				if (strpos($dataAdd, '{manufacturer_product_link}') !== false)
				{
					...
					$manufacturerPLink = RedshopLayoutHelper::render(
						...
						array(
							'class' => 'btn btn-primary',
							'link' => $manuUrl,
							'content' => JText::_("COM_REDSHOP_VIEW_ALL_MANUFACTURER_PRODUCTS") . ' ' . $manufacturerName
						),
						'',
						RedshopLayoutHelper::$layoutOption
					);
				...
				$thumbImage = RedshopLayoutHelper::render(
					...
					array(
						...
						'attr' => '',
						'text' => Redshop\Product\Image\Image::getImage($product->product_id, $link, $pwThumb, $phThumb, 2, 1)
					),
					'',
					RedshopLayoutHelper::$layoutOption
				);
				...
				*/
				...
				$dataAdd = Redshop\Product\Compare::replaceCompareProductsButton($product->product_id, $this->catid, $dataAdd);
