        ...
        if ($total > 0) {
            ...
            foreach ($list as $data) {
                ...
                if ($this->isTagExists('{product_rating_summary}')) {
                    $finalAvgReviewData                             = Redshop\Product\Rating::getRating(
                        $data['item']->productId
                    );
                ...
                if ($this->isTagExists('{products_in_stock}') || $this->isTagExists('{product_stock_amount_image}')) {
                    $productStock                              = RedshopHelperStockroom::getStockAmountWithReserve(
                        $data['item']->productId
                    );
                    $this->replacements['{products_in_stock}'] .= $expDiv . $productStock . $divEnd . $tdEnd . $tdStart;
                    $stockamountList                           = RedshopHelperStockroom::getStockAmountImage(
                        $data['item']->productId,
                        "product",
                        $productStock
                    );
                ...
                if ($this->isTagExists('{product_delivery_time}')) {
                    $productDeliveryTime                           = RedshopHelperProduct::getProductMinDeliveryTime(
                        $data['item']->productId
                    );
                ...
                foreach ($productTag as $aProductTag) {
                    ...
                    $template = Redshop\Helper\ExtraFields::displayExtraFields(
                        "1",
                        $product->product_id,
                        $str,
                        $template
            ...
            $template = Redshop\Template\General::replaceBlank(
                array(
                    '{expand_collapse}',
                    '{product_name}',
                    '{product_image}',
