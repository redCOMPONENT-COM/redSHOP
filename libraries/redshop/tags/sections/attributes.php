...
class RedshopTagsSectionsAttributes extends RedshopTagsAbstract
{
	...
	public function replace()
	{
		...
		if ($displayIndCart && Redshop::getConfig()->getInt('INDIVIDUAL_ADD_TO_CART_ENABLE') === 1)
		{
			...
			return \RedshopHelperAttribute::replaceAttributeWithCartData(
				$productId, $accessoryId, $relatedProductId, $attributes, $this->template, $attributeTemplate, $isChild, $onlySelected
			);
		}

		$attributeTemplate = empty($attributeTemplate) ? \Redshop\Template\Helper::getAttribute($this->template, false) : $attributeTemplate;
		...
		if (count($attributes) > 0)
		{
			...
			{
				$attributeTable .= $this->replaceAttribute($attributes[$a], $attributeTemplateData, $productId,
					$accessoryId, $selectProperty, $relatedProductId, $product);
			}
	...
	public function replaceAttribute($attribute, $attributeTemplateData, $productId, $accessoryId, $selectProperty, $relatedProductId, $product)
	{
		$subDisplay = false;
		$replaceAttr = [];
		...
		if ($attribute->text != "" && count($property) > 0)
		{
			...
			{
				$propertyWoscrollerDiv .= $this->replaceProperty($attributeTable, $imgAdded, $property[$i], $selectProperty, $selectedProperty,
					$propertyId, $productId, $relatedProductId, $accessoryId, $attribute);
			}
			...
			$changePropertyDropdown = "changePropertyDropdown('" . $productId . "','" . $accessoryId . "','"
				. $relatedProductId . "', '" . $attribute->value . "',this.value, '" . $this->mpwThumb . "', '" . $this->mphThumb . "');";
			...
			{
				$pos        = Redshop::getConfig()->get('ASTERISK_POSITION') > 0 ? urldecode($attribute->text) . "<span id='asterisk_right'> * " : "<span id='asterisk_left'>* </span>" . urldecode($attribute->text);
				$attrTitle = $pos;
			}
			else
			{
			...
			if (strpos($attributeTable, '{attribute_tooltip}') !== false)
			{
				...
				{
					$replaceAttr['{attribute_tooltip}'] = JHTML::tooltip($attribute->attribute_description, $attribute->attribute_description, 'tooltip.png', '', '');
				}
				else
				{
...
//				$displaySubProperty .= $this->replaceSubPropertyData(
				$displaySubProperty .= RedshopHelperProduct::replaceSubPropertyData(
					$productId, $accessoryId, $relatedProductId, $attribute->attribute_id, $aDefaultPropertyId, $subPropertyData,
	...
	public function replaceProperty(&$attributeTable, &$imgAdded, $property, $selectProperty, $selectedProperty, $propertyId, $productId, $relatedProductId, $accessoryId, $attribute)
	{
		$propertyWoscrollerDiv = '';
		...
		$propertyStock = isset($propertyStockrooms[$property->value]) ? (int) $propertyStockrooms[$property->value] : 0;
		$propertyStock += $subPropertyStock;

		// Preorder stock data
		$preorderPropertyStock = isset($propertyPreOrderStockrooms[$property->value]) ?
			(int) $propertyPreOrderStockrooms[$property->value] : 0;
		$preorderPropertyStock += $preorderSubPropertyStock;
		...
		if ($property->property_image)
		{
			if (JFile::exists(REDSHOP_FRONT_IMAGES_RELPATH . "product_attributes/" . $property->property_image))
			{
				...
				if ($property->setdefault_selected && $preSelected)
				{
		...
		if ($property->property_price > 0)
		{
			...
			if (!empty($isApplyAttributeVAT))
			{
				...
				{
					$attributesPropertyVatShow     = RedshopHelperProduct::getProductTax($productId, $property->property_price, $userId);
					$attributesPropertyOldPriceVat = RedshopHelperProduct::getProductTax($productId, $attributesPropertyOldPrice, $userId);
				}
			}
			 ...
			 */
			$attributes_property_vat       = RedshopHelperProduct::getProductTax($productId, $property->property_price, $userId);
		...
		$property->preorder_stock = $preorderPropertyStock;
		...
		$attributeTable .= RedshopLayoutHelper::render(
			...
			[
				...
				'preorderPropertyStock' => !empty($preorderPropertyStock) ? $preorderPropertyStock : 0
			],
			'',
			RedshopLayoutHelper::$layoutOption
		);
